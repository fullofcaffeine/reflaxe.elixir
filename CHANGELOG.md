...
* **compiler:** lower proven fresh Array projection loops with exactly one ordered append per input to direct `Enum.map`, while retaining reducers for conditional/multiple appends, partial accumulator reads, control-flow carriers, persistent iterators, and unproven receiver calls; retire the documented but unused `elixir.feature.idiomatic_comprehensions` define so the improvement remains part of the single semantics-preserving pipeline.
...
* **mix:** replace single-root timestamp invalidation with deterministic content fingerprints for recursive HXML, every direct classpath, resources, resolved libraries/package metadata, Haxe toolchain and standard-library identity, relevant environment/configuration, and explicitly declared macro inputs; make watcher rebuilds share the same manifest path and fail closed on legacy or unwritable manifests.
...
* **docs:** add checked Elixir-first, portable-imperative, and PhoenixHx source/output examples to the README; make compile-time HXX view checking explicit; and document the compiler-first native-lowering order plus today's conservative core-helper retention without presenting `-D reflaxe_runtime` as an application mode.
