...
abstract EctoQuery<T>(Term) {
	...
	extern inline public function where<V>(fieldName:String, value:V):EctoQuery<T> {
		...
		return
			new EctoQuery<T>(untyped __elixir__('(require Ecto.Query; Ecto.Query.where({0}, [q], field(q, ^String.to_existing_atom(Macro.underscore({1}))) == ^{2}))',
	...
	extern inline public function preload(associations:Term):EctoQuery<T> {
		return new EctoQuery<T>(untyped __elixir__('(require Ecto.Query; Ecto.Query.preload({0}, ^{1}))', this, associations));
	}
	...
	extern inline public function orderBy(field:String, direction:String = "asc"):EctoQuery<T> {
		// Use field/2 with a binding and convert string to existing atom
		return
			new EctoQuery<T>(direction == "desc" ? untyped __elixir__('(require Ecto.Query; Ecto.Query.order_by({0}, [q], [desc: field(q, ^String.to_existing_atom(Macro.underscore({1})))]))',
				this,
			field) : untyped __elixir__('(require Ecto.Query; Ecto.Query.order_by({0}, [q], [asc: field(q, ^String.to_existing_atom(Macro.underscore({1})))]))',
					this, field));
	}
...
class Query {
	...
	extern inline public static function whereAll<T>(query:EctoQuery<T>, conditions:Map<String, Term>):EctoQuery<T> {
		...
		var elixirQuery = untyped __elixir__('Enum.reduce(Map.to_list({0}), {1}, fn {field_name, value}, acc ->
                import Ecto.Query
                from(q in acc, where: field(q, ^String.to_existing_atom(field_name)) == ^value)
            end)', conditions, query.toElixirQuery());
