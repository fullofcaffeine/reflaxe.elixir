...
abstract TypedQuery<T>(EctoQueryStruct) {
	...
	extern inline public function select<R>(projection:T->R):TypedQuery<R> {
		...
		return cast untyped __elixir__('(require Ecto.Query; Ecto.Query.select({0}, [t], {1}))', this.query, projection);
	}
	...
	extern inline public function limit(count:Int):TypedQuery<T> {
		return cast untyped __elixir__('(require Ecto.Query; Ecto.Query.limit({0}, ^{1}))', this, count);
	}
	...
	extern inline public function offset(count:Int):TypedQuery<T> {
		return cast untyped __elixir__('(require Ecto.Query; Ecto.Query.offset({0}, ^{1}))', this, count);
	}
