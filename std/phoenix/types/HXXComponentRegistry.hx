...
class HXXComponentRegistry {
	...
	public static function isRegisteredElement(elementName:String):Bool {
		return htmlElements.exists(elementName.toLowerCase()) || phoenixComponents.exists(elementName);
	}
	...
	public static function listPhoenixComponents():Array<String> {
		var out:Array<String> = [];
		for (k in phoenixComponents.keys())
			out.push(k);
		out.sort((a, b) -> a < b ? -1 : (a > b ? 1 : 0));
		return out;
	}
	...
	private static function getSelectAttributes():Array<String> {
		return getGlobalAttributes().concat(["name", "multiple", "size", "required", "disabled", "form"]);
	}

	private static function getOptionAttributes():Array<String> {
		return getGlobalAttributes().concat(["value", "label", "selected", "disabled"]);
	}
	...
	private static function getLabelAttributes():Array<String> {
		return getGlobalAttributes().concat(["htmlFor", "form"]);
	}
	...
	private static function getAudioAttributes():Array<String> {
		return getGlobalAttributes().concat(["src", "autoplay", "controls", "loop", "muted", "preload", "crossorigin"]);
	}

	private static function getListAttributes():Array<String> {
		return getGlobalAttributes().concat(["reversed", "start", "type"]);
	}

	private static function getListItemAttributes():Array<String> {
		return getGlobalAttributes().concat(["value"]);
	}

	private static function getTableCellAttributes():Array<String> {
		return getGlobalAttributes().concat(["colspan", "rowspan", "headers", "scope"]);
	}

	private static function getMetaAttributes():Array<String> {
		return ["name", "content", "httpEquiv", "charset", "property"];
	}
	...
	private static function getStyleAttributes():Array<String> {
		return getGlobalAttributes().concat(["media", "nonce"]);
	}
}
