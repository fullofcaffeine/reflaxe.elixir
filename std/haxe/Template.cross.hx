...
/**
 * BEAM-native implementation of the portable `haxe.Template` subset.
 *
 * Supported syntax:
 * - `::name::` and dotted paths such as `::user.name::`
 * - `::if expr::...::else::...::end::`
 * - `::foreach items::...::end::` over lists and maps
 * - `$$macro(arg, ...)` where the macro value is a Haxe function
 *
 * The implementation deliberately avoids the upstream parser shape because it
 * relies on string iterators and mutable local closure state that are poor fits
 * for the current Elixir target lowering.
 */
class Template {
	...
	public function execute(context:Dynamic, ?macros:Dynamic):String {
		return render(source, context, macros == null ? {} : macros, globals);
	}

	static function render(source:String, context:Dynamic, macros:Dynamic, globals:Dynamic):String {
