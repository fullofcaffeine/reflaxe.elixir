...
/**
 * Adds immutable map operations for the Elixir target.
 *
 * Use `using elixir.MapTools`. The explicit namespace shows that this is a
 * target-owned helper, not a replacement for an official Haxe module.
 */
class MapTools {
	/**
	...
	public static function filter<K, V>(map:Map<K, V>, predicate:(K, V) -> Bool):Map<K, V> {
		return cast untyped __elixir__("Map.new(Enum.filter({0}, fn {k, v} -> {1}.(k, v) end))", map, predicate);
	}
	...
	public static function map<K, V, U>(map:Map<K, V>, transform:(K, V) -> U):Map<K, U> {
		return cast untyped __elixir__("Map.new(Enum.map({0}, fn {k, v} -> {k, {1}.(k, v)} end))", map, transform);
	}
	...
	public static function mapKeys<K, V, J>(map:Map<K, V>, transform:(K, V) -> J):Map<J, V> {
		return cast untyped __elixir__("Map.new(Enum.map({0}, fn {k, v} -> {{1}.(k, v), v} end))", map, transform);
	}
	...
	public static function fromArray<K, V>(pairs:Array<{key:K, value:V}>):Map<K, V> {
		return cast untyped __elixir__("Map.new(Enum.map({0}, fn %{key: k, value: v} -> {k, v} end))", pairs);
	}
