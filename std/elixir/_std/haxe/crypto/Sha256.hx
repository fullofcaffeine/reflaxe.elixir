...
/**
 * Sha256 (Elixir target)
 *
 * WHAT
 * - Canonical Haxe `haxe.crypto.Sha256` helpers for SHA-256 hex and raw digest bytes.
 *
 * WHY
 * - SHA-256 is common in portable code for content digests and protocol fingerprints.
 *   BEAM provides this primitive directly via `:crypto`, so generated Elixir should
 *   not carry the portable reference implementation at runtime.
 *
 * HOW
 * - Runtime delegates to `:crypto.hash(:sha256, ...)` and wraps binary output as `Bytes`.
 ...
 */
@:native("Haxe.Crypto.Sha256")
class Sha256 {
	public static function encode(s:String):String {
		#if (macro || (!reflaxe_runtime && !elixir))
		return Sha256Pure.makeHex(Bytes.ofString(s));
		#else
		return untyped __elixir__('Base.encode16(:crypto.hash(:sha256, {0}), case: :lower)', s);
		#end
	}

	public static function make(b:Bytes):Bytes {
		#if (macro || (!reflaxe_runtime && !elixir))
		return Sha256Pure.makeBytes(b);
		#else
		var digest = untyped __elixir__(':crypto.hash(:sha256, {0})', b.getData());
