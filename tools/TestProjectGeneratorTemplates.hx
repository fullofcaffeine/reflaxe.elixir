...
class TestProjectGeneratorTemplates {
	public static function main():Void {
		...
		assertNotContains(buildHxml, "-D reflaxe_runtime", "build.hxml keeps compiler-development defines internal");
	...
	private static function assertMixTaskScaffold(root:String):Void {
		...
		assertNotContains(buildHxml, "-D reflaxe_runtime", "mix task build.hxml keeps compiler-development defines internal");
	...
	private static function ensureRepoMixDepsInstalled():Void {
		var exitDepsGet = Sys.command("mix", ["deps.get"]);
		if (exitDepsGet != 0) {
			Sys.println("FAIL: mix deps.get failed (exit: " + exitDepsGet + ")");
			Sys.exit(1);
		}
	}
