...
class LiveReactRegistry {
	...
	public static function renderBoundary(value:LiveReactComponent):String {
		...
		var eventName = ElixirMacro.underscore(value.name) + "_action";
		return joinLines([
			...
			"",
			'export interface ${value.name}RawProps extends Record<string, unknown> {',
	...
	public static function renderInnerComponent(value:LiveReactComponent):String {
		...
		return joinLines([
			...
			'export function ${value.name}({title, onAction}: ${value.name}Props) {',
			"  return (",
