...
class UserRegistration {
	...
	static function main() {
		...
		var invalidUsers = [
			...
			{
				userId: "toolongusername123456789012345678901234567890",
	...
	public static function registerUser(userIdStr:String, emailStr:String, displayNameStr:String, ageStr:String):Result<RegisteredUser, String> {
		...
		return UserId.parse(userIdStr).mapError(e -> 'Invalid User ID: ${e}').flatMap(userId -> {
			return Email.parse(StringTools.trim(emailStr)).mapError(e -> 'Invalid Email: ${e}').flatMap(email -> {
				return NonEmptyString.parseAndTrim(displayNameStr).mapError(e -> 'Invalid Display Name: ${e}').flatMap(displayName -> {
					var ageInt = Std.parseInt(ageStr);
					if (ageInt == null) {
						return Error('Invalid Age: "${ageStr}" is not a number');
					}

					return PositiveInt.parse(ageInt).mapError(e -> 'Invalid Age: ${e}').map(age -> {
						return {
							userId: userId,
							email: email,
	...
	static function updateUserEmail(user:RegisteredUser, newEmailStr:String):Result<RegisteredUser, String> {
		return Email.parse(newEmailStr).mapError(e -> 'Invalid new email: ${e}').map(newEmail -> {
			return {
				userId: user.userId,
				email: newEmail,
	...
	static function updateUserDisplayName(user:RegisteredUser, newDisplayNameStr:String):Result<RegisteredUser, String> {
		return NonEmptyString.parseAndTrim(newDisplayNameStr).mapError(e -> 'Invalid new display name: ${e}').map(newDisplayName -> {
			return {
				userId: user.userId,
				email: user.email,
