defmodule OptionPatterns.UserRepositoryTest do
  ...
  test "find returns option for valid id" do
    ...
    _ = assert(match?({:some, _}, user), "Should find user with ID 1")
    (case user do
      {:some, u} ->
        _ = assert("Alice Johnson" == u.name, "Should have correct name")
        _ = assert("alice@example.com" == u.email, "Should have correct email")
        _ = assert(u.active, "Should be active")
      {:none} ->
  ...
  test "find returns none for invalid id" do
    ...
    _ = assert(match?({:none}, user), "Should not find user with invalid ID")
  end
  test "find returns none for negative id" do
    user = OptionPatterns.UserRepository.find(-1)
    _ = assert(match?({:none}, user), "Should not find user with negative ID")
  end
  test "find returns none for zero id" do
    user = OptionPatterns.UserRepository.find(0)
    _ = assert(match?({:none}, user), "Should not find user with zero ID")
  end
  test "find by email returns option for valid email" do
    user = OptionPatterns.UserRepository.find_by_email("bob@example.com")
    _ = assert(match?({:some, _}, user), "Should find user with valid email")
    (case user do
      {:some, u} ->
        _ = assert(2 == u.id, "Should have correct ID")
        _ = assert("Bob Smith" == u.name, "Should have correct name")
      {:none} ->
  ...
  test "get user email returns email for valid user" do
    ...
    _ = assert(match?({:some, _}, email), "Should get email for valid user")
    _ = assert({:some, "alice@example.com"} == email, "Should have correct email")
  end
  test "get user email returns none for invalid user" do
    email = OptionPatterns.UserRepository.get_user_email(999)
    _ = assert(match?({:none}, email), "Should not get email for invalid user")
  end
  test "get user display name returns name for valid user" do
    ...
    _ = assert("Alice Johnson" == display_name, "Should return user's display name")
  end
  ...
  test "update email fails for invalid user" do
    ...
    _ = assert(match?({:error, _}, result), "Should fail to update email for invalid user")
    (case result do
      {:ok, _value} ->
  ...
  test "update email fails for invalid email format" do
    ...
    _ = assert(match?({:error, _}, result), "Should fail for invalid email format")
    (case result do
      {:ok, _value} ->
  ...
  test "get users by status returns active users" do
    ...
    _ = assert(length(active_users) >= 3, "Should have at least 3 active users")
    _g = 0
    _ = Enum.each(active_users, fn user -> assert(user.active, "All returned users should be active") end)
  end
  test "get users by status returns inactive users" do
    inactive_users = OptionPatterns.UserRepository.get_users_by_status(false)
    _ = assert(length(inactive_users) >= 1, "Should have at least 1 inactive user")
    _g = 0
    _ = Enum.each(inactive_users, fn user -> refute(user.active, "All returned users should be inactive") end)
  end
  test "create succeeds for valid data" do
    result = OptionPatterns.UserRepository.create("Test User", "test@example.com")
    _ = assert(match?({:ok, _}, result), "Should successfully create user with valid data")
    (case result do
      {:ok, user} ->
        _ = assert("Test User" == user.name, "Should have correct name")
        _ = assert("test@example.com" == user.email, "Should have correct email")
        _ = assert(user.active, "New user should be active")
      {:error, msg} ->
        flunk("Unexpected error: " <> msg)
    end)
  end
  test "create fails for empty name" do
    ...
    _ = assert(match?({:error, _}, result), "Should fail for empty name")
    (case result do
      {:ok, _value} ->
  ...
  test "create fails for invalid email" do
    ...
    _ = assert(match?({:error, _}, result), "Should fail for invalid email")
    (case result do
      {:ok, _value} ->
        flunk("Expected error for invalid email")
      {:error, msg} ->
  ...
  test "create fails for duplicate email" do
    ...
    _ = assert(match?({:error, _}, result), "Should fail for duplicate email")
    (case result do
      {:ok, _value} ->
