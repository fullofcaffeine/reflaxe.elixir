...
class Main {
	...
	static function demonstrateRepositoryPatterns() {
		...
		switch (user) {
			case Some(u):
				trace('Found user: ${u.getDisplayName()}');
			case None:
	...
	static function demonstrateConfigurationManagement() {
		...
		switch (ConfigManager.getIntWithRange("max_connections", 1, 1000)) {
			case Ok(value):
				trace('Max connections: ${value}');
			case Error(msg):
				trace('Config error: ${msg}');
		}

		// Required configuration
		switch (ConfigManager.getDatabaseUrl()) {
			case Ok(url):
				trace('Database URL validated successfully');
			case Error(msg):
				trace('Database config error: ${msg}');
		}
	...
	static function demonstrateNotificationService() {
		...
		switch (NotificationService.sendToUser(1, "Welcome to our service!", Email)) {
			case Ok(record):
				trace('Notification sent successfully to user ${record.userId}');
			case Error(reason):
				trace('Failed to send notification: ${reason}');
		}
		...
		switch (NotificationService.sendToEmail("alice@example.com", "Email notification", Email)) {
			case Ok(record):
				trace('Email notification sent successfully');
			case Error(reason):
				trace('Email send failed: ${reason}');
		}
	...
	static function demonstrateErrorHandling() {
		...
		var userResult = UserRepository.find(999).toResult("User 999 not found").flatMap(user -> {
		...
		switch (userResult) {
			case Ok(user):
				trace('User validated: ${user.email}');
			case Error(msg):
				trace('Validation failed: ${msg}');
		}
		...
		switch (userOption) {
			...
			case None:
				trace("User creation failed");
		}

		// Chaining validations
		var validationChain = ConfigManager.getRequired("timeout").flatMap(timeoutStr -> {
		...
		switch (validationChain) {
			case Ok(timeout):
				trace('Validated timeout: ${timeout}s');
			case Error(msg):
				trace('Validation error: ${msg}');
		}
	...
	static function demonstrateFunctionalComposition() {
		...
		switch (composedOperation) {
			case Some(result):
				trace('Composed result: ${result}');
			case None:
				trace("Composition chain failed at some point");
		}
		...
		var activeUserNames = [];
		for (id in userIds) {
			switch (UserRepository.find(id).filter(user -> user.active).map(user -> user.name)) {
				case Some(name):
