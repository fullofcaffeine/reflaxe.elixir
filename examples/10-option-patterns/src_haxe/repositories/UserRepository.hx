...
class UserRepository {
	...
	public static function getUserDisplayName(id:Int):String {
		return find(id).map(user -> user.getDisplayName()).unwrap("Unknown User");
	}
	...
	public static function isUserActive(id:Int):Bool {
		return find(id).map(user -> user.active).unwrap(false);
	}
	...
	public static function updateEmail(id:Int, newEmail:String):Result<User, String> {
		...
		return find(id).toResult("User not found").map(user -> {
			user.email = newEmail;
			user;
		});
	}
