...
class Accounts {
	...
	public static function listOrganizationInvites(organizationId:Int):Array<OrganizationInvite> {
		var query:ecto.TypedQuery.TypedQuery<OrganizationInvite> = TypedQuery.from(OrganizationInvite).where(i -> i.organizationId == organizationId);
		var invites:Array<OrganizationInvite> = Repo.all(query);
		invites.sort((a, b) -> a.id - b.id);
		return invites;
	}

	public static function createOrganizationInvite(organizationId:Int, email:String,
			role:String):Result<OrganizationInvite, Changeset<OrganizationInvite, server.schemas.OrganizationInvite.OrganizationInviteParams>> {
	...
	static function getPendingInviteByEmail(normalizedEmail:String):Null<OrganizationInvite> {
		var query:ecto.TypedQuery.TypedQuery<OrganizationInvite> = TypedQuery.from(OrganizationInvite).where(i -> i.email == normalizedEmail);
		var invites:Array<OrganizationInvite> = Repo.all(query);
	...
	public static function getOrCreateUserForLogin(email:String, name:String):Result<User, Changeset<User, Term>> {
		...
		if (existing != null) {
			...
			return switch (Repo.update(changeset)) {
				case Ok(updated):
					if (pendingInvite != null)
						acceptInvite(pendingInvite, updated.id, now);
					Ok(updated);
		...
		var invitedRole = pendingInvite != null
			&& pendingInvite.role != null
			&& StringTools.trim(pendingInvite.role) != "" ? pendingInvite.role : "user";
		var role = isFirstUserInOrg ? "admin" : invitedRole;
