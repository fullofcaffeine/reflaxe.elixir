...
class GithubOAuth {
	...
	public static function defaultRedirectUri(conn:plug.Conn<{}>):String {
		...
		if (redirectUriOverride != null && redirectUriOverride.trim() != "")
	...
	public static function buildAuthorizeUrl(conn:plug.Conn<{}>, state:String):Result<String, String> {
		...
		if (clientId == null || clientId.trim() == "")
			return Error("GitHub OAuth is not configured (missing GITHUB_CLIENT_ID).");
	...
	public static function authenticate(conn:plug.Conn<{}>, code:String):Result<GithubIdentity, String> {
		...
		if (clientSecret == null || clientSecret.trim() == "")
			return Error("GitHub OAuth is not configured (missing GITHUB_CLIENT_SECRET).");
		...
		return switch (userLookupResult) {
			case Ok(user):
				...
				if (login == null || login.trim() == "")
					return Error("GitHub user lookup failed (missing login).");
				...
				if (email == null || email.trim() == "") {
					...
					var finalEmail = (resolvedEmail != null && resolvedEmail.trim() != "") ? resolvedEmail : (login + "@users.noreply.github.com");
