...
class CreateUsers extends Migration {
	public function up():Void {
		createTable("users") // User identification and profile fields
			.addColumn("name", ColumnType.String(), {nullable: false})
			.addColumn("email", ColumnType.String(), {nullable: false}) // Authentication fields
			...
			.addColumn("active", ColumnType.Boolean, {defaultValue: true}) // Timestamps

			.addTimestamps() // Indexes for performance

			.addUniqueConstraint(["email"], "users_email_unique")
			.addIndex(["active"])
			.addIndex(["confirmed_at"])
			.addIndex(["last_login_at"]) // Data integrity constraints
