...
class HookRegistry {
	public static macro function build(defs:Expr):ExprOf<DynamicAccess<Hook>> {
		...
		for (d in declared)
			declaredByName.set(d.name, d.value);
		...
		for (d in declared) {
			if (!provided.exists(d.name))
				missing.push(d.name);
		}

		if (missing.length > 0) {
			Context.error("Missing hook implementations for HookName: " + missing.join(", ") + " (add them to HookRegistry.build(...))", pos);
		}
