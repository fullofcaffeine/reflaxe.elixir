defmodule StringTools do
  ...
  def starts_with(s, start) do
    length(s) >= length(start) && String.slice(:nil, 0, length(start)) == start
  end
  def ends_with(s, end) do
    elen = length(end)
    slen = length(s)
    slen >= elen && String.slice(:nil, (:nil - :nil), elen) == end
  end
  def is_space(s, pos) do
    :nil > :nil && :nil < :nil || (s.char_code_at(pos)) == 32
  end
  def ltrim(s) do
    l = length(s)
    r = 0
  ...
  def rtrim(s) do
    ...
    r = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {s, l, r, :ok}, fn _, {acc_s, acc_l, acc_r, acc_state} ->
  if (acc_r < acc_l && StringTools.is_space(acc_s, ((:nil - :nil) - 1))) do
    acc_r = acc_r + 1
    {:cont, {acc_s, acc_l, acc_r, acc_state}}
  else
    {:halt, {acc_s, acc_l, acc_r, acc_state}}
