defmodule StringTools do
  ...
  if (acc_g < acc_g1) do
    ...
    if (c >= 65 && c <= 90 || c >= 97 && c <= 122 || c >= 48 && c <= 57 || c == 45 || c == 95 || c == 46 || c == 126) do
      ...
      acc_result = acc_result <> "%" <> StringTools.hex(c, 2).toUpperCase()
    end
  ...
  if (acc_i < acc_s.length) do
    ...
    if (c == "%") do
      if (acc_i + 2 < acc_s.length) do
        ...
        code = StringTools.parse_int("0x" <> hex)
        if (code != nil) do
          throw(:continue)
        end
      end
  ...
  def lpad(s, c, l) do
    if (c.length <= 0), do: s
    buf = ""
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {s, l, buf, :ok}, fn _, {acc_s, acc_l, acc_buf, acc_state} ->
  if (acc_buf.length + acc_s.length < acc_l), do: {:cont, {acc_s, acc_l, acc_buf <> c, acc_state}}, else: {:halt, {acc_s, acc_l, acc_buf, acc_state}}
end)
    buf <> s
  end
  def rpad(s, c, l) do
  ...
  if (acc_buf.length < acc_l), do: {:cont, {acc_buf <> c, acc_l, acc_state}}, else: {:halt, {acc_buf, acc_l, acc_state}}
end)
    buf
  end
  ...
  if (acc_s.length < acc_digits), do: {:cont, {acc_digits, "0" <> acc_s, acc_state}}, else: {:halt, {acc_digits, acc_s, acc_state}}
end)
    end
    s
  end
  ...
  def quote_regexp_meta(s) do
    special_chars = ["\\", "^", "$", ".", "|", "?", "*", "+", "(", ")", "[", "]", "{", "}"]
    g = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {s, g, special_chars, :ok}, fn _, {acc_s, acc_g, acc_special_chars, acc_state} ->
  if (acc_g < acc_special_chars.length) do
    char = special_chars[g]
    acc_g = acc_g + 1
    {:cont, {acc_s, acc_g, acc_special_chars, acc_state}}
  else
    {:halt, {acc_s, acc_g, acc_special_chars, acc_state}}
  end
end)
    s
  end
