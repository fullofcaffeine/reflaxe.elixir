defmodule StringTools do
  ...
  def url_decode(s) do
    result = ""
    i = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {result, s, i, :ok}, fn _, {acc_result, acc_s, acc_i, acc_state} -> nil end)
    result
  end
  ...
  def lpad(s, c, l) do
    if (length(c) <= 0), do: s
    buf = ""
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {buf, l, s, :ok}, fn _, {acc_buf, acc_l, acc_s, acc_state} -> nil end)
    buf <> s
  end
  def rpad(s, c, l) do
    if (length(c) <= 0), do: s
