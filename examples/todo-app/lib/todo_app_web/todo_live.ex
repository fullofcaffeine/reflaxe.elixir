defmodule TodoAppWeb.TodoLive do
  ...
  def mount(_params, session, socket) do
    now = Date.now()
    Log.trace("Current time: " <> DateTime.to_iso8601(now.datetime), %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 108, :class_name => "server.live.TodoLive", :method_name => "mount"})
      ...
      {:ok, _} ->
        ...
        case (g) do
          ...
          {:error, _} ->
            g = elem(g, 1)
            reason = g
            Log.trace("Failed to broadcast todo update: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 348, :class_name => "server.live.TodoLive", :method_name => "toggleTodoStatus"})
        end
      ...
      {:ok, _} ->
        ...
        case (g) do
          ...
          {:error, _} ->
            g = elem(g, 1)
            reason = g
            Log.trace("Failed to broadcast todo deletion: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 370, :class_name => "server.live.TodoLive", :method_name => "deleteTodo"})
        end
        remove_todo_from_list(id, socket)
      {:error, _} ->
        g = elem(g, 1)
      ...
      {:ok, _} ->
        ...
        case (g) do
          ...
          {:error, _} ->
            g = elem(g, 1)
            reason = g
            Log.trace("Failed to broadcast todo priority update: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 394, :class_name => "server.live.TodoLive", :method_name => "updateTodoPriority"})
        end
        update_todo_in_list(updated_todo, socket)
      {:error, _} ->
        g = elem(g, 1)
  ...
  def get_user_from_session(session) do
    ...
    %{:id => (if (user_id != nil), do: user_id, else: 1), :name => "Demo User", :email => "demo@example.com", :password_hash => "hashed_password", :confirmed_at => nil, :last_login_at => nil, :active => true}
  end
  ...
  def complete_all_todos(socket) do
    ...
    case (g) do
      ...
      {:error, _} ->
        g = elem(g, 1)
        reason = g
        Log.trace("Failed to broadcast bulk complete: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 540, :class_name => "server.live.TodoLive", :method_name => "completeAllTodos"})
    end
      ...
      {:ok, _} ->
        ...
        case (g) do
          ...
          {:error, _} ->
            g = elem(g, 1)
            reason = g
            Log.trace("Failed to broadcast todo update: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 621, :class_name => "server.live.TodoLive", :method_name => "saveEditedTodoTyped"})
        end
        presence_socket = TodoAppWeb.Presence.update_user_editing(socket, socket.assigns.current_user, nil)
      ...
      {:ok, _} ->
        ...
        case (g) do
          ...
          {:error, _} ->
            g = elem(g, 1)
            reason = g
            Log.trace("Failed to broadcast todo save: " <> reason, %{:file_name => "src_haxe/server/live/TodoLive.hx", :line_number => 658, :class_name => "server.live.TodoLive", :method_name => "saveEditedTodo"})
        end
