defmodule TodoAppWeb.UserLive do
  ...
  defp handle_event(event, params, socket) do
    case (event) do
      ...
      "delete_user" ->
        handle_delete_user(params, socket)
      "edit_user" ->
        handle_edit_user(params, socket)
      "new_user" ->
        handle_new_user(params, socket)
      "save_user" ->
        handle_save_user(params, socket)
      "search" ->
        handle_search(params, socket)
      _ ->
  ...
  Users.update_user(selected_user, user_params)
    ...
    case (g) do
      "error" ->
        %{:status => "noreply", :socket => assign(socket, "changeset", result[:changeset])}
      "ok" ->
        users = Users.list_users(nil)
        show_form = false
  ...
  defp handle_delete_user(params, socket) do
    ...
    if (result[:status] == "ok") do
      users = Users.list_users(nil)
      %{:status => "noreply", :socket => assign(socket, "users", users)}
    end
    %{:status => "noreply", :socket => socket}
  end
