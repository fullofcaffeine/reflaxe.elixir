defmodule TodoAppWeb.UserLive do
  ...
  defp handle_event(event, socket) do
    ...
    case (event) do
      {:new_user} ->
        temp_result = TodoAppWeb.UserLive.handle_new_user(live_socket)
      {:edit_user, id} ->
        id = g
        temp_result = TodoAppWeb.UserLive.handle_edit_user(id, live_socket)
      {:save_user, params} ->
        params = g
        temp_result = TodoAppWeb.UserLive.handle_save_user(params, live_socket)
      {:delete_user, id} ->
        id = g
        temp_result = TodoAppWeb.UserLive.handle_delete_user(id, live_socket)
      {:search, params} ->
        params = g
        temp_result = TodoAppWeb.UserLive.handle_search(params.search_term, live_socket)
      {:filter_status, params} ->
        params = g
        temp_result = TodoAppWeb.UserLive.handle_filter_status(params.status, live_socket)
      {:clear_search} ->
        temp_result = TodoAppWeb.UserLive.handle_clear_search(live_socket)
      {:cancel} ->
        temp_result = TodoAppWeb.UserLive.handle_cancel(live_socket)
    end
    temp_result
  end
  ...
  defp handle_save_user(params, socket) do
    ...
    if (selected_user == nil) do
      temp_result = Users.create_user(user_params)
    else
      temp_result = Users.update_user(selected_user, user_params)
    end
    temp_result2 = nil
    case (temp_result) do
      {:ok, user} ->
        user = g
        users = Users.list_users(nil)
        temp_result2 = %{:status => "noreply", :socket => Phoenix.Component.assign([socket, users, false, nil, Users.change_user(nil)], %{:users => {1}, :show_form => {2}, :selected_user => {3}, :changeset => {4}})}
      {:error, changeset} ->
        changeset = g
  ...
  defp handle_delete_user(user_id, socket) do
    ...
    case (result) do
      {:ok, deleted_user} ->
        deleted_user = g
        users = Users.list_users(nil)
        temp_result = %{:status => "noreply", :socket => Phoenix.Component.assign(socket, :users, users)}
      {:error, changeset} ->
        changeset = g
  ...
  defp handle_search(search_term, socket) do
    ...
    if (length(search_term) > 0) do
      temp_maybe_struct = %{:name => search_term, :email => search_term, :is_active => nil}
    else
      temp_maybe_struct = nil
    end
    filter = temp_maybe_struct
