defmodule TodoPubSub do
  ...
  def message_to_elixir(message) do
    ...
    case (message) do
      ...
      {:bulk_update, _action} ->
        action = g
        temp_struct = %{:type => "bulk_update", :action => :nil.bulk_action_to_string(action)}
    ...
    Phoenix.SafePubSub.add_timestamp(:nil)
  end
  def parse_message_impl(msg) do
    ...
    case (g) do
      ...
      "todo_deleted" ->
        if (msg.todo_id != nil), do: :nil, else: :nil
      "todo_updated" ->
        if (msg.todo != nil), do: :nil, else: :nil
      "user_offline" ->
        if (msg.user_id != nil), do: :nil, else: :nil
      "user_online" ->
        if (msg.user_id != nil), do: :nil, else: :nil
      _ ->
