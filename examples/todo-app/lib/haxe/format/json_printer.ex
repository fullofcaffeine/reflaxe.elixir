defmodule JsonPrinter do
  defp write_value(v, key) do
    v = if (self.replacer != nil), do: self.replacer(key, v), else: v
    if (v == nil), do: "null"
    if (Std.is(v, Bool)) do
      temp_result = nil
      if v do
        temp_result = "true"
      else
        temp_result = "false"
      end
      tempResult
    end
    if (Std.is(v, Int)) do
      Std.string(v)
    end
    if (Std.is(v, Float)) do
      s = Std.string(v)
      if (:nil == :nil || :nil == :nil || s == "-Infinity"), do: "null"
      s
    end
    if (Std.is(v, String)), do: struct.quote_string(v)
    if (Std.is(v, Array)), do: struct.write_array(v)
    struct.write_object(v)
  end
  defp write_array(arr) do
    items = []
    g = 0
    g1 = length(arr)
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {g, g1, :ok}, fn _, {acc_g, acc_g1, acc_state} ->
  if (g < g1) do
    i = g = g + 1
    items = items ++ [self.write_value(arr[i], Std.string(i))]
    {:cont, {acc_g, acc_g1, acc_state}}
  else
    {:halt, {acc_g, acc_g1, acc_state}}
  end
end)
    if (self.space != nil && length(items) > 0) do
      "[\n  " <> Enum.join(:nil, :nil) <> "\n]"
    else
      "[" <> Enum.join(:nil, :nil) <> "]"
    end
  end
  defp write_object(obj) do
    fields = Map.keys(obj)
    pairs = []
    g = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {fields, g, :ok}, fn _, {acc_fields, acc_g, acc_state} ->
  if (g < length(acc_fields)) do
    field = acc_fields[_g]
    g = g + 1
    value = Map.get(obj, String.to_atom(field))
    key = self.quote_string(field)
    val = self.write_value(value, field)
    if (self.space != nil) do
      pairs = pairs ++ [key <> ": " <> val]
    else
      pairs = pairs ++ [key <> ":" <> val]
    end
    {:cont, {acc_fields, acc_g, acc_state}}
  else
    {:halt, {acc_fields, acc_g, acc_state}}
  end
end)
    if (self.space != nil && length(pairs) > 0) do
      "{\n  " <> Enum.join(:nil, :nil) <> "\n}"
    else
      "{" <> Enum.join(:nil, :nil) <> "}"
    end
  end
  defp quote_string(s) do
    result = "\""
    g = 0
    g1 = length(s)
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {result, g, g1, :ok}, fn _, {acc_result, acc_g, acc_g1, acc_state} ->
  if (g < g1) do
    i = g = g + 1
    c = s.char_code_at(i)
    if (c == nil), do: nil, else: nil
    {:cont, {acc_result, acc_g, acc_g1, acc_state}}
  else
    {:halt, {acc_result, acc_g, acc_g1, acc_state}}
  end
end)
    result = result <> "\""
    result
  end
  def write(k, v) do
    struct.write_value(v, k)
  end
  def print(o, replacer, space) do
    (JsonPrinter.new(replacer2, space2)).write_value(o, "")
  end
end