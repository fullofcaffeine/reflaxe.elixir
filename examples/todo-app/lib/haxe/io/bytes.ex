defmodule Bytes do
  @length nil
  @b nil
  def get_string(_struct, _pos, _len, _encoding) do
    if (encoding == nil) do
      encoding = {:UTF8}
    end
  ...
  def blit(_struct, _pos, _src, _srcpos, _len) do
  ...
  def fill(_struct, _pos, _len, _value) do
    if (pos < 0 || len < 0 || pos + len > length(struct)) do
      throw("Out of bounds")
    end
  ...
  def get_int32(_struct, _pos) do
    if (pos < 0 || pos + 4 > length(struct)) do
      throw("Out of bounds")
    end
    <<value::little-signed-size(32)>> = :binary.part(struct.b, pos, 4); value
  end
  def set_int32(_struct, _pos, _v) do
    if (pos < 0 || pos + 4 > length(struct)) do
      throw("Out of bounds")
    end
    before_part = if (pos > 0) do
  ...
  def get_int64(_struct, _pos) do
    if (pos < 0 || pos + 8 > length(struct)) do
      throw("Out of bounds")
    end
    <<value::little-signed-size(64)>> = :binary.part(struct.b, pos, 8); value
  end
  def set_int64(_struct, _pos, _v) do
    if (pos < 0 || pos + 8 > length(struct)) do
      throw("Out of bounds")
    end
    before_part = if (pos > 0) do
  ...
  def read_string(_struct, _pos, _len) do
    :unicode.characters_to_list(struct, :utf8)
  end
