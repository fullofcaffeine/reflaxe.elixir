...
defmodule Users do
  ...
  # Static functions
  @doc "Get all users with optional filtering"
  @spec list_users(Null.t()) :: Array.t()
  def list_users(filter) do
    []
  end

  @doc "Create changeset for user (required by LiveView example)"
  @spec change_user(Null.t()) :: term()
  def change_user(user) do
    %{valid: true}
  end

  @doc "Main function for compilation testing"
  @spec main() :: nil
  def main() do
  ...
  @doc "Get user by ID with error handling"
  @spec get_user(integer()) :: User.t()
  def get_user(id) do
    nil
  end

  @doc "Get user by ID, returns null if not found"
  ...
  @doc "Delete user (soft delete by setting active: false)"
  ...
  @doc "Get users with their posts (preload association)"
