defmodule Phoenix.SafePubSub do
  def subscribe_with_converter(topic, topic_converter) do
    ...
    subscribe_result = Phoenix.PubSub.subscribe(pubsubModule, topicString)
    is_ok = subscribeResult == :ok
    if isOk do
      {:ok, nil}
    else
      error_reason = 
                case subscribeResult do
  ...
  def is_valid_message(msg) do
    :nil != :nil && Map.has_key?(msg, String.to_atom("type")) && Map.get(msg, String.to_atom("type")) != nil
  end
  ...
  def create_malformed_message_error(msg) do
    ...
    "Malformed PubSub message: " <> tempString <> ". Expected message with \"type\" field."
  end
