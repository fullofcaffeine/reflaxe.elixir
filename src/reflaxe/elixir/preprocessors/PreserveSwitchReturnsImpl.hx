...
class PreserveSwitchReturnsImpl extends BasePreprocessor {
	/**
	 * Gives every compiler-created temporary a distinct internal Haxe variable identity.
	 *
	 * A synthetic variable is a local variable that this preprocessor inserts even though the user
	 * did not write it. For example, `return switch (...)` temporarily becomes “store the switch
	 * result in a local, then return that local.” Later compiler steps use the negative `TVar.id` to
	 * connect each reference to the right inserted declaration. Reusing an ID could make two such
	 * locals appear to be the same variable.
	 *
	 * This process-wide counter must not also determine the printed Elixir name. A persistent Haxe
	 * compilation server may reuse previously typed modules, so two equivalent builds can visit a
	 * different number of functions before reaching the same source function. Without the separate
	 * function-local counter below, a fresh build could print `switch_result_27` while a warm-server
	 * build prints `switch_result_8`. The program would behave the same, but generated bytes would
	 * be unstable and could trigger needless downstream recompilation.
	 */
	static var syntheticVariableIdCounter = 0;

	/**
	 * Numbers visible switch-result temporaries within the function currently being processed.
	 *
	 * Elixir locals need to be unique only inside their lexical function. Restarting this counter
	 * for every `ClassFuncData` makes the generated name depend on that function's own source shape,
	 * not on which unrelated modules the Haxe server happened to reuse.
	 */
	var functionTempCounter = 0;
			...
			case TReturn(e) if (e != null):
				...
				switch (innerExpr.expr) {
					case TSwitch(scrutinee, cases, defaultCase):
						...
						// Printed names are function-local so fresh and warm-server builds agree.
						var varName = "switch_result_" + (++functionTempCounter);
						var syntheticVariableId = -(++syntheticVariableIdCounter);

						// Create a typed variable to hold the switch result
						var tvar:TVar = {
							id: syntheticVariableId,
							name: varName,
