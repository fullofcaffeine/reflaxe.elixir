...
class EctoSchemaAssociationValidator {
	...
	public static function ensureAfterTypingHook():Void {
		if (afterTypingRegistered)
			return;
		if (Context.defined("ecto_no_assoc_validation"))
			return;
	...
	static function validateAllTypedModules(modules:Array<ModuleType>):Void {
		...
		for (schema in index.schemas) {
			if (!schemaValidationEnabled(schema))
				continue;
	...
	static function schemaValidationEnabled(schema:ClassType):Bool {
		if (schema == null || schema.meta == null)
			return false;
		if (schema.meta.has(":ecto_no_assoc_validation") || schema.meta.has("ecto_no_assoc_validation"))
			return false;
		return true;
	}
	...
	static function detectAssociationKind(field:ClassField):Null<AssocKind> {
		...
		if (field.meta.has(":belongs_to"))
			return BelongsTo;
		if (field.meta.has(":has_many"))
			return HasMany;
		if (field.meta.has(":has_one"))
			return HasOne;
		if (field.meta.has(":many_to_many"))
			return ManyToMany;
		return null;
	}
				...
				switch (params[i].expr) {
					case EObjectDecl(fields):
						for (f in fields) {
							if (f == null)
								continue;
							if (f.field != "foreign_key" && f.field != "foreignKey")
								continue;
							switch (f.expr.expr) {
	...
	static function validateBelongsTo(schema:ClassType, field:ClassField, args:AssocArgs, schemaFieldSnakes:Map<String, Bool>, warnOnly:Bool,
			pos:Position):Void {
		var expectedFk = args.foreignKeyOverride != null ? args.foreignKeyOverride : (args.assocName + "_id");
		var expectedFkSnake = NameUtils.toSnakeCase(expectedFk);

		if (!schemaFieldSnakes.exists(expectedFkSnake)) {
			reportProblem(warnOnly,
	...
	static function validateHasSide(schema:ClassType, field:ClassField, kind:AssocKind, args:AssocArgs, index:TypeIndex, warnOnly:Bool, pos:Position):Void {
		var expectedFkOnTarget = args.foreignKeyOverride != null ? args.foreignKeyOverride : (NameUtils.toSnakeCase(schema.name) + "_id");
		...
		if (target.meta == null || !target.meta.has(":schema")) {
			Context.warning('Ecto association validation: ${schema.name}.${field.name} targets `${target.name}` but that type is not annotated with `@:schema`; skipping cross-schema foreign key validation.',
				pos);
			return;
		}
	...
	static function resolveAssociationTarget(schema:ClassType, field:ClassField, kind:AssocKind, args:AssocArgs, index:TypeIndex,
			pos:Position):Null<ClassType> {
		var targetFromType = resolveAssociationTargetFromFieldType(kind, field.type);
		if (targetFromType != null)
			return targetFromType;

		if (args.targetHint != null) {
			var resolved = resolveClassByHint(args.targetHint, index, pos);
			if (resolved != null)
				return resolved;
			Context.warning('Ecto association validation: ${schema.name}.${field.name} references target `${args.targetHint}` but it could not be resolved to a Haxe type in this compilation; skipping cross-schema foreign key validation.',
				pos);
			return null;
		}

		Context.warning('Ecto association validation: ${schema.name}.${field.name} uses a dynamic/unresolvable target type; skipping cross-schema foreign key validation.',
			pos);
		return null;
	}
	...
	static function resolveClassByHint(hint:String, index:TypeIndex, pos:Position):Null<ClassType> {
		...
		Context.warning('Ecto association validation: type name `${hint}` is ambiguous; candidates: ${candidates.join(", ")}. Skipping cross-schema validation.',
			pos);
		return null;
	}
	...
	static function classFqcn(cls:ClassType):String {
		if (cls == null)
			return "";
		return cls.pack != null && cls.pack.length > 0 ? cls.pack.join(".") + "." + cls.name : cls.name;
	}
}
#end
