...
class StrictModeEnforcer {
	...
	static function enforceNoDynamicType(type:Type, pos:haxe.macro.Expr.Position, label:String):Void {
		...
		Context.error("Strict mode forbids `Dynamic` in application types ("
			+ label
			+ "). "
	...
	static function containsDynamic(type:Type):Bool {
		return switch (type) {
			...
			case TMono(monoRef): var resolved = monoRef.get(); resolved != null && containsDynamic(resolved);
		}
	}
		...
		switch (expr.expr) {
			case TMeta(meta, inner):
				...
				if (metaName == ":untyped" || metaName == "untyped") {
					Context.error("Strict mode forbids `untyped` in application code (including `untyped __elixir__()` injections). "
				...
				if (isElixirInjectionCall(inner)) {
					Context.error("Strict mode forbids `__elixir__()` code injection in application code. "
