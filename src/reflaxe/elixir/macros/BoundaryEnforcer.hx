...
class BoundaryEnforcer {
	public static function init():Void {
		...
		// For user projects, enable `-D reflaxe_elixir_strict` (see StrictModeEnforcer) to apply
		// similar guardrails across project-local sources.
	...
	static function enforceExternClassRules(classType:ClassType):Void {
		...
		Context.error("Extern classes are not allowed in example app sources. "
	...
	static function scanForElixirInjection(expr:TypedExpr):Void {
		if (isElixirInjectionCall(expr)) {
			Context.error("`__elixir__()` code injection is disallowed in example app sources. "
