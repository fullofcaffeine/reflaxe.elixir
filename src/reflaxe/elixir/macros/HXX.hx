...
class HXX {
	...
	public static macro function block(content:Expr):Expr {
		...
		if (mode == HxxMode.Tsx) {
			Context.error("HXX: block('...') is not allowed in @:hxx_mode(\"tsx\") code.\n"
	...
	static function allowRawHeexRequestedByMeta():Bool {
		...
		for (f in cls.statics.get())
			if (f != null && f.name == methodName && fieldHasAllowHeex(f))
				return true;
	...
	static function hxxInternal(templateStr:Expr):Expr {
		return switch (templateStr.expr) {
			case EConst(CString(s, _)):
				var mode = HxxModeResolver.resolveFromMacroContext();
				if (mode == HxxMode.Tsx) {
					Context.error("HXX: string templates (hxx('...')) are not allowed in @:hxx_mode(\"tsx\") code.\n"
						+ "Use inline markup (`return <div>...</div>`) or `phoenix.hxx.HeexTemplate.root(...)` instead.",
						templateStr.pos);
				}
				...
				if (containsRawHeexMarkers(s)) {
					...
					if (!allowRawHeex) {
						Context.error("Raw HEEx markers (<% ... %>) are not allowed inside hxx('...') by default.\n"
	...
	static function rewriteTopLevelTernary(expr:String):String {
		...
		while (i < expr.length) {
			...
			if (ch == "(")
				...
				parenDepth = Std.int(Math.max(0, parenDepth - 1));
			else if (ch == "[")
				bracketDepth++;
			else if (ch == "]")
				bracketDepth = Std.int(Math.max(0, bracketDepth - 1));
			else if (ch == "{")
				braceDepth++;
			else if (ch == "}")
				braceDepth = Std.int(Math.max(0, braceDepth - 1));
			else if (parenDepth == 0 && bracketDepth == 0 && braceDepth == 0) {
				if (questionIndex == -1 && ch == "?")
					questionIndex = i;
				else if (questionIndex != -1 && ch == ":") {
					colonIndex = i;
					break;
				}
			}
		...
		if (cond.length == 0 || thenPart.length == 0 || elsePart.length == 0)
			return expr;
	...
	static function processTemplateStringInternal(template:String, ?pos:haxe.macro.Expr.Position):String {
		...
		processed = interp.map(processed, function(re) {
			...
			if (expr.length >= 2) {
				...
				if ((first == "\"" || first == "'") && expr.length >= 2) {
					...
					var idx = 1;
					while (idx < expr.length && ~/^\s$/.match(expr.charAt(idx)))
						idx++;
					if (idx < expr.length && expr.charAt(idx) == '<') {
						#if macro
						haxe.macro.Context.error("HXX: injecting HTML via string inside ${...} is not allowed. Use inline markup or HXX.block('...') as a deliberate escape hatch.",
							pos != null ? pos : haxe.macro.Context.currentPos());
						#end
					}
				}
			}
		...
		processed = eexIf.map(processed, function(re) {
			...
			if (!(StringTools.startsWith(th, "\"") && StringTools.endsWith(th, "\""))
				&& !(StringTools.startsWith(th, "'") && StringTools.endsWith(th, "'")))
				th = "\"" + th + "\"";
			if (!(StringTools.startsWith(el, "\"") && StringTools.endsWith(el, "\""))
				&& !(StringTools.startsWith(el, "'") && StringTools.endsWith(el, "'")))
				el = "\"" + el + "\"";
