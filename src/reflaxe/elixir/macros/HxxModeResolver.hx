...
class HxxModeResolver {
    ...
    public static function extractModeFromMetaEntries(entries: Null<Array<MetadataEntry>>): Null<HxxMode> {
        ...
        for (e in entries) {
            if (e == null) continue;
            if (e.name != META_NAME && e.name != "hxx_mode") continue;
    ...
    static function findMethodField(cls: ClassType, methodName: String): Null<ClassField> {
        if (cls == null || methodName == null || methodName.length == 0) return null;
        for (f in cls.statics.get()) if (f != null && f.name == methodName) return f;
        for (f in cls.fields.get()) if (f != null && f.name == methodName) return f;
        return null;
    }
    ...
    public static function resolveFromTypes(cls: Null<ClassType>, field: Null<ClassField>): HxxMode {
        ...
        if (defined != null) return parseModeString(defined, Context.currentPos());

        return HxxMode.Balanced;
    }

    public static function hasAllowHeexMeta(cls: Null<ClassType>, field: Null<ClassField>): Bool {
        if (field != null && field.meta != null && field.meta.has(":allow_heex")) return true;
        if (cls != null && cls.meta != null && cls.meta.has(":allow_heex")) return true;
        return false;
    }
    ...
    public static function allowRawHeexMarkers(mode: HxxMode, allowHeexRequested: Bool, globalAllowHeex: Bool): Bool {
        return switch (mode) {
            case Metal:
                true;
            case Balanced:
                allowHeexRequested || globalAllowHeex;
            case Tsx:
                false;
            default:
                false;
