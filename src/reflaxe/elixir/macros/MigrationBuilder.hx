...
class MigrationBuilder {
	...
	public static macro function build():Array<Field> {
		...
		if (Context.defined("ecto_migrations_exs")) {
			...
			for (field in fields) {
				if (field.name != "up" && field.name != "down")
					continue;
				if (field.meta == null)
					field.meta = [];
		...
		if (migrationInfo.timestamp != null) {
			...
			Context.error('Missing migration timestamp for ${localClass.name}. Provide one via @:migration({timestamp: \"20240101120000\"}) or generate with `mix haxe.gen.migration`.',
				localClass.pos);
		}
			...
			switch (step.name) {
				case "addId":
					if (Context.defined("ecto_migrations_exs") && step.args.length > 0) {
						Context.error("addId(...) with custom options is not supported in ecto_migrations_exs builds. Use addId() or a manual Elixir migration.",
							step.pos);
					}
