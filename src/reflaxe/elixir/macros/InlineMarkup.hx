...
class InlineMarkup {
    ...
    public static macro function build(): Array<Field> {
        ...
        for (f in fields) rewriteField(f, classMode);
        return fields;
    }
    ...
    static function fieldUsesLegacyRewrite(field: Field): Bool {
        if (field != null && field.meta != null) {
            for (m in field.meta) {
                if (m == null) continue;
                if (m.name == ":hxx_legacy" || m.name == "hxx_legacy") return true;
            }
        }
    ...
    static function rewriteField(field: Field, classMode: HxxMode): Void {
        ...
        if (mode == HxxMode.Tsx && legacy) {
            Context.error(
                "Inline markup: @:hxx_legacy is not allowed in @:hxx_mode(\"tsx\") code.\n" +
        ...
        switch (field.kind) {
            case FFun(fn):
                if (fn != null && fn.expr != null) fn.expr = rewriteExpr(fn.expr, false, mode, legacy);
            case FVar(t, e):
                if (e != null) field.kind = FVar(t, rewriteExpr(e, false, mode, legacy));
            case FProp(get, set, t, e):
                if (e != null) field.kind = FProp(get, set, t, rewriteExpr(e, false, mode, legacy));
        }
    }
    ...
    static function validateTsxInlineMarkupPayload(payload: String, payloadPos: Position): Void {
        ...
        if (payload.indexOf("<%") != -1) {
            Context.error(
                "Inline markup (tsx): raw `<% ... %>` blocks are not allowed.\n" +
                "Use Haxe expressions inside `${ ... }` instead.",
                payloadPos
            );
        }
        if (payload.indexOf("#{") != -1) {
            Context.error(
                "Inline markup (tsx): `#{ ... }` interpolation is not allowed.\n" +
