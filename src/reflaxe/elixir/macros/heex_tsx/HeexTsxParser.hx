...
private class Parser {
	...
	function readName():String {
		skipWs();
		if (eof())
			failHere("Expected name");
		var start = i;
		while (!eof()) {
			var c = ch(i);
			var ok = (c >= "a" && c <= "z") || (c >= "A" && c <= "Z") || (c >= "0" && c <= "9") || c == "_" || c == "-" || c == ":" || c == "."; // include . for Phoenix components
			if (!ok)
				break;
			i++;
		}
	...
	function parseTextNodesUntilTag():Array<Expr> {
		...
		if (lit.length > 0)
			out.push(mkText(lit, makeSubPos(textStart, i)));
		...
		while (startsWith("${")) {
			...
			if (seg.length > 0)
				out.push(mkText(seg, makeSubPos(segStart, i)));
		}
	...
	public function parseNodesUntil(closing:Null<String>):Array<Expr> {
		...
		while (!eof()) {
			...
			if (startsWith("<!--")) {
				...
				if (end == -1)
					Context.error("Unclosed HTML comment", makeSubPos(cStart, src.length));
				i = end + 3;
				continue;
			}
	...
	function parseNodesUntilStop(shouldStop:() -> Bool):Array<Expr> {
		...
		while (!eof() && !shouldStop()) {
			...
			if (startsWith("<else>") || startsWith("</else>") || startsWith("</if>"))
				break;
			...
			if (startsWith("=")) {
				...
				if (attrName == ":for" && (startsWith("${") || startsWith("{"))) {
					...
					var token = readUntil((at) -> {
						...
						|| c == "\r"
						|| c == ">"
						|| (c == "/" && at + 1 < src.length && src.charAt(at + 1) == ">");
					});
