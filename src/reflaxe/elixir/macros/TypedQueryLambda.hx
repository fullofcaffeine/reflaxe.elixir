...
class TypedQueryLambda {
	...
	public static function buildOrderByExpr(ethis:Expr, ordering:Expr):Expr {
		...
		var specs:Array<Expr> = switch (unwrapToExpr(body).expr) {
			...
			default:
				Context.error('orderBy() must return an array literal like `[{field: t.id, direction: SortDirection.Desc}]`', body.pos);
				[];
		};
		...
		for (spec in specs) {
			switch (unwrapToExpr(spec).expr) {
				...
				default:
					Context.error('orderBy() specs must be objects like `{field: t.id, direction: SortDirection.Desc}`', spec.pos);
			}
		}
	...
	private static function opToString(op:haxe.macro.Binop):String {
		return switch (op) {
			...
			case OpGte: '>=';
			default:
				Context.error('Unsupported operator', Context.currentPos());
				'';
		}
	}
	...
	private static function combineBool(op:String, left:{template:String, values:Array<Expr>},
			right:{template:String, values:Array<Expr>}):{template:String, values:Array<Expr>} {
	...
	private static function extractLambdaBody(expr:Expr):Expr {
		var body:Expr = switch (expr.expr) {
			...
			default:
				Context.error('Expected lambda function for where()', expr.pos);
				expr;
		};
	...
	private static function parseSortDirection(expr:Null<Expr>):String {
		...
		return switch (normalizedExpr.expr) {
			...
			case EField(_, name):
				parseSortDirectionValue(name, normalizedExpr.pos);
			default:
				Context.error('orderBy() direction must be SortDirection.Asc/Desc (or "asc"/"desc")', normalizedExpr.pos);
				"asc";
		}
	}
		...
		return switch (t) {
			case TAbstract(_, params) if (params != null && params.length > 0):
				switch (params[0]) {
					...
					default:
						Context.error('Unable to infer schema type parameter for TypedQuery<T>', expr.pos);
						null;
				}
			default:
