...
class TypedExprPreprocessor {
	...
	static function containsInfrastructurePattern(expr:TypedExpr):Bool {
		...
		return switch (expr.expr) {
			...
			case TCast(e, _):
				containsInfrastructurePattern(e);
			case TVar(v, init): // This preprocessor is triggered by infra-temp declarations, but we still
				// need to recurse into non-infra TVar initializers (they may contain blocks).
				(init != null && isInfrastructureVar(v.name)) || (init != null && containsInfrastructurePattern(init));
			case TBlock(exprs):
				Lambda.exists(exprs, e -> containsInfrastructurePattern(e));
			case TReturn(e) if (e != null):
				containsInfrastructurePattern(e);
			case TFunction(func): func.expr != null && containsInfrastructurePattern(func.expr);
			case TIf(cond, e1, e2): containsInfrastructurePattern(cond) || containsInfrastructurePattern(e1) || (e2 != null
					&& containsInfrastructurePattern(e2));
			case TSwitch(e, cases, edef): containsInfrastructurePattern(e) || Lambda.exists(cases,
					c -> Lambda.exists(c.values, v -> containsInfrastructurePattern(v))
						|| containsInfrastructurePattern(c.expr)) || (edef != null && containsInfrastructurePattern(edef));
			case TTry(e, catches): containsInfrastructurePattern(e) || Lambda.exists(catches, c -> containsInfrastructurePattern(c.expr));
			case TWhile(cond, e, _): containsInfrastructurePattern(cond) || containsInfrastructurePattern(e);
			case TFor(_, iter, e): containsInfrastructurePattern(iter) || containsInfrastructurePattern(e);
			default:
				false;
		};
	}
	 ...
	 */
	static function processSwitchExpr(e:TypedExpr, cases:Array<{values:Array<TypedExpr>, expr:TypedExpr}>, edef:Null<TypedExpr>, pos:haxe.macro.Expr.Position,
			t:Type, substitutions:Map<Int, TypedExpr>):TypedExpr {
		#if debug_infrastructure_vars
