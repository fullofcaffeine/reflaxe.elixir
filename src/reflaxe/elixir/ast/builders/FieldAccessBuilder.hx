...
class FieldAccessBuilder {
	...
	static function buildStaticField(e:TypedExpr, classRef:Ref<ClassType>, cf:Ref<ClassField>, context:CompilationContext):Null<ElixirASTDef> {
		...
		if (objAST == null) {
			...
			return ERemoteCall(makeAST(EVar(ModuleBuilder.extractModuleName(classRef.get()))), NameUtils.toSnakeCase(fieldName), []);
		}
	...
	static function buildDynamicField(e:TypedExpr, fieldName:String, context:CompilationContext):Null<ElixirASTDef> {
		...
		var fetchByString = makeAST(ERemoteCall(makeAST(EVar("Map")), "fetch", [objVar, makeAST(EString(snakeField))]));

		var getByAtom = makeAST(ERemoteCall(makeAST(EVar("Map")), "get", [objVar, makeAST(EAtom(snakeField))]));
	 ...
	 */
	static function buildClosure(e:TypedExpr, closureType:Null<{c:Ref<ClassType>, params:Array<Type>}>, cf:Ref<ClassField>,
