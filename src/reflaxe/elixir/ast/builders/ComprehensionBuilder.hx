	...
	public static function tryBuildArrayComprehensionFromBlock(statements:Array<TypedExpr>, context:BuildContext):Null<ElixirAST> {
		...
		if (isComprehensionPattern(statements)) {
			...
			if (data != null) {
							...
							if (iifeStmts != null) {
								if (looksLikeListBuildingBlock(iifeStmts)) {
									...
									if (looseIIFE != null && looseIIFE.length > 0)
										return makeAST(EList(looseIIFE));
								} else {
				...
				], filters, body, null, // into: null for list comprehension
					false // uniq: false
				));
			}
		}
						...
						case TLocal(returnVar) if (returnVar.name == v.name):
							...
							for (mid in middleStmts) {
								switch (unwrapMetaParens(mid).expr) {
									case TBlock(inner):
										for (s in inner)
											switch (unwrapMetaParens(s).expr) {
														...
														var valueAst = (function() {
															switch (push.value.expr) {
																case TBlock(nested) if (looksLikeListBuildingBlock(nested)):
																	...
																	return
																		(loose != null && loose.length > 0) ? makeAST(EList(loose)) : context.getExpressionBuilder()(push.value);
																default:
																	return context.getExpressionBuilder()(push.value);
															}
														})();
														return makeAST(EFor([{pattern: PVar(loopVar.name), expr: context.getExpressionBuilder()(iterator)}],
				...
				case TBinop(OpAssignOp(OpAdd), {expr: TLocal(v)}, {expr: TArrayDecl([value])}) if (v.name == tempVarName):
					...
					var built = switch (value.expr) {
						...
						case TBlock(innerStmts):
							...
							if (nested != null) nested else
								(listFromBlock(innerStmts) != null ? listFromBlock(innerStmts) : context.getExpressionBuilder()(value));
						default:
							context.getExpressionBuilder()(value);
					};
					elements.push(built);
				...
				case TFor(_, iterator, body):
					...
					if (filterAndValue != null) {
						...
						return makeAST(EFor([
							...
							[context.getExpressionBuilder()(filterAndValue.condition)], context.getExpressionBuilder()(filterAndValue.value), null, // into
							false // uniq
						));
					}
				...
				case TBinop(OpAdd, _, {expr: TCall(_, _)}):
					...
					if (iifeSt != null) {
						...
						if (nestedI != null && nestedI.length > 0)
							out.push(makeAST(EList(nestedI)));
					}
				...
				case TBinop(OpAssign, _, {expr: TBinop(OpAdd, _, {expr: TCall(_, _)})}):
					...
					if (iifeSt2 != null) {
						...
						if (nestedI2 != null && nestedI2.length > 0)
							out.push(makeAST(EList(nestedI2)));
					}
