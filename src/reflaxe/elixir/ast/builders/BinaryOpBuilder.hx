...
class BinaryOpBuilder {
	...
	public static function buildBinopFromAST(op:Binop, leftAST:ElixirAST, rightAST:ElixirAST, leftExpr:TypedExpr,
		...
		var def = switch (op) {
			case OpAdd:
				...
				var leftIsString = isStringType(leftExpr.t);
				var rightIsString = isStringType(rightExpr.t);
	 ...
	 */
	public static function buildBinop(op:Binop, leftExpr:TypedExpr, rightExpr:TypedExpr, buildExpr:TypedExpr->ElixirAST, extractPattern:TypedExpr->EPattern,
			toSnakeCase:String->String, metadata:ElixirMetadata = null):ElixirAST {
		// Special handling for field != nil or field == nil comparisons
		var isNilComparison = switch (op) {
			case OpEq | OpNotEq:
				switch (rightExpr.expr) {
		...
		var def = switch (op) {
			case OpAdd:
				...
				var isStringConcat = isStringType(leftExpr.t);

				if (isStringConcat) {
					// For string concatenation, ensure right operand is a string
					var rightStr = if (isStringType(rightExpr.t)) {
						right;
					} else {
			...
			case OpAssign: EMatch(extractPattern(leftExpr), right);

			case OpAssignOp(op2):
				// Transform compound assignment: a += b becomes a = a + b
				var innerOp = if (op2 == OpAdd) {
					// Detect string concatenation
					isStringType(leftExpr.t) ? StringConcat : Add;
				} else {
					convertAssignOp(op2);
				};
				EMatch(extractPattern(leftExpr), makeAST(EBinary(innerOp, left, right)));
