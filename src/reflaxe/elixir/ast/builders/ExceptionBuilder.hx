...
class ExceptionBuilder {
	...
	public static function buildThrow(e:TypedExpr, context:CompilationContext):Null<ElixirASTDef> {
		...
		var attrs = makeAST(EKeywordList([{key: HAXE_THROW_VALUE_FIELD, value: throwExpr}]));
	...
	static function isHaxeExceptionCatchType(t:Null<Type>):Bool {
		...
		return switch (t) {
			...
			case TInst(clsRef, _): var cls = clsRef.get(); cls.pack.length == 1 && cls.pack[0] == "haxe" && cls.name == "Exception";
			default:
				false;
		};
	}
