...
private class Parser {
		...
		if (!selfClosing) {
			...
			while (!eof()) {
				if (peek() == '<' && peek2() == '/') {
					...
					if (txt.length > 0)
						children.push(makeAST(EString(txt)));
				}
			}
		}
	...
	function isTagChar(ch:String):Bool {
		...
		return (c >= 'A'.code && c <= 'Z'.code) || (c >= 'a'.code && c <= 'z'.code) || (c >= '0'.code && c <= '9'.code) || ch == '_' || ch == '-'
			|| ch == '.' || ch == ':';
	}
		...
		while (!eof()) {
			...
			if (consume('=')) {
				...
				if (q == '"' || q == '\'') {
					...
					valueSpanEnd = valueStart + (switch (valueAst.def) {
						case EString(s): s != null ? s.length : 0;
						default: 0;
					});
...
private class ExprParser {
	...
	function parseInlineIf():ElixirAST {
		...
		if (commaElse != -1) {
			...
			if (elseStr != null)
				elseExpr = makeAST(EString(elseStr));
		}
	...
	function parsePostfix(base:ElixirAST):ElixirAST {
		while (true) {
			...
			if (peek() == '[') {
				...
				if (peek() == '"' || peek() == '\'')
					idxAst = makeAST(EString(parseQuoted()));
				else if (peek() == '@') {
					advance(1);
					idxAst = makeAST(EAssign(parseIdent()));
				} else if (isIdentStart(peek()))
					idxAst = makeAST(EVar(parseIdent()));
				else {
	...
	inline function startsWith(t:String):Bool
		return s.substr(i, t.length) == t;

	inline function remaining():String
