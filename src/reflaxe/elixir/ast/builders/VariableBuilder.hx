...
class VariableBuilder {
	static function ensureStableNameForAnonymousLocal(tvar:TVar, context:CompilationContext):Void {
		...
		function isAlreadyUsedValue(candidate:String):Bool {
			for (key in context.tempVarRenameMap.keys()) {
				if (context.tempVarRenameMap.get(key) == candidate)
					return true;
			}
			return false;
		}
		...
		var candidate = (context.anonymousTempVarCounter == 0) ? "_g" : "_g" + context.anonymousTempVarCounter;
	...
	static function deriveStableNameForAnonymousLocalFromInit(tvar:TVar, init:Null<TypedExpr>, context:CompilationContext):Void {
		if (tvar == null || init == null || context == null)
			return;
		if (context.tempVarRenameMap == null)
			context.tempVarRenameMap = new Map();
		...
		if (!isGeneratedPlaceholder(existing))
			return;
			...
			case TField(obj, fa):
				switch (obj.expr) {
					case TLocal(localVar):
						...
						for (key in context.tempVarRenameMap.keys()) {
							if (context.tempVarRenameMap.get(key) == derived)
								return;
						}
	...
	static function isInfrastructureVariable(name:String):Bool {
		return name.startsWith("g_") || name.startsWith("rec_") || name.startsWith("__") || name == "this" || name == "self";
	}
