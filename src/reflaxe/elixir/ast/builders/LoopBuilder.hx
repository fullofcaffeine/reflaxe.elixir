...
class LoopBuilder {
			...
			case EnumEachRange(varName, startExpr, endExpr, body):
				...
				if (accumulation != null) {
					...
					if (pushInfo != null && accumulation.isListAppend) {
						var rangeFor = makeAST(ERange(buildExpr(startExpr), buildExpr(endExpr), false, makeAST(EInteger(1))));
				...
				var range = makeAST(ERange(buildExpr(startExpr), buildExpr(endExpr), false, // inclusive
					makeAST(EInteger(1))));
				...
				#if debug_loop_builder
				#end
				var loopAst = makeAST(ERemoteCall(makeAST(EVar("Enum")), "each", [
					range,
					makeAST(EFn([
						{
							args: [PVar(snakeVar)],
			...
			case EnumEachCollection(varName, collection, body):
				...
				switch (collectionAst.def) {
					case ERemoteCall({def: EVar(mn)}, func, args) if (mn == "Reflect" && func == "fields" && args != null && args.length == 1):
						...
						var loopAst = makeAST(ERemoteCall(makeAST(EVar("Enum")), "each", [
							makeAST(EVar(fieldsVar)),
							makeAST(EFn([{args: [PVar(binder)], body: adjustedBody}]))
						]));
	...
	static function detectAccumulationPattern(body:TypedExpr):Null<{
		...
		switch (body.expr) {
			...
			case TBinop(OpAssignOp(OpAdd), {expr: TLocal(v)}, rhs):
				...
				#if debug_loop_builder
				#end
				var isList = isArrayType(v.t) || switch (rhs.expr) {
			...
			case TBinop(OpAssign, {expr: TLocal(v1)}, {expr: TBinop(OpAdd, {expr: TLocal(v2)}, rhsAdd)}) if (v1.name == v2.name):
				...
				var isListAppend = isArrayType(v1.t) || switch (rhsAdd.expr) {
					case TArrayDecl(_): true;
					default: false;
				};
			...
			case TCall(e, _):
				...
				switch (e.expr) {
					...
					case TField(_, FStatic(_, cf)):
						...
						return name == "trace" || name == "log" || name == "println" || name == "print" || name == "debug" || name == "info"
	 ...
	 */
	public static function buildWhile(econd:TypedExpr, e:TypedExpr, normalWhile:Bool, buildExpr:TypedExpr->ElixirAST,
	...
	public static function detectDesugarForLoopPattern(cond:TypedExpr, body:TypedExpr):Null<{
		...
		var bounds:Null<{
			counter:String,
			limit:String,
			op:haxe.macro.Expr.Binop,
			counterExpr:TypedExpr,
			limitExpr:TypedExpr
	...
	static function extractInfrastructureVarName(expr:TypedExpr):Null<String> {
		return switch (expr.expr) {
			case TLocal(v):
				...
				if (name == "g" || name == "_g" || ~/^_?g[0-9]*$/.match(name)) { // Matches g, g1, g2, _g, _g1, _g2, etc.
					name;
				} else null;
			default: null;
		};
	...
	public static function buildFromForPattern(pattern:{
		...
		// Build range
		var range = makeAST(ERange(buildExpr(pattern.startExpr), buildExpr(pattern.endExpr), false, // inclusive
			makeAST(EInteger(1))));
		...
		if (pattern.hasSideEffectsOnly) {
			return makeAST(ERemoteCall(makeAST(EVar("Enum")), "each", [
				range,
				makeAST(EFn([
					{
						args: [PVar(varName)],
			...
			return makeAST(EFor([{pattern: PVar(varName), expr: range}], [], body, null, false));
		}
	}
	 ...
	 */
	public static function buildWithFullContext(startExpr:TypedExpr, endExpr:TypedExpr, whileBody:TypedExpr,
		...
		if (analysis == null) {
			...
			var range = makeAST(ERange(buildExpr(startExpr), buildExpr(endExpr), false, makeAST(EInteger(1))));
			...
			if (accumulation != null) {
				return buildAccumulationLoop(defaultVar, range, cleanedBody, accumulation, buildExpr, toSnakeCase);
			}
		...
		if (accumulation != null) {
			var range = makeAST(ERange(buildExpr(startExpr), buildExpr(endExpr), false, makeAST(EInteger(1))));
			return buildAccumulationLoop(analysis.userVar, range, analysis.userCode, accumulation, buildExpr, toSnakeCase);
		}
	...
	static function analyzeLoop(expr:TypedExpr, buildExpr:TypedExpr->ElixirAST):LoopIR {
		...
		// Run analyzers
		var analyzers = [
			new RangeIterationAnalyzer(buildExpr) // Additional analyzers can be added here as they become production-ready.
		];
	 ...
	 */
	static function emitFromIR(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, extractPattern:Null<TypedExpr->EPattern>, toSnakeCase:String->String):ElixirAST {
	 ...
	 */
	static function emitEnumEach(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
	 ...
	 */
	static function emitEnumMap(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
		// Build source from original expression
		var source = switch (ir.originalExpr.expr) {
			case TFor(_, e1, _):
		...
		return makeAST(ERemoteCall(makeAST(EVar("Enum")), "map", [
			source,
			makeAST(EFn([
				{
					args: [PVar(varName)],
	 ...
	 */
	static function emitComprehension(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
		// Build source from original expression
		var source = switch (ir.originalExpr.expr) {
			case TFor(_, e1, _):
				// Check if it's a range iteration
	 ...
	 */
	static function emitEnumReduce(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
	 ...
	 */
	static function emitSimpleLoop(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
	 ...
	 */
	static function buildLegacyFor(v:TVar, e1:TypedExpr, e2:TypedExpr, buildExpr:TypedExpr->ElixirAST, extractPattern:TypedExpr->EPattern,
	 ...
	 */
	static function buildLegacyWhile(cond:ElixirAST, body:ElixirAST, normalWhile:Bool, buildExpr:TypedExpr->ElixirAST):ElixirAST {
		#if debug_loop_builder
		if (body != null) {} else {}
		#end

		// Simple reduce_while implementation
		var stream = makeAST(ERemoteCall(makeAST(EVar("Stream")), "iterate", [
			makeAST(EInteger(0)),
			makeAST(EFn([
				{
					args: [PVar("n")],
		...
		var reducerBody = makeAST(EIf(cond, makeAST(ETuple([makeAST(EAtom("cont")), makeAST(EBlock([body, makeAST(EAtom("ok"))]))])),
			makeAST(ETuple([makeAST(EAtom("halt")), makeAST(EAtom("ok"))]))));
		...
		return makeAST(ERemoteCall(makeAST(EVar("Enum")), "reduce_while", [stream, initAcc, reducerFn]));
	}
	 ...
	 */
	static function buildLegacyForFromIR(ir:LoopIR, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):ElixirAST {
	 ...
	 */
	static function buildLegacyWhileFromIR(ir:LoopIR, buildExpr:TypedExpr->ElixirAST):ElixirAST {
	 ...
	 */
	static function analyzeVariableScopes(loopVar:TVar, iterator:TypedExpr, body:TypedExpr):VariableScopeAnalysis {
	 ...
	 */
	static function buildEnvironmentCapture(analysis:VariableScopeAnalysis, buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String):Null<{
	 ...
	 */
	static function wrapWithInitializations(loopAst:ElixirAST, initializations:Map<String, ElixirAST>, toSnakeCase:String->String):ElixirAST {
		...
		for (varName => initValue in initializations) {
			...
			statements.push(makeAST(EBinary(Match, makeAST(EVar(snakeName)), initValue)));
		}
	...
	static function buildAccumulationLoop(iteratorVar:String, source:ElixirAST, body:TypedExpr,
		...
		function mentionsIterator(ast:ElixirAST):Bool {
			...
			function mentionsInRaw(code:String):Bool {
				if (code == null || snakeIterator == null || snakeIterator.length == 0)
					return false;
				if (code.indexOf(snakeIterator) == -1)
					return false;
				try {
	 ...
	 */
	static function transformBodyForReduce(expr:TypedExpr, accumulation:{varName:String, isStringConcat:Bool, isListAppend:Bool},
			buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String, accumulatorVarName:String):ElixirAST {
		function unwrap(e:TypedExpr):TypedExpr {
			return switch (e.expr) {
		...
		switch (unwrapped.expr) {
			case TBlock(exprs):
				...
				for (i in 0...exprs.length) {
					...
					if (isAccumulation) {
						...
						if (astString.indexOf(" = g ") >= 0 || astString.indexOf(" = g + ") >= 0 || astString.indexOf(" = _g ") >= 0
			...
			default:
				...
				return makeAST(EBlock([buildExpr(unwrapped), makeAST(EVar(accumulatorVarName))]));
		}
	}
	 ...
	 */
	static function extractAccumulationValue(expr:TypedExpr, accumulation:{varName:String, isStringConcat:Bool, isListAppend:Bool},
			buildExpr:TypedExpr->ElixirAST, toSnakeCase:String->String, accumulatorVarName:String):ElixirAST {
	 ...
	 */
	static function extractMapIterationVariables(body:TypedExpr, iteratorVar:String):{
		key:Null<String>,
		value:Null<String>,
		keyUsed:Bool,
		valueUsed:Bool
	 ...
	 */
	static function buildIdiomaticMapIteration(v:TVar, iterator:TypedExpr, body:TypedExpr, context:BuildContext, toElixirVarName:String->String):ElixirASTDef {
		...
		#if debug_map_iteration
		if (vars.keyUsed && vars.valueUsed) {} else if (vars.keyUsed) {} else if (vars.valueUsed) {} else {}
		#end

		// Build the Enum call
		return ERemoteCall(makeAST(EVar("Enum")), enumFunc, [
			mapAst,
			makeAST(EFn([
				{
					args: [pattern],
	 ...
	 */
	public static function buildFor(v:TVar, e1:TypedExpr, e2:TypedExpr, expr:TypedExpr, context:BuildContext, toElixirVarName:String->String):ElixirASTDef {
		var buildExpression = context.getExpressionBuilder();
	...
	public static function buildWhileComplete(econd:TypedExpr, e:TypedExpr, normalWhile:Bool, expr:TypedExpr, context:BuildContext,
			...
			if (compilationContext != null) {
				for (s in savedMappings) {
					if (s.had)
						...
						compilationContext.tempVarRenameMap.remove(s.key);
				}
			}
		...
		if (arrayPattern != null) {
			return generateIdiomaticEnumCall(arrayPattern.arrayRef, arrayPattern.operation, e, context, toElixirVarName);
		}
	...
	static function buildDesugaredForRangeLoop(econd:TypedExpr, body:TypedExpr, context:BuildContext, toElixirVarName:String->String):Null<ElixirASTDef> {
		...
		if (compilationContext != null
			&& bounds != null
			&& bounds.counterVar != null
			&& bounds.counterVar.name != null
			...
			trace('[haxe5-loop-seeds] range candidate: counter name="${bounds.counterVar.name}" id=$counterId'
				+ ' localInitCount='
				+ (compilationContext.localVarInitValuesById != null ? Lambda.count(compilationContext.localVarInitValuesById) : -1)
				+ ' hasLocalInit='
				+ (compilationContext.localVarInitValuesById != null && compilationContext.localVarInitValuesById.exists(counterId))
				+ ' infraInitCount='
				+ (compilationContext.infrastructureVarInitValues != null ? Lambda.count(compilationContext.infrastructureVarInitValues) : -1)
				+ ' hasInfraInitByName='
				+ (compilationContext.infrastructureVarInitValues != null
					&& compilationContext.infrastructureVarInitValues.exists(bounds.counterVar.name))
				+ ' tempMapHasId='
				+ (compilationContext.tempVarRenameMap != null && compilationContext.tempVarRenameMap.exists(idKey))
				+ ' tempMapIdVal='
				+ (compilationContext.tempVarRenameMap != null
					&& compilationContext.tempVarRenameMap.exists(idKey) ? compilationContext.tempVarRenameMap.get(idKey) : '<none>'));
		}
		#end
		...
		if (compilationContext != null) {
			if (compilationContext.localVarInitValuesById != null
				&& compilationContext.localVarInitValuesById.exists(bounds.counterVar.id)) {
				var init = compilationContext.localVarInitValuesById.get(bounds.counterVar.id);
				if (init != null) {
					startAst = init;
					hasStartSeed = true;
				}
			} else if (compilationContext.infrastructureVarInitValues != null
				&& compilationContext.infrastructureVarInitValues.exists(bounds.counterVar.name)) {
	...
	static function detectForInArrayPattern(econd:TypedExpr, body:TypedExpr):Null<{
		...
		if (counterVar == null || arrayExpr == null)
			return null;
		...
		trace('[detectForInArrayPattern] userBody='
			+ reflaxe.elixir.util.EnumReflection.enumConstructor(userBody.expr)
			+ ' containsReturn='
			+ containsNonLocalReturn(userBody));
		#end

		// Avoid rewriting if the user body references the counter variable (index-based loops).
		var usesCounter = false;
		function scan(e:TypedExpr):Void {
			if (e == null || usesCounter)
				return;
			switch (e.expr) {
	...
	static function buildWhileLoop(econd:TypedExpr, e:TypedExpr, normalWhile:Bool, context:BuildContext, toElixirVarName:String->String):ElixirASTDef {
		...
		if (Lambda.count(mutatedVars) > 0) {
			...
			return ERemoteCall(makeAST(EVar("Enum")), "reduce_while", [
				makeAST(ERemoteCall(makeAST(EVar("Stream")), "iterate", [
					makeAST(EInteger(0)),
					makeAST(EFn([
						{
				...
				makeAST(EFn([
					{
						args: [PWildcard, PVar("acc")],
						guard: null,
						body: wrapLoopControlTry(makeAST(EIf(condition, buildThenBlock(body),
	...
	static function buildReduceWhileWithState(mutatedVars:Map<Int, TVar>, condition:ElixirAST, body:ElixirAST, context:BuildContext,
		...
		var transformedCondition = transformExpressionWithMapping(condition, outerToReducerVar);
		...
		var lambdaIfBody = makeAST(EIf(transformedCondition, buildThenBlock(transformedBody),
		...
		var result = ERemoteCall(makeAST(EVar("Enum")), "reduce_while", [
			makeAST(ERemoteCall(makeAST(EVar("Stream")), "iterate", [
				makeAST(EInteger(0)),
				makeAST(EFn([
					{
	 ...
	 */
	static function generateIdiomaticEnumCall(arrayRef:TypedExpr, operation:String, body:TypedExpr, context:BuildContext,
			toElixirVarName:String->String):ElixirASTDef {
		var buildExpression = context.getExpressionBuilder();
		...
		switch (operation) {
			case "map":
				...
				return ERemoteCall(makeAST(EVar("Enum")), "map", [
					array,
					makeAST(EFn([
						{
							args: [PVar(itemVar)],
			...
			case "filter":
				...
				return ERemoteCall(makeAST(EVar("Enum")), "filter", [
					array,
					makeAST(EFn([
						{
							args: [PVar(itemVar)],
					...
					function scan(e:TypedExpr):Void {
						...
						switch (e.expr) {
							case TField(obj, _):
								...
								scan(obj);
							case TLocal(v2):
								if (candidate == null && !StringTools.startsWith(v2.name, "_"))
									candidate = v2.name;
							case TCall(f, args):
								scan(f);
								for (a in args)
			...
			default:
				...
				return buildWhileLoop(arrayRef, // Use as condition (simplified)
					body, true, context, toElixirVarName);
		}
	}
