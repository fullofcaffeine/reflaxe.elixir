...
class BlockBuilder {
		...
		switch ([el[0].expr, el[1].expr]) {
			case [TVar(tmpVar, init), TBinop(OpNullCoal, {expr: TLocal(v)}, defaultExpr)] if (v.id == tmpVar.id && init != null):
				...
				if (context.compiler != null) {
					...
					var ifExpr = makeAST(EIf(makeAST(EBinary(EBinaryOp.NotEqual, makeAST(EMatch(PVar(tmpVarName), initAst)), makeAST(ENil))),
	...
	static function checkForInlineExpansion(el:Array<TypedExpr>, context:CompilationContext):Null<ElixirASTDef> {
		if (ElixirASTPatterns.isInlineExpansionBlock(el)) {
			return ElixirASTPatterns.transformInlineExpansion(el, // CRITICAL FIX: Call ElixirASTBuilder.buildFromTypedExpr directly to preserve context
				// Using compiler.compileExpressionImpl creates a NEW context, losing ClauseContext registrations
				function(e) return reflaxe.elixir.ast.ElixirASTBuilder.buildFromTypedExpr(e,
					context), function(name) return VariableAnalyzer.toElixirVarName(name));
		}
		return null;
	}
	...
	static function trackLocalInitValuesById(el:Array<TypedExpr>, context:CompilationContext):Array<{id:Int, had:Bool, value:Null<ElixirAST>}> {
		if (el == null)
			return [];
		if (context.localVarInitValuesById == null)
	...
	static function restoreLocalInitValuesById(snapshots:Array<{id:Int, had:Bool, value:Null<ElixirAST>}>, context:CompilationContext):Void {
		if (snapshots == null || context == null || context.localVarInitValuesById == null)
			return;
	 ...
	 */
	public static function withScopedInitTracking<T>(el:Array<TypedExpr>, context:CompilationContext, fn:() -> T):T {
