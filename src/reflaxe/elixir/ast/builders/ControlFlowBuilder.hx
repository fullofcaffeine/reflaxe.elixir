...
class ControlFlowBuilder {
	...
	static function detectOptimizedEnumSwitch(econd:TypedExpr, eif:TypedExpr, eelse:Null<TypedExpr>, context:CompilationContext):Null<ElixirASTDef> {
		...
		if (enumValue != null && enumIndex >= 0 && enumTypeRef != null) {
			...
			if (matchingConstructor != null) {
				...
				#if debug_enum_param_recovery
				trace('[EnumParamRecovery] final pattern ctor='
					+ matchingConstructor
					+ ' pattern='
		...
		function scrutineeMatches(sourceExpr:TypedExpr):Bool {
			...
			function eq(a:TypedExpr, b:TypedExpr):Bool {
				...
				return switch (aa.expr) {
					...
					case TField(oa, fa):
						switch (bb.expr) {
							case TField(ob, fb): // Match field names; ignore resolution differences across fa variants.
								var nameA = switch (fa) {
				...
				case TLocal(src):
					...
					for (index in 0...paramCount) {
						if (extractedIdByIndex[index] != null && extractedIdByIndex[index] == src.id) {
							...
							matched = true;
							#if debug_enum_param_recovery
							trace('[EnumParamRecovery] alias by extractedId index=' + index + ' target=' + targetVar.name + '#' + targetVar.id + ' src='
								+ src.name + '#' + src.id + ' out=' + out[index]);
							#end
							break;
						}
					}
					...
					if (!matched
						&& context != null
						&& context.infraVarSubstitutions != null
						&& context.infraVarSubstitutions.exists(src.id)) {
						var subst = unwrapNoOpWrappers(context.infraVarSubstitutions.get(src.id));
						switch (subst.expr) {
							case TEnumParameter(source, ef, index)
								if (ef != null && ef.name == constructorName && index >= 0 && index < paramCount && scrutineeMatches(source)):
								if (out[index] == null || out[index] == src.name) {
									out[index] = targetVar.name;
								}
								...
								matched = true;
								#if debug_enum_param_recovery
								trace('[EnumParamRecovery] alias by substitution index=' + index + ' target=' + targetVar.name + '#' + targetVar.id
									+ ' src=' + src.name + '#' + src.id + ' out=' + out[index]);
								#end
							case TArray(source, indexExpr):
								var idxU = unwrapNoOpWrappers(indexExpr);
								switch (idxU.expr) {
									case TConst(TInt(i)) if (i >= 0 && i < paramCount && scrutineeMatches(source)):
										...
										matched = true;
										#if debug_enum_param_recovery
										trace('[EnumParamRecovery] alias by substitution index=' + i + ' target=' + targetVar.name + '#' + targetVar.id
											+ ' src=' + src.name + '#' + src.id + ' out=' + out[i]);
										#end
									default:
								}
							default:
			...
			switch (expr.expr) {
				case TVar(v, init) if (init != null):
					...
					if (info != null && info.ctorName == constructorName && info.index >= 0 && info.index < paramCount && scrutineeMatches(info.source)) {
						...
						#if debug_enum_param_recovery
						var srcDbg = switch (info.source.expr) {
							case TLocal(sv): sv.name + "#" + sv.id;
							default: reflaxe.elixir.util.EnumReflection.enumConstructor(info.source.expr);
						};
	...
	static function branchUsesHaxeVarName(expr:TypedExpr, name:String):Bool {
		if (expr == null || name == null || name.length == 0)
			return false;
		var found = false;
			...
			switch (e.expr) {
				...
				case TSwitch(switchTarget, cases, defaultExpr):
					...
					for (caseClause in cases)
						traverse(caseClause.expr);
					if (defaultExpr != null)
						traverse(defaultExpr);
