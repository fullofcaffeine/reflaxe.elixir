...
class HeexTsxAstLowerer {
	public static function lowerRoot(nodeExpr:TypedExpr, buildExpression:(TypedExpr) -> ElixirAST, context:CompilationContext):String {
	...
	static function lowerNode(e:TypedExpr, buildExpression:(TypedExpr) -> ElixirAST, context:CompilationContext):String {
		...
		context.error("TSX template: expected a phoenix.hxx.ast.HeexNode constructor expression", context.getCurrentPosition());
		throw "tsx_heex_node_unexpected_expr";
	}

	static function lowerHeexNodeCtor(ctor:String, params:Array<TypedExpr>, buildExpression:(TypedExpr) -> ElixirAST, context:CompilationContext,
			pos:Position):String {
		return switch (ctor) {
			case "Text":
			...
			case "Fragment":
				...
				var out = "";
				for (c in children)
					out += lowerNode(c, buildExpression, context);
				out;
			case "Element":
	...
	static function lowerElement(name:String, attrs:Array<TypedExpr>, children:Array<TypedExpr>, buildExpression:(TypedExpr) -> ElixirAST,
			context:CompilationContext):String {
		var attrsStr = lowerAttrs(attrs, buildExpression, context);
		var childrenStr = "";
		for (c in children)
			childrenStr += lowerNode(c, buildExpression, context);
	...
	static function lowerAttrs(attrs:Array<TypedExpr>, buildExpression:(TypedExpr) -> ElixirAST, context:CompilationContext):String {
		...
		var out = "";
		for (a in attrs)
			out += lowerAttr(unwrap(a), buildExpression, context);
		return out;
	}
	...
	static function lowerHeexAttrCtor(ctor:String, params:Array<TypedExpr>, buildExpression:(TypedExpr) -> ElixirAST, context:CompilationContext):String {
		return switch (ctor) {
			case "Static":
				if (params == null || params.length != 2) {
