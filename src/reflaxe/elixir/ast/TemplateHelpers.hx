...
class TemplateHelpers {
	...
	static function renderExpr(ast:ElixirAST):String {
		return switch (ast.def) {
			...
			case EField(obj, field):
				...
				var heexBase = StringTools.startsWith(base, "assigns.") ? ('@' + base.substr("assigns.".length)) : base;
				if (base == "assigns") {
					'@' + field;
		...
		var __result = switch (ast.def) {
			...
			case EIf(condition, thenBranch, elseBranch):
				...
				if (StringTools.startsWith(condStr, "assigns."))
					condStr = '@' + condStr.substr("assigns.".length);
			...
			case ECall(module, func, args):
				...
				if (StringTools.startsWith(callStr, "assigns."))
					callStr = '@' + callStr.substr("assigns.".length);
				'<%= ' + callStr + ' %>';
			...
			case ERemoteCall(module, func, args):
				...
				function isHeexTemplateModule(m:ElixirAST):Bool {
					...
					return switch (m.def) {
						...
						case EField(_, fld): fld == "HeexTemplate" || fld == "HXX2" || fld == "H";
						default:
							false;
					}
				}
				...
				if (isHeexTemplateModule(module) && (func == "for_each" || func == "forEach" || func == "each") && args.length == 2) {
					...
					if (StringTools.startsWith(itemsExpr, "assigns."))
						itemsExpr = '@' + itemsExpr.substr("assigns.".length);
				...
				if (StringTools.startsWith(full, "assigns."))
					full = '@' + full.substr("assigns.".length);
				'<%= ' + full + ' %>';
			...
			default:
				...
				if (StringTools.startsWith(exprAny, "assigns."))
					exprAny = '@' + exprAny.substr("assigns.".length);
				'<%= ' + exprAny + ' %>';
		};
		return __result;
	}
	...
	static function rewriteInterpolationsInternal(s:String):String {
		...
		while (i < s.length) {
			...
			if (expr.length >= 2 && expr.charAt(0) == '"' && expr.charAt(1) == '<') {
				#if macro
				haxe.macro.Context.error("HXX: injecting HTML via string inside interpolation is not allowed. Use HXX.block('...') or inline markup.",
	...
	static function buildInterpolatedAttrExpr(value:String):String {
		...
		while (i < value.length) {
			...
			if (hashInterpIndex != -1)
				nextMarkerIndex = hashInterpIndex;
			if (dollarInterpIndex != -1 && (nextMarkerIndex == -1 || dollarInterpIndex < nextMarkerIndex))
				nextMarkerIndex = dollarInterpIndex;
			if (eexInterpIndex != -1 && (nextMarkerIndex == -1 || eexInterpIndex < nextMarkerIndex))
				nextMarkerIndex = eexInterpIndex;
			if (nextMarkerIndex == -1) {
				var tail = value.substr(i);
				if (tail != "")
					parts.push('"' + escapeElixirStringLiteral(tail) + '"');
				break;
			}

			var staticPart = value.substr(i, nextMarkerIndex - i);
			if (staticPart != "")
				parts.push('"' + escapeElixirStringLiteral(staticPart) + '"');
			...
			if (eexInterpIndex != -1 && nextMarkerIndex == eexInterpIndex) {
				...
				if (endTag == -1) {
					...
					if (rest != "")
						parts.push('"' + escapeElixirStringLiteral(rest) + '"');
					break;
				}
			...
			if (expr.length >= 2 && expr.charAt(0) == '"' && expr.charAt(1) == '<') {
				#if macro
				haxe.macro.Context.error("HXX: injecting HTML via string inside attribute interpolation is not allowed. Use HXX.block('...') or inline markup.",
					haxe.macro.Context.currentPos());
				#else
	...
	static function rewriteQuotedAttributeMixedInterpolations(s:String):String {
		...
		while (i < s.length) {
			...
			if (ch == "=") {
				var vpos = i + 1;
				while (vpos < s.length && ~/^\\s$/.match(s.charAt(vpos)))
					vpos++;
	...
	static function rewriteAttributeEexInterpolations(s:String):String {
		...
		function normalizeAttrExpr(expr:String):String {
			...
			if (iife.match(e))
				e = StringTools.trim(iife.matched(1));
			// Unwrap inspect(...)
			var inspectWrap = ~/^(?:Kernel\.)?inspect\((.*)\)\s*$/s;
			if (inspectWrap.match(e))
				e = StringTools.trim(inspectWrap.matched(1));
			// Unwrap to_string(...)
			var toStringWrap = ~/^(?:Kernel\.)?to_string\((.*)\)\s*$/s;
			if (toStringWrap.match(e))
				e = StringTools.trim(toStringWrap.matched(1));
			...
			if (implToStringWrap.match(e))
				e = StringTools.trim(implToStringWrap.matched(1));
			return e;
		}

		function isQuoted(v:String):Bool {
			var t = StringTools.trim(v);
			return (StringTools.startsWith(t, "\"") && StringTools.endsWith(t, "\""))
				|| (StringTools.startsWith(t, "'") && StringTools.endsWith(t, "'"));
		}
		...
		while (i < s.length) {
			...
			if (StringTools.startsWith(expr, "if ") && StringTools.endsWith(expr, " do")) {
				...
				var elseContentRaw:Null<String> = (elseMarkerPos != -1 && elseMarkerPos < endMarkerPos) ? StringTools.trim(s.substr(elseMarkerPos + 10,
					endMarkerPos - (elseMarkerPos + 10))) : null;
				...
				if (postEndPos < s.length && (s.charAt(postEndPos) == '"' || s.charAt(postEndPos) == '\'')) {
					if (quote == null || s.charAt(postEndPos) == quote)
						postEndPos++;
				}
				i = postEndPos;
				continue;
			...
			var qpos = nextPos;
			while (qpos < s.length && ~/^\s$/.match(s.charAt(qpos)))
				qpos++;
			if (qpos < s.length && (s.charAt(qpos) == '"' || s.charAt(qpos) == '\'')) {
				if (quote == null || s.charAt(qpos) == quote)
					qpos++;
			}
			nextPos = qpos;
			i = nextPos;
	...
	static function rewriteAttributeBraceTrailingQuotes(s:String):String {
		...
		while (i < s.length) {
			...
			var vpos = k + 1;
			while (vpos < s.length && isWhitespace(s.charAt(vpos)))
				vpos++;
	...
	static function rewriteAttributeInterpolations(s:String):String {
		...
		while (i < s.length) {
			...
			var nameEnd = k - 1;
			while (nameEnd >= i && ~/^\s$/.match(s.charAt(nameEnd)))
				nameEnd--;
			var nameStart = nameEnd;
			while (nameStart >= i && ~/^[A-Za-z0-9_:\-]$/.match(s.charAt(nameStart)))
				nameStart--;
			nameStart++;
				...
				if (idxDo != -1) {
					...
					if (qv != null) {
						...
						if (StringTools.startsWith(rem, ","))
							rem = StringTools.trim(rem.substr(1));
	...
	public static function rewriteControlTags(s:String):String {
		...
		while (i < s.length) {
			...
			while (j < s.length && ~/^\s$/.match(s.charAt(j)))
				j++;
			...
			while (k < s.length && depth > 0) {
				...
				if (tag == 1) {
					depth++;
					k = next + 3;
				} else if (tag == 2 && depth == 1 && elsePos == -1) {
					elsePos = next;
					k = next + 6;
				} else if (tag == 3) {
					depth--;
			...
			if (elseHtml != null)
				elseHtml = rewriteControlTags(elseHtml);
