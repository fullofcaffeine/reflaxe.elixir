...
class ElixirASTHelpers {
	...
	public static function isArrayType(t:Type):Bool {
		return switch (t) {
			case TInst(cls, _): var clsType = cls.get(); clsType.name == "Array" || (clsType.pack.length == 0 && clsType.name == "Array");
			case TAbstract(abs, _): var absType = abs.get(); absType.name == "Array" && absType.pack.length == 0;
			default:
				false;
		};
	}
	...
	public static function isMapType(t:Type):Bool {
		return switch (t) {
			case TInst(cls, _): var clsType = cls.get(); clsType.name == "Map" || (clsType.module == "haxe.ds.Map");
			case TAbstract(abs, _): var absType = abs.get(); absType.name == "Map" && (absType.module == "haxe.ds.Map" || absType.pack.join(".") == "haxe.ds");
			default:
				false;
		};
	}
	...
	public static function countVarOccurrencesInAST(ast:ElixirAST, name:String):Int {
		...
		return switch (ast.def) {
			...
			case EIf(cond, thenBranch, elseBranch):
				countVarOccurrencesInAST(cond,
					name) + countVarOccurrencesInAST(thenBranch, name) + (elseBranch != null ? countVarOccurrencesInAST(elseBranch, name) : 0);
	...
	public static function replaceVarInAST(ast:ElixirAST, name:String, replacement:ElixirAST):ElixirAST {
		...
		return switch (ast.def) {
			...
			case EIf(cond, thenBranch, elseBranch):
				make(EIf(replaceVarInAST(cond, name, replacement), replaceVarInAST(thenBranch, name, replacement),
			...
			case EMap(fields): make(EMap(fields.map(f -> {
					key: replaceVarInAST(f.key, name, replacement),
					value: replaceVarInAST(f.value, name, replacement)
				})));
