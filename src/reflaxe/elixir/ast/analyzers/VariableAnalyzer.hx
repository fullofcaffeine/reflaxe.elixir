...
class VariableAnalyzer {
	...
	public static function usesVariableInNode(node:ElixirAST, varName:String):Bool {
		...
		return switch (node.def) {
			...
			case ERemoteCall(target, _, args): (target != null && usesVariableInNode(target, varName)) || usesVariable(args, varName);
			case EMatch(_, expr): usesVariableInNode(expr, varName);
			case EBinary(_, left, right): usesVariableInNode(left, varName) || usesVariableInNode(right, varName);
			...
			case EMap(fields):
				var used = false;
				for (field in fields) {
					if (usesVariableInNode(field.key, varName) || usesVariableInNode(field.value, varName)) {
						used = true;
						break;
					}
				}
				used;
			case EIf(cond, thenBranch, elseBranch): usesVariableInNode(cond,
					varName) || usesVariableInNode(thenBranch, varName) || (elseBranch != null && usesVariableInNode(elseBranch, varName));
			...
			case EFn(clauses):
				...
				used;
			case EPipe(left, right): usesVariableInNode(left, varName) || usesVariableInNode(right, varName);
			...
			case ECond(clauses):
				var used = false;
				for (clause in clauses) {
					if (usesVariableInNode(clause.condition, varName) || usesVariableInNode(clause.body, varName)) {
						used = true;
						break;
					}
				}
	...
	public static function isForbiddenInElixir(name:String):Bool {
		var forbidden = [
			"and", "or", "not", "when", "in", "fn", "do", "end", "catch", "rescue", "after", "else", "true", "false", "nil"
		];
	...
	public static function isPatternVariableUsedById(body:TypedExpr, varId:Int):Bool {
		...
		switch (body.expr) {
			...
			case TBlock(exprs):
				for (expr in exprs) {
					if (isPatternVariableUsedById(expr, varId))
						return true;
				}
				return false;

			case TBinop(_, e1, e2):
				return isPatternVariableUsedById(e1, varId) || isPatternVariableUsedById(e2, varId);

			case TCall(e, el):
				if (isPatternVariableUsedById(e, varId))
					return true;
				for (arg in el) {
					if (isPatternVariableUsedById(arg, varId))
						return true;
				}
				return false;
			...
			case TSwitch(e, cases, edef):
				if (isPatternVariableUsedById(e, varId))
					return true;
				for (c in cases) {
					if (isPatternVariableUsedById(c.expr, varId))
						return true;
				}
			...
			case TObjectDecl(fields):
				for (field in fields) {
					if (isPatternVariableUsedById(field.expr, varId))
						return true;
				}
				return false;
			...
			case TArrayDecl(values):
				for (value in values) {
					if (isPatternVariableUsedById(value, varId))
						return true;
				}
				return false;
