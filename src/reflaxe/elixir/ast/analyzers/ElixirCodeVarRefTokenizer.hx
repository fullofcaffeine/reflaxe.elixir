...
class ElixirCodeVarRefTokenizer {
  public static function collectFromInterpolatedStringText(text: String, out: Map<String, Bool>): Void {
    if (text == null || text.length == 0 || out == null) return;
  ...
  public static function collectFromElixirCode(code: String, out: Map<String, Bool>): Void {
    if (code == null || code.length == 0 || out == null) return;
    ...
    function scanDoubleQuotedString(startIdx: Int): Int {
      ...
      while (i < code.length) {
        ...
        if (ch == "#" && i + 1 < code.length && code.charAt(i + 1) == "{") {
          ...
          if (innerEnd > innerStart) collectFromElixirCode(code.substr(innerStart, innerEnd - innerStart), out);
          i = j;
          continue;
        }
        if (ch == "\"") return i + 1;
    ...
    while (i < code.length) {
      ...
      if (!isLowercaseVar(name)) continue;
      ...
      if (isAtom || isKeywordKey || isAssign || isFieldName || isCall) continue;
