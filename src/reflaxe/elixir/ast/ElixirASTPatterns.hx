...
class ElixirASTPatterns {
	...
	static function isComplexInlineExpansionBlock(block:Array<TypedExpr>):Bool {
		// Complex inline expansion detection is intentionally disabled for now.
		// This needs robust coverage + tests before enabling.
		return false;
	 ...
	 */
	public static function transformInlineExpansion(block:Array<TypedExpr>, buildFromTypedExpr:TypedExpr->ElixirAST,
		...
		var tmpVarName = toElixirVarName(pattern.tmpVar.name.charAt(0) == "_" ? pattern.tmpVar.name.substr(1) : pattern.tmpVar.name);
		...
		var ifExpr = makeAST(EIf(makeAST(EBinary(Equal, makeAST(EMatch(PVar(tmpVarName), initAst)), makeAST(ENil))), buildFromTypedExpr(nullBranch),
	 ...
	 */
	static function transformComplexInlineExpansion(block:Array<TypedExpr>, buildFromTypedExpr:TypedExpr->ElixirAST,
			toElixirVarName:String->String):ElixirASTDef {
		#if debug_inline_expansion
