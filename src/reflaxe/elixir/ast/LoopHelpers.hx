...
class LoopHelpers {
	...
	public static function buildMapIteration(pattern:MapIterationPattern, context:CompilationContext):ElixirAST {
		...
		return makeAST(ECall(makeAST(EVar("Enum")), enumFunction, [mapAst, makeAST(EFn([fnClause]))]));
	}
	...
	public static function containsLoop(expr:TypedExpr):Bool {
		return switch (expr.expr) {
			...
			case TIf(_, thenExpr, elseExpr): containsLoop(thenExpr) || (elseExpr != null && containsLoop(elseExpr));
			case TSwitch(_, cases, def): Lambda.exists(cases, c -> containsLoop(c.expr)) || (def != null && containsLoop(def));
			case TTry(e, catches): containsLoop(e) || Lambda.exists(catches, c -> containsLoop(c.expr));
			default: false;
		};
	}
}
