...
class ElixirASTPrinter {
		...
		var result = switch (ast.def) {
			case EDefmodule(name, doBlock):
				...
				if (observedAppPrefix == null) {
					...
					if (idxCap > 0)
						observedAppPrefix = name.substring(0, idxCap);
					var idxWeb = name.indexOf("Web");
					if (idxWeb > 0)
						observedAppPrefix = name.substring(0, idxWeb);
				}
				...
				if (isException) {
					...
					if (ast.metadata != null && ast.metadata.instanceFields != null) {
						for (f in ast.metadata.instanceFields) {
							if (f == null || f.length == 0)
								continue;
							if (f == "message" || f == "previous" || f == "native" || f == "stack")
								continue;
							exceptionFields.push(f);
						}
					}
					...
					if (observedAppPrefix == null) {
						...
						if (idxCap2 > 0)
							observedAppPrefix = name.substring(0, idxCap2);
						var idxWeb2 = name.indexOf("Web");
						if (idxWeb2 > 0)
							observedAppPrefix = name.substring(0, idxWeb2);
					}
							...
							function scan(n:ElixirAST):Void {
								...
								switch (n.def) {
									...
									case ERemoteCall(mod, _, args):
										switch (mod.def) {
											case EVar(m) if (m == 'Ecto.Query'): found = true;
											default:
										}
										if (args != null)
											for (a in args)
	...
	static function printNode(node:ElixirASTDef, indent:Int):String {
		return switch (node) {
			...
			case EModule(name, attributes, body):
					...
					for (s in stmts)
						switch (s.def) {
							case EAlias(module, as) if ((as == null || as == "Repo")):
								...
								if (module != null && module.indexOf(".Repo") > 0)
									hasAlias = true;
							default:
						}
				...
				for (expr in body) {
					var normalizedExpr = switch (expr.def) {
						case EParen(inner): inner;
						default: expr;
					};
					result += indentStr(indent + 1) + print(normalizedExpr, indent + 1) + '\n';
				}
			...
			case ECase(expr, clauses):
				...
				var scrutineeNeedsParens = switch (expr.def) {
					case EIf(_, _, _) | EUnless(_, _, _) | ETry(_, _, _, _, _) | ECase(_, _) | ECond(_) | EWith(_, _, _) | EReceive(_, _) | EFn(_):
						true;
					case EBlock(stmts) if (stmts.length > 1):
						true;
					default:
			...
			case EMatch(pattern, expr):
				...
				var isZeroPat = switch (pattern) {
					case PLiteral({def: EInteger(v)}) if (v == 0): true;
					default: false;
				};
						...
						if (expr != null) {
							switch (expr.def) {
								...
								case EMatch(innerPattern, rhsExpr2):
									switch (innerPattern) {
										case PVar(innerName) if (innerName == name): return name + ' = ' + print(rhsExpr2, indent);
										default:
									}
								default:
							}
			...
			case EIf(condition, thenBranch, elseBranch):
				...
				if (isInline && elseBranch != null) { // Inline if-else expression: if condition, do: then_val, else: else_val
					...
					+ ', else: '
					+ print(elseBranch, 0);
				} else if (elseBranch != null) { // Multi-line if-else block
					'if '
					+ conditionStr
					+ ' do\n'
					+ indentStr(indent + 1)
				...
				} else { // Multi-line if without else
					'if '
					+ conditionStr
					+ ' do\n'
					+ indentStr(indent + 1)
				...
				'{' + [
					for (e in elements) {
						switch (e.def) {
							...
							case EParen(inner) if (switch (inner.def) {
									case EBlock(es) if (es.length > 1): true;
									case EDo(ds) if (ds.length > 1): true;
									default: false;
								}):
					...
					if (target != null) {
						...
						if (funcName == "") {
							...
							var isEnumMethod = switch (funcName) {
								case "map" | "filter" | "reduce" | "each" | "find" | "reject" | "take" | "drop" | "any" | "all" | "count" | "member" |
									"sort" | "reverse" | "zip" | "concat" | "flat_map" | "group_by" | "split" | "join" | "at" | "fetch" | "empty" | "sum" |
			...
			case ERemoteCall(module, funcName, args):
				...
				var argStr = (function() {
					...
					if (funcName == "changeset" && args.length >= 1) {
								...
								if (raw.length > 0) {
									var app = (function() {
										...
										if (currentModuleName != null) {
											...
											if (w > 0)
												pfx = currentModuleName.substring(0, w);
										}
										if (pfx == null)
											pfx = observedAppPrefix;
										if (pfx == null) {
											try
						...
						if ((mstr == reflaxe.elixir.PhoenixMapper.getAppModuleName() + ".Repo" || StringTools.endsWith(mstr, ".Repo"))
							&& (funcName == "get" || funcName == "one")) {
							var comma = s.indexOf(',');
			...
			case EString(value):
				...
				inline function sanitizeJoinArgInInterpolatedString(s:String):String {
					if (s == null || s.indexOf("#{") == -1 || s.indexOf("Enum.join(") == -1)
						return s;
			...
			case EVar(name):
				...
				inline function safeIdent(nm:String):String {
					return switch (nm) {
						case "fn" | "do" | "end" | "case" | "cond" | "try" | "rescue" | "catch" | "after" | "receive" | "quote" | "unquote" | "when" | "and" |
							"or" | "not": nm
							+ "_";
						default: nm;
					}
			...
			case EFor(generators, filters, body, into, uniq):
				...
				var options = [];
				if (into != null)
					options.push('into: ' + print(into, 0));
				if (uniq)
			...
			case EFn(clauses):
				...
				if (clauses.length == 1 && clauses[0].guard == null) {
					...
					if (isMultiLine) { // Multi-line format for complex bodies
						'fn'
						+ paramPart
						+ ' ->\n'
						+ indentStr(indent + 1)
							...
							'use ' + module + ', ' + [
								for (p in pairs) {
									var value = switch (p.value.def) {
										case EIf(cond, thenBranch, elseBranch)
											if (elseBranch != null && isSimpleExpression(thenBranch) && isSimpleExpression(elseBranch)):
											'('
											+ print(p.value, 0)
											+ ')';
										case _:
			...
			case ERaw(code):
				...
				if (out.indexOf("Ecto.Query.from(") != -1) {
					...
					if (currentModuleName != null) {
						...
						if (idxA > 0)
							pfx = currentModuleName.substring(0, idxA)
						else {
							var idxB = currentModuleName.indexOf(".Repo");
							if (idxB > 0)
								pfx = currentModuleName.substring(0, idxB);
						}
					}
					if (pfx == null && observedAppPrefix != null)
						pfx = observedAppPrefix;
					if (pfx == null) {
						try
							pfx = reflaxe.elixir.PhoenixMapper.getAppModuleName()
	...
	static function printBinarySegment(segment:EBinarySegment):String {
		...
		if (segment.size != null)
			specs.push('size(' + print(segment.size, 0) + ')');
	...
	static function isNonAssociative(op:EBinaryOp):Bool {
		return switch (op) {
			case Subtract | Divide | Remainder | Equal | NotEqual | Greater | GreaterEqual | Less | LessEqual | In:
				true;
			default:
				false;
		};
	...
	static function isSimpleExpression(ast:ElixirAST):Bool {
		...
		return switch (ast.def) {
			...
			case EMatch(_, _):
				...
				false;
			case EBlock(expressions): // Empty blocks MUST use block syntax (not inline)
	...
	static function containsAssignment(ast:ElixirAST):Bool {
		...
		return switch (ast.def) {
			...
			case EBlock(expressions):
				...
				false;
			case EIf(_, thenBranch, elseBranch): containsAssignment(thenBranch) || (elseBranch != null && containsAssignment(elseBranch));
			case _: false;
		};
	}
		...
		function looksLikeNativeMapRemoveRebindBlock(block:ElixirAST):Bool {
			...
			switch (meaningful[1].def) {
				case EMatch(PVar(m2), {def: ERemoteCall({def: EVar("Map")}, "delete", args1)}) if (args1 != null && args1.length == 2):
					if (mapVarName == null || m2 != mapVarName)
						return false;
					switch (args1[0].def) {
						case EVar(m3) if (m3 == mapVarName):
						default: return false;
					}
					if (keyExpr == null || !astEq(keyExpr, args1[1]))
						return false;
				default:
					return false;
			}
	...
	static function maybeWrapRawMultilineRhs(expr:ElixirAST, printed:String):String {
		if (printed == null || expr == null || expr.def == null)
			return printed;
		...
		switch (module.def) {
			...
			case EVar(n):
				...
				inline function isPhoenixWebRoot(name:String):Bool {
					return name == "Routes" || name == "Gettext" || name == "HTML" || name == "CoreComponents" || name == "Components" || name == "Layouts"
						|| StringTools.endsWith(name, "Live");
				}
				var appPrefix = observedAppPrefix;
				if (appPrefix == null && currentModuleName != null) {
					var idx2 = currentModuleName.indexOf("Web");
					if (idx2 > 0)
						appPrefix = currentModuleName.substring(0, idx2);
				}
	...
	static function isAppModuleName(s:String):Bool {
		...
		return s.indexOf("App") != -1 || s.endsWith(".PubSub") || s.endsWith(".Repo") || s.endsWith(".Endpoint") || s.endsWith(".Telemetry")
			|| s.endsWith(".Supervisor") || s.endsWith(".Application") || s.endsWith("Web");
	}
