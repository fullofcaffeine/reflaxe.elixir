		...
		function tx(n:ElixirAST):ElixirAST {
			...
			return switch (n.def) {
				...
				case ECase(expr, clauses):
					...
					for (cl in clauses)
						cls.push({pattern: renamePattern(cl.pattern), guard: cl.guard, body: tx(cl.body)});
	...
	static function collectPattern(p:EPattern, declared:Map<String, Bool>):Void {
		switch (p) {
			...
			case PTuple(es) | PList(es):
				for (e in es)
					collectPattern(e, declared);
			case PCons(h, t):
				collectPattern(h, declared);
				collectPattern(t, declared);
			case PMap(kvs):
				for (kv in kvs)
					collectPattern(kv.value, declared);
			case PStruct(_, fs):
				for (f in fs)
					collectPattern(f.value, declared);
			case PPin(inner):
