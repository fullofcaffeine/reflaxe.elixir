								...
								if (two != null) {
									...
									switch (two.second.def) {
										case EIf(cond, thenE, elseE) if (b != null && elseReturnsVar(elseE, b.name)):
											...
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b.name), two.first.metadata, two.first.pos), b.rhs),
												two.first.metadata, two.first.pos));
											out.push(makeASTWithMeta(EBinary(Match, left,
												makeASTWithMeta(EIf(cond, thenE, elseE), two.second.metadata, two.second.pos)),
												s.metadata, s.pos));
											rewritten = true;
										default:
									}
									...
									if (two2 != null) {
										...
										switch (two2.second.def) {
											case EIf(cond2, thenE2, elseE2) if (b2 != null && elseReturnsVar(elseE2, b2.name)):
												...
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b2.name), two2.first.metadata, two2.first.pos),
													b2.rhs),
													two2.first.metadata, two2.first.pos));
												out.push(makeASTWithMeta(EMatch(pat,
													makeASTWithMeta(EIf(cond2, thenE2, elseE2), two2.second.metadata, two2.second.pos)),
													s.metadata, s.pos));
												rewritten = true;
											default:
										}
								...
								if (tw != null) {
									...
									switch (tw.second.def) {
										case EIf(condX, thenX, elseX) if (bX != null && elseReturnsVar(elseX, bX.name)):
											out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(bX.name), tw.first.metadata, tw.first.pos), bX.rhs),
												tw.first.metadata, tw.first.pos));
											out2.push(makeASTWithMeta(EBinary(Match, left2,
												makeASTWithMeta(EIf(condX, thenX, elseX), tw.second.metadata, tw.second.pos)),
												s2.metadata, s2.pos));
											done = true;
										default:
									}
									...
									if (tw2 != null) {
										...
										switch (tw2.second.def) {
											case EIf(condY, thenY, elseY) if (bY != null && elseReturnsVar(elseY, bY.name)):
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(bY.name), tw2.first.metadata, tw2.first.pos),
													bY.rhs),
													tw2.first.metadata, tw2.first.pos));
												out2.push(makeASTWithMeta(EMatch(p2,
													makeASTWithMeta(EIf(condY, thenY, elseY), tw2.second.metadata, tw2.second.pos)), s2.metadata,
													s2.pos));
												done = true;
											default:
										}
