...
class ControllerLocalUnusedUnderscoreTransforms {
				...
				for (i in 0...stmts.length) {
					...
					var rewrittenStmt = switch (stmt.def) {
						...
						case EBinary(Match, left, right):
							switch (left.def) {
								case EVar(binderName) if (canUnderscoreBinder(binderName)
									&& !OptimizedVarUseAnalyzer.usedLater(useIndex, i + 1, binderName)):
						...
						case ECase(expr, clauses):
							var newClauses = [];
							for (cl in clauses)
								newClauses.push({pattern: cl.pattern, guard: cl.guard, body: underscoreUnused(cl.body)});
	...
	static inline function isControllerModule(node:ElixirAST, name:String):Bool {
		if (node.metadata?.isPhoenixWeb == true && node.metadata?.phoenixContext == PhoenixContext.Controller)
			return true;
