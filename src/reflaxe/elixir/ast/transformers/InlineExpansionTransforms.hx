...
class InlineExpansionTransforms {
	...
	static function containsVariable(expr:ElixirAST, varName:String):Bool {
		return switch (expr.def) {
			...
			case ECall(target, _, args): (target != null && containsVariable(target, varName)) || usesVariable(args, varName);
	...
	static function containsInlineAssignment(expr:ElixirAST):Bool {
		...
		switch (expr.def) {
			case EBlock(_):
				trace('[XRay InlineTupleExtraction] Checking EBlock for assignments');
			case EIf(_, _, _):
				trace('[XRay InlineTupleExtraction] Checking EIf for assignments');
			case EMatch(_, _):
				trace('[XRay InlineTupleExtraction] Found EMatch!');
			case EBinary(Match, _, _):
				trace('[XRay InlineTupleExtraction] Found EBinary(Match)!');
			default:
		}
		#end
		...
		return switch (expr.def) {
			...
			case EIf(cond, then, els): containsInlineAssignment(cond) || containsInlineAssignment(then) || (els != null && containsInlineAssignment(els));
