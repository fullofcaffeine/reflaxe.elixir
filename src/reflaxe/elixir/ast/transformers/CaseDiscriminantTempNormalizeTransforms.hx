			...
			return switch (n.def) {
				case EBlock(stmts) if (stmts.length >= 2):
					...
					for (i in 0...stmts.length) {
							...
							while (k >= 0) {
								switch (out[k].def) {
									case EBinary(Match, leftK, _):
										switch (leftK.def) {
											case EVar(nm): if (equalModuloLeadingUnderscore(targetVar, nm)) return nm;
											default:
										}
									case EMatch(patK, _):
										switch (patK) {
											case PVar(nmk): if (equalModuloLeadingUnderscore(targetVar, nmk)) return nmk;
											default:
										}
									default:
								}
						...
						switch (cur.def) {
							...
							case EParen(inner):
								switch (inner.def) {
									case ECase(tgt2, cls2): switch (tgt2.def) {
											case EVar(tv2):
												var lhs2 = nearestLhsFor(tv2);
												if (lhs2 != null) replaced = makeASTWithMeta(EParen(makeAST(ECase(makeAST(EVar(lhs2)), cls2))), cur.metadata,
													cur.pos);
											default:
										}
									default:
							...
							case EBinary(Match, leftN, rhsN):
								switch (rhsN.def) {
									case ECase(tN, csN): switch (tN.def) {
											case EVar(tvN):
							...
							case EMatch(patM, rhsM):
								switch (rhsM.def) {
									case ECase(tM, csM): switch (tM.def) {
											case EVar(tvM):
				...
				case EDo(stmts2) if (stmts2.length >= 2):
					...
					for (j in 0...stmts2.length) {
						...
						function nearestLhs2(targetVar:String):Null<String> {
							...
							while (k2 >= 0) {
								switch (out2[k2].def) {
									...
									case EMatch(p2, _):
										switch (p2) {
											case PVar(nm2): if (equalModuloLeadingUnderscore(targetVar, nm2)) return nm2;
											default:
										}
									default:
								}
						...
						switch (cur2.def) {
							case ECase(t2, c2): switch (t2.def) {
									case EVar(tv):
										var lhs = nearestLhs2(tv);
										if (lhs != null) rep2 = makeASTWithMeta(ECase(makeAST(EVar(lhs)), c2), cur2.metadata, cur2.pos);
									default:
								}
							case EParen(in2): switch (in2.def) {
									case ECase(tt, cc): switch (tt.def) {
											case EVar(tv3):
												var lhs3 = nearestLhs2(tv3);
												if (lhs3 != null) rep2 = makeASTWithMeta(EParen(makeAST(ECase(makeAST(EVar(lhs3)), cc))), cur2.metadata,
													cur2.pos);
											default:
										}
									default:
								}
							case EBinary(Match, lN, rN): switch (rN.def) {
									case ECase(tN2, csN2): switch (tN2.def) {
											case EVar(tvn):
							...
							case EMatch(pN2, rN2): switch (rN2.def) {
									case ECase(tM2, csM2): switch (tM2.def) {
											case EVar(tvM2):
