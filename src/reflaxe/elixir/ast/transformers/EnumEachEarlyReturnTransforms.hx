...
class EnumEachEarlyReturnTransforms {
	...
	public static function pass(ast:ElixirAST):ElixirAST {
		return ElixirASTTransformer.transformNode(ast, function(node:ElixirAST):ElixirAST {
			return switch (node.def) {
				case EBlock(stmts):
					rewriteSequenceAsSameKind(stmts, exprs -> makeASTWithMeta(EBlock(exprs), node.metadata, node.pos), node.metadata, node.pos);
				case EDo(stmts):
					rewriteSequenceAsSameKind(stmts, exprs -> makeASTWithMeta(EDo(exprs), node.metadata, node.pos), node.metadata, node.pos);
				default:
					node;
			}
		});
	...
	static function subtreeContainsFromReturn(node:ElixirAST):Bool {
		...
		ElixirASTTransformer.iterateAST(node, function(child:ElixirAST):Void {
			if (found)
				return;
			if (subtreeContainsFromReturn(child))
				found = true;
		});
		return found;
	}

	static function rewriteSequenceAsSameKind(stmts:Array<ElixirAST>, wrap:Array<ElixirAST>->ElixirAST, meta:ElixirMetadata,
		...
		while (index < stmts.length) {
			...
			if (extracted != null) {
				...
				var caseExpr = makeASTWithMeta(ECase(reduceWhileExpr, [
					{
	...
	static function extractEarlyReturnEnumEach(stmt:ElixirAST):Null<{
		...
		function tryExtractSingle(singleStmt:ElixirAST):Null<{
			...
			if (singleStmt == null || singleStmt.def == null)
				return null;
			...
			switch (bodyExpr.def) {
				case EIf(_c, thenBranch, elseBranch):
					debugLog('Enum.each binder=' + binderName + ' meta=' + debugMetaInfo(callExpr) + ' if? elseNil=' + isImplicitNilElse(elseBranch)
						+ ' loopContainsReturn=' + loopContainsReturn + ' then.fromReturn=' + isFromReturn(thenBranch) + ' then.subtreeFromReturn='
						+ subtreeContainsFromReturn(thenBranch) + ' then.unwrapped.fromReturn=' + isFromReturn(unwrapSingleStmtBlock(thenBranch)));
				case EUnless(_c, body, elseBranch):
					debugLog('Enum.each binder=' + binderName + ' meta=' + debugMetaInfo(callExpr) + ' unless? elseNil=' + isImplicitNilElse(elseBranch)
						+ ' loopContainsReturn=' + loopContainsReturn + ' body.fromReturn=' + isFromReturn(body) + ' body.subtreeFromReturn='
						+ subtreeContainsFromReturn(body) + ' body.unwrapped.fromReturn=' + isFromReturn(unwrapSingleStmtBlock(body)));
				default:
			}
			#end

			var earlyReturn = switch (bodyExpr.def) {
				case EIf(cond, thenBranch, elseBranch) if (isImplicitNilElse(elseBranch)
					&& (branchSignalsReturn(thenBranch) || loopContainsReturn)):
					{condition: cond, value: thenBranch};
				case EUnless(cond, body, elseBranch) if (isImplicitNilElse(elseBranch)
					&& (branchSignalsReturn(body) || loopContainsReturn)):
					{condition: cond, value: body};
				default:
					null;
			};
			if (earlyReturn == null) {
				debugLog('Found Enum.each but body did not match early-return shape; binder='
					+ binderName
					+ ' body='
					+ reflaxe.elixir.util.EnumReflection.enumConstructor(bodyExpr.def));
				return null;
			}
		...
		if (innerStmts != null) {
			var meaningful = [
				for (s in innerStmts)
					if (s != null && s.def != null && !isBareNumericSentinel(s)) s
			];
	...
	static function unwrapIifeReturningFn(target:ElixirAST):Null<ElixirAST> {
		...
		if (outerFn == null || outerFn.length != 1)
			return null;
	...
	static function buildReduceWhile(collection:ElixirAST, binderName:String, condition:ElixirAST, returnValue:ElixirAST, meta:ElixirMetadata,
		...
		return makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "reduce_while", [collection, noReturnAst, reducerFn]), meta, pos);
	}
}
#end
