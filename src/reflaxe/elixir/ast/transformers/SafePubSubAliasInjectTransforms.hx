...
class SafePubSubAliasInjectTransforms {
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body):
					...
					if (name == "Phoenix.SafePubSub")
						return n;
					if (!(moduleUsesSafePubSub(body) || moduleQualifiedUsed(body, "Phoenix.SafePubSub")))
						return n;
				...
				case EDefmodule(name, doBlock):
					...
					if (!(moduleUsesSafePubSub(stmts) || moduleQualifiedUsed(stmts, "Phoenix.SafePubSub")))
						return n;
		...
		function scan(e:ElixirAST):Void {
			...
			switch (e.def) {
				...
				case ECall(tgt, _, args):
					if (tgt != null)
						switch (tgt.def) {
							case EVar(m) if (m == "SafePubSub"): found = true;
							default:
						}
					if (tgt != null)
						scan(tgt);
				...
				case ERemoteCall(remoteTarget, _, remoteArgs):
					switch (remoteTarget.def) {
						case EVar(m) if (m == "SafePubSub"): found = true;
						default:
					}
					scan(remoteTarget);
					if (remoteArgs != null)
						for (arg in remoteArgs)
							scan(arg);
				case EDef(_, _, _, b):
					scan(b);
	...
	static function moduleQualifiedUsed(body:Array<ElixirAST>, moduleName:String):Bool {
		var used = false;
		function scan(e:ElixirAST):Void {
			if (used || e == null || e.def == null)
				return;
			switch (e.def) {
				case ERemoteCall(mod, _, args):
					switch (mod.def) {
						case EVar(m) if (m == moduleName): used = true;
						default:
					}
					scan(mod);
					if (args != null)
				...
				case ECall(tgt, _, argsList):
					if (tgt != null)
						switch (tgt.def) {
							case EVar(moduleVar) if (moduleVar == moduleName): used = true;
							default:
						}
					if (tgt != null)
						scan(tgt);
					if (argsList != null)
						for (a in argsList)
							scan(a);
				case ERaw(code):
					if (code != null && code.indexOf(moduleName + ".") != -1)
						used = true;
				case EDef(_, _, _, b):
					scan(b);
				case EDefp(_, _, _, privateBody):
