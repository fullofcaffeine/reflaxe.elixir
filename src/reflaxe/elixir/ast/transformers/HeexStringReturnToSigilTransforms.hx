...
class HeexStringReturnToSigilTransforms {
	...
	static function toHeex(node:ElixirAST):ElixirAST {
		...
		while (true) {
			...
			if (reflaxe.elixir.util.EnumReflection.enumConstructor(cur.def) != "EParen")
				break;
		}
		switch (cur.def) {
		...
		return ElixirASTTransformer.transformNode(ret, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case EBlock(stmts):
					...
					switch (last.def) {
						case EVar(varName):
							...
							if (rewritten != null) {
								...
								if (ensureAssigns)
									updated.insert(updated.length - 1, makeAST(EMatch(PVar("assigns"), makeAST(EMap([])))));
					...
					if (ensureAssigns)
						newStmts.insert(newStmts.length - 1, makeAST(EMatch(PVar("assigns"), makeAST(EMap([])))));
	...
	static function tryConvertHxxCallToHeex(node:ElixirAST):Null<ElixirAST> {
		switch (node.def) {
			case ECall(mod, fnName, args) if (args != null && args.length >= 1):
				if (!isTemplateProducerCall(mod, fnName))
					return null;
							...
							switch (l.def) {
								case EVar(lhs) if (lhs == varName):
									...
									switch (r.def) {
										...
										case ECall(m, f, a):
											if (isTemplateProducerCall(m,
												f)) return makeASTWithMeta(ESigil("H", '<%= Phoenix.HTML.raw(' + varName + ') %>', ""), last.metadata,
													last.pos);
										default:
									}
								default:
								...
								case PVar(lhs2) if (lhs2 == varName):
									switch (rhs.def) {
										case EString(s2) if (looksLikeHtml(s2)):
											return makeASTWithMeta(ESigil("H", '<%= Phoenix.HTML.raw(' + varName + ') %>', ""), last.metadata, last.pos);
										case ECall(m2, f2, a2):
											if (isTemplateProducerCall(m2,
												f2)) return makeASTWithMeta(ESigil("H", '<%= Phoenix.HTML.raw(' + varName + ') %>', ""), last.metadata,
													last.pos);
										default:
									}
								default:
			...
			return switch (n.def) {
				case EDef(name, args, guards, body) | EDefp(name, args, guards, body):
					...
					if (newBody != body) {
						...
						var newDef = reflaxe.elixir.util.EnumReflection.enumConstructor(n.def) == "EDef" ? EDef(name, argsRenamed, guards,
							newBody) : EDefp(name, argsRenamed, guards, newBody);
						...
						if (name == "render" && args.length == 1 && (hasAssignsParam || hasUnderscoredAssigns)) {
							if (containsHSigilAST(body)) {
								...
								var defWithSigil = reflaxe.elixir.util.EnumReflection.enumConstructor(n.def) == "EDef" ? EDef(name, argsRenamed, guards,
									sig2) : EDefp(name, argsRenamed, guards, sig2);
