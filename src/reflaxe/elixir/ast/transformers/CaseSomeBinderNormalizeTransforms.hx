								...
								case PTuple(parts) if (parts.length == 2):
									...
									if (isSome) switch (parts[1]) {
										case PVar(nm) if (nm != null):
											...
											if (candidate == null || candidate == "" || candidate == "socket")
												candidate = "payload";
											// rewrite clause body
											var body2 = cl.body;
											...
											body2 = ElixirASTTransformer.transformNode(body2, function(x:ElixirAST):ElixirAST {
												return switch (x.def) {
													...
													case ECase(tgt, cls2):
														switch (tgt.def) {
															case EVar(v2) if (v2 == nm): makeASTWithMeta(ECase(makeAST(EVar(candidate)), cls2), x.metadata,
																	x.pos);
															default: x;
														}
													// Repair wrong `{:noreply, nm_or_alias}` returns inside handle_info/2 only
													case ETuple(elems):
														if ((currentFunc == "handleInfo" || currentFunc == "handle_info")
															&& elems.length == 2) {
															switch (elems[0].def) {
