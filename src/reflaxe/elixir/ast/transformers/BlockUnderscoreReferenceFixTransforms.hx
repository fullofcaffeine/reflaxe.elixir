		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EBlock(stmts):
					...
					for (s in stmts)
						collectDecls(s, underscored, baseDecl);
					...
					for (s in stmts)
						newStmts.push(rewriteRefs(s, rename));
	...
	static function collectDecls(node:ElixirAST, underscored:Map<String, Bool>, baseDecl:Map<String, Bool>):Void {
		...
		switch (node.def) {
			...
			case EIf(c, t, e):
				...
				if (e != null)
					collectDecls(e, underscored, baseDecl);
			case ECase(expr, cs):
				collectDecls(expr, underscored, baseDecl);
				for (cl in cs) {
					if (cl.guard != null)
						collectDecls(cl.guard, underscored, baseDecl);
			...
			case ECall(t, _, as):
				...
				if (as != null)
					for (a in as)
						collectDecls(a, underscored, baseDecl);
	...
	static function collectPatternDecls(p:EPattern, underscored:Map<String, Bool>, baseDecl:Map<String, Bool>):Void {
		switch (p) {
			case PVar(n) if (n != null && n.length > 0):
				if (n.charAt(0) == '_')
					underscored.set(n.substr(1), true)
				else
					baseDecl.set(n, true);
			case PTuple(es):
				for (e in es)
					collectPatternDecls(e, underscored, baseDecl);
			case PList(es):
				for (e in es)
					collectPatternDecls(e, underscored, baseDecl);
			case PCons(h, t):
			...
			case PMap(kvs):
				for (kv in kvs)
					collectPatternDecls(kv.value, underscored, baseDecl);
			case PStruct(_, fs):
				for (f in fs)
					collectPatternDecls(f.value, underscored, baseDecl);
			case PPin(inner):
