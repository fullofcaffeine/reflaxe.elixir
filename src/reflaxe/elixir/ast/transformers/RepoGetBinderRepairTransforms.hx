...
class RepoGetBinderRepairTransforms {
	...
	private static function rewriteBodyIfBareUndeclaredVar(body:ElixirAST, params:Array<EPattern>,
			info:Map<String, {repoMod:ElixirAST, schemaArg:ElixirAST}>):ElixirAST {
		...
		if (declared.exists(bareVar))
			return body; // already declared; not our target

		var base = bareVar.toLowerCase();
		var repoInfo:{repoMod:ElixirAST, schemaArg:ElixirAST} = null;
		if (info.exists(base))
			repoInfo = info.get(base)
		else if (info.exists("__repo__")) {
