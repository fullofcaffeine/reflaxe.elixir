...
class HandleInfoReturnSocketNormalizeTransforms {
	...
	static function collectPayloadBinders(body:ElixirAST, msgVar:String, socketVar:String):Map<String, Bool> {
		...
		function collectPatternVars(p:EPattern):Void {
			switch (p) {
				case PVar(name):
					if (name != null && name != msgVar && name != socketVar && name.charAt(0) != '_')
						out.set(name, true);
				case PTuple(items) | PList(items):
					for (it in items)
						collectPatternVars(it);
				case PCons(h, t):
					collectPatternVars(h);
					collectPatternVars(t);
				case PMap(kvs):
					for (kv in kvs)
						collectPatternVars(kv.value);
				case PStruct(_, fields):
					for (f in fields)
						collectPatternVars(f.value);
				case PPin(inner):
	...
	static function renamePVar(p:EPattern, from:String, to:String):EPattern {
		return switch (p) {
			...
			case PMap(kvs):
				...
				for (kv in kvs)
					out.push({key: kv.key, value: renamePVar(kv.value, from, to)});
				PMap(out);
			case PStruct(n, fields):
				var out = new Array<{key:String, value:EPattern}>();
				for (f in fields)
					out.push({key: f.key, value: renamePVar(f.value, from, to)});
				PStruct(n, out);
	...
	static function sameFirstLast(args:Array<ElixirAST>):Null<String> {
		...
		if (firstName == null)
			return null;
		switch (args[args.length - 1].def) {
			case EVar(lastName) if (lastName == firstName):
				return lastName;
			default:
		}
		return null;
