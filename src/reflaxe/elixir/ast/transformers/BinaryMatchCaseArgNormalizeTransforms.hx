...
class BinaryMatchCaseArgNormalizeTransforms {
	static inline function isBinaryModule(mod:ElixirAST):Bool {
		return switch (mod.def) {
			...
			case EAtom(a): var s:String = a; s == ":binary" || s == "binary";
			default:
				false;
		};
	}
		...
		var stmts:Array<ElixirAST> = switch (candidate.def) {
			...
			case EParen(innerExpr):
				switch (innerExpr.def) {
					case EBlock(ss3) if (ss3.length >= 2): ss3;
					case EDo(ss4) if (ss4.length >= 2): ss4;
					default: null;
				}
			default: null;
		}
					...
					case ECase(matchExpr, clauses):
						...
						switch (matchExpr.def) {
							case ERemoteCall(mod, fnName, margs) if (fnName == "match" && margs != null && margs.length == 2 && isBinaryModule(mod)):
								switch (margs[0].def) {
									case EVar(v) if (v == varName): ok = true;
									default:
								}
								if (ok) subExpr = margs[1];
							case ECall(target, fnName2, margs2)
								if (target != null && fnName2 == "match" && margs2 != null && margs2.length == 2 && isBinaryModule(target)):
								switch (margs2[0].def) {
									case EVar(v2) if (v2 == varName): ok = true;
									default:
								}
