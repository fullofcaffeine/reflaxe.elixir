...
class HaxeMapModuleCallRewriteTransforms {
						...
						case ERemoteCall({def: EVar(modName)}, "set", args) if (isHaxeMapModule(modName) && args != null && args.length == 3):
							switch (args[0].def) {
								...
								default:
									...
									makeASTWithMeta(EMatch(PVar("_"), makeAST(ERemoteCall(makeAST(EVar("Map")), "put", [args[0], args[1], args[2]]))),
								...
								case ERemoteCall({def: EVar(modName)}, "set", args) if (isHaxeMapModule(modName) && args != null && args.length == 3):
									switch (args[0].def) {
										...
										default:
											...
											makeASTWithMeta(EBinary(Match, left,
												makeAST(ERemoteCall(makeAST(EVar("Map")), "put", [args[0], args[1], args[2]]))),
												node.metadata, node.pos);
									}
	...
	static function tryRewriteFieldTmpSet(first:ElixirAST, second:ElixirAST):Null<ElixirAST> {
		...
		if (tmpName == null || receiverName == null || fieldName == null)
			return null;

		// second: _ = IntMap.set(tmp, k, v)
		var secondAssign = extractVarAssign(second);
		if (secondAssign == null || secondAssign.name != "_")
			return null;
