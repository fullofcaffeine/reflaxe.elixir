...
class BinderTransforms {
				...
				switch (b.def) {
					case EUse(module, args):
						if (module == appPrefix + "Web") {
							for (a in args)
								switch (a.def) {
									case EAtom(x) if (x == "live_view"): found = true;
									default:
								}
						}
					...
					switch (doBlock.def) {
						case EBlock(stmts) if (app2 != null && !hasLiveUse(stmts, app2)):
							...
							var liveViewOptions2 = makeAST(EKeywordList([
								{
									key: "layout",
									value: makeAST(ETuple([makeAST(EVar(appWebModule2 + ".Layouts")), makeAST(EAtom(ElixirAtom.raw("app")))]))
								}
							]));
		...
		ElixirASTTransformer.transformNode(ast, function(n) {
			switch (n.def) {
				case EModule(name, _, _):
					...
					if (idx > 0)
						derivedApp = name.substring(0, idx);
				default:
			}
			return n;
		});
			...
			function scan(x:ElixirAST):Void {
				...
				switch (x.def) {
					case EField(target, f):
						switch (target.def) {
							case EVar(name) if (name == v && f == field): found = true;
							default: scan(target);
						}
					case EMatch(_, rhs):
						...
						switch (rhs.def) {
							case EField(tgt, f) if (f == field):
								switch (tgt.def) {
									case EVar(name) if (name == v): found = true;
									default:
								}
							default:
						}
				...
				default:
					...
					switch (n.def) {
						...
						case ECase(ex, cls):
							...
							for (c in cls) {
								if (c.guard != null)
									collectModules(c.guard);
								collectModules(c.body);
							}
						case EFn(cs): for (cl in cs)
								collectModules(cl.body);
						case ECall(t, _, args):
							if (t != null)
								collectModules(t);
							...
							case ERemoteCall(mod, func, args):
								switch (mod.def) {
									case EVar(m) if (isSingleSegmentModule(m) && isUpperCamel(m) && !isGlobalWhitelisted(m)):
										if (m == "SafePubSub") {
											return makeASTWithMeta(ECapture(makeAST(ERemoteCall(makeAST(EVar("Phoenix.SafePubSub")), func, args)), arity),
												n.metadata, n.pos);
										}
										var fq = resolveQualifiedModule(m, appPrefix);
										fq != null ? makeASTWithMeta(ECapture(makeAST(ERemoteCall(makeAST(EVar(fq)), func, args)), arity), n.metadata,
											n.pos) : n;
									default: n;
								}
							default: n;
		...
		return ElixirASTTransformer.transformNode(ast, function(node:ElixirAST):ElixirAST {
			return switch (node.def) {
				case EModule(name, attrs, body) if (name.indexOf("Web") != -1):
					...
					var newBody = [];
					for (b in body)
						newBody.push(ElixirASTTransformer.transformNode(b, function(n:ElixirAST):ElixirAST {
							return switch (n.def) {
	...
	public static function erawWebModuleQualificationPass(ast:ElixirAST):ElixirAST {
		inline function isUpper(c:String):Bool
			return c.toUpperCase() == c && c.toLowerCase() != c;
			...
			while (i < code.length) {
				...
				if (!isPrevIdent && isUpper(ch)) {
					...
					if (j < code.length && code.charAt(j) == '.') {
						...
						if (!isAppOrWebRoot
							&& !reflaxe.elixir.ast.StdModuleWhitelist.isWhitelistedRoot(token)
							&& app != null
							&& app.length > 0) {
							out.add(app);
							out.add(".");
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body) if (name.indexOf("Web") != -1):
					...
					var newBody:Array<ElixirAST> = [];
					for (b in body)
						newBody.push(ElixirASTTransformer.transformNode(b, function(x) {
							return switch (x.def) {
								case ERaw(code):
	...
	public static function stringToAtomLiteralPass(ast:ElixirAST):ElixirAST {
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ERemoteCall(mod, func, args):
					switch (mod.def) {
						case EVar(m) if (m == "String"
							&& (func == "to_atom" || func == "to_existing_atom")
							&& args != null
							&& args.length == 1):
							switch (args[0].def) {
  ...
  _ ->
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body):
					...
					var newBody = [
						for (b in body)
							ElixirASTTransformer.transformNode(b, function(x) {
								return switch (x.def) {
	...
	public static function renameSwitchResultVarsPass(ast:ElixirAST):ElixirAST {
		inline function rename(name:String):String {
			return (name != null
				&& name.length >= 23
				&& name.substr(0, 23) == "__elixir_switch_result_") ? ("switch_result_" + name.substr(23)) : name;
		}
	...
	public static function repoAliasInjectionPass(ast:ElixirAST):ElixirAST {
		...
		function referencesRepo(node:ElixirAST):Bool {
			...
			ElixirASTTransformer.transformNode(node, function(n) {
				if (found)
					return n; // early stop not supported; just skip
				switch (n.def) {
					case ERemoteCall(mod, _, _):
						switch (mod.def) {
							case EVar(m) if (m == "Repo"): found = true;
							default:
						}
									...
									switch pat {
										case PTuple(elements) if (elements.length == 3):
											...
											if (tag == "system_alert") {
												...
												var newThird = PVar(third != null
													&& third != "flash_type" ? "flash_type" : (third == null ? "flash_type" : third));
				...
				case ECase(target, clauses):
					...
					for (clause in clauses) {
						...
						var tag:Null<String> = switch (pat) {
							case PTuple(e) if (e.length > 0): extractAtom(e[0]);
							default: null;
						};
						if (tag == "error") {
							var binder = switch (pat) {
								case PTuple(e) if (e.length == 2): switch (e[1]) {
										case PVar(n): n;
										default: null;
									};
								default: null;
										...
										case ECase(target, clauses):
											...
											for (clause in clauses) {
												...
												if ((tag == "ok" || tag == "error") && usesPhoenixController(clause.body)) {
													...
													if (used.indexOf("user") != -1 && desired != "user")
														assigns.push(makeAST(EMatch(PVar("user"), makeAST(EVar(desired)))));
													if (used.indexOf("changeset") != -1 && desired != "changeset")
														assigns.push(makeAST(EMatch(PVar("changeset"), makeAST(EVar(desired)))));
													if (used.indexOf("data") != -1)
														assigns.push(makeAST(EMatch(PVar("data"), makeAST(EVar(desired)))));
													var nb2 = if (assigns.length > 0) switch (clause.body.def) {
														case EBlock(exprs): makeAST(EBlock(assigns.concat(exprs)));
					...
					for (clause in clauses) {
						...
						if (tag == "ok" || tag == "error") {
							...
							if (tag == "ok") {
								...
								if (used.indexOf("user") != -1)
									preferred = "user";
								else if (used.indexOf("data") != -1)
									preferred = "data";
								...
								if (used.indexOf("reason") != -1)
									preferred = "reason";
								else if (used.indexOf("changeset") != -1)
									preferred = "changeset";
							}
	...
	static function preferredNameForTag(tag:String):Null<String> {
		...
		var t = tag;
		inline function contains(substr:String):Bool
			return t.indexOf(substr) != -1;
		inline function startsWith(prefix:String):Bool
			return StringTools.startsWith(t, prefix);
		...
		if (contains("search"))
			return "params"; // search forms typically submit params
		if (contains("query"))
			return "query";
		...
		switch (tag) {
			case "validate":
				return "params";
			case "clear_filters":
				return null; // no payload expected
			case "bulk_update":
				return "action";
	...
	static function collectPatternBinders(pat:EPattern):Array<String> {
		...
		function walk(p:EPattern):Void {
			switch (p) {
				case PVar(name):
					if (name != null && name.length > 0 && isLower(name))
						result.push(name);
				case PTuple(items):
					for (i in items)
						walk(i);
