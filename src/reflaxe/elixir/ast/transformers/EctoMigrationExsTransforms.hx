...
class EctoMigrationExsTransforms {
		...
		return ElixirASTTransformer.transformNode(ast, function(node:ElixirAST):ElixirAST {
			return switch (node.def) {
				case EModule(name, _attrs, body):
					if (!isMigrationModule(node, body))
						return node;
	...
	static function isMigrationModule(node:ElixirAST, body:Array<ElixirAST>):Bool {
		...
		if (node.metadata != null && node.metadata.ectoContext != null)
			switch (node.metadata.ectoContext) {
				case Migration:
					return true;
				default:
			}
	...
	static function buildUpStatements(body:ElixirAST, pos:Position):Null<Array<ElixirAST>> {
		...
		for (si in 0...statements.length) {
			...
			var extra = switch (stmt.def) {
				...
				case EBinary(Match, left, right):
					' match=EBinary(Match, '
					+ Type.enumConstructor(left.def)
					+ ', '
					+ Type.enumConstructor(right.def)
					+ ')';
				default:
					"";
			};
		...
		while (i < statements.length) {
			...
			if (chain == null) {
				...
				if (rendered.length > 220)
					rendered = rendered.substr(0, 220) + "...";
	...
	static function buildCreateTableStatementsFromChain(chain:MigrationCallChain, pos:Position):Null<Array<ElixirAST>> {
		...
		for (i in 1...chain.calls.length) {
			...
			switch (call.name) {
				case "add_id":
					compilerError("Unsupported migration up/0: add_id is not supported in ecto_migrations_exs builds (custom primary keys require create table(..., primary_key: false)). Write a manual Elixir migration for custom primary keys.",
						pos);
					return null;
				...
				case "add_column":
					...
					if (columnName == "id") {
						compilerError("Unsupported migration up/0: adding an explicit \"id\" column is not supported in ecto_migrations_exs builds (create table defaults to an id primary key). Write a manual Elixir migration for custom primary keys.",
							pos);
						return null;
					}
					var addStmt = buildAddColumn(call.args, pos);
					if (addStmt != null)
						tableBody.push(addStmt);
		...
		for (i in 1...chain.calls.length) {
			...
			switch (call.name) {
				...
				case "remove_column":
					...
					if (removeStmt != null)
						tableBody.push(removeStmt);

				case "modify_column":
					var modifyStmt = buildModifyColumn(call.args, pos);
					if (modifyStmt != null)
						tableBody.push(modifyStmt);
	...
	static function buildCreateIndex(tableAtom:ElixirAST, args:Array<ElixirAST>, pos:Position):Null<ElixirAST> {
		...
		if (filteredPairs.length > 0)
			indexArgs.push(makeAST(EKeywordList(filteredPairs)));
	...
	static function buildUniqueConstraint(tableAtom:ElixirAST, args:Array<ElixirAST>, pos:Position):Null<ElixirAST> {
		...
		if (args.length >= 2) {
			...
			if (name != null && name != "")
				pairs.push({key: "name", value: makeAtom(name)});
		}

		var indexArgs:Array<ElixirAST> = [tableAtom, columns];
		if (pairs.length > 0)
			indexArgs.push(makeAST(EKeywordList(pairs)));
		...
		while (true) {
			switch (current.def) {
				...
				case ECall(_target, "apply", applyArgs) if (applyArgs.length == 3):
					...
					if (methodName == null || argElements == null || argElements.length == 0)
						break;
	...
	static function flattenTopLevelStatements(statements:Array<ElixirAST>):Array<ElixirAST> {
		...
		for (statement in statements) {
			for (expanded in expandStatement(statement))
				out.push(expanded);
		}
		return out;
	}
		...
		return switch (unwrapped.def) {
			case EBlock(expressions) | EDo(expressions):
				...
				for (e in expressions) {
					for (expanded in expandStatement(e))
						out.push(expanded);
				}
				out;
			case EMatch(pattern, value):
			...
			case "decimal":
				...
				if (elements.length >= 3) {
					...
					return {
						...
						extraOptions: [{key: "precision", value: precision}, {key: "scale", value: scale}]
					};
				}
		...
		for (p in pairs) {
			switch (p.key) {
				...
				case "name":
					...
					if (n != null && n != "")
						out.push({key: "name", value: makeAtom(n)});
				case "where":
					var w = extractString(p.value);
					if (w != null && w != "")
						out.push({key: "where", value: makeAST(EString(w))});
				case "concurrently":
					out.push({key: "concurrently", value: p.value});
				case "method":
					var usingAtom = mapIndexMethod(p.value);
					if (usingAtom != null)
						out.push({key: "using", value: usingAtom});
				default:
			}
		}
		...
		return switch (unwrap(expr).def) {
			case EMap(mapPairs):
				...
				for (pair in mapPairs) {
					...
					if (keyString != null)
						out.push({key: keyString, value: pair.value});
				}
				out;
			case ENil:
				[];
