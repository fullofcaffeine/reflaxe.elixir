...
class ListMapFilterFixTransforms {
	...
	public static function filterRemoveFixPass(ast:ElixirAST):ElixirAST {
		...
		transformChildren = function(node:ElixirAST, fnParams:Array<String>):ElixirAST {
			return switch (node.def) {
				case EIf(c, t,
					e): makeASTWithMeta(EIf(transformWithParams(c, fnParams), transformWithParams(t, fnParams),
						e != null ? transformWithParams(e, fnParams) : null),
				...
				case ECase(expr, clauses):
					...
					for (c in clauses)
						newClauses.push({
							pattern: c.pattern,
							guard: c.guard != null ? transformWithParams(c.guard, fnParams) : null,
							body: transformWithParams(c.body, fnParams)
						});
					makeASTWithMeta(ECase(transformWithParams(expr, fnParams), newClauses), node.metadata, node.pos);
				case ECall(t, f,
					as): makeASTWithMeta(ECall(t != null ? transformWithParams(t, fnParams) : null, f, [for (a in as) transformWithParams(a, fnParams)]),
						node.metadata, node.pos);
				case ERemoteCall(m, f,
					as): makeASTWithMeta(ERemoteCall(transformWithParams(m, fnParams), f, [for (a in as) transformWithParams(a, fnParams)]), node.metadata,
						node.pos);
				...
				case EMap(ps): makeASTWithMeta(EMap([
						for (p in ps)
							{key: transformWithParams(p.key, fnParams), value: transformWithParams(p.value, fnParams)}
					]), node.metadata, node.pos);
				case EBinary(op, l,
					r): makeASTWithMeta(EBinary(op, transformWithParams(l, fnParams), transformWithParams(r, fnParams)), node.metadata, node.pos);
				case EUnary(op, ex): makeASTWithMeta(EUnary(op, transformWithParams(ex, fnParams)), node.metadata, node.pos);
				case EFn(cs): makeASTWithMeta(EFn([
						for (cl in cs)
							{
								args: cl.args,
								guard: cl.guard != null ? transformWithParams(cl.guard, fnParams) : null,
								body: transformWithParams(cl.body, fnParams)
							}
					]), node.metadata, node.pos);
				...
				switch (bodyExpr.def) {
					case EBinary(NotEqual | StrictNotEqual, left, right):
						...
						if (isFieldIdOfVar(left, param) && varNameEquals(right, param)) {
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(NotEqual, left, newRight))}
							]));
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(NotEqual, newLeft, right))}
							]));
						} else {
							return null;
						}
				...
				switch (bodyExpr.def) {
					case EBinary(Equal, left, right):
						...
						if (isFieldIdOfVar(left, param) && varNameEquals(right, param)) {
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(Equal, left, makeAST(EVar(outerId))))}
							]));
						} else if (varNameEquals(left, param) && isFieldIdOfVar(right, param)) {
							#if debug_list_fix
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(Equal, makeAST(EVar(outerId)), right))}
							]));
						} else {
							return null;
						}
					case EBinary(StrictEqual, left2, right2):
						if (isFieldIdOfVar(left2, param) && varNameEquals(right2, param)) {
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(StrictEqual, left2, makeAST(EVar(outerId))))}
							]));
							...
							#end
							return makeAST(EFn([
								{args: cl.args, guard: cl.guard, body: makeAST(EBinary(StrictEqual, makeAST(EVar(outerId)), right2))}
							]));
						} else {
							return null;
						}
	...
	static function extractOtherIdVarFromEquality(cond:ElixirAST, param:String):Null<String> {
		return switch (cond.def) {
			case EBinary(Equal | StrictEqual, l, r):
				...
				if (lOther != null && rOther == null) lOther else if (rOther != null && lOther == null) rOther else
					null; // Require exactly one side to be the param.id and the other to be other.id
			default:
				null;
		}
	}
	...
	static function resolveOuterIdParam(fnParams:Array<String>):Null<String> {
		...
		for (n in fnParams)
			if (n == "_id")
				return n; // allow underscored variant
		return null;
	}
}
#end
