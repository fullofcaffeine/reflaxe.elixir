...
class HeexEnsureAssignsForNestedSigilsTransforms {
			...
			return switch (n.def) {
				case EDef(name, args, guards, body) | EDefp(name, args, guards, body):
					...
					if (hasAssigns || !containsHSigilAST(body)) {
						...
						var wrapped = makeAST(EBlock([makeAST(EMatch(PVar("assigns"), assignsExpr)), rewrittenBody]));
						var def = reflaxe.elixir.util.EnumReflection.enumConstructor(n.def) == "EDef" ? EDef(name, args, guards,
							wrapped) : EDefp(name, args, guards, wrapped);
		...
		for (a in args)
			switch (a) {
				case PVar(p):
					...
					if ((p == "assigns") || (p == "_assigns"))
						continue;
					if (StringTools.startsWith(p, "_"))
						continue;
					names.push(p);
				default:
			}
	...
	static function rewriteHeexContentParamRefs(content:String, paramNames:Array<String>):String {
		if (content == null || paramNames == null || paramNames.length == 0)
			return content;
	...
	static function rewriteElixirExprParamRefs(code:String, paramNames:Array<String>):String {
		...
		for (p in paramNames)
			if (p != null && p.length > 0 && code.indexOf(p) != -1) {
				mightContain = true;
				break;
			}
		if (!mightContain)
		...
		function skipWs(idx:Int):Int {
			var j = idx;
			while (j < code.length && isWhitespace(code.charAt(j)))
				j++;
			return j;
		}
		function isBindingPosition(tokenEnd:Int):Bool {
			...
			if (ch == "=")
				return true;
			if (ch == "<" && j + 1 < code.length && code.charAt(j + 1) == "-")
				return true;
			if (ch == "-" && j + 1 < code.length && code.charAt(j + 1) == ">")
				return true;
			return false;
		...
		while (i < code.length) {
			...
			if (isIdentStart(ch)) {
				...
				var shouldRewrite = isParamName(token) && prev != "@" && prev != ":" && next != ":" && !isBindingPosition(j);
