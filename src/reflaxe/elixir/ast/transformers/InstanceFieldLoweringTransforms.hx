...
class InstanceFieldLoweringTransforms {
			...
			return switch (node.def) {
				case EModule(name, attributes, body):
					...
					if (instanceFieldSet == null) {
						...
						changed ? makeASTWithMeta(EModule(name, attributes, newBody), node.metadata, node.pos) : node;
					}
				default:
					node;
			}
	...
	static function buildInstanceFieldSet(meta:ElixirMetadata):Null<Map<String, Bool>> {
		if (meta == null || meta.instanceFields == null || meta.instanceFields.length == 0)
			return null;
		var set:Map<String, Bool> = new Map();
		for (f in meta.instanceFields)
			set.set(f, true);
		return set;
	}
	...
	static function rewriteFunctionLike(original:ElixirAST, name:String, args:Array<EPattern>, guards:Null<ElixirAST>, body:ElixirAST,
			instanceFieldSet:Map<String, Bool>):ElixirAST {
		...
		if (receiverBinder == null && name != "new")
			return original;
		...
		function resolveFieldKey(varName:String):Null<String> {
			if (varName == null || varName == "" || varName == structName)
				return null;
			if (instanceFieldSet.exists(varName))
				return varName;
			if (varName.startsWith("_")) {
				var stripped = varName.substr(1);
				if (instanceFieldSet.exists(stripped))
					return stripped;
			}
			return null;
		}
	...
	static function collectPatternVars(p:EPattern, out:Map<String, Bool>):Void {
		...
		switch (p) {
			...
			case PMap(pairs):
				if (pairs != null)
					for (pair in pairs)
						collectPatternVars(pair.value, out);
			case PStruct(_, fields):
				if (fields != null)
					for (f in fields)
						collectPatternVars(f.value, out);
			case PPin(inner):
				collectPatternVars(inner, out);
			case PWildcard:
			case PAlias(varName, inner):
				if (varName != null && varName != "" && varName != "_")
					out.set(varName, true);
				collectPatternVars(inner, out);
			case PBinary(segments):
				if (segments != null)
					for (seg in segments)
						collectPatternVars(seg.pattern, out);
		}
	}
}
#end
