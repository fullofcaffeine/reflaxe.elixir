  ...
  public static function pass(ast: ElixirAST): ElixirAST {
	    return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
	      return switch (n.def) {
	        ...
	        case EDefmodule(name, doBlock) if (isController(name)):
  ...
  static function applyToDefs(node: ElixirAST): ElixirAST {
	    return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
	      return switch (n.def) {
	        ...
	        case EDefp(fn, args, guards, body):
	          makeASTWithMeta(EDefp(fn, args, guards, transformBody(body)), n.metadata, n.pos);
	        default: n;
	      }
	    });
	  }
	  ...
	  static function transformBody(n: ElixirAST): ElixirAST {
	    return switch (n.def) {
	      ...
	      case EDo(stmts): makeASTWithMeta(EDo(rewrite(stmts)), n.metadata, n.pos);
	      default: n;
	    }
	  }
    ...
    var i = 0;
	    while (i < stmts.length) {
	      ...
	      switch (s.def) {
	        ...
	        case EBinary(Match, {def: EVar(binder)}, rhs):
	          var res = tryInline(stmts, i, binder, rhs);
	          if (res.inlined) { out.push(res.nextStmt); i += 2; continue; } else { out.push(s); i++; continue; }
	        default:
	          out.push(s); i++;
	      }
    ...
    if (idx + 1 >= stmts.length) return {inlined:false, nextStmt:null};
	    ...
	    for (j in idx+1...stmts.length) {
	      ...
	      switch (stmts[j].def) {
	        ...
	        case EBinary(Match, {def: EVar(n)}, _) if (n == name): reassigned = true;
	        default:
	      }
      if (reassigned) break;
      var found = false;
