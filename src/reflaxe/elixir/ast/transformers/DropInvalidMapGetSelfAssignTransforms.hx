...
class DropInvalidMapGetSelfAssignTransforms {
	...
	static function filterBody(body:ElixirAST):ElixirAST {
		return switch (body.def) {
			case EBlock(stmts):
				...
				for (s in stmts)
					if (!isSelfAssignMapGet(s))
						out.push(s);
				makeASTWithMeta(EBlock(out), body.metadata, body.pos);
			case EDo(statements):
				var out:Array<ElixirAST> = [];
				for (stmt in statements)
					if (!isSelfAssignMapGet(stmt))
						out.push(stmt);
	...
	static function isSelfAssignMapGet(n:ElixirAST):Bool {
		return switch (n.def) {
			case EBinary(Match, lhs, rhs): var keyL = extractMapGetKey(lhs); var keyR = extractMapGetKey(rhs); keyL != null && keyR != null && keyL == keyR;
			default: false;
		}
	}
