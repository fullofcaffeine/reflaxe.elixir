...
class ControllerJsonSecondArgUndefinedRewriteUltimateTransforms {
	...
	static function rewriteStmts(stmts:Array<ElixirAST>, binder:Null<String>):Array<ElixirAST> {
		var out:Array<ElixirAST> = [];
		for (s in stmts)
			out.push(rewriteWithContext(s, binder));
		return out;
	}
				...
				case ERemoteCall(t, fnName, args) if (fnName == "json" && args != null && args.length == 2 && isPhoenixController(t)):
					switch (args[1].def) {
						case EVar(v) if (v == "data"):
							...
							if (binder != null)
								replacement = makeAST(EVar(binder));
							if (replacement == null) {
								var cand = singleLowerUsedVar(node);
								if (cand != null)
									replacement = makeAST(EVar(cand));
							}
							if (replacement == null)
								replacement = makeAST(EMap([]));
			...
			switch (n.def) {
				case EVar(v):
					if (v != null && v.length > 0) {
						var c = v.charAt(0);
						if (c.toLowerCase() == c && v != "conn" && v != "params" && v != "socket" && v != "live_socket")
							used.set(v, true);
					}
				default:
			}
