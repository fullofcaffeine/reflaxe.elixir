	...
	static function promoteAt(n:ElixirAST, isPrivate:Bool):ElixirAST {
		...
		switch (third) {
			...
			case PVar(pname) if (pname != null && pname != "socket"):
				...
				return makeASTWithMeta(isPrivate ? EDefp("mount", newParams, getGuards(n), newBody) : EDef("mount", newParams, getGuards(n), newBody),
					n.metadata, n.pos);
			...
			case PVar(pname2) if (pname2 == "socket"):
				...
				return makeASTWithMeta(isPrivate ? EDefp("mount", params, getGuards(n), fixedBody) : EDef("mount", params, getGuards(n), fixedBody),
					n.metadata, n.pos);
			default:
				return n;
		}
