...
class LocalVarNameNormalizeTransforms {
		...
		function normalizeTree(node:ElixirAST):ElixirAST {
			...
			return switch (withChildren.def) {
				...
				case EDef(name, args, guards, body):
					makeASTWithMeta(EDef(name, args.map(normalizePattern), guards, body), withChildren.metadata, withChildren.pos);

				case EDefp(name, args, guards, body):
					makeASTWithMeta(EDefp(name, args.map(normalizePattern), guards, body), withChildren.metadata, withChildren.pos);
				...
				case EFn(clauses):
					makeASTWithMeta(EFn(clauses.map(cl -> {
						...
						guard: cl.guard,
						body: cl.body
					})), withChildren.metadata, withChildren.pos);
				...
				case EFor(generators, filters, body, into, uniq):
					makeASTWithMeta(EFor(generators.map(g -> {pattern: normalizePattern(g.pattern), expr: g.expr}), filters, body, into, uniq),
						withChildren.metadata, withChildren.pos);

				case ECase(expr, clauses):
					makeASTWithMeta(ECase(expr, clauses.map(cl -> {
				...
				case EWith(clauses, doBlock, elseBlock):
					makeASTWithMeta(EWith(clauses.map(c -> {pattern: normalizePattern(c.pattern), expr: c.expr}), doBlock, elseBlock), withChildren.metadata,
						withChildren.pos);

				case ETry(body, rescueClauses, catchClauses, afterBlock, elseBlock):
					makeASTWithMeta(ETry(body, rescueClauses.map(r -> {
						...
						body: c.body
					}), afterBlock, elseBlock), withChildren.metadata, withChildren.pos);

				case EReceive(clauses, afterClause):
					makeASTWithMeta(EReceive(clauses.map(cl -> {
						pattern: normalizePattern(cl.pattern),
						guard: cl.guard,
						body: cl.body
					}), afterClause), withChildren.metadata, withChildren.pos);
	...
	static function isSimpleIdent(code:String):Bool {
		...
		for (i in 0...code.length) {
			...
			var ok = (c >= 'A'.code && c <= 'Z'.code) || (c >= 'a'.code && c <= 'z'.code) || (c >= '0'.code && c <= '9'.code) || c == '_'.code;
			if (!ok)
				return false;
		}
		return true;
