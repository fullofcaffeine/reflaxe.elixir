								...
								case EBinary(Match, {def: EVar(a2)}, {def: EVar(b2)}):
									switch (stmts2[j + 1].def) {
										case EIf(cond2, then2, else2):
											...
											if (elseIsB2) {
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a2), stmts2[j].metadata, stmts2[j].pos),
													makeASTWithMeta(EIf(cond2, then2, else2), stmts2[j + 1].metadata, stmts2[j + 1].pos)),
													stmts2[j].metadata, stmts2[j].pos));
												j += 2;
												continue;
											}
										default:
