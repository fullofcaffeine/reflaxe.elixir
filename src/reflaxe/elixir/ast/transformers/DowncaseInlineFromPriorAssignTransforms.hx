...
/**
	...
	*   (generated by std/elixir/_std/String.hx's toLowerCase() via __elixir__)
	*
	* WHY
		...
		for (i in 0...stmts.length) {
			...
			switch (s.def) {
				case EBinary(Match, left, rhs):
					switch (left.def) {
						case EVar(v): assigns.set(v, {idx: out.length, rhs: rhs});
						default:
					}
				case ERemoteCall({def: EVar("String")}, "downcase", args) if (args != null && args.length == 1):
					switch (args[0].def) {
						case EVar(vname)
							if (assigns.exists(vname)
								&& !usedBetween(out, stmts, assigns.get(vname).idx + 1, i, vname)
								&& !OptimizedVarUseAnalyzer.usedLater(useIndex, i + 1, vname)):
							var rec = assigns.get(vname);
							// remove earlier assign from out
							out = out.slice(0, rec.idx).concat(out.slice(rec.idx + 1));
							replaced = makeASTWithMeta(ERemoteCall(makeASTWithMeta(EVar("String"), s.metadata, s.pos), "downcase", [rec.rhs]), s.metadata,
								s.pos);
						default:
					}
