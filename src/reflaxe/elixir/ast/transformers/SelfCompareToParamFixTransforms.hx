		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EDef(name, args, guards, body):
					...
					for (a in args)
						switch (a) {
							case PVar(nm) if (nm == "id" || nm == "_id"): idName = nm;
							default:
						}
				...
				case EDefp(name2, args2, guards2, body2):
					...
					for (a2 in args2)
						switch (a2) {
							case PVar(nm2) if (nm2 == "id" || nm2 == "_id"): idName2 = nm2;
							default:
						}
					...
					if (binder == null) x else {
						...
						switch (bodyExpr.def) {
							...
							case EBinary(NotEqual | StrictNotEqual, l, r):
								inline function isBinderVar(e:ElixirAST):Bool
									return switch (unwrapSingleExpr(e).def) {
										case EVar(n) if (n == binder): true;
										default: false;
									};
								inline function isBinderIdField(e:ElixirAST):Bool
									return switch (unwrapSingleExpr(e).def) {
										case EField({def: EVar(n2)}, fld) if (n2 == binder && fld == "id"): true;
										default: false;
									};
							...
							case EBinary(Equal | StrictEqual, l2, r2):
								inline function isBinderVar2(e:ElixirAST):Bool
									return switch (unwrapSingleExpr(e).def) {
										case EVar(n) if (n == binder): true;
										default: false;
									};
								inline function isBinderIdField2(e:ElixirAST):Bool
									return switch (unwrapSingleExpr(e).def) {
										case EField({def: EVar(n2)}, fld) if (n2 == binder && fld == "id"): true;
										default: false;
									};
								var eqOp = switch (bodyExpr.def) {
