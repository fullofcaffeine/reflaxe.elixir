...
class DuplicateEffectfulCallPruneTransforms {
	...
	static function callsEqual(a:ElixirAST, b:ElixirAST):Bool {
		return switch [a.def, b.def] {
			case [ERemoteCall(ma, fa, aa), ERemoteCall(mb, fb, ab)]: exprEqual(ma, mb) && fa == fb && argsEqual(aa, ab);
			case [ECall(ta, fa, aa), ECall(tb, fb, ab)]: exprEqual(ta, tb) && fa == fb && argsEqual(aa, ab);
			default: false;
		}
	}
