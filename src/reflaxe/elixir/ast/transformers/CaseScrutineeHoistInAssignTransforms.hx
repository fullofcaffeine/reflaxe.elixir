				...
				case EBinary(Match, {def: EVar(lhs)}, rhs):
					...
					if (rhsCaseInfo == null) n else {
						...
						var scrutineeIsListOrBinary = switch (caseScrutinee.def) {
							case EList(_) | EBitstring(_): true;
							default: false;
						};
						if (!scrutineeIsListOrBinary)
							...
							var assignToLhs = makeASTWithMeta(EBinary(Match, makeAST(EVar(lhs)),
								makeAST(ECase(makeAST(EVar(hoistedVarName)), repairedClauses))), n.metadata, n.pos);
				...
				case EMatch(PVar(lhsName), rhsMatched):
					...
					if (rhsCaseInfoInner == null) n else {
						...
						if (!scrutineeIsListOrBinaryInner)
							...
							var assignToLhsInner = makeASTWithMeta(EBinary(Match, makeAST(EVar(lhsName)),
								makeAST(ECase(makeAST(EVar(hoistedVarNameInner)), repairedClausesInner))), n.metadata,
								n.pos);
