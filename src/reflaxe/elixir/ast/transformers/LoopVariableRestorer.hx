	...
	static function transformWithMetadata(node:ElixirAST):ElixirAST {
		...
		return switch (node.def) {
			...
			case EModule(name, attributes, body):
				makeASTWithMeta(EModule(name, attributes, body.map(b -> {
			...
			case EBlock(statements):
				makeASTWithMeta(EBlock(statements.map(s -> {
					...
					switch (arg.def) {
						case EFn(clauses):
							var newClauses = clauses.map(clause -> {
								...
								if (bodyWithMeta.metadata == null)
									bodyWithMeta.metadata = {};
		...
		return switch (node.def) {
			...
			case ERemoteCall(module, "each", [range, fn]):
				...
				if (isEnumModule(module) && isAnonymousFunction(fn)) {
					...
					makeASTWithMeta(ERemoteCall(transformWithContext(module, context), "each",
			...
			case EModule(name, attributes, body):
				...
				if (name == "Main") {}
				#end
				makeASTWithMeta(EModule(name, attributes, body.map(b -> transformWithContext(b, context))), node.metadata, node.pos);
			...
			case EBlock(statements):
				makeASTWithMeta(EBlock(statements.map(s -> transformWithContext(s, context))), node.metadata, node.pos);

			case ECall(target, func, args):
				makeASTWithMeta(ECall(target != null ? transformWithContext(target, context) : null, func, args.map(a -> transformWithContext(a, context))),
					node.metadata, node.pos);

			case ERemoteCall(module, func, args):
				makeASTWithMeta(ERemoteCall(transformWithContext(module, context), func, args.map(a -> transformWithContext(a, context))), node.metadata,
					node.pos);
