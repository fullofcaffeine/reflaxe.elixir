		...
		return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case ECase(expr, clauses):
					var newClauses = [];
					for (cl in clauses)
						newClauses.push({pattern: cl.pattern, guard: cl.guard, body: drop(cl.body)});
		...
		for (i in 0...stmts.length) {
			...
			switch (s.def) {
				case EMatch(PVar(nm), rhs) if (isAliasName(nm) && isPure(rhs) && !OptimizedVarUseAnalyzer.usedLater(useIndex, i + 1, nm)):
					// drop
				case EBinary(Match, {def: EVar(binderName)}, rhs)
					if (isAliasName(binderName) && isPure(rhs) && !OptimizedVarUseAnalyzer.usedLater(useIndex, i + 1, binderName)):
					// drop
				default:
					out.push(s);
			}
