				...
				case EBinary(Match, {def: EVar(a)}, rhsAny):
					switch (rhsAny.def) {
						case EBinary(Match, {def: EVar(b)}, rhs):
							...
							out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos), makeASTWithMeta(EVar(b), s.metadata, s.pos)),
								s.metadata, s.pos));
							splitDone = true;
						default:
					}
					...
					if (!splitDone) {
						switch (rhsAny2.def) {
							case EBinary(Match, {def: EVar(rightVar)}, rhsExpr):
								...
								out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(leftVar), s.metadata, s.pos),
									makeASTWithMeta(EVar(rightVar), s.metadata, s.pos)),
									s.metadata, s.pos));
								splitDone = true;
							default:
						}
