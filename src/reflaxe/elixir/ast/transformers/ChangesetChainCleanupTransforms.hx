					...
					switch (rhs.def) {
						...
						case EBinary(Match, leftInner2, expr2):
							var innerIsCs = switch (leftInner2.def) {
								case EVar(n2) if (n2 == "cs"): true;
								default: false;
							};
							var outerIsThis = switch (left.def) {
								case EVar(n3) if (n3 != null && (n3.indexOf("this") == 0 || n3.indexOf("_this") == 0)): true;
								default: false;
							};
			...
			return switch (x.def) {
				...
				case EDo(stmts):
					...
					while (trimmedDo.length > 0) {
						...
						if (isAliasDo)
							trimmedDo = trimmedDo.slice(0, trimmedDo.length - 1);
						else
							break;
					}
