...
class KernelImportExceptThenTransforms {
		...
		return ElixirASTTransformer.transformNode(ast, function(node:ElixirAST):ElixirAST {
			return switch (node.def) {
				case EModule(name, attrs, body):
					...
					updatedBody == body ? node : makeASTWithMeta(EModule(name, attrs, updatedBody), node.metadata, node.pos);
		...
		for (i in 0...body.length) {
			...
			switch (stmt.def) {
				case EImport(module, only, except, warn) if (module == "Kernel"):
					...
					if (updatedExcept == except && warn == updatedWarn)
						return body;
	...
	static function collectKernelConflictExcepts(body:Array<ElixirAST>):Array<EImportOption> {
		...
		for (stmt in body) {
			if (stmt == null || stmt.def == null)
				continue;
			switch (stmt.def) {
				case EDef(name, args, _, _) | EDefp(name, args, _, _) | EDefmacro(name, args, _, _) | EDefmacrop(name, args, _, _):
		...
		if (required.exists("then/2"))
			out.push({name: "then", arity: 2});
		if (required.exists("to_string/1"))
			out.push({name: "to_string", arity: 1});
		if (required.exists("length/1"))
			out.push({name: "length", arity: 1});
		return out;
	}
	...
	static function ensureExceptHasAll(except:Null<Array<EImportOption>>, required:Array<EImportOption>):Array<EImportOption> {
		...
		function has(name:String, arity:Int):Bool {
			for (opt in options) {
				if (opt != null && opt.name == name && opt.arity == arity)
					return true;
			}
			return false;
		}
