	...
	static function ensureInDef(d:ElixirAST):ElixirAST {
		return switch (d.def) {
			case EDef(fname, args, guards, body) if (fname == "start" && args.length == 2):
				...
				var transformed = ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
					return switch (x.def) {
						...
						case EBinary(Match, left, rhs):
							switch (left.def) {
								case EVar(v) if (v == "children" || v == "_children"): childrenName = v;
								default:
							}
							x;
				...
				if (hasStartLink)
					return makeASTWithMeta(EDef(fname, args, guards, transformed), d.metadata, d.pos);
