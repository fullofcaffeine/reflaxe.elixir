...
class UnusedDefpPrune {
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body):
					...
					var isPhoenixWeb = (n.metadata?.isPhoenixWeb == true)
				...
				case EDefmodule(name, doBlock):
					var isPhoenixWeb2 = (n.metadata?.isPhoenixWeb == true)
						|| (name != null && StringTools.endsWith(name, "Web") && name.indexOf(".") == -1);
					if (isPhoenixWeb2)
						return n;
	...
	static function pruneModuleBody(moduleName:String, stmts:Array<ElixirAST>, attrs:Array<EAttribute>):Array<ElixirAST> {
		...
		for (k in defpNames.keys()) {
			...
			var snake = toSnake(k);
			var camel = toCamel(k);
			if (list.indexOf(snake) == -1)
				list.push(snake);
			if (list.indexOf(camel) == -1)
				list.push(camel);
		...
		function visit(e:ElixirAST):Void {
			...
			switch (e.def) {
				...
				case ECall(tgt, fname, args):
					if (tgt != null && isSelfModuleExpr(tgt) && defpNames.exists(fname))
						used.set(fname, true);
					if (tgt != null)
						visit(tgt);
					for (a in args)
				...
				case EReceive(clauses, afterClause):
					...
					if (afterClause != null) {
						visit(afterClause.timeout);
						visit(afterClause.body);
					}
				default:
			}
		}
				...
				switch (a.value.def) {
					case ETuple([kind, kws]):
						var isNowarn = switch (kind.def) {
							case EAtom(atom) if (atom == "nowarn_unused_function"): true;
							default: false;
						};
						if (!isNowarn)
							break;
