			...
			switch (b.def) {
				case EDef(name, args, guards, inner):
					var newInner = switch (inner.def) {
						case EBlock(ss): makeASTWithMeta(EBlock([
								for (s in ss)
									switch (s.def) {
							...
							].filter(x -> x != null)), inner.metadata, inner.pos);
						case EDo(ss2): makeASTWithMeta(EDo([
								for (s in ss2)
									switch (s.def) {
										case ERequire(mod, _) if (mod == "Ecto.Query"):
											null;
				...
				case EDefp(privateName, privateArgs, privateGuards, inner):
					var newInner = switch (inner.def) {
						case EBlock(ss3): makeASTWithMeta(EBlock([
								for (s in ss3)
									switch (s.def) {
										case ERequire(mod, _) if (mod == "Ecto.Query"):
											null;
						...
						case EDo(ss4): makeASTWithMeta(EDo([
								for (s in ss4)
									switch (s.def) {
										case ERequire(mod, _) if (mod == "Ecto.Query"):
											null;
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body):
					...
					if (!hasModuleLevelRequire(newBody))
						newBody = [makeAST(ERequire("Ecto.Query", null))].concat(newBody);
				...
				case EDefmodule(name, doBlock):
					...
					if (!hasModuleLevelRequire(newStmts))
						newStmts = [makeAST(ERequire("Ecto.Query", null))].concat(newStmts);
