...
class HeexLetUnusedBinderUnderscoreTransforms {
	...
	static function rewriteSigil(original:ElixirAST, sigilType:String, content:String, modifiers:String):ElixirAST {
		...
		try
			nodes = reflaxe.elixir.ast.builders.HeexFragmentBuilder.build(content)
		catch (_:Dynamic)
			nodes = null;
		...
		for (node in nodes)
			collectEditsFromNode(node, edits);
		if (edits.length == 0)
			return original;
		...
		for (e in edits) {
			if (e == null)
				continue;
			if (e.start < 0 || e.end < e.start || e.end > nextContent.length)
				continue;
	...
	static function maybeAddLetBinderEdit(attributes:Array<EAttribute>, children:Array<ElixirAST>, edits:Array<{start:Int, end:Int, replacement:String}>):Void {
		...
		if (letAttr == null || letAttr.value == null)
			return;
		...
		if (varName == null || varName.length == 0)
			return;
		if (varName == "_" || varName.charAt(0) == "_")
			return;

		if (children != null && isVarUsedInHeexNodes(varName, children))
			return;
	...
	static function isVarUsedInHeexNodes(varName:String, nodes:Array<ElixirAST>):Bool {
		...
		for (n in nodes) {
			if (n == null || n.def == null)
				continue;
			if (isVarUsedInHeexNode(varName, n))
				return true;
		}
		return false;
	}
				...
				if (bindsLetVarName(attributes, varName)) {
					...
					if (attributes != null) {
						for (a in attributes) {
							...
							if (a.name == ":let")
								continue;
							if (exprContainsVar(varName, a.value))
								return true;
						}
					}
					if (children != null) {
						for (c in children)
							if (isVarUsedInHeexNode(varName, c))
								return true;
					}
					false;
				}
	...
	static function bindsLetVarName(attributes:Array<EAttribute>, varName:String):Bool {
		if (attributes == null || attributes.length == 0)
			return false;
		for (a in attributes) {
			if (a == null || a.name != ":let" || a.value == null)
				continue;
			switch (a.value.def) {
			...
			switch (e.def) {
				case EVar(name):
					if (name == varName)
						found = true;
				case ERaw(code):
					if (code != null && containsElixirVarToken(code, varName))
						found = true;
				case EFragment(_tag, attributes, children):
