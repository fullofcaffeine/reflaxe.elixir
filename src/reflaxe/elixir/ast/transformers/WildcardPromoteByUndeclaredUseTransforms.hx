...
class WildcardPromoteByUndeclaredUseTransforms {
				...
				case EDef(fname, args, guards, body):
					...
					switch (b.def) {
						case EBlock(stmts):
							...
							for (a in args)
								switch (a) {
									case PVar(nm): declared.set(nm, true);
									default:
								}
									...
									case EBinary(Match, left, rhs2):
										...
										if (isWild) {
											...
											if (promoted2 != null) {
												...
												if (cands2.length == 0)
													cands2 = collectUndeclaredRefs(stmts, i + 1, declared);
												if (cands2.length == 1) {
													var nm2 = cands2[0];
						...
						case EDo(stmts2):
							...
							for (a in args)
								switch (a) {
									case PVar(nm): declared2.set(nm, true);
									default:
								}
							for (s in stmts2)
								switch (s.def) {
									case EMatch(PVar(n3), _): declared2.set(n3, true);
									case EBinary(Match, left3, _): switch (left3.def) {
											case EVar(n4): declared2.set(n4, true);
											default:
										}
									default:
								}
								...
								switch (s2.def) {
									case EMatch(PWildcard, rhs5):
										...
										if (p1 != null) {
											...
											if (c1.length == 0)
												c1 = collectUndeclaredRefs(stmts2, i + 1, declared2);
											if (c1.length == 1) {
												var nm = c1[0];
									...
									case EBinary(Match, left4, rhs6):
										...
										if (isWild2) {
											...
											if (p2 != null) {
												...
												if (c2.length == 0)
													c2 = collectUndeclaredRefs(stmts2, i + 1, declared2);
												if (c2.length == 1) {
													var nm2 = c2[0];
	...
	static function laterUsesLengthOf(stmts:Array<ElixirAST>, start:Int, name:String):Bool {
		for (j in start...stmts.length)
			if (hasLengthCallWith(stmts[j], name))
				return true;
		return false;
		return false;
	}
			...
			case EMap(_) | EKeywordList(_):
				...
				if (vars.length == 1) {
					...
					if (!declared.exists(v))
						return makeASTWithMeta(EMatch(PVar(v), rhs), rhs.metadata, rhs.pos);
				}
			case ECall(_, _, _):
				/*
						...
						switch (argsA[1].def) {
							case EMap(pairsA):
								...
								for (p in pairsA) {
									...
									for (k in vals.keys())
										seen.set(k, (seen.exists(k) ? seen.get(k) : 0) + 1);
								}
	...
	static function tryPromoteWildcardBinary(rhs:ElixirAST, stmts:Array<ElixirAST>, idx:Int, declared:Map<String, Bool>, meta:ElixirMetadata,
			pos:Position):Null<ElixirAST> {
	...
	static function hasLengthCallWith(n:ElixirAST, name:String):Bool {
		var seen = false;
		function walk(x:ElixirAST):Void {
			if (seen || x == null || x.def == null)
				return;
			switch (x.def) {
				case ECall(_, fn, args) if (fn == "length"):
					for (a in args)
						switch (a.def) {
							case EVar(v) if (v == name): seen = true;
							default:
						}
				case ERemoteCall(_, fn2, args2) if (fn2 == "length"):
					for (a2 in args2)
						switch (a2.def) {
							case EVar(v2) if (v2 == name): seen = true;
							default:
						}
				case EBlock(ss):
					for (s in ss)
	...
	static function laterUsesDateTimeToIso(stmts:Array<ElixirAST>, idx:Int, name:String):Bool {
		for (k in idx + 1...stmts.length)
			if (hasDateTimeToIso(stmts[k], name))
				return true;
		return false;
	}
	...
	static function collectLengthArgNames(stmts:Array<ElixirAST>, start:Int, declared:Map<String, Bool>, window:Int):Array<String> {
		...
		for (k in start...endIdx)
			collectNamesInLength(stmts[k], out);
		var names = [];
		for (nm in out.keys())
				...
				if (modStr == "DateTime")
					for (a in args)
						switch (a.def) {
							...
							default: scanDateTimeToIso(a, out);
						}
			case EBlock(ss):
				for (s in ss)
					scanDateTimeToIso(s, out);
			case EIf(c, t, e):
				scanDateTimeToIso(c, out);
				scanDateTimeToIso(t, out);
				if (e != null)
					scanDateTimeToIso(e, out);
			case EBinary(_, l, r):
				scanDateTimeToIso(l, out);
				scanDateTimeToIso(r, out);
			case ECall(tgt, _, argsX):
				if (tgt != null)
					scanDateTimeToIso(tgt, out);
				for (aX in argsX)
					scanDateTimeToIso(aX, out);
			case ERemoteCall(tgt2, _, argsY):
				scanDateTimeToIso(tgt2, out);
				for (aY in argsY)
					scanDateTimeToIso(aY, out);
			case ETuple(elems):
				for (e in elems)
					scanDateTimeToIso(e, out);
			default:
		...
		switch (n.def) {
			case ECall(_, fn, args) if (fn == "length"):
				for (a in args)
					switch (a.def) {
						case EVar(v): out.set(v, true);
						default: collectNamesInLength(a, out);
					}
			case ERemoteCall(_, fn2, args2) if (fn2 == "length"):
				for (a2 in args2)
					switch (a2.def) {
						case EVar(v2): out.set(v2, true);
						default: collectNamesInLength(a2, out);
					}
			case EBlock(ss):
				for (s in ss)
					collectNamesInLength(s, out);
			case EIf(c, t, e):
				collectNamesInLength(c, out);
				collectNamesInLength(t, out);
				if (e != null)
					collectNamesInLength(e, out);
			case EBinary(_, l, r):
				collectNamesInLength(l, out);
				collectNamesInLength(r, out);
			case ECall(tgt, _, argsX):
				if (tgt != null)
					collectNamesInLength(tgt, out);
				for (aX in argsX)
					collectNamesInLength(aX, out);
			case ERemoteCall(tgt2, _, argsY):
				collectNamesInLength(tgt2, out);
				for (aY in argsY)
					collectNamesInLength(aY, out);
			case EKeywordList(pairs):
				for (p in pairs)
					collectNamesInLength(p.value, out);
			case EMap(pairs):
				for (p in pairs)
					collectNamesInLength(p.value, out);
			case EStructUpdate(base, fields):
				...
				for (f in fields)
					collectNamesInLength(f.value, out);
			case ETuple(elems):
				for (e in elems)
					collectNamesInLength(e, out);
			default:
	...
	static function collectTargetedCandidates(stmts:Array<ElixirAST>, idx:Int, declared:Map<String, Bool>, window:Int):Array<String> {
		var out:Map<String, Bool> = new Map();
		for (nm in collectLengthArgNames(stmts, idx + 1, declared, window))
			out.set(nm, true);
		for (nm in collectRemoteArgVars(stmts, idx + 1, declared, window, "Phoenix.Component", "assign", 0))
			out.set(nm, true);
		for (nm in collectRemoteArgVars(stmts, idx + 1, declared, window, "Phoenix.Component", "assign", 1))
			out.set(nm, true);
		for (nm in collectAssignMapValueVars(stmts, idx + 1, window))
			out.set(nm, true);
		var names = [];
		for (nm in out.keys())
			if (!declared.exists(nm))
	...
	static function collectRemoteArgVars(stmts:Array<ElixirAST>, start:Int, declared:Map<String, Bool>, window:Int, modName:String, fnName:String,
			argIndex:Int):Array<String> {
		var out:Map<String, Bool> = new Map();
		var endIdx = Std.int(Math.min(stmts.length, start + window));
		for (k in start...endIdx)
			scanRemoteArgVar(stmts[k], out, modName, fnName, argIndex);
		var names = [];
		for (nm in out.keys())
			if (!declared.exists(nm))
				names.push(nm);
		...
		switch (n.def) {
			case ERemoteCall(mod, fn, args) if (fn == fnName):
				...
				if (m == modName && argIndex < args.length)
					switch (args[argIndex].def) {
						case EVar(v): out.set(v, true);
						default:
					}
				for (a in args)
					scanRemoteArgVar(a, out, modName, fnName, argIndex);
			case EBlock(ss):
				for (s in ss)
					scanRemoteArgVar(s, out, modName, fnName, argIndex);
			case EIf(c, t, e):
				scanRemoteArgVar(c, out, modName, fnName, argIndex);
				scanRemoteArgVar(t, out, modName, fnName, argIndex);
				if (e != null)
					scanRemoteArgVar(e, out, modName, fnName, argIndex);
			case EBinary(_, l, r):
				scanRemoteArgVar(l, out, modName, fnName, argIndex);
				scanRemoteArgVar(r, out, modName, fnName, argIndex);
			case ECall(tgt, _, argsX):
				if (tgt != null)
					scanRemoteArgVar(tgt, out, modName, fnName, argIndex);
				for (aX in argsX)
					scanRemoteArgVar(aX, out, modName, fnName, argIndex);
			case ERemoteCall(tgt2, _, argsY):
				scanRemoteArgVar(tgt2, out, modName, fnName, argIndex);
				for (aY in argsY)
					scanRemoteArgVar(aY, out, modName, fnName, argIndex);
			case ETuple(elems):
				for (e in elems)
					scanRemoteArgVar(e, out, modName, fnName, argIndex);
			case EMap(pairs):
				for (p in pairs)
					scanRemoteArgVar(p.value, out, modName, fnName, argIndex);
			default:
		}
	}
	...
	static function collectAssignMapValueVars(stmts:Array<ElixirAST>, start:Int, window:Int):Array<String> {
		...
		var endIdx = Std.int(Math.min(stmts.length, start + window));
		for (k in start...endIdx)
			scanAssignMapValueVars(stmts[k], out);
		var names = [];
		for (nm in out.keys())
			names.push(nm);
		return names;
			...
			case ERemoteCall(mod, fn, args) if (fn == "assign"):
				...
				if (m == "Phoenix.Component" && args.length >= 2) {
					switch (args[1].def) {
						case EMap(pairs):
							for (p in pairs)
								collectFieldBasesFromExpr(p.value, out);
						default:
					}
				}
				for (a in args)
					scanAssignMapValueVars(a, out);
			case EBlock(ss):
				for (s in ss)
					scanAssignMapValueVars(s, out);
			case EIf(c, t, e):
				scanAssignMapValueVars(c, out);
				scanAssignMapValueVars(t, out);
				if (e != null)
					scanAssignMapValueVars(e, out);
			case EBinary(_, l, r):
				scanAssignMapValueVars(l, out);
				scanAssignMapValueVars(r, out);
			case ECall(tgt, _, argsX):
				if (tgt != null)
					scanAssignMapValueVars(tgt, out);
				for (aX in argsX)
					scanAssignMapValueVars(aX, out);
			case ERemoteCall(tgt2, _, argsY):
				scanAssignMapValueVars(tgt2, out);
				for (aY in argsY)
					scanAssignMapValueVars(aY, out);
			case ETuple(elems):
				for (e in elems)
					scanAssignMapValueVars(e, out);
			case EMap(pairs):
	...
	static function collectFieldBasesFromExpr(x:ElixirAST, out:Map<String, Bool>):Void {
		...
		switch (x.def) {
			case EField(obj, _):
				switch (obj.def) {
					case EVar(vf): out.set(vf, true);
					default: collectFieldBasesFromExpr(obj, out);
				}
			case EBlock(ss):
				for (s in ss)
					collectFieldBasesFromExpr(s, out);
			case EIf(c, t, e):
				collectFieldBasesFromExpr(c, out);
				collectFieldBasesFromExpr(t, out);
				if (e != null)
					collectFieldBasesFromExpr(e, out);
			case EBinary(_, l, r):
				collectFieldBasesFromExpr(l, out);
				collectFieldBasesFromExpr(r, out);
			case ECall(tgt, _, args):
				if (tgt != null)
					collectFieldBasesFromExpr(tgt, out);
				for (a in args)
					collectFieldBasesFromExpr(a, out);
			case ERemoteCall(tgt2, _, args2):
				collectFieldBasesFromExpr(tgt2, out);
				for (a2 in args2)
					collectFieldBasesFromExpr(a2, out);
			case ETuple(elems):
				for (e in elems)
					collectFieldBasesFromExpr(e, out);
			case EMap(pairs):
	...
	static function collectVarsFromExpr(x:ElixirAST, out:Map<String, Bool>):Void {
		...
		switch (x.def) {
			...
			case ECall(tgt, _, args):
				if (tgt != null)
					collectVarsFromExpr(tgt, out);
				for (a in args)
					collectVarsFromExpr(a, out);
			case ERemoteCall(tgt2, _, args2):
				collectVarsFromExpr(tgt2, out);
				for (a2 in args2)
					collectVarsFromExpr(a2, out);
			case EFn(clauses):
				for (cl in clauses) {
					// Collect variables inside the fn body, excluding clause parameter names
					var inner:Map<String, Bool> = new Map();
					if (cl.guard != null)
						collectVarsFromExpr(cl.guard, inner);
					if (cl.body != null)
						collectVarsFromExpr(cl.body, inner);
					var params:Map<String, Bool> = new Map();
					for (arg in cl.args)
						switch (arg) {
							case PVar(nm): params.set(nm, true);
							default:
						}
					for (k in inner.keys())
						if (!params.exists(k))
			...
			case EMap(pairs):
				for (p in pairs)
					collectVarsFromExpr(p.value, out);
			case EField(obj, _):
				switch (obj.def) {
					case EVar(vf): out.set(vf, true);
					default: collectVarsFromExpr(obj, out);
				}
			default:
		}
	}
		...
		switch (n.def) {
			...
			case EFn(clauses):
				for (cl in clauses) {
					...
					var inner:Map<String, Bool> = new Map();
					if (cl.guard != null)
						collectVars(cl.guard, inner);
					if (cl.body != null)
						collectVars(cl.body, inner);
					var params:Map<String, Bool> = new Map();
					for (arg in cl.args)
						switch (arg) {
							case PVar(nm): params.set(nm, true);
