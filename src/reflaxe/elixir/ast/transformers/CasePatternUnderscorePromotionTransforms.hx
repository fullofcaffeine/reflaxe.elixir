...
class CasePatternUnderscorePromotionTransforms {
	...
	static function promotePattern(p:EPattern, used:Map<String, Bool>, renames:Map<String, String>, patternBinders:Map<String, Bool>):EPattern {
		return switch (p) {
			...
			case PMap(kvs): PMap([
					for (kv in kvs)
						{key: kv.key, value: promotePattern(kv.value, used, renames, patternBinders)}
				]);
			case PStruct(m, fs): PStruct(m, [
					for (f in fs)
						{key: f.key, value: promotePattern(f.value, used, renames, patternBinders)}
				]);
	...
	static function renameVars(node:ElixirAST, renames:Map<String, String>):ElixirAST {
		if (node == null || renames == null)
			return node;
		if (!renames.keys().hasNext())
			return node;
