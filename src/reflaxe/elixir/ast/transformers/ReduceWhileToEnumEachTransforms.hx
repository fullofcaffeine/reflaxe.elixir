	...
	static function extractElementBinder(stmts:Array<ElixirAST>, listVarName:String):Null<{binder:String, remaining:Array<ElixirAST>}> {
		...
		for (i in 0...remaining.length) {
			...
			var binder:Null<String> = switch (s.def) {
				case EBinary(Match, {def: EVar(lhs)}, rhs): lhs != null && rhs != null && isListHeadAccess(rhs, listVarName) ? lhs : null;
				case EMatch(PVar(lhs2), rhs2): lhs2 != null && rhs2 != null && isListHeadAccess(rhs2, listVarName) ? lhs2 : null;
				default:
					null;
			};
	...
	static function isListHeadAccess(expr:ElixirAST, listVarName:String):Bool {
		...
		return switch (expr.def) {
			case EAccess({def: EVar(v)}, {def: EInteger(i)}) if (v == listVarName && i == 0):
				true;
			case ERemoteCall({def: EVar("Enum")}, "at", args) if (args != null && args.length >= 2):
				switch (args[0].def) {
					case EVar(v2) if (v2 == listVarName):
						switch (args[1].def) {
							case EInteger(i2) if (i2 == 0): true;
							default: false;
						}
					default:
						false;
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ERemoteCall({def: EVar("Enum")}, "reduce_while", args) if (args != null && args.length == 3 && isStreamIterateZero(args[0])):
											...
											if (st != null)
												switch (st.def) {
													...
													case EBinary(Match, left, rhs):
														var lt = left == null
															|| left.def == null ? '<null>' : reflaxe.elixir.util.EnumReflection.enumConstructor(left.def);
														var rt = rhs == null
															|| rhs.def == null ? '<null>' : reflaxe.elixir.util.EnumReflection.enumConstructor(rhs.def);
														Sys.println('    lhs=' + lt + ' rhs=' + rt);
													case EMatch(_, rhsExpr):
														var rhsTag = rhsExpr == null
															|| rhsExpr.def == null ? '<null>' : reflaxe.elixir.util.EnumReflection.enumConstructor(rhsExpr.def);
					...
					if (bodyBlock == null || elementBinder == null)
						return n;
