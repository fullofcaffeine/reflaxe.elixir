		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EDef(fname, args, guards, body) if (fname == "changeset" && args.length == 2):
					...
					inline function baseName(n:String):String
					...
					function scanForDesired(n:ElixirAST):Void {
						if (desired0 != null && desired1 != null)
							return;
						switch (n.def) {
							case ERemoteCall(mod, fn, args) if (args != null && args.length >= 2):
								...
								if (m == "Ecto.Changeset" && (fn == "change" || fn == "cast")) {
									switch (args[0].def) {
										case EVar(v): desired0 = v;
										default:
									}
									switch (args[1].def) {
										case EVar(v2): desired1 = v2;
										default:
									}
								}
							case EBlock(es):
					...
					if (desired0 == null && rename0)
						desired0 = base0; // when renaming by usage, use base
					if (desired1 == null && rename1)
						desired1 = base1;
				...
				case EDefp(fname2, args2, guards2, body2) if (fname2 == "changeset" && args2.length == 2):
					...
					inline function baseNameNoUnderscore(n:String):String
						return (n != null && n.length > 0 && n.charAt(0) == '_') ? n.substr(1) : n;
