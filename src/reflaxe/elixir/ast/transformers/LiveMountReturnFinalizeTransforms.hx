				...
				case EDef(name, args, guards, body) if (name == "mount"):
					...
					switch (body.def) {
						case EBlock(stmts):
							...
							if (stmts.length > 0)
								switch (stmts[stmts.length - 1].def) {
									case ETuple(elems) if (elems.length == 2):
										alreadyTuple = (switch (elems[0].def) {
											case EAtom(_): true;
											default: false;
										});
									default:
									...
									case EBinary(Match, leftAny, rhs):
										switch (rhs.def) {
											case EMap(_):
												...
												var tupleVal0 = makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
													[makeAST(EVar(socketParam)), rhs]));
									...
									case EMatch(patAny, rhsM):
										switch (rhsM.def) {
											case EMap(_):
												...
												var tupleValM = makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
													[makeAST(EVar(socketParam)), rhsM]));
							...
							var tupleVal:ElixirAST = (assignsVar != null) ? makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
								[makeAST(EVar(socketParam)), makeAST(EVar(assignsVar))])) : makeAST(EVar(socketParam));
						...
						case EDo(stmts2):
							...
							if (stmts2.length > 0)
								switch (stmts2[stmts2.length - 1].def) {
									case ETuple(elems) if (elems.length == 2):
										alreadyTuple2 = (switch (elems[0].def) {
											case EAtom(_): true;
											default: false;
										});
									default:
									...
									case EBinary(Match, leftAny2, rhs2):
										switch (rhs2.def) {
											case EMap(_):
												...
												var tupleValX = makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
													[makeAST(EVar(socketParam)), rhs2]));
									...
									case EMatch(patAny2, rhsM2):
										switch (rhsM2.def) {
											case EMap(_):
												...
												var tupleValY = makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
													[makeAST(EVar(socketParam)), rhsM2]));
							...
							var tupleVal2:ElixirAST = (assignsVar2 != null) ? makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign",
								[makeAST(EVar(socketParam)), makeAST(EVar(assignsVar2))])) : makeAST(EVar(socketParam));
