	...
	static function filter(stmts:Array<ElixirAST>, ctx:ElixirAST):Array<ElixirAST> {
		...
		for (s in stmts) {
			var drop = switch (s.def) {
				case EBinary(Match, left, right): var isNil = switch (left.def) {
				...
				case EMatch(pat, rhs): var isNilPat = switch (pat) {
						case PVar(nm3) if (nm3 == "nil"): true;
						case PLiteral(lit): switch (lit.def) {
								case EAtom(v) if (v == ":nil" || v == "nil"): true;
								case ENil: true;
								default: false;
							};
						default: false;
					}; var isUnderscoredRhs = switch (rhs.def) {
						case EVar(nm4) if (nm4 != null && nm4.length > 0 && nm4.charAt(0) == '_'): true;
						default: false;
