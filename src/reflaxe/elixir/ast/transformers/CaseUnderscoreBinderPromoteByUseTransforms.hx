...
class CaseUnderscoreBinderPromoteByUseTransforms {
	...
	static function usageDrivenRenameSecondBinder(p:EPattern, used:Map<String, Bool>, patNames:Map<String, Bool>):EPattern {
		return switch (p) {
			...
			case PMap(kvs): PMap([
					for (kv in kvs)
						{key: kv.key, value: usageDrivenRenameSecondBinder(kv.value, used, patNames)}
				]);
			case PStruct(nm, fs): PStruct(nm, [
					for (f in fs)
						{key: f.key, value: usageDrivenRenameSecondBinder(f.value, used, patNames)}
				]);
	...
	static function promoteBinders(p:EPattern, used:Map<String, Bool>, patNames:Map<String, Bool>):EPattern {
		return switch (p) {
			...
			case PAlias(nm, pat) if (nm != null && nm.length > 1 && nm.charAt(0) == '_'):
				...
				if (used.exists(trimmed2) && !patNames.exists(trimmed2)) PAlias(trimmed2,
					promoteBinders(pat, used, patNames)) else PAlias(nm, promoteBinders(pat, used, patNames));
