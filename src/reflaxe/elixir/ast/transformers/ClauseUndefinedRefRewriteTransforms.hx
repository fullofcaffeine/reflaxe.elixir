...
class ClauseUndefinedRefRewriteTransforms {
			...
			case ECase(target, clauses):
				...
				for (cl in clauses) {
					...
					if (binder != null) {
						...
						for (u in used.keys())
							if (!declared.exists(u) && u != binder && allow(u))
								undef.push(u);
	...
	static function replaceVar(body:ElixirAST, from:String, to:String):ElixirAST {
		return ElixirASTTransformer.transformNode(body, function(e:ElixirAST):ElixirAST {
			return switch (e.def) {
				case EVar(v) if (v == from): makeASTWithMeta(EVar(to), e.metadata, e.pos);
				default: e;
			};
		});
	}
	...
	static function collectDeclaredFromStatement(stmt:ElixirAST, vars:Map<String, Bool>):Void {
		if (stmt == null || stmt.def == null || vars == null)
			return;
		switch (stmt.def) {
			case EMatch(p, _):
				collectPatternVars(p, vars);
			case EBinary(Match, left, _):
				collectLhs(left, vars);
			case ECase(_, cs):
				for (c in cs)
	...
	static function collectLhs(l:ElixirAST, vars:Map<String, Bool>):Void {
		switch (l.def) {
			case EVar(n):
				vars.set(n, true);
			case EBinary(Match, nestedLeft, _):
				collectLhs(nestedLeft, vars);
			default:
		}
	}
}
