					...
					for (i in 0...n) {
						...
						switch (s.def) {
							case EBinary(Match, leftOuter, rightOuter):
								...
								if ((innerIsWildcard
									|| (innerAliasName != null && !OptimizedVarUseAnalyzer.usedLater(usage, i + 1, innerAliasName)))
									&& innerRightExpr != null) {
						...
						switch (s.def) {
							case EBinary(Match, leftOuter, rightOuter):
								...
								switch (rightOuter.def) {
									case EBinary(Match, leftInner, innerExpr):
										switch (leftInner.def) {
											case EVar(variableName): innerAliasNameDo = variableName;
											default:
										}
										innerRightExprDo = innerExpr;
									case EMatch(innerPattern, innerMatchExpr):
										switch (innerPattern) {
											case PVar(variableName): innerAliasNameDo = variableName;
											default:
										}
										innerRightExprDo = innerMatchExpr;
									default:
								}
								if (innerAliasNameDo != null
									&& innerRightExprDo != null
									&& !OptimizedVarUseAnalyzer.usedLater(usage, i + 1, innerAliasNameDo)) {
							...
							case EMatch(patOuter, rhsOuter):
								...
								switch (rhsOuter.def) {
									case EBinary(Match, aliasLeft, aliasRightExpr):
										switch (aliasLeft.def) {
											case EVar(variableName): innerAliasNameDoMatch = variableName;
											default:
										}
										innerRightExprDoMatch = aliasRightExpr;
									case EMatch(matchPattern, matchExpr):
										switch (matchPattern) {
											case PVar(variableName): innerAliasNameDoMatch = variableName;
											default:
										}
										innerRightExprDoMatch = matchExpr;
									default:
								}
								if (innerAliasNameDoMatch != null
									&& innerRightExprDoMatch != null
									&& !OptimizedVarUseAnalyzer.usedLater(usage, i + 1, innerAliasNameDoMatch)) {
