					...
					for (i in 0...stmts.length) {
						...
						switch (s.def) {
							case EMatch(pat, {def: ECase(scrut, clauses)}) if (isDiscardPattern(pat)):
								var assign = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("parsed_result"), s.metadata, s.pos), scrut), s.metadata,
									s.pos);
							...
							case EBinary(Match, {def: EVar("_")}, {def: ECase(scr2, cls2)}):
								var assign2 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("parsed_result"), s.metadata, s.pos), scr2), s.metadata,
									s.pos);
						...
						if (!hoisted) {
							var extra = switch (s.def) {
								case EMatch(lhsPattern, rhs): ' (lhsPattern='
									+ reflaxe.elixir.util.EnumReflection.enumConstructor(lhsPattern)
									+ ', rhs='
									+ reflaxe.elixir.util.EnumReflection.enumConstructor(rhs.def)
									+ ')';
								default: '';
							};
					...
					for (i2 in 0...stmts2.length) {
						...
						switch (s2.def) {
							case EMatch(p3, {def: ECase(scr3, cls3)}) if (isDiscardPattern(p3)):
								var assign3 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("parsed_result"), s2.metadata, s2.pos), scr3), s2.metadata,
									s2.pos);
							...
							case EBinary(Match, {def: EVar("_")}, {def: ECase(scr4, cls4)}):
								var assign4 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("parsed_result"), s2.metadata, s2.pos), scr4), s2.metadata,
									s2.pos);
