			...
			switch (a) {
				...
				case PAlias(name, _):
					if (name != null && name.length > 0)
						names.push(name);
				case PTuple(targs):
					for (x in collectBinderNames(targs))
						names.push(x);
				case PList(largs):
					for (x in collectBinderNames(largs))
						names.push(x);
				case PCons(h, t):
					for (x in collectBinderNames([h]))
						names.push(x);
					for (x in collectBinderNames([t]))
						names.push(x);
				case PMap(pairs):
					for (p in pairs)
						for (x in collectBinderNames([p.value]))
							names.push(x);
				case PStruct(_, fields):
					for (f in fields)
						for (x in collectBinderNames([f.value]))
							names.push(x);
				default:
			}
		return names;
