									...
									case EIf(cond, thenE, elseE) if (elseE != null):
										...
										if (elseIsB) {
											...
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos),
												makeASTWithMeta(EIf(cond, thenE, elseE), stmts[j].metadata, stmts[j].pos)),
												s.metadata, s.pos));
								...
								switch (stmts[scanIndexSecondary].def) {
									case EIf(condM, thenM, elseM) if (elseM != null):
										...
										if (elseIsBM) {
											...
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(aM), s.metadata, s.pos),
												makeASTWithMeta(EIf(condM, thenM, elseM), stmts[scanIndexSecondary].metadata, stmts[scanIndexSecondary].pos)),
												s.metadata, s.pos));
											for (k2 in i + 1...scanIndexSecondary)
												out.push(stmts[k2]);
								...
								switch (stmts[scanIndexTertiary].def) {
									case EIf(nestedCond, nestedThen, nestedElse) if (nestedElse != null):
										var elseReturnsBinder = switch (nestedElse.def) {
											case EVar(binderName) if (binderName == rhsBinder): true;
											default: false;
										};
										if (elseReturnsBinder) {
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(rhsBinder), s.metadata, s.pos), rhsValue),
												s.metadata, s.pos));
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(lhsName), s.metadata, s.pos),
												makeASTWithMeta(EIf(nestedCond, nestedThen, nestedElse), stmts[scanIndexTertiary].metadata,
													stmts[scanIndexTertiary].pos)),
												s.metadata, s.pos));
											for (k3 in i + 1...scanIndexTertiary)
												out.push(stmts[k3]);
