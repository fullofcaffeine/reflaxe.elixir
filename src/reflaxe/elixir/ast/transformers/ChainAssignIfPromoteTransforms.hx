								...
								if (ch != null && ch.inner != null && ch.rhs != null && i + 1 < stmts.length) {
									switch (stmts[i + 1].def) {
										case EIf(cond, thenE, elseE) if (isElseVar(elseE, ch.inner)):
											// debug removed
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(ch.inner), s.metadata, s.pos), ch.rhs), s.metadata,
												s.pos));
											out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos),
												makeASTWithMeta(EIf(cond, thenE, elseE), stmts[i + 1].metadata, stmts[i + 1].pos)),
												s.metadata, s.pos));
											i += 2;
											consumed = true;
										default:
									...
									if (ch2 != null && ch2.inner != null && ch2.rhs != null && i + 1 < stmts.length) {
										switch (stmts[i + 1].def) {
											case EIf(cond2, thenE2, elseE2) if (isElseVar(elseE2, ch2.inner)):
												// debug removed
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(ch2.inner), s.metadata, s.pos), ch2.rhs),
													s.metadata, s.pos));
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a2), s.metadata, s.pos),
													makeASTWithMeta(EIf(cond2, thenE2, elseE2), stmts[i + 1].metadata, stmts[i + 1].pos)),
													s.metadata, s.pos));
												i += 2;
												consumed = true;
											default:
								...
								if (ch3 != null && ch3.inner != null && ch3.rhs != null && j + 1 < stmts2.length) {
									switch (stmts2[j + 1].def) {
										case EIf(cond3, thenE3, elseE3) if (isElseVar(elseE3, ch3.inner)):
											// debug removed
											out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(ch3.inner), s2.metadata, s2.pos), ch3.rhs),
												s2.metadata, s2.pos));
											out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a3), s2.metadata, s2.pos),
												makeASTWithMeta(EIf(cond3, thenE3, elseE3), stmts2[j + 1].metadata, stmts2[j + 1].pos)),
												s2.metadata, s2.pos));
											j += 2;
											consumed2 = true;
										default:
									...
									if (ch4 != null && ch4.inner != null && ch4.rhs != null && j + 1 < stmts2.length) {
										switch (stmts2[j + 1].def) {
											case EIf(cond4, thenE4, elseE4) if (isElseVar(elseE4, ch4.inner)):
												// debug removed
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(ch4.inner), s2.metadata, s2.pos), ch4.rhs),
													s2.metadata, s2.pos));
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a4), s2.metadata, s2.pos),
													makeASTWithMeta(EIf(cond4, thenE4, elseE4), stmts2[j + 1].metadata, stmts2[j + 1].pos)),
													s2.metadata, s2.pos));
												j += 2;
												consumed2 = true;
											default:
