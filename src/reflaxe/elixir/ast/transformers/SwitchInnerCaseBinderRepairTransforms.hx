		...
		switch (p) {
			case PTuple(es) if (es.length >= 2):
				...
				switch (es[0]) {
					case PLiteral(ast):
						switch (ast.def) {
							case EAtom(nm) if (nm == "some"): name = extractVar(es[1]);
							default:
						}
					default:
				}
		...
		function rw(n:ElixirAST):ElixirAST {
			...
			return switch (n.def) {
				case ECase(scr, cls):
					...
					var ncls = [];
					for (c in cls)
						ncls.push({pattern: c.pattern, guard: c.guard == null ? null : rw(c.guard), body: rw(c.body)});
		...
		return ElixirASTTransformer.transformNode(body, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EVar(v):
					...
					if (v == "g" || v == "_g")
						isInfra = true;
					else if (v != null && v.length > 1 && StringTools.startsWith(v, "g") && isDigits(v.substr(1)))
						isInfra = true;
					else if (v != null && v.length > 2 && StringTools.startsWith(v, "_g") && isDigits(v.substr(2)))
						isInfra = true;
