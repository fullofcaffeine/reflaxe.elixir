        ...
        return ElixirASTTransformer.transformNode(ast, function(n: ElixirAST): ElixirAST {
            return switch (n.def) {
                case EBinary(Match, left, rhs):
                    ...
                    if (fixed != null) {
                        #if sys Sys.println('[BinderRepair] Applied repair in EBinary context'); #end
                        fixed;
                    } else n;
                case EMatch(pat, rhs2):
                    var fixed2 = tryRepairMatch(pat, rhs2);
                    if (fixed2 != null) {
                        #if sys Sys.println('[BinderRepair] Applied repair in EMatch context'); #end
                        fixed2;
                    } else n;
                case ECase(target, clauses):
                    ...
                    if (changed) {
                        #if sys Sys.println('[BinderRepair] Applied repair in ECase clause bodies'); #end
    ...
    static function tryRepairBinary(lhs: ElixirAST, rhs: ElixirAST): Null<ElixirAST> {
        ...
        return switch (rhs.def) {
            case ECase(target, clauses):
                ...
                #if sys Sys.println('[BinderRepair] Rewrote inner case scrutinee from ' + lhsName + ' to binder in one or more clauses'); #end
                #end
    ...
    static function tryRepairMatch(pat: EPattern, rhs: ElixirAST): Null<ElixirAST> {
        ...
        return switch (rhs.def) {
            case ECase(target, clauses):
                ...
                if (!changed) return null;
                #if debug_switch_binder_repair
                #if sys Sys.println('[BinderRepair] (EMatch) Rewrote inner case scrutinee from ' + lhsName + ' to binder'); #end
                #end
