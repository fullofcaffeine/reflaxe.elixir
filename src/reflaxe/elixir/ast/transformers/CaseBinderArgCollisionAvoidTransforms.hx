		...
		return ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
			return switch (x.def) {
				case ECase(expr, clauses):
					...
					for (c in clauses)
						out.push(renameCollidingBinders(c, argNames, scrutineeName));
	...
	static function renamePattern(p:EPattern, argNames:Map<String, Bool>, renameMap:Map<String, String>, scrutineeName:Null<String>):EPattern {
		return switch (p) {
			...
			case PMap(kvs):
				PMap([
					for (kv in kvs)
						{key: kv.key, value: renamePattern(kv.value, argNames, renameMap, scrutineeName)}
				]);
			case PStruct(m, fs):
				PStruct(m, [
					for (f in fs)
						{key: f.key, value: renamePattern(f.value, argNames, renameMap, scrutineeName)}
				]);
			case PBinary(segs):
				PBinary([
					for (s in segs)
						{
							pattern: renamePattern(s.pattern, argNames, renameMap, scrutineeName),
							size: s.size,
							type: s.type,
	...
	static function rewriteVars(expr:ElixirAST, renameMap:Map<String, String>):ElixirAST {
		...
		function collectPatternBinders(p:EPattern, out:Map<String, Bool>):Void {
			switch (p) {
				...
				case PBinary(segs):
					for (s in segs)
						collectPatternBinders(s.pattern, out);
				case PPin(inner):
					collectPatternBinders(inner, out);
				default:
			}
