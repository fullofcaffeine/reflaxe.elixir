...
class ClauseUnusedBinderUnderscoreTransforms {
				...
				case ECase(target, clauses):
					...
					for (cl in clauses) {
						...
						if (cl.guard != null) {
							...
							for (g in guardUsed)
								if (!arrayContains(used, g))
									used.push(g);
						}
						// Treat EString/ERaw interpolations ("#{name}") as usage as well
						for (b in binders) {
							if (!arrayContains(used, b) && stringInterpolatesName(cl.body, b))
								used.push(b);
							if (cl.guard != null && !arrayContains(used, b) && stringInterpolatesName(cl.guard, b))
								used.push(b);
						}
		...
		return switch (p) {
			...
			case PBinary(segments):
				PBinary([
					for (seg in segments)
						{
							pattern: underscoreBinders(seg.pattern, names),
							size: seg.size,
							type: seg.type,
	...
	static function stringInterpolatesName(body:ElixirAST, name:String):Bool {
		...
		function visit(n:ElixirAST):Void {
			...
			switch (n.def) {
				case EString(v):
					if (v != null && name != null && v.indexOf("#{" + name) != -1)
						found = true;
				case ERaw(code):
					if (code != null && name != null && code.indexOf("#{" + name) != -1)
						found = true;
				case EBlock(ss):
					for (s in ss)
						visit(s);
