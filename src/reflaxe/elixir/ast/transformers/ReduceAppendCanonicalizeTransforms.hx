					...
					switch (fnNode.def) {
						case EFn(clauses) if (clauses.length == 1):
								...
								switch (s.def) {
									case EBinary(Match, left, right):
										...
										var rhsIsBinder = switch (right.def) {
											case EVar(n2) if (n2 == binderName): true;
											default: false;
										};
									...
									case EMatch(patX, rhsX):
										...
										var rhsIsBinder2 = switch (rhsX.def) {
											case EVar(n4) if (n4 == binderName): true;
											default: false;
										};
								...
								var rewritten = ElixirASTTransformer.transformNode(stmt, function(node:ElixirAST):ElixirAST {
									return switch (node.def) {
										case EBinary(Match, left, rhs):
											#if debug_reduce_append_canon
											// DEBUG: Sys.println('[ReduceAppendCanonicalize] HIT EBinary(Match) ' + ElixirASTPrinter.print(node, 0));
											#end
											var lhs:Null<String> = switch (left.def) {
												case EVar(n): n;
												default: null;
											...
											if (lhs != null && containsSelfAppend(rhs, lhs)) {
												...
												var rhs2 = ElixirASTTransformer.transformNode(rhs, function(t:ElixirAST):ElixirAST {
													return switch (t.def) {
														...
														case EVar(v2) if (elementAlias != null && v2 == elementAlias): makeASTWithMeta(EVar(binderName),
																t.metadata, t.pos);
														default: t;
													}
												});
												...
												case ERemoteCall(_, "concat", cargs) if (lhs != null && cargs.length == 2):
													...
													if (arg0IsLhs) {
														localRewrote = true;
														var rhsList2 = (elementAlias == null) ? cargs[1] : ElixirASTTransformer.transformNode(cargs[1],
															function(x:ElixirAST):ElixirAST {
																return switch (x.def) {
																	case EVar(v) if (v == elementAlias): makeASTWithMeta(EVar(binderName), x.metadata, x.pos);
																	default: x;
																}
															});
														makeASTWithMeta(EBinary(Match, makeAST(EVar(accName)),
															makeAST(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), rhsList2]))),
															node.metadata, node.pos);
													} else node;
												case EBinary(Concat, lcat, rcat):
													var lhsIsCat = switch (lcat.def) {
														case EVar(nlc) if (lhs != null && nlc == lhs): true;
														default: false;
													};
													if (lhsIsCat) {
														localRewrote = true;
														var list2 = (elementAlias == null) ? rcat : ElixirASTTransformer.transformNode(rcat,
															function(x2:ElixirAST):ElixirAST {
																return switch (x2.def) {
																	case EVar(vv) if (vv == elementAlias): makeASTWithMeta(EVar(binderName), x2.metadata,
																			x2.pos);
																	default: x2;
																}
															});
														makeASTWithMeta(EBinary(Match, makeAST(EVar(accName)),
															makeAST(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), list2]))),
															node.metadata, node.pos);
													} else node;
												case ECall(_, "concat", cargsC) if (lhs != null && cargsC.length == 2):
													var isSelfC = switch (cargsC[0].def) {
														case EVar(nc) if (nc == lhs): true;
														default: false;
													};
													if (isSelfC) {
														localRewrote = true;
														var rhsL2 = (elementAlias == null) ? cargsC[1] : ElixirASTTransformer.transformNode(cargsC[1],
															function(z:ElixirAST):ElixirAST {
																return switch (z.def) {
														...
														makeASTWithMeta(EBinary(Match, makeAST(EVar(accName)),
															makeAST(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), rhsL2]))),
															node.metadata, node.pos);
													} else node;
												default:
													node;
											...
											if (lhs2 != null && containsSelfAppend(rhs2, lhs2)) {
												...
												var rhs2b = ElixirASTTransformer.transformNode(rhs2, function(t2:ElixirAST):ElixirAST {
													return switch (t2.def) {
														...
														case EVar(vp2) if (elementAlias != null && vp2 == elementAlias): makeASTWithMeta(EVar(binderName),
																t2.metadata, t2.pos);
														default: t2;
													}
												});
												...
												case ERemoteCall(_, "concat", cargs2) if (lhs2 != null && cargs2.length == 2):
													var arg0IsLhs2 = switch (cargs2[0].def) {
														case EVar(n3): (n3 == lhs2);
														default: false;
													};
													if (arg0IsLhs2) {
														localRewrote = true;
														var rhsListM2 = (elementAlias == null) ? cargs2[1] : ElixirASTTransformer.transformNode(cargs2[1],
															function(y:ElixirAST):ElixirAST {
																return switch (y.def) {
														...
														makeASTWithMeta(EBinary(Match, makeAST(EVar(accName)),
															makeAST(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), rhsListM2]))),
															node.metadata, node.pos);
													} else node;
												default:
													node;
							...
							if (!usedSpecificRewrite) {
									...
									ASTUtils.walk(stmt2, function(n2:ElixirAST) {
										switch (n2.def) {
											case ERemoteCall(modX, "concat", argsX) if (argsX.length == 2):
												...
												foundList = argsX[1];
												#if debug_reduce_append_canon
												// DEBUG: Sys.println('[ReduceAppendCanonicalize] walk found ERemoteCall concat: ' + ElixirASTPrinter.print(n2, 0));
												#end
											case ECall(_, "concat", argsCX) if (argsCX.length == 2):
												foundList = argsCX[1];
												#if debug_reduce_append_canon
												// DEBUG: Sys.println('[ReduceAppendCanonicalize] walk found ECall concat: ' + ElixirASTPrinter.print(n2, 0));
												#end
								...
								#if debug_reduce_append_canon
								// DEBUG: Sys.println('[ReduceAppendCanonicalize] fallback foundList=' + (foundList == null ? 'null' : ElixirASTPrinter.print(foundList, 0)) + ', foundExpr=' + (foundExpr == null ? 'null' : ElixirASTPrinter.print(foundExpr, 0)));
								#end
								...
								var returnsAcc = switch (bodyStmts.length > 0 ? bodyStmts[bodyStmts.length - 1].def : ENil) {
									case EVar(v) if (v == accName): true;
									default: false;
								};
								if ((foundList != null || foundExpr != null) && returnsAcc) {
									var listNode:ElixirAST = (foundList != null) ? foundList : makeAST(EList([foundExpr]));
									var listPrime = (elementAlias == null) ? listNode : ElixirASTTransformer.transformNode(listNode,
										function(w:ElixirAST):ElixirAST {
											return switch (w.def) {
									...
									#if debug_reduce_append_canon
									// DEBUG: Sys.println('[ReduceAppendCanonicalize] fallback rebuilding acc concat with list=' + ElixirASTPrinter.print(listPrime, 0));
									#end
									newBody = [
										makeAST(EBinary(Match, makeAST(EVar(accName)),
											makeAST(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), listPrime])))),
