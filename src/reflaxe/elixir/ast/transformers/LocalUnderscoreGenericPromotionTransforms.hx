		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case EFn(clauses):
					var outClauses = [];
					for (cl in clauses)
						outClauses.push({args: cl.args, guard: cl.guard, body: rewriteBody(cl.body)});
	...
	static function rewriteSeq(stmts:Array<ElixirAST>):Array<ElixirAST> {
		...
		for (i in 0...stmts.length) {
			...
			var rewrittenStmt = switch (stmt.def) {
				...
				case EBinary(Match, {def: EVar(binderName)}, rhs) if (isUnderscored(binderName)
					&& OptimizedVarUseAnalyzer.usedLater(useIndex, i, binderName)):
