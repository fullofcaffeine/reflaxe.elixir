		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case EBinary(Match, left, {def: EBinary(Match, _innerLeft, innerRhs)}):
					switch (innerRhs.def) {
						case EBinary(Concat, {def: EList(empty)}, {def: EList(single)})
							if (empty != null && empty.length == 0 && single != null && single.length == 1):
							makeASTWithMeta(EBinary(Match, left, makeASTWithMeta(EList(single), n.metadata, n.pos)), n.metadata, n.pos);
						default:
							n;
					}
				// data = [] ++ [expr]  →  data = [expr]
				case EBinary(Match, left, {def: EBinary(Concat, {def: EList(empty)}, {def: EList(single)})})
					if (empty != null && empty.length == 0 && single != null && single.length == 1):
					makeASTWithMeta(EBinary(Match, left, makeASTWithMeta(EList(single), n.metadata, n.pos)), n.metadata, n.pos);
				// _ = [] ++ [expr] → _ = [expr]
				case EMatch(PWildcard, {def: EBinary(Concat, {def: EList(empty2)}, {def: EList(single2)})})
					if (empty2 != null && empty2.length == 0 && single2 != null && single2.length == 1):
