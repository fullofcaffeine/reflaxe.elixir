...
class HeexAssignsLocalVarRenameTransforms {
	...
	static function containsHSigil(node:ElixirAST):Bool {
		...
		ElixirASTTransformer.transformNode(node, function(x:ElixirAST):ElixirAST {
			...
			switch (x.def) {
				...
				case ERaw(code)
					if (code != null
						&& (code.indexOf("~H\"") != -1 || code.indexOf("~H'''") != -1 || code.indexOf("~H\"\"\"") != -1 || code.indexOf("~H'") != -1)):
					found = true;
				default:
			}
			return x;
	...
	static function renameAssignsInPattern(p:EPattern):EPattern {
		return switch (p) {
			...
			case PBinary(segs):
				PBinary(segs.map(s -> {
					pattern: renameAssignsInPattern(s.pattern),
					size: s.size,
					type: s.type,
