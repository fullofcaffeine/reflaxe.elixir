...
class LoopTransforms {
	...
	static function transformChildrenManually(node:ElixirAST, transformer:ElixirAST->ElixirAST):ElixirAST {
		...
		switch (node.def) {
			case EIf(cond, thenBranch, elseBranch):
				return makeASTWithMeta(EIf(transformer(cond), transformer(thenBranch), elseBranch != null ? transformer(elseBranch) : null), meta, node.pos);

			case ECase(expr, clauses):
				return makeASTWithMeta(ECase(transformer(expr), clauses.map(c -> {
					pattern: c.pattern,
			...
			case ECall(target, funcName, args):
				return makeASTWithMeta(ECall(target != null ? transformer(target) : null, funcName, args.map(a -> transformer(a))), meta, node.pos);

			case ERemoteCall(module, funcName, args):
				return makeASTWithMeta(ERemoteCall(transformer(module), funcName, args.map(a -> transformer(a))), meta, node.pos);
		...
		function detectAndTransformUnrolledLoops(node:ElixirAST):ElixirAST {
			...
			if (nodeType == "EMatch") {
				var rhsIsBlock = switch (node.def) {
					case EMatch(_, rhs): switch (rhs.def) {
							case EBlock(_): true;
							default: false;
						};
					default: false;
			...
			switch (node.def) {
				...
				case EMatch(pattern, rhsBlock):
					...
					var rhsIsBlock = (rhsBlock != null) && (switch (rhsBlock.def) {
						case EBlock(_): true;
						default: false;
					});
					if (!rhsIsBlock) {
	...
	static function isEnumEachCall(ast:ElixirAST):Bool {
		return switch (ast.def) {
			case ERemoteCall(module, func, _):
				switch (module.def) {
					case EAtom(atom): // ElixirAtom converts to String, check if it's "Enum"
						var atomStr:String = atom; atomStr == "Enum" && func == "each";
					default: false;
				}
			default: false;
		};
	...
	static function reconstructNestedLoop(outerStatements:Array<ElixirAST>, innerLoops:Array<ElixirAST>):ElixirAST {
		...
		var outerRange = makeAST(ERange(makeAST(EInteger(0)), makeAST(EInteger(count - 1)), false, // inclusive range
			makeAST(EInteger(1))));
		...
		return makeAST(ERemoteCall(makeAST(EAtom(enumAtom)), "each", [outerRange, outerFunction]));
	}
	...
	static function detectComprehensionInReduceWhile(stmt:ElixirAST):Null<ElixirAST> {
		...
		switch (stmt.def) {
			case ERemoteCall({def: EVar("Enum")}, "reduce_while", args) if (args.length == 3):
				...
				var comprehension = makeAST(EFor([generator], comprehensionInfo.filter != null ? [comprehensionInfo.filter] : [], comprehensionInfo.bodyExpr,
					null, false));
	...
	static function detectComprehensionPattern(stmts:Array<ElixirAST>, startIdx:Int):Null<{transformed:ElixirAST, count:Int}> {
		if (startIdx + 2 >= stmts.length)
			return null; // Need at least 3 statements
		...
		var filters = comprehensionInfo.filterCondition != null ? [comprehensionInfo.filterCondition] : [];

		var comprehension = makeAST(EFor([generator], // generators array
	...
	static function detectBlockComprehension(resultVar:String, stmts:Array<ElixirAST>):Null<ComprehensionInfo> {
		...
		for (idx in 0...Math.floor(Math.min(stmts.length, 3))) {
			var desc = switch (stmts[idx].def) {
				case EMatch(p,
					e): 'EMatch(${reflaxe.elixir.util.EnumReflection.enumConstructor(p)}, ${reflaxe.elixir.util.EnumReflection.enumConstructor(e.def)})';
				...
				case EBlock(innerStmts):
					...
					for (j in 0...innerStmts.length) {
						var desc = switch (innerStmts[j].def) {
							case ECall(target, name,
								args): 'ECall(target=${target != null ? reflaxe.elixir.util.EnumReflection.enumConstructor(target.def) : "null"}, name=$name, ${args.length} args)';
							case EBinary(op, left,
								right): 'EBinary($op, ${reflaxe.elixir.util.EnumReflection.enumConstructor(left.def)}, ${reflaxe.elixir.util.EnumReflection.enumConstructor(right.def)})';
	...
	static function detectUnrolledFilteredComprehension(stmts:Array<ElixirAST>, startIdx:Int):Null<{transformed:ElixirAST, count:Int}> {
		if (startIdx + 2 >= stmts.length)
			return null; // Need at least: init + if + terminator
		...
		if (blockStmts.length < 2)
			return null; // Need at least init + terminator
	...
	static function containsVariable(expr:ElixirAST, varName:String):Bool {
		return switch (expr.def) {
			case EVar(name):
				name == varName;
			case EBinary(_, left, right): containsVariable(left, varName) || containsVariable(right, varName);
			case EUnary(_, innerExpr):
				containsVariable(innerExpr, varName);
			case ECall(target, _, args): var hasInTarget = (target != null && containsVariable(target,
					varName)); var hasInArgs = Lambda.exists(args, arg -> containsVariable(arg, varName)); hasInTarget || hasInArgs;
			default:
				false;
		};
	}
	...
	static function detectUnrolledLoop(stmts:Array<ElixirAST>):Null<ElixirAST> {
		...
		if (complexity > STRING_COMPLEXITY_THRESHOLD)
			return null;
	...
	static function transformToEnumEach(callInfo:{module:String, func:String, args:Array<ElixirAST>}, count:Int):ElixirAST {
		...
		var range = makeAST(ERange(makeAST(EInteger(0)), makeAST(EInteger(count - 1)), false, makeAST(EInteger(1))));
		...
		var body = if (callInfo.module != "") {
			makeAST(ERemoteCall(makeAST(EVar(callInfo.module)), callInfo.func, bodyArgs));
		} else {
		...
		return makeAST(ERemoteCall(makeAST(EVar("Enum")), "each", [range, func]));
	}
