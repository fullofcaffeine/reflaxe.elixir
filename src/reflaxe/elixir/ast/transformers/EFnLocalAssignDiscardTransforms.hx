...
class EFnLocalAssignDiscardTransforms {
	...
	static function collectAssignedAndUsed(n:ElixirAST, assigned:Map<String, Bool>, used:Map<String, Bool>):Void {
		...
		switch (n.def) {
			...
			case EBlock(stmts):
				for (s in stmts)
					collectAssignedAndUsed(s, assigned, used);
			case EDo(stmts2):
				for (s2 in stmts2)
					collectAssignedAndUsed(s2, assigned, used);
			case EIf(c, t, e):
				collectAssignedAndUsed(c, assigned, used);
				collectAssignedAndUsed(t, assigned, used);
				if (e != null)
					collectAssignedAndUsed(e, assigned, used);
			case ECase(expr, cs):
				collectAssignedAndUsed(expr, assigned, used);
				for (c in cs)
					collectAssignedAndUsed(c.body, assigned, used);
			case ECall(tgt, _, args):
				if (tgt != null)
					collectAssignedAndUsed(tgt, assigned, used);
				for (a in args)
					collectAssignedAndUsed(a, assigned, used);
					...
					for (i in 0...stmts2.length) {
						...
						switch (s2.def) {
							case EBinary(Match, left2, rhs2):
								switch (left2.def) {
									case EVar(name2):
										if ((name2 != null && name2.length > 0 && name2.charAt(0) == '_')
											&& !nameUsedLater(stmts2, i + 1, name2)) {
	...
	static function nameUsedLater(stmts:Array<ElixirAST>, start:Int, name:String):Bool {
		for (k in start...stmts.length)
			if (statementUsesName(stmts[k], name))
				return true;
		return false;
	}
