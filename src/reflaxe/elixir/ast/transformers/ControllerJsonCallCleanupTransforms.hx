...
class ControllerJsonCallCleanupTransforms {
	...
	static function maybeRenameBinderFromBodyMapKeys(pat:EPattern, body:ElixirAST):EPattern {
		...
		switch (pat) {
			case PTuple(es) if (es.length == 2):
				switch (es[0]) {
					case PLiteral(l) if (isOkAtom(l)): headOk = true;
					default:
				}
				switch (es[1]) {
				...
				case ERemoteCall(target, fnName, args) if (fnName == "json" && args != null && args.length == 2):
					switch (args[1].def) {
						case EMap(pairs):
							for (p in pairs)
								switch (p.value.def) {
									case EVar(vn): if (!declared.exists(vn)) undefInMap.set(vn, true);
									default:
								}
						default:
					}
	...
	static function mapUndefinedVarsToBinder(body:ElixirAST, binder:String, env:Map<String, Bool>):ElixirAST {
		...
		reflaxe.elixir.ast.ASTUtils.walk(body, function(n:ElixirAST) {
			switch (n.def) {
				...
				case ECase(_, cs):
					for (c in cs)
						collectPatternDecls(c.pattern, declared);
				default:
			}
		});
		for (k in env.keys())
	...
	static function collectPatternDecls(p:EPattern, acc:Map<String, Bool>):Void {
		switch (p) {
			...
			case PTuple(es) | PList(es):
				for (e in es)
					collectPatternDecls(e, acc);
			case PCons(h, t):
				collectPatternDecls(h, acc);
				collectPatternDecls(t, acc);
			case PMap(kvs):
				for (kv in kvs)
					collectPatternDecls(kv.value, acc);
			case PStruct(_, fs):
				for (f in fs)
					collectPatternDecls(f.value, acc);
			case PPin(inner):
	...
	static function prefixBindUndefinedToBinder(body:ElixirAST, binder:String):ElixirAST {
		...
		for (u in undef)
			assigns.push(makeAST(EMatch(PVar(u), makeAST(EVar(binder)))));
		...
		return switch (e.def) {
			case ERemoteCall(target, fnName, args):
				if (fnName != "json" || args == null || args.length != 2)
					return false;
				switch (target.def) {
					case EVar(m) if (m == "Phoenix.Controller"): true;
					default: false;
				}
			default: false;
		}
