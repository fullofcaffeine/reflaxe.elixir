...
class ClauseUndefinedVarBindToBinderTransforms {
	...
	static function processClause(cl:ECaseClause, outerScope:Map<String, Bool>):ECaseClause {
		...
		var best = undef[0];
		var binderName = b;
		if (best == null || best.length == 0 || best == binderName)
			return cl;
		if (hasAliasInBody(cl.body, best, binderName))
			return cl;

		var prefixes:Array<ElixirAST> = [makeAST(EBinary(Match, makeAST(EVar(best)), makeAST(EVar(binderName))))];
