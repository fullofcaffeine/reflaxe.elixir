		...
		for (s in stmts)
			switch (s.def) {
				case EBinary(Match, left, right):
					...
					if (!ok)
						out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("_"), left.metadata, left.pos), right), s.metadata, s.pos))
					else
						out.push(s);
				case EMatch(pat, rhs):
					var pat2 = switch (pat) {
					...
					if (pat2 != pat)
						out.push(makeASTWithMeta(EMatch(pat2, rhs), s.metadata, s.pos))
					else
						out.push(s);
				default:
					out.push(s);
