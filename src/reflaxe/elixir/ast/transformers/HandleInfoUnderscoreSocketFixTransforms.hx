					...
					var nb = ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
						return switch (x.def) {
							...
							case EBinary(Match, left, right):
								...
								var normRight = switch (right.def) {
									case EVar(rv) if (rv == "_socket"): makeASTWithMeta(EVar("socket"), right.metadata, right.pos);
									default: right;
								};
								switch (normRight.def) {
									case EVar(rv2) if (rv2 == "socket"):
										switch (left.def) {
											case EVar(_): makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("_"), left.metadata, left.pos), normRight),
													x.metadata, x.pos);
