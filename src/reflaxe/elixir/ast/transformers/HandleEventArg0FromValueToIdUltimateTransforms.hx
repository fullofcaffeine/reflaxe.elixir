 ...
 *
 * WHAT
 * - In handle_event/3 clause bodies for id-based events (toggle_todo, delete_todo, edit_todo),
 *   when a call uses `Map.get(params, "value", ...)` as its first argument and `socket` as the
 *   last argument, replace that first argument with an integer-converted id extracted from the
 *   nested value map or from the top-level params fallback.
 *
 * WHY
 * - Click events often send payload as `%{"value" => %{"id" => "123", ...}}` while form submits
 *   omit `value` entirely. Helpers like `toggle_todo_status/2` expect an `id`, not the entire map.
 *   This pass bridges that shape generically without app-specific names.
 *
 * HOW
 * - Match inside `def handle_event(<string>, params, socket)` for event names that conventionally
 *   take an id. For calls where first arg is `Map.get(params, "value", ...)` (or call-form Map.get)
...
class HandleEventArg0FromValueToIdUltimateTransforms {
    ...
    return ElixirASTTransformer.transformNode(ast, function(n: ElixirAST): ElixirAST {
      return switch (n.def) {
        case EDef(name, args, guards, body) if (isHandleEvent3(name, args)):
          var ev:Null<String> = switch (args[0]) { case PLiteral({def: EString(s)}): s.toLowerCase(); default: null; };
        ...
        case EDefp(name2, args2, guards2, body2) if (isHandleEvent3(name2, args2)):
          var ev2:Null<String> = switch (args2[0]) { case PLiteral({def: EString(s2)}): s2.toLowerCase(); default: null; };
  ...
  static inline function isIdBasedEvent(ev:Null<String>):Bool {
