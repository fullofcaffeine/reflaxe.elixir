				...
				case ECase(target, clauses):
					...
					for (cl in clauses) {
						...
						if (okBinder != null) {
							...
							var locked = cl.body != null && cl.body.metadata != null && (cl.body.metadata.lockPayloadBinder == true);
							if (okBinder == "_value" || locked) {
								newClauses.push(cl);
								continue;
							}
							...
							var binderUsed = VarUseAnalyzer.stmtUsesVarExact(cl.body, okBinder)
								|| (cl.guard != null && VarUseAnalyzer.stmtUsesVarExact(cl.guard, okBinder));
							if (binderUsed) {
								newClauses.push(cl);
								continue;
							}
							...
							if (fnDeclared != null)
								for (k in fnDeclared.keys())
									clauseDeclared.set(k, true);
							...
							var undef = [];
							for (u in used.keys())
								if (!clauseDeclared.exists(u) && u != okBinder && allowUndefined(u))
									undef.push(u);
							if (undef.length == 1) {
								var newName = undef[0];
								// Guard: never rename binder to an underscore/infra temp or reserved env name
								if (!allowUndefined(newName) || isHygieneBinder(newName)) {
									newClauses.push(cl);
									continue;
								}
