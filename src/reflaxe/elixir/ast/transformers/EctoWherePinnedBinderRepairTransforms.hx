			...
			switch (x.def) {
				...
				case ERaw(code) if (code != null):
					...
					while (i < code.length && found == null) {
						...
						if (~/^[A-Za-z_][A-Za-z0-9_]*$/.match(candidate))
							found = candidate;
						i = j + 1;
					}
				default:
		...
		inline function markDeclared(s:ElixirAST):Void {
			switch (s.def) {
				...
				case EBinary(Match, left, _):
					switch (left.def) {
						case EVar(nm2): declared.set(nm2, true);
						default:
					}
				default:
			}
		...
		for (i in 0...stmts.length) {
			...
			if (!pushed) {
				...
				inline function tryRepairForArgs(args:Array<ElixirAST>):Void {
					...
					if (pinnedName == null)
						pinnedVar = findPinnedInnerVar(cond);
					if (pinnedName != null && !declared.exists(pinnedName)) {
						...
						if (lastWildcardIdx >= 0) {
							...
							if (prevId != null && prevId == pvId) {
								...
								var pinnedName2 = switch (pinnedVar.def) {
									case EVar(nm): nm;
									default: null;
								};
								if (pinnedName2 != null) {
									switch (prev2.def) {
										...
										case EBinary(Match, leftPrev2, rhsPrev2):
											...
											if (isWildPrev2) {
												out[lastWildcardIdx] = makeASTWithMeta(EBinary(Match, makeAST(EVar(pinnedName2)), rhsPrev2), prev2.metadata,
													prev2.pos);
												declared.set(pinnedName2, true);
												repaired = true;
											}
