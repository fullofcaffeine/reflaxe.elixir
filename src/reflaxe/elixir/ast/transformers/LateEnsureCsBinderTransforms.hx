...
class LateEnsureCsBinderTransforms {
	...
	static function ensureBinder(body:ElixirAST):ElixirAST {
			...
			function scan(n:ElixirAST):Void {
				...
				switch (n.def) {
					case ERemoteCall(mod, fn, _):
						switch (mod.def) {
							case EVar(m) if (m == "Ecto.Changeset" && (StringTools.startsWith(fn, "validate_"))): found = true;
							default:
						}
					case ERaw(code):
						if (code != null && code.indexOf("Ecto.Changeset.validate_") != -1)
							found = true;
					case EBlock(ss):
						for (s in ss)
							scan(s);
		...
		function ensureTrailingCsBlock(stmts:Array<ElixirAST>):Array<ElixirAST> {
			...
			var endsWithCs = switch (last.def) {
	...
	static function patternDeclares(p:EPattern, name:String):Bool {
		return switch (p) {
			...
			case PMap(kvs):
				for (kv in kvs)
					if (patternDeclares(kv.value, name))
						return true;
				false;
			case PStruct(_, fs):
				for (f in fs)
					if (patternDeclares(f.value, name))
						return true;
				false;
	...
	static function rhsIfChangesetProducer(s:ElixirAST):Null<ElixirAST> {
		function containsChangesetProducer(e:ElixirAST):Bool {
			return switch (e.def) {
				case ERemoteCall(mod, fn, _):
					...
					false;
				case ERaw(code): code != null && (code.indexOf("Ecto.Changeset.cast(") != -1
						|| code.indexOf("Ecto.Changeset.change(") != -1);
		...
		switch (s.def) {
			...
			case ERaw(code) if (code != null
				&& (code.indexOf("Ecto.Changeset.cast(") != -1 || code.indexOf("Ecto.Changeset.change(") != -1)):
				return s;
	...
	static function peel(n:ElixirAST):ElixirAST {
		...
		while (cur != null && cur.def != null) {
			switch (cur.def) {
				case EBinary(Match, _, inner):
					cur = inner;
				case EMatch(_, inner2):
					cur = inner2;
				case EParen(inner3):
					cur = inner3;
				default:
