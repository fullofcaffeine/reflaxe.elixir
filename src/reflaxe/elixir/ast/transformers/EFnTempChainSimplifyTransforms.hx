				...
				case EVar(name):
					...
					for (i in 0...stmts.length - 1) {
						...
						switch (stmts[idx].def) {
							...
							case EMatch(pat, r2):
								switch (pat) {
									case PVar(varName) if (varName == name):
										assignIdx = idx;
										rhs = r2;
									default:
								}
						...
						for (j in assignIdx + 1...stmts.length - 1) {
							...
							var isNilSentinel = switch (s.def) {
								case EBinary(Match, matchLeft, rnil):
									...
									if (isNil) switch (matchLeft.def) {
										case EVar(varName) if (varName == name): true;
										default: false;
									} else false;
								case EMatch(matchPattern, matchRhs):
									var rhsIsNil = switch (matchRhs.def) {
										case ENil: true;
										default: false;
									};
									if (rhsIsNil) switch (matchPattern) {
										case PVar(varName) if (varName == name): true;
										default: false;
									} else false;
								default:
									false;
