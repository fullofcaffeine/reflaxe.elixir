...
class CaseTupleResultBindingTransforms {
					...
					case ECase(expr, clauses):
						{
							build: function(resultVarNames:Array<String>, newCaseExpr:ElixirAST):ElixirAST {
								return makeASTWithMeta(EMatch(PTuple([for (nm in resultVarNames) PVar(nm)]), newCaseExpr), stmt.metadata, stmt.pos);
							},
	...
	private static function extractConsistentTupleVarNames(caseExpr:{expr:ElixirAST, clauses:Array<ECaseClause>}):Null<Array<String>> {
		if (caseExpr == null || caseExpr.clauses == null || caseExpr.clauses.length == 0)
			return null;
		...
		for (cl in caseExpr.clauses) {
			...
			for (e in tupleElems) {
				var v = switch (unwrapParen(e).def) {
					case EVar(nm): nm;
					default: null;
				};
			...
			if (names == null) {
				...
				if (names.length != clauseNames.length)
					return null;
				for (i in 0...names.length)
					if (names[i] != clauseNames[i])
						return null;
			}
		}
	...
	private static function rewriteClauseBody(body:ElixirAST, resultVarNames:Array<String>, nilInitNames:StringMap<Bool>):ElixirAST {
		...
		return makeASTWithMeta(bodyStmts.wrapper == "do" ? EDo(rebuilt) : EBlock(rebuilt), body.metadata, body.pos);
	}
