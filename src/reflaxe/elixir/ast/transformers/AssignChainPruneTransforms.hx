...
class AssignChainPruneTransforms {
	...
	static function isPresenceEffectCall(e:ElixirAST):Bool {
		return switch (e.def) {
			case ERemoteCall({def: EVar(mod)}, func, _): var isPresence = (mod == "Phoenix.Presence")
					|| StringTools.endsWith(mod, ".Presence")
					|| (mod == "Presence"); isPresence && (func == "track" || func == "update" || func == "untrack");
			default:
				false;
		};
	}
