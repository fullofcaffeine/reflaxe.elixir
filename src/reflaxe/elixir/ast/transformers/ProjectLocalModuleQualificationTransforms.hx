...
class ProjectLocalModuleQualificationTransforms {
	...
	public static function transformPass(ast:ElixirAST):ElixirAST {
		...
		try
			app = Context.definedValue("app_name")
		catch (e) {}
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case ERemoteCall({def: EVar(moduleName)}, functionName, argumentList) if (localRoots.exists(moduleName)):
				...
				case ECall({def: EVar(moduleName)}, functionName, argumentList) if (localRoots.exists(moduleName)):
					var qualifiedModule = qualifyAppLocalModule(moduleName, app);
					makeASTWithMeta(ERemoteCall(makeAST(EVar(qualifiedModule)), functionName, argumentList), n.metadata, n.pos);
