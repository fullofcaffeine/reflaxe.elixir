...
class ExUnitAssertTransforms {
  ...
  static function rewriteAssertRemoteCall(n: ElixirAST, fnName: String, args: Array<ElixirAST>): ElixirAST {
    return switch (fnName) {
      ...
      case "equals":
        if (args == null || args.length < 2) n else assertOrRefute("assert", makeAST(EBinary(EBinaryOp.Equal, args[0], args[1])), takeOptionalNonNilArg(args, 2), n);
      case "not_equals":
        if (args == null || args.length < 2) n else assertOrRefute("assert", makeAST(EBinary(EBinaryOp.NotEqual, args[0], args[1])), takeOptionalNonNilArg(args, 2), n);
      case "is_null":
        if (args == null || args.length < 1) n else assertOrRefute("assert", makeAST(ECall(null, "is_nil", [args[0]])), takeOptionalNonNilArg(args, 1), n);
      case "is_not_null":
        if (args == null || args.length < 1) n else assertOrRefute("refute", makeAST(ECall(null, "is_nil", [args[0]])), takeOptionalNonNilArg(args, 1), n);
      case "is_some":
      ...
      case "contains":
        if (args == null || args.length < 2) n else assertOrRefute("assert", makeAST(ERemoteCall(makeAST(EVar("Enum")), "member?", [args[0], args[1]])), takeOptionalNonNilArg(args, 2), n);
      case "contains_string":
        if (args == null || args.length < 2) n else assertOrRefute("assert", makeAST(ERemoteCall(makeAST(EVar("String")), "contains?", [args[0], args[1]])), takeOptionalNonNilArg(args, 2), n);
      case "does_not_contain_string":
        if (args == null || args.length < 2) n else assertOrRefute("refute", makeAST(ERemoteCall(makeAST(EVar("String")), "contains?", [args[0], args[1]])), takeOptionalNonNilArg(args, 2), n);
      case "is_empty":
        if (args == null || args.length < 1) n else assertOrRefute("assert", makeAST(ERemoteCall(makeAST(EVar("Enum")), "empty?", [args[0]])), takeOptionalNonNilArg(args, 1), n);
      case "is_not_empty":
        if (args == null || args.length < 1) n else assertOrRefute("refute", makeAST(ERemoteCall(makeAST(EVar("Enum")), "empty?", [args[0]])), takeOptionalNonNilArg(args, 1), n);
      case "in_delta":
        rewriteInDelta(n, args);
      case "fail":
        if (args == null || args.length < 1) n else makeASTWithMeta(ECall(null, "flunk", [args[0]]), n.metadata, n.pos);
      case "matches":
        if (args == null || args.length < 2) n else assertOrRefute("assert", makeAST(ECall(null, "match?", [args[0], args[1]])), takeOptionalNonNilArg(args, 2), n);
      case "received":
  ...
  static function rewriteRaises(n: ElixirAST, args: Array<ElixirAST>): ElixirAST {
    if (args == null || args.length < 1) return n;
    var fnExpr = args[0];
    ...
    if (exceptionModule != null) {
      ...
      if (message != null) assertArgs.push(message);
  ...
  static function rewriteInDelta(n: ElixirAST, args: Array<ElixirAST>): ElixirAST {
    if (args == null || args.length < 3) return n;
  ...
  static function rewriteReceived(n: ElixirAST, args: Array<ElixirAST>): ElixirAST {
    ...
    if (timeout != null) callArgs.push(timeout);
    if (message != null) {
      if (timeout == null) callArgs.push(makeAST(EInteger(100)));
  ...
  static function resultIsOkExpr(value: ElixirAST): ElixirAST {
    return makeAST(ECall(null, "match?", [
      makeAST(ETuple([
        makeAST(EAtom(ElixirAtom.raw("ok"))),
        makeAST(EUnderscore)
      ])),
      value
    ]));
  ...
  static function resultIsErrorExpr(value: ElixirAST): ElixirAST {
    return makeAST(ECall(null, "match?", [
      makeAST(ETuple([
        makeAST(EAtom(ElixirAtom.raw("error"))),
        makeAST(EUnderscore)
      ])),
      value
    ]));
  ...
  static function isExUnitModule(moduleAst: ElixirAST, body: ElixirAST): Bool {
    if (moduleAst.metadata?.isExunit == true) return true;
  ...
  static function containsUseExUnitCase(ast: ElixirAST): Bool {
    ...
    ElixirASTTransformer.transformNode(ast, function(n: ElixirAST): ElixirAST {
      if (!found) {
        switch (n.def) {
          case EUse("ExUnit.Case", _): found = true;
          default:
        }
      }
      return n;
