								...
								switch (out[i].def) {
									...
									case ERemoteCall({def: EVar(mod)}, "assign", [firstArg, secArg]) if (mod == "Phoenix.Component"):
										...
										if (firstVar != null && !declared.exists(firstVar)) {
											...
											if (paramName != null)
												firstArg = makeAST(ElixirASTDef.EVar(paramName));
										}
										var needsBinding = false;
										for (k in i + 1...out.length)
											switch (out[k].def) {
											...
											for (k in i + 1...out.length)
												switch (out[k].def) {
													case ETuple(elems2) if (elems2.length == 2):
														...
														switch (elems2[1].def) {
															case EVar(vn3) if (ok2): tupleVar = vn3;
															default:
														}
													default:
												}
											if (tupleVar != null) {
												out[i] = makeASTWithMeta(EBinary(Match, makeAST(ElixirASTDef.EVar(tupleVar)),
													makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [firstArg, secArg]))),
													out[i].metadata, out[i].pos);
												declared.set(tupleVar, true);
											} else {
												out[i] = makeASTWithMeta(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [firstArg, secArg]),
													out[i].metadata, out[i].pos);
											}
										} else {
											out[i] = makeASTWithMeta(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [firstArg, secArg]),
									...
									case EBinary(Match, leftVar, {def: ERemoteCall({def: EVar(mod2)}, "assign", args2)}) if (mod2 == "Phoenix.Component"):
										...
										if (fv != null && !declared.exists(fv)) {
											...
											if (paramName2 != null)
												a0 = makeAST(ElixirASTDef.EVar(paramName2));
										}
											...
											for (k in i + 1...out.length)
												switch (out[k].def) {
													case ETuple(elems3) if (elems3.length == 2):
														...
														switch (elems3[1].def) {
															case EVar(vn4) if (ok3): tupleVar2 = vn4;
															default:
														}
													default:
												}
										...
										out[i] = makeASTWithMeta(EBinary(Match, finalLeft,
											makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [a0, a1]))),
											out[i].metadata, out[i].pos);
									default:
								}
