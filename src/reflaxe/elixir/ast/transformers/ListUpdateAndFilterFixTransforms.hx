			...
			switch (n.def) {
				case EDef(fname, fargs, fguards, fbody):
					...
					if (fargs != null)
						for (p in fargs)
							switch (p) {
								case PVar(nm) if (baseName(nm) == "id"): idParam = nm;
								default:
							}
					#if debug_list_fix
					if (idParam != null)
						trace('[ListFix] EDef ' + fname + ' has id param: ' + idParam);
					#end
					if (idParam == null)
						return n;
								...
								switch (findFnClauses) {
									case _ if (findFnClauses != null && findFnClauses.length == 1):
										...
										var fixedBody = switch (body.def) {
											case EBinary(Equal | StrictEqual, l, r):
												inline function isBinderVar(e:ElixirAST):Bool
													return isVarNamed(e, binder);
												inline function isIdFieldOfBinder(e:ElixirAST):Bool
													return switch (unwrapSingleExpr(e).def) {
														case EField(target, fld) if (fld == "id"): isVarNamed(target, binder);
														default: false;
													};
												var eqOp = switch (body.def) {
													case EBinary(StrictEqual, _, _): StrictEqual;
													default: Equal;
												};
												if (isIdFieldOfBinder(l) && isBinderVar(r)) makeAST(EBinary(eqOp, l,
													makeAST(EVar(idParam)))) else if (isBinderVar(l) && isIdFieldOfBinder(r)) makeAST(EBinary(eqOp,
													makeAST(EVar(idParam)), r)) else cl.body;
											default:
												cl.body;
										};
										...
										var fixedBody = switch (cl.body.def) {
											case EBinary(NotEqual | StrictNotEqual, l, r):
												inline function isElemId(e:ElixirAST):Bool {
													return switch (e.def) {
														case EField({def: EVar(_)}, fld) if (fld == "id"): true;
														default: false;
													}
												}
							...
							case EFn(clauses) if (clauses.length == 1):
								...
								switch (unwrappedBody.def) {
									case EBinary(NotEqual | StrictNotEqual, l2, r2):
										inline function isBinder(e:ElixirAST):Bool
											return switch (e.def) {
									...
									case EBinary(Equal | StrictEqual, lEq, rEq):
										...
										inline function isIdFieldOfBinderEq(e:ElixirAST):Bool
											return switch (unwrapSingleExpr(e).def) {
												case EField(target, fld) if (fld == "id"): isVarNamed(target, binder);
												default: false;
											};
										var eqOp = switch (unwrappedBody.def) {
											case EBinary(StrictEqual, _, _): StrictEqual;
											default: Equal;
										};
				...
				case EDefp(fnamep, fargsp, fguardsp, fbodyp):
					...
					if (fargsp != null)
						for (p in fargsp)
							switch (p) {
								case PVar(nm) if (baseName(nm) == "id"): idParamP = nm;
								default:
							}
					#if debug_list_fix
					if (idParamP != null)
						trace('[ListFix] EDefp ' + fnamep + ' has id param: ' + idParamP);
					#end
					if (idParamP == null)
						return n;
									...
									case _ if (findFnClauses != null && findFnClauses.length == 1):
										...
										var fixedBody = switch (body.def) {
											case EBinary(Equal | StrictEqual, l, r):
												...
												if (isIdFieldOfBinder(l) && isBinderVar(r)) makeAST(EBinary(eqOp, l,
													makeAST(EVar(idParamP)))) else if (isBinderVar(l) && isIdFieldOfBinder(r)) makeAST(EBinary(eqOp,
													makeAST(EVar(idParamP)), r)) else cl.body;
											default:
												cl.body;
										};
										if (fixedBody != cl.body) {
							...
							case EFn(clauses) if (clauses.length == 1):
								...
								switch (unwrappedBodyP.def) {
									case EBinary(NotEqual | StrictNotEqual, lB, rB):
										inline function isBinder(e:ElixirAST):Bool
											return switch (e.def) {
												case EVar(nm2) if (nm2 == binder2): true;
												default: false;
											};
										inline function isIdFieldOfBinder(e:ElixirAST):Bool
											return switch (unwrapSingleExpr(e).def) {
												case EField(target, fld) if (fld == "id"): isVarNamed(target, binder2);
												default: false;
											};
									...
									case EBinary(Equal | StrictEqual, lEqP, rEqP):
										inline function isBinderVarP(e:ElixirAST):Bool
											return switch (unwrapSingleExpr(e).def) {
												case EVar(nm) if (nm == binder2): true;
												default: false;
											};
										inline function isIdFieldOfBinderP(e:ElixirAST):Bool
											return switch (unwrapSingleExpr(e).def) {
												case EField(target, fld) if (fld == "id"): isVarNamed(target, binder2);
												default: false;
											};
										var eqOpP = switch (unwrappedBodyP.def) {
											case EBinary(StrictEqual, _, _): StrictEqual;
											default: Equal;
										};
