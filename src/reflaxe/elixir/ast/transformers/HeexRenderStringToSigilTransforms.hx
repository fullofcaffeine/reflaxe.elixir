										...
										case EMatch(PVar(lhs), rhs) if (lhs == varName):
											...
											if (!usedLater) {
												...
												if (sBind != null && looksLikeHtml(sBind)) {
													...
													updatedStmt = makeASTWithMeta(EMatch(PVar(lhs), makeAST(ESigil("H", convBind, ""))),
														stmts[assignIdx].metadata, stmts[assignIdx].pos);
												}
											}
												...
												case EVar(lhs2) if (lhs2 == varName):
													...
													if (!usedLater2) {
														...
														if (sBind2 != null && looksLikeHtml(sBind2)) {
															...
															updatedStmt = makeASTWithMeta(EBinary(Match, left, makeAST(ESigil("H", convBind2, ""))),
																stmts[assignIdx].metadata, stmts[assignIdx].pos);
														}
													}
												default:
									...
									switch (stmts[assignIdx2].def) {
										case EMatch(PVar(lhs3), rhs3) if (lhs3 == varName2):
											...
											if (sBind3 != null && looksLikeHtml(sBind3)) {
												...
												updatedStmt2 = makeASTWithMeta(EMatch(PVar(lhs3), makeAST(ESigil("H", convBind3, ""))),
													stmts[assignIdx2].metadata, stmts[assignIdx2].pos);
											}
											...
											switch (left3.def) {
												case EVar(lhs4) if (lhs4 == varName2):
													...
													if (sBind4 != null && looksLikeHtml(sBind4)) {
														...
														updatedStmt2 = makeASTWithMeta(EBinary(Match, left3, makeAST(ESigil("H", convBind4, ""))),
															stmts[assignIdx2].metadata, stmts[assignIdx2].pos);
													}
												default:
											}
