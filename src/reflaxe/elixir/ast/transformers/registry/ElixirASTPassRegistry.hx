...
class ElixirASTPassRegistry {
	...
	static function getGranularPasses():Array<ElixirASTTransformer.PassConfig> {
		...
		// Override select Haxe stdlib pieces with native Elixir implementations when justified.
		passes.push({
		...
		passes.push({
			name: "EqNilToIsNil",
			description: "Replace (x == nil) with Kernel.is_nil(x) (post opts rewrites)",
			enabled: true,
		...
		// NOTE: We intentionally avoid auto-underscoring unused locals here.
		// It can mis-detect usage inside nested expressions (e.g. HEEx arg maps) and cause undefined refs.
		...
		passes.push({
			...
			runAfter: ["SuccessBinderPrefixMostUsedUndefined_Final"]
		});
					...
					return switch (n.def) {
						case EModule(name, attrs, body):
							...
							function scan(x:ElixirAST):Void {
								if (hasAssign || x == null || x.def == null)
									return;
								switch (x.def) {
									case ECall(_, func, args) if (func == "assign" && args != null && args.length == 2):
										switch (args[0].def) {
											case EVar(v) if (v.indexOf("socket") != -1): hasAssign = true;
											default:
										}
							...
							if (hasAssign) {
								...
								for (b in body)
									switch (b.def) {
										case EUse(module, _) if (module == "Phoenix.Component"): hasUse = true;
										default:
									}
								if (!hasUse) {
					...
					return switch (n.def) {
						...
						case EDefmodule(name, doBlock) if (name != null && (name.indexOf("Live") != -1)):
							...
							switch (doBlock.def) {
								...
								default: if (!hasUse) makeASTWithMeta(EDefmodule(name, makeAST(EBlock([makeAST(EUse("Phoenix.Component", [])), doBlock]))),
										n.metadata, n.pos) else n;
							}
						default:
							n;
					}
							...
							switch (s.def) {
								case EUse(module, opts) if (module == webModule):
									if (opts != null)
										for (o in opts)
											switch (o.def) {
												case EAtom(a) if (a == "html"): return true;
												default:
											}
								default:
							}
		...
		passes.push({
			...
			runAfter: ["EctoQueryBranchSelfAssignUnderscore", "AssignWhereSelfBinderUnderscore"]
		});
		passes.push({
		...
		// NOTE: Prefer the guarded ModuleNewToStructLiteral pass; the BinderTransforms variant was removed to avoid
		...
		passes.push({
			name: "ListFindByIdFix",
			description: "Fix Enum.find self-compare v.id == v using enclosing id/_id param",
			enabled: true,
			pass: reflaxe.elixir.ast.transformers.ListMapFilterFixTransforms.filterRemoveFixPass
		});
		passes.push({
		...
		passes.push({
			...
			enabled: true,
			pass: reflaxe.elixir.ast.transformers.OkValueGlobalCleanupTransforms.pass,
			runAfter: ["WebJsonSecondArgRewrite_Ultimate", "FinalLocalReferenceAlign"]
		});
		...
		passes.push({
			...
			runAfter: ["CaseBinderRefNormalizeByFlattenUnderscores_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["FunctionArgMultiStmtIIFE_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["StringIndexOf_Normalize_Final", "FunctionArgMultiStmtIIFE_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["BinaryMatchCaseArgNormalize_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["AssignWhereSelfBinderUnderscore_Final", "DefParamUnusedUnderscoreSafe"]
		});
		...
		passes.push({
			...
			enabled: true,
			pass: reflaxe.elixir.ast.transformers.ParamUnderscoreArgRefAlignGlobalTransforms.pass,
			runAfter: ["DefParamHeadUnderscoreWhenUnused_Final", "HeexAssignsParamRename_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["HandleEventMapGetUnderscoreParams_Final"]
		});
		passes.push({
		...
		passes.push({
			...
			runAfter: ["HandleInfoUnderscoreSocketFix_Final", "HandleInfoReturnSocketNormalize_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["ListUpdateAndFilterFix", "WebParamFinalFix", "HandleEventParamRepair_Final"]
		});
		...
		passes.push({
			...
			runAfter: ["UnderscoreTempInlineDowncase", "DowncaseInlineFromPriorAssign_Final"]
		});
	...
	static function sortPassesByConstraints(passes:Array<ElixirASTTransformer.PassConfig>):Array<ElixirASTTransformer.PassConfig> {
		...
		for (i in 0...passes.length)
			indexByName.set(passes[i].name, i);
		...
		inline function addEdge(from:String, to:String):Void {
			if (!adj.exists(from) || !adj.exists(to))
				return;
