...
class ElixirASTPassRegistry {
    public static function getEnabledPasses(): Array<ElixirASTTransformer.PassConfig> {
        ...
        passes.push({
            ...
            description: "Repair {:tag, socket} in handle_info/2 clauses by renaming binder and fixing local helper calls (runs before generic binder-avoid)",
            enabled: true,
        ...
        */
        ...
        // Disabled: unsafe for nested usage sites (e.g., HEEx arg maps) — can underscore
        // locals that are referenced immediately in the next statement inside nested expressions.
        passes.push({
        ...
        // Ultra-final safeguard: if a predicate still references `query` and there is no
        // local binder but `search_query` exists in the surrounding block, inline `query` now.
        passes.push({
        ...
        // Debug: scan case {:tag, binder} for undefined body locals (to validate binder passes)
        passes.push({
        ...
        // Disabled: over-aggressive discards caused undefined-variable errors
        // in LiveView flows where nested expressions reference immediately-bound locals.
        // Correctness-first: warnings are handled by underscore-prefix passes without dropping semantics.
        passes.push({
        ...
        passes.push({
            name: "ClauseUndefinedVarToBinder_Final",
            description: "(absolute final) Harmonize clause payload binder with the sole undefined local used in body",
            enabled: true,
            pass: reflaxe.elixir.ast.transformers.ClauseUndefinedVarToBinderTransforms.replaceUndefinedVarWithBinderPass
        });
        passes.push({
            name: "CaseBinderAlignFinal",
            description: "(absolute final) Rename {:tag, binder} pattern to match the body’s sole undefined local",
            enabled: true,
            pass: reflaxe.elixir.ast.transformers.CaseBinderAlignFinalTransforms.pass
        });
