...
class PresenceConcatAccumulatorInitTransforms {
	...
	static inline function looksPresence(node:ElixirAST, name:String):Bool {
		return (node.metadata?.isPresence == true)
			|| (name != null && name.indexOf("Web.Presence") > 0)
			|| StringTools.endsWith(name, ".Presence");
	}
	...
	static function ensureInit(body:ElixirAST):ElixirAST {
		...
		function scan(n:ElixirAST):Void {
			...
			switch (n.def) {
				case EBinary(Match, {def: EVar(lhs)}, rhs0):
					switch (rhs0.def) {
						case EList(_): hasInitList.set(lhs, true);
						default:
					}
				case EMatch(PVar(lhs2), rhs1):
					switch (rhs1.def) {
						case EList(_): hasInitList.set(lhs2, true);
						default:
					}
				case ERemoteCall({def: EVar(modName)}, fn, args) if (fn == "concat" && args != null && args.length >= 1):
					...
					if (isEnum)
						switch (args[0].def) {
							case EVar(v): needsInit.set(v, true);
							default:
						}
				default:
			}
		...
		for (v in initList)
			inits.push(makeAST(EBinary(Match, makeAST(EVar(v)), makeAST(EList([])))));

		inline function rewriteTailToVar(n:ElixirAST, v:String):ElixirAST {
			return switch (n.def) {
				case EBlock(stmts) if (stmts.length > 0):
					var last = stmts[stmts.length - 1];
					var newLast = switch (last.def) {
						case EList(_): makeAST(EVar(v));
						default: last;
					};
					var prefix = stmts.copy();
					prefix.pop();
				...
				case EDo(stmts2) if (stmts2.length > 0):
					...
					var newLast2 = switch (last2.def) {
						case EList(_): makeAST(EVar(v));
						default: last2;
					};
					var prefix2 = stmts2.copy();
					prefix2.pop();
