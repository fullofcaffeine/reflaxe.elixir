...
class ControllerJsonDataArgPickSingleVarTransforms {
		...
		return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case ECase(expr, clauses):
					var newClauses = [];
					for (cl in clauses)
						newClauses.push({pattern: cl.pattern, guard: cl.guard, body: rewriteInBody(cl.body)});
		...
		return ElixirASTTransformer.transformNode(body, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ERemoteCall(t, fnName, args) if (fnName == "json" && args != null && args.length == 2):
					switch (args[1].def) {
						case EVar(v) if (v == "data"): makeASTWithMeta(ERemoteCall(t, fnName, [args[0], makeAST(EVar(varName))]), n.metadata, n.pos);
						default: n;
					}
				default: n;
			}
	...
	static inline function isCandidate(v:String):Bool {
		if (v == null || v.length == 0)
			return false;
		if (v == "conn" || v == "params" || v == "socket" || v == "live_socket")
			return false;
		var c = v.charAt(0);
