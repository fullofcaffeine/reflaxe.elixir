...
class BlockUnusedAssignmentDiscardTransforms {
							...
							switch (left.def) {
								case EVar(nm):
									...
									if (nm == "query" && filterPredicateUsesQueryLater(stmts, i + 1)) {
										...
										out.push(s);
									} else if (!(inChangeset && rhsContainsChangesetCall(rhs))
										...
										&& (nm.length > 0 && nm.charAt(0) == '_') // Do not discard underscored binders if their non-underscored
										// form is referenced later via string interpolation or raw
										&& !(nm.length > 1
											&& (interpolationReferencesFrom(i + 1, nm.substr(1))
												|| rawIdentifierUsedLater(stmts, i + 1, nm.substr(1))))) {
						...
						case EMatch(pat, rhs2):
							switch (pat) {
								case PVar(nm2):
									...
									if (isPresenceEffectCall(rhs2)) {
										out.push(s);
										break;
									}
									if (isDowncaseSearch(rhs2)) {
										out.push(s);
									} else if (!(inChangeset && rhsContainsChangesetCall(rhs2))
										...
										&& !(nm2.length > 1
											&& (interpolationReferencesFrom(i + 1, nm2.substr(1))
												|| rawIdentifierUsedLater(stmts, i + 1, nm2.substr(1))))) {
				...
				for (i in 0...stmts2.length) {
					...
					switch (s2.def) {
							...
							switch (left2.def) {
								case EVar(nm2):
									...
									if (nm2 == "query" && filterPredicateUsesQueryLater(stmts2, i + 1)) {
										...
										&& (nm2.length > 0 && nm2.charAt(0) == '_')
										&& !(nm2.length > 1
											&& (interpolationReferencesFromDo(i + 1, nm2.substr(1))
												|| rawIdentifierUsedLater(stmts2, i + 1, nm2.substr(1))))) {
						...
						case EMatch(pat2, rhsDo):
							switch (pat2) {
								case PVar(nmDo):
									if (!(inChangeset && rhsContainsChangesetCall(rhsDo))
		...
		function scan(n:ElixirAST):Void {
			...
			switch (n.def) {
				case ERemoteCall(mod, _, args):
					switch (mod.def) {
						case EVar(m) if (m == "Ecto.Changeset"): found = true;
						default:
					}
					scan(mod);
					if (args != null)
						for (a in args)
							scan(a);
				case ERaw(code):
					if (code != null && code.indexOf("Ecto.Changeset.") != -1)
						found = true;
				case ECall(t, _, as):
					if (t != null)
						scan(t);
	...
	static function isDowncaseSearch(e:ElixirAST):Bool {
		return switch (e.def) {
			case ERemoteCall({def: EVar(m)}, "downcase", args) if (m == "String" && args != null && args.length == 1):
				switch (args[0].def) {
					case EVar(v) if (v == "search_query"): true;
					default: false;
				}
			default: false;
		};
	...
	static function filterPredicateUsesQueryLater(stmts:Array<ElixirAST>, startIdx:Int):Bool {
		for (i in startIdx...stmts.length)
			if (stmtHasFilterQueryUse(stmts[i]))
				return true;
		return false;
	}
	...
	static function rawIdentifierUsedLater(stmts:Array<ElixirAST>, startIdx:Int, name:String):Bool {
		...
		for (i in startIdx...stmts.length) {
			...
			switch (s.def) {
				...
				case EBlock(ss):
					if (rawIdentifierUsedLater(ss, 0, name))
						return true;
				case EDo(ss2):
					if (rawIdentifierUsedLater(ss2, 0, name))
						return true;
				case EIf(c, t, e):
					// scan nested branches conservatively
					var tmpS = [c, t].concat(e != null ? [e] : []);
					if (rawIdentifierUsedLater(tmpS, 0, name))
						return true;
				default:
			}
		}
	...
	static function stmtHasFilterQueryUse(n:ElixirAST):Bool {
		...
		function scan(x:ElixirAST):Void {
			if (x == null || x.def == null || found)
				return;
			switch (x.def) {
				case ERaw(code) if (code != null):
					if (code.indexOf('Enum.filter(') != -1 && rawContainsIdent(code, 'query'))
						found = true;
				case ERemoteCall({def: EVar(m)}, "filter", args) if (m == "Enum" && args != null && args.length == 2):
					switch (args[1].def) {
						case EFn(cs) if (cs.length == 1): if (hasQueryInEFnBody(args[1])) found = true;
						default:
					}
				case ECall(_, "filter", args2) if (args2 != null && args2.length >= 1):
					var pred = args2[args2.length - 1];
					switch (pred.def) {
						case EFn(cs2) if (cs2.length == 1): if (hasQueryInEFnBody(pred)) found = true;
						default:
					}
				case EBlock(ss):
					for (s in ss)
