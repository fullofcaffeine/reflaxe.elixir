...
class TempVariableTransforms {
	...
	private static function isSingleUseOfVar(expr:ElixirAST, varName:String):Bool {
		...
		return switch (expr.def) {
			...
			case ECall(_, _, args): args.length > 0 && switch (args[0].def) {
					case EVar(v) if (v == varName): true;
					default: false;
				} default: false;
		};
	...
	private static function iterateAST(node:ElixirAST, visitor:ElixirAST->Void):Void {
		...
		switch (node.def) {
			case EBlock(exprs):
				for (e in exprs)
					iterateAST(e, visitor);
			case ECall(target, method, args):
				iterateAST(target, visitor);
				for (a in args)
					iterateAST(a, visitor);
			case EMap(pairs):
				for (p in pairs)
					iterateAST(p.value, visitor);
			case EKeywordList(pairs):
				for (p in pairs)
					iterateAST(p.value, visitor);
			case ETuple(values):
				for (v in values)
					iterateAST(v, visitor);
			case EList(items):
				for (i in items)
					iterateAST(i, visitor);
			case EBinary(op, left, right):
