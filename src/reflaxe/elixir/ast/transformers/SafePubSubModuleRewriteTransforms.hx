                ...
                case ERemoteCall(mod, func, args):
                    switch (mod.def) {
                        case EVar(m) if (m == 'SafePubSub'):
                            ...
                            trace('[SafePubSubRewrite] Rewriting SafePubSub.' + func + ' to Phoenix.SafePubSub.' + func);
                            #end
                ...
                case ECall(tgt, func2, args2):
                    if (tgt != null) switch (tgt.def) {
                        case EVar(m2) if (m2 == 'SafePubSub'):
                            #if debug_safe_pubsub_rewrite
                            trace('[SafePubSubRewrite] Rewriting SafePubSub.' + func2 + ' (target call) to Phoenix.SafePubSub.' + func2);
                            #end
