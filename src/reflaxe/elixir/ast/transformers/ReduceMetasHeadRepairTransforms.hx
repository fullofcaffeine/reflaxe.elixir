		...
		for (i in 0...stmts.length) {
			...
			switch (stmt.def) {
				case EBinary(Match, left, rhs):
					metaVar = switch (left.def) {
						case EVar(nm): nm;
						default: null;
					};
					isBinderAliasAssign = metaVar != null && (switch (rhs.def) {
						case EVar(rn) if (rn == binderName): true;
						default: false;
					});
				case EMatch(pat, rhs2):
					...
					isBinderAliasAssign = metaVar != null && (switch (rhs2.def) {
						case EVar(rn2) if (rn2 == binderName): true;
						default: false;
					});
				default:
			}
