								...
								case EBinary(Match, {def: EVar(a0)}, {def: EBinary(Match, {def: EVar(b0)}, rhs0)}):
									switch (stmts[i + 1].def) {
										case EIf(cond0, then0, else0):
											...
											if (elseIsB0) {
												// Emit: b = rhs; a = if ... end
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b0), s.metadata, s.pos), rhs0), s.metadata,
													s.pos));
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a0), s.metadata, s.pos),
													makeASTWithMeta(EIf(cond0, then0, else0), stmts[i + 1].metadata, stmts[i + 1].pos)),
													s.metadata, s.pos));
												i += 2;
												continue;
											}
								...
								while (j <= windowEnd) {
									switch (stmts[j].def) {
										case EIf(condW, thenW, elseW):
											...
											if (elseIsBW) {
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(bW), s.metadata, s.pos), rhsW), s.metadata,
													s.pos));
												out.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(aW), s.metadata, s.pos),
													makeASTWithMeta(EIf(condW, thenW, elseW), stmts[j].metadata, stmts[j].pos)),
													s.metadata, s.pos));
											...
											switch (stmts[i + 2].def) {
												case EIf(cond, thenExpr, elseExpr):
													...
													if (elseIsB) {
														...
														var assignAIf = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos),
															makeASTWithMeta(EIf(cond, thenExpr, elseExpr), stmts[i + 2].metadata, stmts[i + 2].pos)),
															stmts[i + 1].metadata, stmts[i + 1].pos);
														out.push(assignAIf);
														i += 3;
														folded = true;
													}
											...
											case EBinary(Match, innerLeft, rhs):
												switch (innerLeft.def) {
													case EVar(b):
														...
														var second = makeASTWithMeta(EBinary(Match, left,
															makeASTWithMeta(EVar(b), right.metadata, right.pos)), s.metadata, s.pos);
														out.push(first);
														out.push(second);
													default:
														out.push(s);
									...
									case EBinary(Match, left2, mid):
										...
										switch (left2.def) {
											case EVar(a2):
												switch (mid.def) {
													case EVar(b):
														var firstL = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b), mid.metadata, mid.pos), right),
															s.metadata, s.pos);
														var secondL = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a2), left2.metadata, left2.pos),
															makeASTWithMeta(EVar(b), mid.metadata, mid.pos)),
															s.metadata, s.pos);
														out.push(firstL);
														out.push(secondL);
														i++;
											...
											case EBinary(Match, innerLeft2, rhs2):
												switch (innerLeft2.def) {
													case EVar(b2):
														...
														var second2 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), rhs.metadata, rhs.pos),
															makeASTWithMeta(EVar(b2), rhs.metadata, rhs.pos)),
															s.metadata, s.pos);
														out.push(first2);
														out.push(second2);
													default:
															...
															case EBinary(Match, innerLeft3, rhs3):
																switch (innerLeft3.def) {
																	case EVar(b3):
																		...
																		var second3 = makeASTWithMeta(EBinary(Match,
																			makeASTWithMeta(EVar(a3), rhs.metadata, rhs.pos),
																			makeASTWithMeta(EVar(b3), rhs.metadata, rhs.pos)),
																			s.metadata, s.pos);
																		out.push(first3);
																		out.push(second3);
																	default:
								...
								case EBinary(Match, {def: EVar(a0)}, {def: EBinary(Match, {def: EVar(b0)}, rhs0)}):
									switch (stmts2[i2 + 1].def) {
										case EIf(cond0, then0, else0):
											...
											if (elseIsB0) {
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b0), s.metadata, s.pos), rhs0), s.metadata,
													s.pos));
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a0), s.metadata, s.pos),
													makeASTWithMeta(EIf(cond0, then0, else0), stmts2[i2 + 1].metadata, stmts2[i2 + 1].pos)),
													s.metadata, s.pos));
												i2 += 2;
												continue;
											}
								...
								while (j2 <= windowEnd2) {
									switch (stmts2[j2].def) {
										case EIf(condW2, thenW2, elseW2):
											...
											if (elseIsBW2) {
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(bW2), s.metadata, s.pos), rhsW2), s.metadata,
													s.pos));
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(aW2), s.metadata, s.pos),
													makeASTWithMeta(EIf(condW2, thenW2, elseW2), stmts2[j2].metadata, stmts2[j2].pos)),
													s.metadata, s.pos));
												for (k2 in i2 + 1...j2)
													out2.push(stmts2[k2]);
												i2 = j2 + 1;
										...
										case EBinary(Match, {def: EVar(a)}, {def: EVar(b2)}) if (b2 == b):
											switch (stmts2[i2 + 2].def) {
												case EIf(cond, thenExpr, elseExpr):
													...
													if (elseIsB2) {
														out2.push(s);
														var assignAIf2 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos),
															makeASTWithMeta(EIf(cond, thenExpr, elseExpr), stmts2[i2 + 2].metadata, stmts2[i2 + 2].pos)),
															stmts2[i2 + 1].metadata, stmts2[i2 + 1].pos);
										...
										switch (rhs.def) {
											case EBinary(Match, innerLeft2, rhs2):
												switch (innerLeft2.def) {
													case EVar(b2):
														var first2 = makeASTWithMeta(EBinary(Match, innerLeft2, rhs2), s.metadata, s.pos);
														var second2 = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a2), rhs.metadata, rhs.pos),
															makeASTWithMeta(EVar(b2), rhs.metadata, rhs.pos)),
															s.metadata, s.pos);
														out2.push(first2);
														out2.push(second2);
												...
												case EBinary(Match, {def: EVar(a)}, {def: EVar(b2)}) if (b2 == b):
													switch (stmts2[i2 + 2].def) {
														case EIf(cond, thenExpr, elseExpr):
															...
															if (elseIsB) {
																var assignAIf = makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a), s.metadata, s.pos),
																	makeASTWithMeta(EIf(cond, thenExpr, elseExpr), stmts2[i2 + 2].metadata,
																		stmts2[i2 + 2].pos)),
																	stmts2[i2 + 1].metadata, stmts2[i2 + 1].pos);
																out2.push(s);
																out2.push(assignAIf);
