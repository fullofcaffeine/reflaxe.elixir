	...
	static function rewriteStmt(s:ElixirAST):ElixirAST {
		return switch (s.def) {
			case ECall(target, fname, args):
				makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("_"), s.metadata, s.pos), makeASTWithMeta(ECall(target, fname, args), s.metadata, s.pos)),
					s.metadata, s.pos);
			case ERemoteCall(mod, fname2, args2):
				makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar("_"), s.metadata, s.pos),
					makeASTWithMeta(ERemoteCall(mod, fname2, args2), s.metadata, s.pos)),
					s.metadata, s.pos);
			default:
				s;
		}
