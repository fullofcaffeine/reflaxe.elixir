...
class ControllerResultBinderNormalizeTransforms {
		...
		return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ECase(expr, clauses):
					var newClauses = [];
					for (cl in clauses)
						newClauses.push(normalizeClause(cl));
			...
			case PTuple(es) if (es.length == 2):
				switch (es[0]) {
					case PLiteral(lit) if (isAtom(lit, ":ok")):
						switch (es[1]) {
							case PVar(n) if (n != null && n.length > 0 && n.charAt(0) == '_'): PTuple([es[0], PVar("value")]);
							default: p;
						}
					case PLiteral(lit2) if (isAtom(lit2, ":error")):
						switch (es[1]) {
							case PVar(n2) if (n2 != null && n2.length > 0 && n2.charAt(0) == '_'): PTuple([es[0], PVar("reason")]);
							default: p;
						}
					default: p;
				}
	...
	static inline function isAtom(ast:ElixirAST, name:String):Bool {
		return switch (ast.def) {
			case EAtom(v): v == name || v == name.substr(1);
			default: false;
		};
	}
}
