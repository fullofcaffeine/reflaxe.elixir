		...
		return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case EDo(stmts2):
					...
					for (s2 in stmts2)
						out2.push(cleanse(s2));
					makeASTWithMeta(EDo(out2), n.metadata, n.pos);
				case ECase(expr, clauses):
					var newClauses = [];
					for (cl in clauses)
						newClauses.push(rewriteArm(cl));
		...
		var newBody = ElixirASTTransformer.transformNode(cl.body, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ERemoteCall(t, fnName, args) if (fnName == "json" && args != null && args.length == 2):
					switch (args[1].def) {
						case EVar(v) if (v == "data"): makeASTWithMeta(ERemoteCall(t, fnName, [args[0], makeAST(EVar(binder))]), n.metadata, n.pos);
						default: n;
					}
				default: n;
			}
