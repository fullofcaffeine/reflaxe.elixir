				...
				case ERemoteCall({def: EVar(mod)}, "start_link", [firstArg, opts]) if (mod == "Supervisor"):
					switch (firstArg.def) {
						case EVar(v) if (v == "children"):
							...
							if (hasUnderscoredChildren) {
								makeASTWithMeta(ERemoteCall(makeAST(EVar("Supervisor")), "start_link", [makeAST(ElixirASTDef.EVar("_children")), opts]),
									e.metadata, e.pos);
							} else {
								makeASTWithMeta(ERemoteCall(makeAST(EVar("Supervisor")), "start_link", [makeAST(ElixirASTDef.EList([])), opts]), e.metadata,
									e.pos);
							}
						default:
							e;
							...
							switch (stmts[i].def) {
								...
								case EBinary(Match, leftAssign, _):
									switch (leftAssign.def) {
										case EVar(nm) if (nm == "children"): if (childrenAssignIdx == -1) childrenAssignIdx = i;
										default:
									}
								case ERemoteCall({def: EVar(mod)}, "start_link", [firstArg, _]) if (mod == "Supervisor"):
									switch (firstArg.def) {
										case EVar(v) if (v == "children"): startLinkIdx = i;
										default:
									}
								default:
							}
						if (startLinkIdx != -1
							&& ((wildcardIdx != -1 && wildcardIdx < startLinkIdx)
								|| (childrenAssignIdx != -1 && childrenAssignIdx < startLinkIdx))) {
							...
							switch (w.def) {
								case EMatch(_, rhs): rhsExpr = rhs;
								default:
							}
							switch (w.def) {
								case EBinary(Match, _, rhs): if (rhsExpr == null) rhsExpr = rhs;
								default:
							}
							if (rhsExpr == null)
								return node;
							...
							for (i in 0...stmts.length)
								if (i != dropIdx)
									newStmts.push(i == startLinkIdx ? newCall : stmts[i]);
