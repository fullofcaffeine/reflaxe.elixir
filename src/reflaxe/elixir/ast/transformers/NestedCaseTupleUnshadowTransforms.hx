...
class NestedCaseTupleUnshadowTransforms {
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ECase(outerExpr, outerClauses):
					...
					for (oc in outerClauses)
						out.push(rewriteNested(oc));
	...
	static function rewriteNested(oc:ECaseClause):ECaseClause {
		...
		var inner:Null<{
			expr:ElixirAST,
			clauses:Array<ECaseClause>,
			meta:ElixirMetadata,
			pos:haxe.macro.Expr.Position,
			parenDepth:Int
		}> = null;
		function unwrapParens(e:ElixirAST):{
			node:ElixirAST,
			depth:Int,
			meta:ElixirMetadata,
			pos:haxe.macro.Expr.Position
		...
		switch (oc.body.def) {
			...
			default:
				...
				switch (uwBody.node.def) {
					case ECase(e3, cls3): inner = {
							expr: e3,
							clauses: cls3,
							meta: uwBody.meta,
							pos: uwBody.pos,
							parenDepth: uwBody.depth
						};
					default:
				}
		}
		...
		var innerVar:Null<String> = switch (inner.expr.def) {
			case EVar(v): v;
			default: null;
		};
		if (innerVar == null)
		...
		while (d > 0) {
			wrapped = makeASTWithMeta(EParen(wrapped), wrapped.metadata, wrapped.pos);
			d--;
		}
