		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EDefp(name, args, guards, body) | EDef(name, args, guards, body):
										...
										ElixirASTTransformer.transformNode(thenB, function(z:ElixirAST):ElixirAST {
											switch (z.def) {
												case ERemoteCall({def: EVar("Enum")}, "filter", [lX, _]) if (switch (lX.def) {
														case EVar(vx) if (vx == retVar): true;
														default: false;
													}):
													if (filterExpr == null)
														filterExpr = z;
												...
												case EMatch(_, rhs):
													...
													switch (rhs.def) {
														case ERemoteCall({def: EVar("Enum")}, "filter", [lY, _]) if (switch (lY.def) {
																case EVar(vy) if (vy == retVar): true;
																default: false;
															}):
															if (filterExpr == null)
																filterExpr = rhs;
					...
					if (newBody == body) n else makeASTWithMeta(reflaxe.elixir.util.EnumReflection.enumConstructor(n.def) == "EDef" ? EDef(name, args, guards,
						newBody) : EDefp(name, args, guards, newBody),
						n.metadata, n.pos);
				default:
					n;
			}
				...
				case ECase(expr, clauses):
					...
					for (cl in clauses) {
						...
						var newBody = ElixirASTTransformer.transformNode(cl.body, function(x:ElixirAST):ElixirAST {
							switch (x.def) {
								case ECall(t, fname, args)
									if (args != null
										&& args.length >= 1
										&& scrut != null
										&& fname != null
										&& (StringTools.endsWith(fname, "_from_list") || StringTools.endsWith(fname, "_in_list"))):
									var newArgs = args.copy();
									if (switch (newArgs[0].def) {
											case EVar(v0) if (v0 == scrut): true;
											default: false;
										})
										newArgs[0] = makeAST(EVar(idVar));
									return makeASTWithMeta(ECall(t, fname, newArgs), x.metadata, x.pos);
								case ERemoteCall(m, fname2, args2)
									if (args2 != null
										&& args2.length >= 1
										&& scrut != null
										&& fname2 != null
										&& (StringTools.endsWith(fname2, "_from_list") || StringTools.endsWith(fname2, "_in_list"))):
									var newArgs2 = args2.copy();
									if (switch (newArgs2[0].def) {
											case EVar(v1) if (v1 == scrut): true;
											default: false;
										})
