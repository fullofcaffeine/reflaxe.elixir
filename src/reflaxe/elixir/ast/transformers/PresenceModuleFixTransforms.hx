...
class PresenceModuleFixTransforms {
	...
	static inline function shouldRewrite(fname:String):Bool {
		return fname == "track_user" || fname == "update_user_editing" || fname == "track_with_socket" || fname == "update_with_socket"
			|| fname == "untrack_with_socket";
	}
			...
			switch (p) {
				case PVar(n):
					if (keep.indexOf(n) != -1)
						...
						out.push(PVar((n != null && n.length > 0 && n.charAt(0) != '_') ? '_' + n : n));
				default:
					out.push(p);
			}
		return out;
