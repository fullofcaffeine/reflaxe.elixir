...
class UnderscoreVarUsageFixTransforms {
		...
		function collect(x:ElixirAST, inPattern:Bool):Void {
			...
			switch (x.def) {
				...
				case EBinary(_, l, r):
					...
					collect(r, false);
				case EMatch(p, rhs):
					collect(rhs, false); // pattern ignored
				case EBlock(ss):
					for (s in ss)
						collect(s, false);
				case EDo(ss2):
		...
		function rename(x:ElixirAST, inPattern:Bool):ElixirAST {
			...
			return switch (x.def) {
				...
				case EBlock(ss):
					var out = [];
					for (s in ss)
						out.push(rename(s, false));
					makeASTWithMeta(EBlock(out), x.metadata, x.pos);
				case EDo(ss2):
					var out2 = [];
					for (s in ss2)
						out2.push(rename(s, false));
				...
				case ECase(expr, clauses):
					var ncs = [];
					for (cl in clauses)
						ncs.push({pattern: cl.pattern, guard: cl.guard != null ? rename(cl.guard, false) : null, body: rename(cl.body, false)});
				...
				case EKeywordList(pairs):
					var ps = [];
					for (p in pairs)
						ps.push({key: p.key, value: rename(p.value, false)});
					makeASTWithMeta(EKeywordList(ps), x.metadata, x.pos);
				case EMap(pairs):
					var mp = [];
					for (p in pairs)
						mp.push({key: rename(p.key, false), value: rename(p.value, false)});
	...
	static function renamePatternBinders(args:Array<EPattern>, body:ElixirAST):{args:Array<EPattern>, body:ElixirAST} {
		...
		if (underArgs.length == 0)
			return {args: args, body: rewrite(body)};
		...
		for (u in underArgs) {
			if (!usedUnderscored.exists(u.name))
				continue;
