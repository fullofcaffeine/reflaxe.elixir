	...
	static function normalizeInBody(body:ElixirAST):ElixirAST {
							...
							if (canonicalQuery == null && func == "from") {
								switch (mod.def) {
									case EVar(nf) if (nf == "Ecto.Query"):
										switch (pattern) {
											case PVar(vf): canonicalQuery = vf;
											default:
										}
									default:
								}
							...
							switch (innerExpr.def) {
									...
									if (canonicalQuery == null && func3 == "to_query") {
										switch (mod3.def) {
											case EVar(n3) if (n3 == "Ecto.Queryable"):
												switch (innerPat) {
													case PVar(v3): canonicalQuery = v3;
													default:
												}
											default:
										}
									...
									if (canonicalQuery == null && func3 == "from") {
										switch (mod3.def) {
											case EVar(n4) if (n4 == "Ecto.Query"):
												switch (innerPat) {
													case PVar(v4): canonicalQuery = v4;
													default:
												}
											default:
										}
								...
								case ERaw(code2):
									if (canonicalQuery == null && code2.indexOf("Ecto.Query.from") != -1) {
										switch (innerPat) {
											case PVar(v5): canonicalQuery = v5;
											default:
										}
									}
								default:
		...
		function rewrite(n:ElixirAST):ElixirAST {
			...
			return switch (n.def) {
				...
				case ERemoteCall(queryModule, queryFunc, queryArgs)
					if ((queryFunc == "where" || queryFunc == "order_by" || queryFunc == "preload")
						&& queryArgs.length >= 1
						&& canonicalQuery != null):
