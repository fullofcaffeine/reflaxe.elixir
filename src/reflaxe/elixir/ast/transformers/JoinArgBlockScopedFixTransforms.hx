...
class JoinArgBlockScopedFixTransforms {
		...
		while (i < stmts.length) {
			...
			var repairedCurrent = ElixirASTTransformer.transformNode(current, function(m:ElixirAST):ElixirAST {
				return switch (m.def) {
					case ERemoteCall({def: EVar("Enum")}, "join", joinArgs) if (joinArgs != null && joinArgs.length == 2 && matchedWindow == null):
						...
						if (win == null)
							win = findBuilderWindow(stmts, i, null);
						if (win != null) {
							matchedWindow = win;
							sepArg = joinArgs[1];
			...
			if (matchedWindow != null) {
				...
				if (matchedWindow.isContiguous) {
					for (k in 0...matchedWindow.startIndex)
	...
	private static function findBuilderWindow(stmts:Array<ElixirAST>, joinIndex:Int, accName:Null<String>):Null<{
		startIndex:Int,
		endIndex:Int,
		listExpr:ElixirAST,
		binderName:String,
		...
		for (idx in startScan...joinIndex)
