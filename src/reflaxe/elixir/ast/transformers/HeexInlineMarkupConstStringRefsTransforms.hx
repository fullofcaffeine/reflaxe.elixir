...
class HeexInlineMarkupConstStringRefsTransforms {
            ...
            if (ch == "=") {
                ...
                if (cursor < content.length && content.charAt(cursor) == "{") {
                    ...
                    while (cursor < content.length && depth > 0) {
                        ...
                        if (!inS && cj == "\"" && !inD) { inD = true; cursor++; continue; }
                        else if (inD && cj == "\"") { inD = false; cursor++; continue; }
                        if (!inD && cj == "'" && !inS) { inS = true; cursor++; continue; }
                        else if (inS && cj == "'") { inS = false; cursor++; continue; }
                        if (inS || inD) { cursor++; continue; }
                    ...
                    if (trimmed.indexOf(".") != -1 && trimmed.indexOf("@") == -1) {
                        if (constRefToValue == null) constRefToValue = getConstRefToValueMap();
                    }
    ...
    static function getConstRefToValueMap(): Null<Map<String, String>> {
        if (constRefToValueCache != null) return constRefToValueCache;
    ...
    static function buildConstRefToValueMap(): Null<Map<String, String>> {
        ...
        if (discovered == null || discovered.length == 0) return map;

        function addRef(typePath: String, typeName: String, fieldName: String, value: String): Void {
            if (typeName != null && fieldName != null) map.set(typeName + "." + fieldName, value);
            if (typePath != null && fieldName != null) map.set(typePath + "." + fieldName, value);
        }
        ...
        function collectStatics(typePath: String, typeName: String, cls: Null<haxe.macro.Type.ClassType>): Void {
            ...
            for (field in cls.statics.get()) {
                ...
                if (value != null && value.length > 0) addRef(typePath, typeName, field.name, value);
            }
        }
        ...
        for (typePath in discovered) {
            ...
            switch (TypeTools.follow(t)) {
                case TAbstract(aRef, _):
                    var abs = aRef.get();
                    if (abs == null || abs.meta == null) continue;
                    if (!abs.meta.has(":phxHookNames") && !abs.meta.has(":phxEventNames")) continue;
                    collectStatics(typePath, typeName, abs.impl != null ? abs.impl.get() : null);
                case TInst(cRef, _):
                    var cls = cRef.get();
                    if (cls == null || cls.meta == null) continue;
                    if (!cls.meta.has(":phxHookNames") && !cls.meta.has(":phxEventNames")) continue;
