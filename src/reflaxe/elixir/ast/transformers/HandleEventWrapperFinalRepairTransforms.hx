...
class HandleEventWrapperFinalRepairTransforms {
	...
	static function repairBody(body:ElixirAST, paramsVar:String, socketVar:String):ElixirAST {
				...
				return switch (x.def) {
					case ECall(target, fname, args) if (args != null && args.length >= 2):
						var a0IsSocket = switch (args[0].def) {
							case EVar(v) if (v == socketVar): true;
							default: false;
						};
						var lastIsSocket = switch (args[args.length - 1].def) {
							case EVar(v2) if (v2 == socketVar): true;
							default: false;
						};
						if (a0IsSocket && lastIsSocket) {
							...
							var a0IsParams = switch (args[0].def) {
								case EVar(vp) if (vp == paramsVar): true;
								default: false;
							};
							if (a0IsParams) {
					...
					case ERemoteCall(mod, fname2, args2) if (args2 != null && args2.length >= 2):
						var a0IsSocket2 = switch (args2[0].def) {
							case EVar(v3) if (v3 == socketVar): true;
							default: false;
						};
						var lastIsSocket2 = switch (args2[args2.length - 1].def) {
							case EVar(v4) if (v4 == socketVar): true;
							default: false;
						};
						if (a0IsSocket2 && lastIsSocket2) {
							...
							var a0IsParams2 = switch (args2[0].def) {
								case EVar(vp2) if (vp2 == paramsVar): true;
								default: false;
							};
							if (a0IsParams2) {
		...
		for (u in used) {
			var snake = toSnake(u);
			if (!declared.exists(u) && !declared.exists(snake) && allow(u))
				missing.push(u);
		}
		if (missing.length == 0)
			return step2;
		var prefix = [
			for (v in missing)
				makeAST(EBinary(Match, makeAST(EVar(v)), buildExtract(v, paramsVar)))
		];
		...
		reflaxe.elixir.ast.ASTUtils.walk(body, function(n:ElixirAST) {
			...
			switch (n.def) {
				case EBinary(Match, {def: EVar(lhs)}, rhs):
					if (rhsLooksLikeMapGet(rhs, paramsVar))
						m.set(lhs, true);
				case EMatch(PVar(lhs2), rhs2):
					if (rhsLooksLikeMapGet(rhs2, paramsVar))
						m.set(lhs2, true);
				default:
			}
		});
	...
	static function isInternalVariable(name:String):Bool {
		...
		// Socket-related
		if (StringTools.endsWith(lower, "socket"))
			return true;
		// Selection/state-related
		if (StringTools.endsWith(lower, "selected"))
			return true;
		// Processed data
		if (StringTools.startsWith(lower, "refreshed"))
			return true;
		if (StringTools.startsWith(lower, "filtered"))
			return true;
		if (StringTools.startsWith(lower, "updated"))
			return true;
		if (StringTools.startsWith(lower, "new") && lower.length > 3)
			return true; // "newX" but not "new"
		// Result/temp variables
		if (StringTools.endsWith(lower, "result"))
			return true;
		if (StringTools.startsWith(lower, "temp"))
			return true;
		if (StringTools.startsWith(lower, "tmp"))
			return true;
		return false;
	}
	...
	static function collectPat(p:EPattern, out:Map<String, Bool>):Void {
		switch (p) {
			...
			case PMap(kvs):
				for (kv in kvs)
					collectPat(kv.value, out);
			case PStruct(_, fs):
				for (f in fs)
					collectPat(f.value, out);
			case PPin(inner):
