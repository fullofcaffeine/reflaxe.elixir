		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case ERemoteCall(mod, func, args)
					if ((func == "parse_with_converter" || func == "parseWithConverter")
						&& args != null
						&& args.length == 2
						&& isSafePubSub(mod)):
					var msgArg = args[0];
					...
					var captured = switch (parserArg.def) {
						...
						case EAtom(atomName):
							...
							if (fnName != null && fnName.length > 0) makeAST(ECapture(makeAST(ERemoteCall(makeAST(EVar("__MODULE__")), fnName, [])),
								1)) else parserArg;
				...
				case ECall(target, funcName, callArgs)
					if ((target == null)
						&& (funcName == "parse_with_converter" || funcName == "parseWithConverter")
						&& callArgs != null
						&& callArgs.length == 2):
					...
					var capturedParser = switch (parserArgLocal.def) {
						...
						case EAtom(atomValue):
							...
							if (functionName != null && functionName.length > 0) makeAST(ECapture(makeAST(ERemoteCall(makeAST(EVar("__MODULE__")),
								functionName, [])), 1)) else parserArgLocal;
						case EString(str) | ECharlist(str):
							var dotIndex2 = str.lastIndexOf(".");
							var functionName2 = dotIndex2 >= 0 ? str.substr(dotIndex2 + 1) : str;
							if (functionName2 != null && functionName2.length > 0) makeAST(ECapture(makeAST(ERemoteCall(makeAST(EVar("__MODULE__")),
								functionName2, [])), 1)) else parserArgLocal;
