			...
			while (i < code.length) {
				if (i + 5 < code.length && code.substr(i, 5) == " in :") {
					...
					if (raw.length > 0) {
						...
						for (p in parts)
							if (p.length > 0)
								pas.push(p.charAt(0).toUpperCase() + p.substr(1));
						out.add(" in ");
						out.add(app);
						out.add(".");
						out.add(pas.join(""));
	...
	public static function erawEctoQueryableToSchemaPass(ast:ElixirAST):ElixirAST {
		...
		return ElixirASTTransformer.transformNode(ast, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				case EModule(name, attrs, body):
					var app = name != null
						&& name.indexOf("Web") > 0 ? name.substring(0, name.indexOf("Web")) : reflaxe.elixir.PhoenixMapper.getAppModuleName();
					var newBody = [
						for (b in body)
