		...
		ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
			switch (x.def) {
				...
				case EBinary(Match, left, _):
					switch (left.def) {
						case EVar(nm): if (nm != null) declared.set(nm, true);
						default:
					}
			...
			return switch (x.def) {
				case EVar(v):
					if (rewriteOk && v == "ok_value")
						return makeASTWithMeta(EVar("value"), x.metadata, x.pos);
					if (rewriteG && v == "_g")
						return makeASTWithMeta(EVar("g"), x.metadata, x.pos);
					x;
				case ERaw(code) if (code != null):
					var out = code;
					if (rewriteOk && rawContainsIdent(out, "ok_value"))
						out = replaceIdent(out, "ok_value", "value");
					if (rewriteG && rawContainsIdent(out, "_g"))
						out = replaceIdent(out, "_g", "g");
