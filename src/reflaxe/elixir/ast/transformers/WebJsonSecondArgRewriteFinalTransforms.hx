...
class WebJsonSecondArgRewriteFinalTransforms {
		...
		return ElixirASTTransformer.transformNode(node, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
				...
				case ECase(expr, clauses):
					var cls = [];
					for (cl in clauses)
						cls.push({pattern: cl.pattern, guard: cl.guard, body: rewriteWithBinder(cl.body, binderOf(cl.pattern))});
	...
	static function isAliasName(arg:ElixirAST):Bool {
		return switch (arg.def) {
			case EVar(v) if (v == "data" || v == "json"): true;
			default: false;
		}
	}
