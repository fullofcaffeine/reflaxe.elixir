...
class StaticVarMutationRewriteTransforms {
			...
			return switch (node.def) {
				case EModule(moduleName, attrs, body):
					...
					if (accessors == null || accessors.keys().hasNext() == false) {
						...
						changed ? makeASTWithMeta(EModule(moduleName, attrs, rewrittenBody), node.metadata, node.pos) : node;
					}
				case EDefmodule(moduleName, doBlock):
					...
					if (accessors == null || accessors.keys().hasNext() == false) {
						...
						changed ? makeASTWithMeta(EDefmodule(moduleName, rebuildDoBlock(doBlock, rewrittenStatements)), node.metadata, node.pos) : node;
					}
				default:
					node;
			}
	...
	private static function extractStatementsFromDoBlock(doBlock:ElixirAST):Array<ElixirAST> {
		if (doBlock == null || doBlock.def == null)
			return [];
	...
	private static function rebuildDoBlock(original:ElixirAST, statements:Array<ElixirAST>):ElixirAST {
		if (original == null || original.def == null)
			return makeAST(EDo(statements));
	...
	static function rewriteStatementList(stmts:Array<ElixirAST>, moduleName:String, accessors:StringMap<Bool>):Array<ElixirAST> {
		...
		for (stmt in stmts) {
			...
			if (concatInfo != null) {
				...
				#end
				rewritten = makeASTWithMeta(ERemoteCall(concatInfo.moduleExpr, concatInfo.accessor, [concatInfo.concatExpr]), stmt.metadata, stmt.pos);
			}
			...
			if (rewritten == null) {
				...
				if (tmpUpdate != null && tempToAccessor.exists(tmpUpdate.tempName)) {
					...
					var setterCall = makeASTWithMeta(ERemoteCall(moduleExpr, accessorName, [makeAST(EVar(tmpUpdate.tempName))]), stmt.metadata, stmt.pos);
					out.push(tmpAssign);
					out.push(setterCall);
					continue;
				}
			...
			if (rewritten == null) {
				...
				if (tmpConcat != null && tempToAccessor.exists(tmpConcat.tempName)) {
					...
					var setterCall = makeASTWithMeta(ERemoteCall(moduleExpr, accessorName, [makeAST(EVar(tmpConcat.tempName))]), stmt.metadata, stmt.pos);
					out.push(tmpAssign);
					out.push(setterCall);
					continue;
				}
	...
	private static function findStaticAccessors(body:Array<ElixirAST>):StringMap<Bool> {
		...
		for (node in body) {
			switch (node.def) {
				case EDef(name, args, _guards, fnBody):
					if (args == null)
						continue;
					if (args.length == 0 && containsLocalCall(fnBody, "__haxe_static_get__"))
						getters.set(name, true);
					if (args.length == 1 && containsLocalCall(fnBody, "__haxe_static_put__"))
		...
		for (name in getters.keys()) {
			if (setters.exists(name))
				accessors.set(name, true);
		}
		return accessors;
	}
	...
	private static function isStaticGetterCall(expr:ElixirAST, moduleExpr:ElixirAST, accessors:StringMap<Bool>):Null<String> {
		...
		return switch (expr.def) {
			case ERemoteCall(mod, fn, args) if (args != null && args.length == 0 && sameModule(mod, moduleExpr) && accessors.exists(fn)):
				fn;
			case ECall(target, fn, callArgs)
				if (callArgs != null && callArgs.length == 0 && target != null && sameModule(target, moduleExpr) && accessors.exists(fn)):
				fn;
	...
	private static function extractStaticGetterConcat(stmt:ElixirAST, moduleExpr:ElixirAST,
			accessors:StringMap<Bool>):Null<{moduleExpr:ElixirAST, accessor:String, concatExpr:ElixirAST}> {
	...
	private static function extractTempInitFromStaticGetter(stmt:ElixirAST, moduleExpr:ElixirAST,
			accessors:StringMap<Bool>):Null<{tempName:String, accessor:String}> {
		if (stmt == null || stmt.def == null)
			return null;
