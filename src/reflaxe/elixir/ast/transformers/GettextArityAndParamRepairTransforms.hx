...
class GettextArityAndParamRepairTransforms {
	...
	static function insertArityShims(body:Array<ElixirAST>):Array<ElixirAST> {
		...
		var out = body.copy();
		inline function has(name:String, ar:Int):Bool {
			return sigs.exists(name) && sigs.get(name).exists(ar);
		}
		// gettext
		...
		if (has("dgettext", 3) && !has("dgettext", 2)) {
			out.unshift(makeAST(EDef("dgettext", [PVar("domain"), PVar("msgid")], null,
				makeAST(ECall(null, "dgettext", [makeAST(EVar("domain")), makeAST(EVar("msgid")), makeAST(EMap([]))])))));
		}
		// ngettext
		if (has("ngettext", 4) && !has("ngettext", 3)) {
			out.unshift(makeAST(EDef("ngettext", [PVar("msgid"), PVar("msgid_plural"), PVar("count")], null, makeAST(ECall(null, "ngettext", [
				makeAST(EVar("msgid")),
				makeAST(EVar("msgid_plural")),
				makeAST(EVar("count")),
				makeAST(EMap([]))
			])))));
		}
		...
		if (has("dngettext", 5) && !has("dngettext", 4)) {
			out.unshift(makeAST(EDef("dngettext", [PVar("domain"), PVar("msgid"), PVar("msgid_plural"), PVar("count")], null,
				makeAST(ECall(null, "dngettext", [
					makeAST(EVar("domain")),
					makeAST(EVar("msgid")),
					makeAST(EVar("msgid_plural")),
					makeAST(EVar("count")),
