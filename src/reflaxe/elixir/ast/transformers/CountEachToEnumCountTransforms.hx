...
class CountEachToEnumCountTransforms {
	...
	static inline function isEnumEachFn(mod:ElixirAST, func:String, args:Array<ElixirAST>):Bool {
		if (!isEnumEachCall(mod, func, args))
			return false;
		return switch (args[1].def) {
			case EFn(clauses) if (clauses.length == 1): true;
			default: false;
		};
	}
	...
	static function rewriteExternalCounterBlock(node:ElixirAST, stmts:Array<ElixirAST>):Null<ElixirAST> {
		...
		if (countVar == null) {
			...
			if (stmts.length <= 6)
				trace('[CountEachToEnumCount] skip: last expr not var');
			#end
			return null;
		}
		#if debug_count_each_to_count_ext
		...
		if (!isVarInitToZero(stmts[0], countVar)) {
			#if debug_count_each_to_count_ext
			if (debugCandidate)
				trace('[CountEachToEnumCount] skip: init not ' + countVar + ' = 0');
			#end
			return null;
		}
		...
		for (i in 1...(stmts.length - 1)) {
			...
			if (extracted != null) {
				if (each != null) {
					#if debug_count_each_to_count_ext
					if (debugCandidate)
						trace('[CountEachToEnumCount] skip: multiple Enum.each statements');
					#end
					return null;
				}
				each = extracted;
			...
			if (isIgnorableCounterStmt(stmt))
				continue;

			#if debug_count_each_to_count_ext
			if (debugCandidate) {
				var printed = ElixirASTPrinter.print(unwrapParen(stmt), 0);
				trace('[CountEachToEnumCount] skip: unexpected stmt in counter block: '
					+ printed
					+ ' def='
					+ Std.string(unwrapParen(stmt).def));
			}
			#end
			return null;
		}
		if (each == null) {
			#if debug_count_each_to_count_ext
			if (debugCandidate)
				trace('[CountEachToEnumCount] skip: no Enum.each statement found in counter block');
			#end
			return null;
		}
		...
		if (eachFn == null) {
			#if debug_count_each_to_count_ext
			if (debugCandidate)
				trace('[CountEachToEnumCount] skip: could not extract each fn');
			#end
			return null;
		}
		...
		if (countPredicate == null) {
			#if debug_count_each_to_count_ext
			if (debugCandidate)
				trace('[CountEachToEnumCount] skip: could not find count predicate for ' + countVar);
			#end
			return null;
		}
	...
	static function isVarInitToZero(stmt:ElixirAST, name:String):Bool {
		...
		if (s == null || s.def == null || name == null)
			return false;
		return switch (s.def) {
			case EBinary(Match, left, rhs):
				switch (left.def) {
					case EVar(v) if (v == name):
						switch (rhs.def) {
							case EInteger(i) if (i == 0): true;
							default: false;
						}
					default: false;
				}
			case EMatch(pattern, rhs):
				var lhs = switch (pattern) {
					case PVar(varName): varName;
					default: null;
				};
				if (lhs == name) switch (rhs.def) {
					case EInteger(intValue) if (intValue == 0): true;
					default: false;
				} else false;
			default:
				false;
	...
	static function flattenStatements(stmts:Array<ElixirAST>):Array<ElixirAST> {
		...
		for (s in stmts) {
			...
			if (unwrapped == null || unwrapped.def == null)
				continue;
	...
	static function findCountPredicate(fnBody:ElixirAST, countVar:String):Null<ElixirAST> {
		if (fnBody == null || fnBody.def == null)
			return null;
		var stmts:Array<ElixirAST> = switch (fnBody.def) {
			case EBlock(ss): ss;
			default: [fnBody];
		};
		for (s in stmts) {
			switch (s.def) {
				case EIf(cond, thenBr, _):
					if (thenContainsVarIncrement(thenBr, countVar))
						return cond;
				default:
			}
		}
		...
		function walk(x:ElixirAST):Void {
			...
			switch (x.def) {
				case EBinary(Match, left, rhs):
					...
					if (lhs == varName && isVarPlusOne(rhs))
						found = true;
					else {
						walk(left);
						walk(rhs);
				...
				case EMatch(pat, expr):
					var lhsName = switch (pat) {
						case PVar(nm): nm;
						default: null;
					};
					if (lhsName == varName && isVarPlusOne(expr))
						found = true;
					else
						walk(expr);
				case EBlock(ss):
					...
					if (lhs == binder) {
						switch (rhs.def) {
							case EBinary(Add, addLeft, addRight):
								var addVarName = switch (addLeft.def) {
									case EVar(n): n;
									default: null;
								};
								var addIsOne = switch (addRight.def) {
	...
	static function normalizeCond(cond:ElixirAST, binder:String):ElixirAST {
		...
		if (free.length == 1)
			return replaceVar(cond, free[0], binder);
		return cond;
	}
	...
	static function safeBinderName(b:String):String {
		if (b == null || b.length == 0)
			return "item";
		if (b.charAt(0) == '_')
			return b.substr(1) != null && b.substr(1).length > 0 ? b.substr(1) : "item";
		return b;
	}
}
#end
