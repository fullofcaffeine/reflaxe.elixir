								...
								if (j + 1 < stmts2.length) {
									switch (stmts2[j + 1].def) {
										case EIf(cond2, then2, else2):
											...
											if (elseIsB2) {
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(b2), s2.metadata, s2.pos), rhs2), s2.metadata,
													s2.pos));
												out2.push(makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(a2), s2.metadata, s2.pos),
