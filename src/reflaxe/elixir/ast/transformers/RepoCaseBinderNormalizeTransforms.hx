...
class RepoCaseBinderNormalizeTransforms {
	...
	static inline function isRepoDelete(e:ElixirAST):Bool {
		return switch (e.def) {
			case ERemoteCall(mod, func, _args) if (func == 'delete'):
				switch (mod.def) {
					case EVar(m) if (m != null && (StringTools.endsWith(m, '.Repo') || m == 'Repo')): true;
					default: false;
				}
			default: false;
		}
	...
	static function renameVarDeep(body:ElixirAST, fromName:String, toName:String):ElixirAST {
		if (fromName == toName || fromName == null || toName == null)
			return body;
		return ElixirASTTransformer.transformNode(body, function(n:ElixirAST):ElixirAST {
			return switch (n.def) {
	...
	static function collectUsedLowerVars(ast:ElixirAST):Array<String> {
		...
		function scan(n:ElixirAST):Void {
			...
			switch (n.def) {
				case EVar(name):
					if (name != null && name.length > 0 && name.charAt(0).toLowerCase() == name.charAt(0))
						names.set(name, true);
				case EField(t, _):
					scan(t);
				case EAccess(t, k):
