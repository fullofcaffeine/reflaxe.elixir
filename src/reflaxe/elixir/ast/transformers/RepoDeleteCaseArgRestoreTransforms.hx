							...
							while (j >= 0 && scans < 3) {
								switch (stmts[j].def) {
									case EBinary(Match, left, rhs):
										switch (left.def) {
											case EVar(nm) if (nm == matchVar): if (isRepoDelete(rhs)) isDeleteCase = true;
											default:
										}
									case EMatch(pat, rhs2):
										switch (pat) {
											case PVar(nm2) if (nm2 == matchVar): if (isRepoDelete(rhs2)) isDeleteCase = true;
											default:
										}
									default:
								}
			...
			return switch (node.def) {
				case ECall(target, funcName, args) if (args != null && args.length == 2):
					...
					var secondArgIsSocket = switch (secondArg.def) {
						case EVar(varName) if (varName == 'socket'): true;
						default: false;
					};
					var firstArgIsSuccessBinder = switch (firstArg.def) {
						case EVar(varName) if (successBinderSet.exists(varName)): true;
						default: false;
					};
					var firstArgIsNumericArtifact = switch (firstArg.def) {
						case EVar(varName2) if (isNumericArtifact(varName2)): true;
						default: false;
					};
				...
				case ERemoteCall(mod, funcName2, args2) if (args2 != null && args2.length == 2):
					...
					var remoteFirstIsNumericArtifact = switch (remoteFirstArg.def) {
						case EVar(varName) if (isNumericArtifact(varName)): true;
						default: false;
					};
								...
								if (isDeleteCase) {
									...
									for (cl in clauses) {
										...
										if (binder != null) {
											...
											if (assigned != null)
												newBody = rewriteNoreplyReturn(newBody, binder, assigned);
					...
					for (clause in caseClausesFallback) {
						...
						if (successBinderName != null) {
							...
							if (lastAssignedVar != null)
								rewrittenBody = rewriteNoreplyReturn(rewrittenBody, successBinderName, lastAssignedVar);
			...
			return switch (x.def) {
				case ECall(target, name, args) if (args != null && args.length == 2):
					...
					var isSocket2nd = switch (a1.def) {
						case EVar(nm) if (nm == "socket"): true;
						default: false;
					};
				...
				case ERemoteCall(mod, name2, args2) if (args2 != null && args2.length == 2):
					...
					var isSocketSecond = switch (b1.def) {
						case EVar(nm2) if (nm2 == "socket"): true;
						default: false;
					};
					var isBinderFirst = switch (b0.def) {
						case EVar(nm3) if (nm3 == binder): true;
						default: false;
					};
					#if debug_ast_transformer
					if (isSocketSecond) {
						var firstName2 = switch (b0.def) {
							case EVar(n00): n00;
							default: '<non-var>';
						};
					}
				...
				case EMap(pairs):
					...
					for (p in pairs) {
						var isNoreply = switch (p.key.def) {
							case EAtom(v) if (v == 'noreply'): true;
							default: false;
						};
						if (isNoreply) {
							var valueIsBinder = switch (p.value.def) {
								case EVar(nm) if (nm == fromBinder): true;
								default: false;
							};
							if (valueIsBinder) {
