									...
									var newRight = ElixirASTTransformer.transformNode(rhs, function(z:ElixirAST):ElixirAST {
										return switch (z.def) {
											case ERemoteCall(_, "concat", argsX) if (argsX.length == 2):
												makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), argsX[1]]), z.metadata,
													z.pos);
											case ECall(_, "concat", argsCX) if (argsCX.length == 2):
												makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), argsCX[1]]), z.metadata,
													z.pos);
								...
								if (lhsName != null && isSelfAppend(rhsExpr, lhsName)) {
									...
									var rewrittenRight = ElixirASTTransformer.transformNode(rhsExpr, function(node:ElixirAST):ElixirAST {
										return switch (node.def) {
											case ERemoteCall(_, "concat", args) if (args.length == 2):
												makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), args[1]]),
													node.metadata, node.pos);
											case ECall(_, "concat", argsCall) if (argsCall.length == 2):
												makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), argsCall[1]]),
													node.metadata, node.pos);
											case EBinary(Concat, _, right):
												makeASTWithMeta(ERemoteCall(makeAST(EVar("Enum")), "concat", [makeAST(EVar(accName)), right]), node.metadata,
													node.pos);
											default: node;
										}
									});
