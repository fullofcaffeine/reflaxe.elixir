					...
					switch (transformedBody.def) {
							...
							for (s in stmts)
								switch (s.def) {
									...
									case EBinary(Match, _, rhs):
										switch (rhs.def) {
											case ERemoteCall(_, fn2, _) if (fn2 == "start_link"): hasStartLink = true;
											default:
										}
									default:
								}
						...
						default:
							if (!hasStartLink && declaredChildren != null) {
								...
								finalBody = makeASTWithMeta(EBlock([
									transformedBody,
									makeAST(ERemoteCall(makeAST(EVar("Supervisor")), "start_link", [makeAST(EVar(declaredChildren)), defaultOpts2]))
								]), transformedBody.metadata, transformedBody.pos);
							} else finalBody = transformedBody;
					}
