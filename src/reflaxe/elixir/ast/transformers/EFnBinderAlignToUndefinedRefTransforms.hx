	...
	static function rewrite(n:ElixirAST, outerDeclared:Map<String, Bool>):ElixirAST {
		...
		return switch (n.def) {
					...
					ElixirASTTransformer.transformNode(rewrittenBody, function(x:ElixirAST):ElixirAST {
						return switch (x.def) {
							case EField({def: EVar(v)}, _) if (isLocalVarName(v) && !declared.exists(v)):
								...
								if (!outerDeclared.exists(v))
									fieldReceivers.set(v, true);
								x;
							default:
								x;
						};
			...
			case EMap(pairs):
				var np = [
					for (p in pairs)
						{key: rewrite(p.key, outerDeclared), value: rewrite(p.value, outerDeclared)}
				];
			...
			case EReceive(clauses, after):
				var nclauses = [
					for (c in clauses)
						{pattern: c.pattern, guard: c.guard, body: rewrite(c.body, outerDeclared)}
				];
			...
			case ETry(body, rescueClauses, catchClauses, afterBlock, elseBlock):
				...
				var nr = rescueClauses != null ? [
					for (c in rescueClauses)
						{pattern: c.pattern, varName: c.varName, body: rewrite(c.body, outerDeclared)}
				] : null;
				var nc = catchClauses != null ? [
					for (c in catchClauses)
						{kind: c.kind, pattern: c.pattern, body: rewrite(c.body, outerDeclared)}
				] : null;
