		...
		for (s in stmts) {
			switch (s.def) {
				case EBinary(Match, left, _):
					switch (left.def) {
						case EVar(n): defined.set(n, true);
						default:
					}
				case EMatch(pat, _):
					switch (pat) {
						case PVar(patternVarName): defined.set(patternVarName, true);
						default:
					}
				default:
			}
				...
				case EMatch(pat, _):
					switch (pat) {
						case PVar(underscoredName) if (underscoredName.length > 1 && underscoredName.charAt(0) == "_"):
							...
							if (!defined.exists(baseName)
								&& OptimizedVarUseAnalyzer.usedLater(useIndex, i + 1, underscoredName)) toPromote.set(underscoredName, baseName);
						default:
					}
				default:
			}
		}
		if (toPromote.keys().hasNext() == false)
			return stmts;
					...
					case EBinary(Match, left, rhs):
						switch (left.def) {
							case EVar(binderName) if (toPromote.exists(binderName)):
								makeASTWithMeta(EBinary(Match, makeASTWithMeta(EVar(toPromote.get(binderName)), left.metadata, left.pos), rhs), x.metadata,
									x.pos);
							default: x;
						}
