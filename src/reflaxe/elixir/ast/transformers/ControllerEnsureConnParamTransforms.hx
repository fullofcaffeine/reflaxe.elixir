		...
		return ElixirASTTransformer.transformNode(node, function(x:ElixirAST):ElixirAST {
			return switch (x.def) {
				case EDef(fname, args, guards, body):
					...
					inline function patStr(p:EPattern):String
						return switch (p) {
							case PVar(nm): nm;
							case PAlias(nm, _): nm;
							default: Std.string(p);
						};
					...
					if (args != null)
						for (a in args)
							switch (a) {
								case PVar(nm) if (nm == "conn"): hasConn = true;
								case PVar(nm2) if (nm2 == "_conn"): hadUnderscored = true;
								default:
							}
					if (hadUnderscored && !hasConn) {
						...
						for (a in args)
							switch (a) {
								case PVar(n) if (n == "_conn"): newArgs.push(PVar("conn"));
								case PAlias(nm, pat) if (nm == "_conn"): newArgs.push(PAlias("conn", pat));
				...
				case EDefp(fname2, args2, guards2, body2):
					...
					if (args2 != null)
						for (a2 in args2)
							switch (a2) {
								case PVar(nm2) if (nm2 == "conn"): hasConn2 = true;
								case PVar(nm3) if (nm3 == "_conn"): hadUnderscored2 = true;
								default:
							}
					if (hadUnderscored2 && !hasConn2) {
						...
						for (a3 in args2)
							switch (a3) {
								case PVar(nn) if (nn == "_conn"): newArgs3.push(PVar("conn"));
								case PAlias(nn2, pat2) if (nn2 == "_conn"): newArgs3.push(PAlias("conn", pat2));
								default: newArgs3.push(a3);
							}
