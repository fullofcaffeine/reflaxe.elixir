...
class HandleEventArg0FromParamsIdUltraFinalTransforms {
			...
			return switch (n.def) {
				...
				case EDefmodule(modName, doBlock):
					var stmts = switch (doBlock.def) {
						case EDo(s): s;
						case EBlock(blockStmts): blockStmts;
						default: [];
					};
	...
	static function isHandleEvent3(name:String, args:Array<EPattern>):Bool {
		return name == "handle_event" && args != null && args.length == 3 && switch (args[0]) {
			...
			return switch (x.def) {
				case ECall(target, fname, args) if (target == null && args != null && args.length >= 2):
					...
					var firstArgIsParams = switch (args[0].def) {
						case EVar(v) if (v == paramsVar): true;
						default: false;
					};
			...
			switch (s.def) {
				case EDef(n, a, _, _) | EDefp(n, a, _, _):
					...
					if (a != null && a.length > 0)
						switch (a[0]) {
							case PVar(vn): firstName = vn;
							default:
						}
					var byAr = m.get(n);
					if (byAr == null)
						byAr = new Map<Int, Null<String>>();
	...
	static function expectedIdVarName(fname:String, arity:Int, sigs:Map<String, Map<Int, Null<String>>>):Null<String> {
		...
		if (first == null)
			return null;
		if (first == "params" || first == "_params")
			return null;
		var normalized = first;
		if (StringTools.startsWith(normalized, "_") && normalized.length > 1)
	...
	static inline function chooseFirstArgReplacement(paramsVar:String, expectedVar:String, declared:Map<String, Bool>):ElixirAST {
		if (expectedVar != null && declared.exists(expectedVar))
			return makeAST(EVar(expectedVar));
		var key = NameUtils.toSnakeCase(expectedVar);
		if (declared.exists(key))
			return makeAST(EVar(key));
		if (declared.exists('_' + key))
			return makeAST(EVar('_' + key));
