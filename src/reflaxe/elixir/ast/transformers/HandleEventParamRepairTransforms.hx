...
class HandleEventParamRepairTransforms {
			...
			return switch (node.def) {
				...
				case EDef(nameAny, argsAny, _, _):
					...
					if (nameAny == "handle_event") {
						...
						if (argsAny != null)
							for (a in argsAny)
								kinds.push(reflaxe.elixir.util.EnumReflection.enumConstructor(a));
					}
					#end
					node;
				...
				case EDefp(nameAny2, argsAny2, _, _):
					...
					if (nameAny2 == "handle_event") {
						...
						if (argsAny2 != null)
							for (a2 in argsAny2)
								kinds2.push(reflaxe.elixir.util.EnumReflection.enumConstructor(a2));
					}
					#end
					node;
				default:
	...
	static inline function allow(name:String):Bool {
		if (name == null || name.length == 0)
			return false;
		if (name == "socket" || name == "params" || name == "event" || name == "live_socket")
			return false;
	...
	static function collectPattern(p:EPattern, out:Map<String, Bool>):Void {
		switch (p) {
			...
			case PTuple(es) | PList(es):
				for (e in es)
					collectPattern(e, out);
			case PCons(h, t):
				collectPattern(h, out);
				collectPattern(t, out);
			case PMap(kvs):
				for (kv in kvs)
					collectPattern(kv.value, out);
			case PStruct(_, fs):
				for (f in fs)
					collectPattern(f.value, out);
			case PPin(inner):
