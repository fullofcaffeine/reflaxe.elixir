...
class CaseOkBinderAlignTransforms {
	...
	static function alignInBody(body:ElixirAST, funcArgs:Map<String, Bool>):ElixirAST {
		...
		for (k in funcArgs.keys())
			initialDeclared.set(k, true);
		...
		return switch (node.def) {
			...
			case EFn(clauses):
				...
				for (cl in clauses) {
					...
					for (a in cl.args)
						collectPatternDeclsLower(a, fnScope);
			...
			case ECase(expr, clauses):
				...
				for (c in clauses)
					outClauses.push(alignClause(c, funcArgs, declaredBefore));
	...
	static function chooseDesiredName(body:ElixirAST, current:String, funcArgs:Map<String, Bool>, declaredBefore:Map<String, Bool>):Null<String> {
		...
		for (k in declaredBefore.keys())
			declaredInBody.set(k, true);
		...
		inline function considerCandidate(v:String):Void {
			...
			if (seen.exists(vlow))
				return;
			if (!allowLocalCandidate(vlow, currentLow, funcArgs))
				return;
			if (declaredInBody.exists(vlow))
				return;
			seen.set(vlow, true);
			undefined.push(vlow);
			if (!originalByLower.exists(vlow))
				originalByLower.set(vlow, v);
		}
	...
	static function collectDeclaredLocals(body:ElixirAST):Map<String, Bool> {
		...
		function collectFromPattern(p:EPattern):Void {
			switch (p) {
				case PVar(nm) if (nm != null):
					declared.set(nm.toLowerCase(), true);
				case PTuple(es) | PList(es):
					for (e in es)
						collectFromPattern(e);
				case PCons(h, t):
					collectFromPattern(h);
					collectFromPattern(t);
				case PMap(kvs):
					for (kv in kvs)
						collectFromPattern(kv.value);
				case PStruct(_, fs):
					for (f in fs)
						collectFromPattern(f.value);
				case PPin(inner):
					collectFromPattern(inner);
				default:
			}
		}
		ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
			switch (x.def) {
				case EMatch(p, _):
					collectFromPattern(p);
				case EBinary(Match, left, _):
					switch (left.def) {
						case EVar(nm): if (nm != null) declared.set(nm.toLowerCase(), true);
						default:
					}
				case ECase(_, cs):
					for (c in cs)
						collectFromPattern(c.pattern);
				case EFn(clauses):
					for (cl in clauses)
						for (a in cl.args)
							collectFromPattern(a);
				case EReceive(clauses, _):
					for (cl in clauses)
						collectFromPattern(cl.pattern);
				default:
			}
			return x;
		});
	...
	static inline function isIdentChar(c:String):Bool {
		...
		return (cc >= 'a'.code && cc <= 'z'.code) || (cc >= 'A'.code && cc <= 'Z'.code) || (cc >= '0'.code && cc <= '9'.code) || cc == '_'.code;
	}
	...
	static function collectPatternDeclsLower(p:EPattern, out:Map<String, Bool>):Void {
		switch (p) {
			...
			case PTuple(es) | PList(es):
				for (e in es)
					collectPatternDeclsLower(e, out);
			case PCons(h, t):
				collectPatternDeclsLower(h, out);
				collectPatternDeclsLower(t, out);
			case PMap(kvs):
				for (kv in kvs)
					collectPatternDeclsLower(kv.value, out);
			case PStruct(_, fs):
				for (f in fs)
					collectPatternDeclsLower(f.value, out);
			case PPin(inner):
	...
	static function collectDeclaredFromStatement(stmt:ElixirAST, out:Map<String, Bool>):Void {
		...
		switch (stmt.def) {
			...
			case EBinary(Match, left, _):
				switch (left.def) {
					case EVar(nm) if (nm != null): out.set(nm.toLowerCase(), true);
					default:
				}
			default:
		}
