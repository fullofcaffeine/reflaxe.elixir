...
class MapAndCollectionTransforms {
	...
	static function predicateUsesBinder(pred:ElixirAST, binderName:String):Bool {
		if (pred == null || pred.def == null || binderName == null || binderName.length == 0)
			return false;
		if (bodyUsesVar(pred, binderName))
			return true;
		var base = safeBinder(binderName);
		if (base != binderName && bodyUsesVar(pred, base))
			return true;
		if (base != null && base.length > 0 && bodyUsesVar(pred, "_" + base))
			return true;
		return false;
	}
				...
				case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args)):
					...
					switch (fnNode.def) {
						case EFn(clauses) if (clauses.length == 1):
							...
							// Some lowering paths wrap the actual statement list in a singleton EBlock/EDo.
							// Unwrap repeatedly so integrity checks operate on real statements.
							function unwrapSingleton(sts:Array<ElixirAST>):Array<ElixirAST> {
								var cur = sts;
							...
							for (s in stmts) {
								...
								switch (s1.def) {
									case EBinary(Match, l, r):
										...
										var rIsBinder = switch (r.def) {
											case EVar(nm2) if (nm2 == replacementName): true;
											default: false;
										};
									...
									case EMatch(patX, r2):
										...
										var rIsBinder2 = switch (r2.def) {
											case EVar(nm4) if (nm4 == replacementName): true;
											default: false;
										};
								...
								if (droppedAlias != null)
									s1 = replaceVarInExpr(s1, droppedAlias, replacementName);
								newStmts.push(s1);
							}
							...
							function collectUsedVars(n:ElixirAST, out:Map<String, Bool>):Void {
								...
								switch (n.def) {
									...
									case EList(els):
										for (el in els)
											collectUsedVars(el, out);
									case ETuple(els2):
										for (el in els2)
											collectUsedVars(el, out);
									case EKeywordList(ps):
										for (p in ps)
											collectUsedVars(p.value, out);
									case EMap(pairs):
										for (p in pairs) {
											collectUsedVars(p.key, out);
											collectUsedVars(p.value, out);
										}
									case EStructUpdate(base, fs):
										collectUsedVars(base, out);
										for (f in fs)
											collectUsedVars(f.value, out);
									case EStruct(_, fs2):
										for (f2 in fs2)
											collectUsedVars(f2.value, out);
									case EWith(clauses, doBlock, elseBlock):
										for (wc in clauses)
											collectUsedVars(wc.expr, out);
										collectUsedVars(doBlock, out);
										if (elseBlock != null)
											collectUsedVars(elseBlock, out);
									case EFn(_):
							...
							function collectBoundVars(stmts2:Array<ElixirAST>):Map<String, Bool> {
								...
								if (binderName != null)
									m.set(safeBinder(binderName), true);
								function walkStmt(sSt:ElixirAST):Void {
									if (sSt == null || sSt.def == null)
										return;
									switch (sSt.def) {
										case EBinary(Match, lft, _):
											switch (lft.def) {
												case EVar(nv): m.set(nv, true);
												default:
											}
										case EMatch(patY, _):
										...
										case ECase(_, clauses):
											for (cl in clauses)
												collectPatternVars(cl.pattern, m);
								...
								ElixirASTTransformer.transformNode(n, function(x:ElixirAST):ElixirAST {
									...
									switch (x.def) {
										...
										case ECall(_, _, argsC):
											for (a in argsC)
												switch (a.def) {
													case EVar(v2) if (v2 == name): found = true;
													default:
												}
											return x;
										case ERemoteCall(tgt2, _, argsR):
											switch (tgt2.def) {
												case EVar(v3) if (v3 == name): found = true;
												default:
											}
											for (a in argsR)
												switch (a.def) {
													case EVar(v4) if (v4 == name): found = true;
													default:
												}
											return x;
										case EString(str):
											if (str != null
												&& (str.indexOf("#{" + name + "}") != -1 || str.indexOf("#{" + name + ".") != -1))
												found = true;
											return x;
										default:
											return x;
							...
							var finalBinder:EPattern = (didReplace
								|| bodyUsesVar(bodyExpr, replacementName)
								|| bodyUsesVar(bodyExpr, "_" + replacementName)) ? PVar(replacementName) : PWildcard;
							...
							if (args.length >= 2) {
								...
								if (a0 != null && a0.length > 0) {
									...
									if (a1 != null && a1.length > 0)
										body2 = replaceVarInExpr(body2, "_" + a1, a1);
				...
				case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args)):
					...
					switch (fnNode.def) {
						case EFn(clauses) if (clauses.length == 1):
													...
													switch (right.def) {
														case EAccess(tgt, key) if (switch (tgt.def) {
																case ERemoteCall({def: EVar(mod)}, fn, args)
																	if (mod == "Reflect" && fn == "fields" && args != null && args.length == 1
																		&& astEquals(args[0], listExpr)): true;
																default: false;
															}):
										...
										switch (pattern) {
											case PVar(aliasVar):
												if (isHeadAccessOf(rhs, listExpr)) {
													...
													switch (rhs.def) {
														case EAccess(accessTarget, accessKey) if (switch (accessTarget.def) {
																case ERemoteCall({def: EVar(moduleName)}, functionName, callArgs)
																	if (moduleName == "Reflect" && functionName == "fields" && callArgs != null
																		&& callArgs.length == 1 && astEquals(callArgs[0], listExpr)): true;
																default: false;
															}):
															promoteFieldsList = true;
														default:
							...
							// Prefer keeping the original alias name to preserve semantics.
							if (binderName == null && removedAlias != null) {
								binderName = (removedAlias.length > 0 && removedAlias.charAt(0) != '_') ? removedAlias : "elem";
							}
							if (binderName == null)
								return node;
							...
							for (s in newStmts) {
								...
								if (!keep)
									continue;
								var s2 = if (removedAlias != null && removedAlias != binderName) replaceVarInExpr(s, removedAlias, binderName) else s;
								rewritten.push(s2);
							}
							var newBody = (rewritten.length == 1) ? rewritten[0] : makeAST(EBlock(rewritten));
							// Choose wildcard binder if unused to avoid warnings.
							// IMPORTANT: Usage detection must see vars under ECase/blocks/etc; use bodyUsesVar.
							var finalBinder:EPattern = bodyUsesVar(newBody, binderName) ? PVar(binderName) : PWildcard;
							...
							if (promoteFieldsList) {
								...
								var fn2 = makeAST(EFn([
									{args: [PVar(fieldBinder)], guard: clause.guard, body: replaceVarInExpr(newBody, binderName, fieldBinder)}
								]));
				...
				case EBlock(stmts) if (stmts.length >= 3):
					...
					if (stmts.length == 3) {
						...
						switch (stmts[0].def) {
							case EBinary(Match, leftX0, rhsX0):
								switch (leftX0.def) {
									case EVar(nm0): cvar = (switch (rhsX0.def) {
											case EInteger(v0) if (v0 == 0): nm0;
											default: null;
										});
									default:
								}
							case EMatch(pat0, rhs0):
								switch (pat0) {
									case PVar(nm0b): cvar = (switch (rhs0.def) {
											case EInteger(v0b) if (v0b == 0): nm0b;
											default: null;
										});
									default:
								}
						...
						switch (stmts[1].def) {
							...
							case EMatch(_, rhsM0): switch (rhsM0.def) {
									case ERemoteCall(mod1, func1, args1) if (isEnumEach(mod1, func1, args1) && args1.length == 2): eachExpr0 = rhsM0;
									default:
								}
							case EBinary(Match, _, rhsB0): switch (rhsB0.def) {
									case ERemoteCall(mod2, func2, args2) if (isEnumEach(mod2, func2, args2) && args2.length == 2): eachExpr0 = rhsB0;
									default:
								}
							default:
								...
								case ERemoteCall(_m0, _f0, argsE):
									...
									switch (argsE[1].def) {
										case EFn(clauses0) if (clauses0.length == 1):
											...
											for (bs0 in body0)
												switch (bs0.def) {
													case EBinary(Match, lA0, rA0): switch (lA0.def) {
															case EVar(an0): if (isHeadAccessOf(rA0, listExpr0)) alias0 = an0;
															default:
														}
													case EMatch(patA0, rA1): switch (patA0) {
															case PVar(an1): if (isHeadAccessOf(rA1, listExpr0)) alias0 = an1;
															default:
														}
											...
											if (alias0 != null && pred0 != null) {
												// Only rewrite the head-alias when the predicate doesn't already
												// reference the binder; otherwise we can clobber legitimate outer
												// vars (e.g. `id`) that happen to share the alias name.
												var predUsesBinder0 = predicateUsesBinder(pred0, binder0);
												var predUsesAlias0 = bodyUsesVar(pred0, alias0);
												if (!predUsesBinder0 && predUsesAlias0)
						...
						var returns0 = switch (stmts[2].def) {
							case EVar(nr0) if (cvar != null && nr0 == cvar): true;
							default: false;
						};
						...
						switch (stmts[i].def) {
							case EBinary(Match, left0, rhs):
								switch (left0.def) {
									case EVar(name):
										switch (rhs.def) {
											case EInteger(v) if (v == 0): countVar = name;
											default:
										}
									default:
								}
					...
					for (i in 0...stmts.length) {
						...
						switch (stmts[i].def) {
							...
							case EMatch(_, rhs):
								switch (rhs.def) {
									case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args) && args.length == 2): eachCall = rhs;
									default:
								}
							case EBinary(Match, _, rhs2):
								switch (rhs2.def) {
									case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args) && args.length == 2): eachCall = rhs2;
									default:
								}
							default:
						}
												...
												switch (bs.def) {
													case EBinary(Match, leftAlias, rightAlias):
														...
														switch (leftAlias.def) {
															case EVar(an): if (isHeadAccessOf(rightAlias, listExpr)) aliasFromHead = an;
															default:
														}
													case EMatch(patAlias, rightAlias2):
														switch (patAlias) {
															case PVar(an2): if (isHeadAccessOf(rightAlias2, listExpr)) aliasFromHead = an2;
															default:
														}
													case EIf(cond, thenBr, _):
														var inc = false;
														switch (thenBr.def) {
															case EBlock(ts): for (t in ts)
																	if (isCountIncrement(t, countVar)) {
																		inc = true;
																		break;
																	}
														...
														if (inc) {
															...
															if (aliasFromHead != null) {
																var predUsesBinder = predicateUsesBinder(cond, binderName);
																var predUsesAlias = bodyUsesVar(cond, aliasFromHead);
																if (!predUsesBinder && predUsesAlias)
																	adjusted = replaceVarInExpr(cond, aliasFromHead, binderName);
															}
																	...
																	case EBinary(Equal, l2, r2):
																		switch (l2.def) {
																			case EField(objL, fieldL):
																				switch (objL.def) {
																					case EVar(_): adjusted = makeAST(EBinary(Equal,
																							makeAST(EField(makeAST(EVar(binderName)), fieldL)), r2));
																					default:
																				}
																			default:
																		}
																		switch (r2.def) {
																			case EField(objR, fieldR):
																				switch (objR.def) {
																					case EVar(_): adjusted = makeAST(EBinary(Equal, l2,
																							makeAST(EField(makeAST(EVar(binderName)), fieldR))));
																					default:
																				}
																			default:
																		}
					...
					var returnsCount = switch (lastExpr.def) {
						case EVar(n) if (n == countVar): true;
						default: false;
					};
					if (!returnsCount)
						return node;
												...
												case EBinary(Match, leftX, rhsX):
													...
													if (lhsName == temp) switch (rhsX.def) {
														case ERemoteCall({def: EVar(mn)}, "concat", cargs) if (mn == "Enum" && cargs.length == 2):
															switch (cargs[0].def) {
																case EVar(nv) if (nv == temp): switch (cargs[1].def) {
				...
				case EBlock(stmts) if (stmts.length >= 3):
						...
						switch (bs.def) {
							case EBinary(Match, left, right):
								...
								switch (right.def) {
									case EAccess(tgt, _):
										switch (tgt.def) {
											case ERemoteCall({def: EVar("Reflect")}, "fields", [arg])
												if (ElixirASTPrinter.print(arg, 0) == ElixirASTPrinter.print(listExpr, 0)):
												drop = true;
												presenceShape = true;
											default:
										}
								...
								if (!drop)
									switch (left.def) {
										case EVar(aliasName):
											...
											switch (right.def) {
												case ERemoteCall({def: EVar("Map")}, "get", [m, _key])
													if (ElixirASTPrinter.print(m, 0) == ElixirASTPrinter.print(listExpr, 0)):
								...
								switch (rhs.def) {
									case EAccess(accessTarget, _):
										switch (accessTarget.def) {
											case ERemoteCall({def: EVar("Reflect")}, "fields", [reflectFieldsArg])
												if (ElixirASTPrinter.print(reflectFieldsArg, 0) == ElixirASTPrinter.print(listExpr, 0)):
												drop = true;
												presenceShape = true;
											default:
										}
								...
								if (!drop)
									switch (pattern) {
										case PVar(entryAliasName):
											switch (rhs.def) {
												case ERemoteCall({def: EVar("Map")}, "get", [mapExpr, _unusedKey])
													if (ElixirASTPrinter.print(mapExpr, 0) == ElixirASTPrinter.print(listExpr, 0)):
													localEntryAlias = entryAliasName;
													drop = true;
													presenceShape = true;
												default:
											}
					...
					if (presenceShape && presenceUsesMetas) {
										...
										case EVar(v) if (v == name):
											switch (rhs.def) {
												case ERemoteCall({def: EVar("Enum")}, "concat", [a0, _]):
													switch (a0.def) {
														case EVar(v0) if (v0 == name): true;
														default: false;
													}
												case EBinary(Concat, llist, _):
													switch (llist.def) {
														case EVar(v1) if (v1 == name): true;
														default: false;
													}
												default: false;
											}
						...
						if (localEntryAlias != null)
							cond = replaceVarInExpr(cond, localEntryAlias, binderSafe);
									...
									switch (leftX.def) {
										case EVar(mn):
											switch (rightX.def) {
												case EAccess(arrX, _):
													switch (arrX.def) {
														case EField(_, fieldX) if (fieldX == "metas"): metaAlias = mn;
														default:
													}
												default:
											}
						...
						if (metaAlias != null)
							cond = replaceVarWithExpr(cond, metaAlias, metaExpr);

						var appendMeta = makeAST(EBinary(Concat, makeAST(EVar("acc")), makeAST(EList([metaExpr]))));
						var outerCond = makeAST(EBinary(Greater,
							makeAST(ERemoteCall(makeAST(EVar("Kernel")), "length", [makeAST(EField(makeAST(EVar(binderSafe)), "metas"))])),
							makeAST(EInteger(0))));
						var inner = (cond != null) ? makeAST(EIf(cond, appendMeta, makeAST(EVar("acc")))) : appendMeta;
						var reducer = makeAST(EFn([
							{args: [PVar(binderSafe), PVar("acc")], guard: clause.guard, body: makeAST(EIf(outerCond, inner, makeAST(EVar("acc"))))}
						]));
					...
					for (cs in cleanedStmts) {
						var cs2 = cs;
						if (localEntryAlias != null)
							cs2 = replaceVarInExpr(cs2, localEntryAlias, binder);
									...
									case EVar(lhs) if (lhs == temp):
										switch (rhs.def) {
											case ERemoteCall({def: EVar("Enum")}, "concat", [a0, a1]):
												switch (a0.def) {
													case EVar(v0) if (v0 == temp):
														switch (a1.def) {
															case EList(items) if (items.length == 1): makeAST(EBinary(Concat, makeAST(EVar("acc")),
																	makeAST(EList([items[0]]))));
															default: null;
														}
													default: null;
												}
											case EBinary(Concat, llist, rlist):
												switch (llist.def) {
													case EVar(v1) if (v1 == temp): makeAST(EBinary(Concat, makeAST(EVar("acc")), rlist));
													default: null;
												}
											default: null;
										}
					...
					if (localEntryAlias != null)
						reducerBody = replaceVarInExpr(reducerBody, localEntryAlias, binder);

					var reducer = makeAST(EFn([
						{args: [PVar(safeBinder(binder)), PVar("acc")], guard: clause.guard, body: reducerBody}
					]));
							...
							if (cl.args.length == 1) {
								switch (cl.args[0]) {
									case PVar(name) if (name != null && name.length > 0):
										...
										if (name != trimmed)
											newBody = replaceVarInExpr(newBody, name, trimmed);
										#if debug_hygiene
										#end
				...
				case EBlock(stmts) if (stmts.length >= 2):
					...
					for (i in 0...stmts.length - 1)
						switch (stmts[i].def) {
							...
							case EMatch(_, rhs):
								switch (rhs.def) {
									case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args) && args.length == 2): eachStmt = rhs;
									default:
								}
							case EBinary(Match, _, rhs2):
								switch (rhs2.def) {
									case ERemoteCall(mod, func, args) if (isEnumEach(mod, func, args) && args.length == 2): eachStmt = rhs2;
									default:
								}
							default:
						}
					...
					if (listExpr == null || condExpr == null)
						return node;
					// Predicate: use the original condition as-is.
					// NOTE: Any `list[0]`/head-alias artifacts should be resolved earlier by the Enum.each
					// binder integrity/head-extraction passes, not by rewriting other locals here.
					var pred:ElixirAST = condExpr;
						...
						case EVar(modName) if (modName == "Enum" && funcName == "reduce_while" && args != null && args.length >= 3):
											...
											if (target != null) {
												switch (target.def) {
													case EField(_, field):
														#if debug_map_iterator
														#end
														if (field == "key_value_iterator" || field == "has_next" || field == "next" || field == "key"
															|| field == "value") {
															#if debug_map_iterator
															#end
															found = true;
							...
							if (hasMapIteratorCalls(loopFunc)) {
									...
									case EFn(clauses) if (clauses.length > 0):
										...
										switch (body.def) {
											case EIf(_, thenBranch, _):
												...
												loopBody = if (bodyExprs.length == 1) bodyExprs[0] else if (bodyExprs.length > 1)
													makeAST(EBlock(bodyExprs)) else null;
											default:
										}
									default:
								}
								...
								if (loopBody != null) {
									...
									if (keyVarName == null || keyVarName == "")
										return node;
									if (valueVarName == null || valueVarName == "")
										valueVarName = "_value";
									#if debug_map_iterator
									#end
									var transformedAST = makeAST(ERemoteCall(makeAST(EVar("Enum")), "each", [
										makeAST(EVar(mapVar)),
										makeAST(EFn([
											{
	...
	private static function containsIteratorPatterns(ast:ElixirAST):Bool {
		...
		#if debug_map_iterator
		if (result)
			trace('[MapIteratorTransform/scan] ✅ PATTERN DETECTED - hasKeyValueIterator: '
				+ hasKeyValueIterator
				+ ', hasHasNext: '
				+ hasHasNext
				+ ', hasNext: '
