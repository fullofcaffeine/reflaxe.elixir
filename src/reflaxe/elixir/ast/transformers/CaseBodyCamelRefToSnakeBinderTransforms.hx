...
class CaseBodyCamelRefToSnakeBinderTransforms {
				...
				case EVar(v):
					...
					if (isCamel) {
						...
						if (binderBases.exists(s)) {
							...
							if (underscoredBases.exists(s))
								usedUnderscoredBases.set(s, true);
			...
			case PVar(n):
				if (n != null && n.length > 0) {
					...
					if (base != null && base.length > 0) {
						bases.set(base, true);
						if (isUnderscored)
							underscoredBases.set(base, true);
					}
				}
			case PTuple(ps) | PList(ps):
				for (q in ps)
					collectBinders(q, bases, underscoredBases);
			case PCons(h, t):
				collectBinders(h, bases, underscoredBases);
				collectBinders(t, bases, underscoredBases);
			case PMap(kvs):
				for (kv in kvs)
					collectBinders(kv.value, bases, underscoredBases);
			case PStruct(_, fs):
				for (f in fs)
					collectBinders(f.value, bases, underscoredBases);
			case PPin(inner):
				collectBinders(inner, bases, underscoredBases);
			default:
		}
	}
	...
	static function rewriteUnderscoredBindersUsedInBody(p:EPattern, usedBases:Map<String, Bool>):EPattern {
		return switch (p) {
			...
			case PMap(kvs): PMap([
					for (kv in kvs)
						{key: kv.key, value: rewriteUnderscoredBindersUsedInBody(kv.value, usedBases)}
				]);
			case PStruct(name, fs): PStruct(name, [
					for (f in fs)
						{key: f.key, value: rewriteUnderscoredBindersUsedInBody(f.value, usedBases)}
				]);
