...
class BareGetterRepoGetRepairTransforms {
			...
			return switch (n.def) {
				...
				case EDefmodule(name, doBlock):
					...
					if (repoMod2 == null) {
						...
						if (derived2 != null)
							repoMod2 = makeAST(EVar(derived2));
					}
					if (repoMod2 == null)
						return n;
	...
	static function detectRepoMod(stmts:Array<ElixirAST>):Null<ElixirAST> {
		...
		function scan(x:ElixirAST):Void {
			if (found != null || x == null || x.def == null)
				return;
			switch (x.def) {
	...
	static function rewriteBody(body:ElixirAST, params:Array<EPattern>, repoMod:ElixirAST):ElixirAST {
		...
		if (isParamName(params, bareVar))
			return body;
		// If the bare var is declared inside the body, not our case either.
		if (declaresVar(body, bareVar))
			return body;
		var firstParamName:String = deriveFirstParamName(params);
		if (firstParamName == null)
			firstParamName = "id";
		var call = makeASTWithMeta(ERemoteCall(repoMod, "get", [makeAST(EAtom(bareVar.toLowerCase())), makeAST(EVar(firstParamName))]), body.metadata,
			body.pos);
		return call;
	}
