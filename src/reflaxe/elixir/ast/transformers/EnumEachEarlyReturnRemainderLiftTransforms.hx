...
class EnumEachEarlyReturnRemainderLiftTransforms {
	...
	static function extractFromContainer(original:ElixirAST, stmts:Array<ElixirAST>, wrap:Array<ElixirAST>->ElixirAST):Null<EnumEachEarlyReturnTerminalCase> {
	...
	static function findLastMeaningfulIndex(stmts:Array<ElixirAST>):Int {
		...
		while (idx >= 0) {
			var s = stmts[idx];
			if (s != null && s.def != null && !isBareNumericSentinel(s))
				return idx;
			idx--;
		}
		return -1;
		...
		return switch (core.def) {
			case ECase(_scrut, clauses) if (clauses != null && clauses.length > 0):
				...
				for (clause in clauses) {
					if (patternHasReturnTag(clause.pattern))
						return true;
				}
				false;
			default:
