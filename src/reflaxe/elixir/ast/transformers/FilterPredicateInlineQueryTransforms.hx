    ...
    public static function transformPass(ast: ElixirAST): ElixirAST {
        ...
        function replaceInBody(body: ElixirAST): ElixirAST {
            return ElixirASTTransformer.transformNode(body, function(x: ElixirAST): ElixirAST {
                return switch (x.def) {
                    case EVar(nm) if (nm == "query"): #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] inline query -> String.downcase(search_query)'); #end repl;
                    case ERaw(code) if (code != null && rawContainsIdent(code, "query")):
                        var newCode = replaceIdent(code, "query", "String.downcase(search_query)");
                        #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] inline query in ERaw predicate'); #end
        ...
        return ElixirASTTransformer.transformNode(ast, function(n: ElixirAST): ElixirAST {
            return switch (n.def) {
                case ERaw(code) if (code != null && code.indexOf('Enum.filter(') != -1 && rawContainsIdent(code, 'query')):
                    var newCode = replaceIdent(code, 'query', 'String.downcase(search_query)');
                    #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] Inlined query inside ERaw Enum.filter statement'); #end
                    makeAST(ERaw(newCode));
                case ERemoteCall(mod, "filter", args) if (args != null && args.length == 2):
                    #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] Considering Enum.filter(remote)'); #end
                    switch (args[1].def) {
                        case EFn(cs) if (cs.length == 1):
                            ...
                            #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] Inlined query in Enum.filter EFn'); #end
                ...
                case ECall(tgt, "filter", args2) if (args2 != null && args2.length >= 1):
                    #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] Considering Enum.filter(call)'); #end
                    ...
                    switch (predArg.def) {
                        case EFn(cs2) if (cs2.length == 1):
                            ...
                            #if (sys && debug_ast_transformer) Sys.println('[FilterPredicateInlineQuery] Inlined query in call.filter EFn'); #end
