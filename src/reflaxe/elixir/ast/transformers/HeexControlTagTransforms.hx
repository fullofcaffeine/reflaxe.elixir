...
class HeexControlTagTransforms {
						...
						if (open != -1) {
							...
							if (triple != -1) {
								...
								if (contentEnd != -1) {
									...
									if (updatedBody != body)
										return makeASTWithMeta(ERaw(before + updatedBody + after), n.metadata, n.pos);
								}
							}
						}
					}
	...
	public static function rewriteControlTags(s:String):String {
		...
		while (i < s.length) {
			...
			var single = ~/^<%=\s*(.*?)\s*%>$/s;
			if (single.match(cond))
				cond = StringTools.trim(single.matched(1));
