...
class NumericSuffixVarNormalizeTransforms {
			...
			return switch (n.def) {
				...
				case ECase(expr, clauses):
					...
					for (cl in clauses) {
						...
						var refScope = (cl.guard == null) ? cl.body : makeASTWithMeta(EBlock([cl.guard, cl.body]), {}, cl.body.pos);
				...
				case EFn(clauses):
					...
					for (cl in clauses) {
						...
						for (a in cl.args)
							for (nm in collectPatternVars(a).keys())
								paramVars.set(nm, true);
	...
	static function normalizeFunctionScope(params:Array<EPattern>, body:ElixirAST):{params:Array<EPattern>, body:ElixirAST} {
		...
		for (p in params)
			for (k in collectPatternVars(p).keys())
				declared.set(k, true);
		...
		for (k in paramRename.keys())
			rename.set(k, paramRename.get(k));
		for (k in bodyRename.keys())
			rename.set(k, bodyRename.get(k));
		...
		function walk(n:ElixirAST):Void {
			...
			switch (n.def) {
				case EBinary(Match, left, _):
					switch (left.def) {
						case EVar(v) if (isLowerName(v)): vars.set(v, true);
						default:
					}
				case EMatch(pat, _):
					for (k in collectPatternVars(pat).keys())
						vars.set(k, true);
				case EBlock(ss):
					for (s in ss)
						walk(s);
				...
				case ECase(expr, clauses):
					walk(expr);
					for (c in clauses) {
						for (k in collectPatternVars(c.pattern).keys())
							vars.set(k, true);
						if (c.guard != null)
							walk(c.guard);
						walk(c.body);
				...
				case EWith(clauses, doBlock, elseBlock):
					for (wc in clauses) {
						for (k in collectPatternVars(wc.pattern).keys())
							vars.set(k, true);
						walk(wc.expr);
					}
					walk(doBlock);
				...
				case ECall(t, _, args):
					...
					for (a in args)
						walk(a);
				case ERemoteCall(m, _, remoteArgs):
					walk(m);
					for (a in remoteArgs)
						walk(a);
	...
	static function computeNumericRenamesGated(toNormalize:Map<String, Bool>, refs:Map<String, Bool>, ?reserved:Map<String, Bool>):Map<String, String> {
		...
		for (k in toNormalize.keys()) {
			...
			if (!baseDeclared && !baseReferenced)
				continue; // keep original numeric name
			if (!used.exists(base)) {
				rename.set(k, base);
				used.remove(k);
				used.set(base, true);
	...
	static function renameBodyVars(body:ElixirAST, rename:Map<String, String>):ElixirAST {
		if (rename == null || Lambda.count(rename) == 0)
			return body;
		...
		function walk(x:ElixirAST):Void {
			...
			switch (x.def) {
				...
				case ECall(t, _, as):
					...
					for (a in as)
						walk(a);
				case ERemoteCall(m, _, remoteArgs2):
					walk(m);
					for (a in remoteArgs2)
						walk(a);
				case EList(items) | ETuple(items):
					for (i in items)
						walk(i);
