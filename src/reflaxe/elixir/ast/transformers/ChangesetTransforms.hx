...
class ChangesetTransforms {
			...
			return switch (node.def) {
				...
				case EModule(modName, attrs, body):
					...
					if (!usesChangeset) {
						...
						function scan(x:ElixirAST):Void {
							if (usesChangeset || x == null || x.def == null)
								return;
							switch (x.def) {
								case ERemoteCall(m, _, as):
									switch (m.def) {
										case EVar(n) if (n == "Ecto.Changeset"): usesChangeset = true;
										default:
									}
					...
					if (!usesChangeset) node else {
						...
						var binder = if (declared.exists("cs")) "cs" else if (declared.exists("_this2")) "_this2" else if (declared.exists("_this1"))
							"_this1" else if (declared.exists("this2")) "this2" else if (declared.exists("this1")) "this1" else "cs";
				...
				case EDefmodule(modName, doBlock):
					...
					function scanDefBlock(x:ElixirAST):Void {
						if (usesChangeset2 || x == null || x.def == null)
							return;
						switch (x.def) {
							case EImport(m, _, _, _) if (m == "Ecto.Changeset"):
								usesChangeset2 = true;
							case ERemoteCall(m, _, as):
								switch (m.def) {
									case EVar(n) if (n == "Ecto.Changeset"): usesChangeset2 = true;
									default:
								}
								scanDefBlock(m);
								if (as != null)
					...
					if (!usesChangeset2) node else {
						...
						var changesetBinder = if (declaredVars.exists("cs")) "cs" else if (declaredVars.exists("_this2")) "_this2" else
							if (declaredVars.exists("_this1")) "_this1" else if (declaredVars.exists("this2")) "this2" else if (declaredVars.exists("this1"))
								"this1" else "cs";
	...
	public static function normalizeValidateFieldAtomPass(ast:ElixirAST):ElixirAST {
		function normalizeFieldArg(arg:ElixirAST):ElixirAST {
			return switch (arg.def) {
				...
				case ERemoteCall({def: EVar(m1)}, f1, [{def: ECall(null, f2, [inner])}])
					...
					switch (funExpr.def) {
						case ECapture(inner, _):
							switch (inner.def) {
								...
								case EField({def: EVar(cls)}, meth) if (cls == "String"
									&& (meth == "to_atom" || meth == "to_existing_atom")):
			...
			return switch (n.def) {
				case EBinary(Equal, l, r):
					switch [isOptsField(l), r.def, isOptsField(r), l.def] {
						case [f, ENil, _, _] if (f != null):
							makeAST(ERemoteCall(makeAST(EVar("Kernel")), "is_nil", [
								makeAST(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(f))]))
							]));
						case [_, _, f2, ENil] if (f2 != null):
							makeAST(ERemoteCall(makeAST(EVar("Kernel")), "is_nil", [
								makeAST(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(f2))]))
							]));
						default: n;
					}
				case EBinary(NotEqual, l2, r2):
					switch [isOptsField(l2), r2.def, isOptsField(r2), l2.def] {
						case [f, ENil, _, _] if (f != null):
							var inner = makeAST(ERemoteCall(makeAST(EVar("Kernel")), "is_nil", [
								makeAST(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(f))]))
							]));
							makeAST(ElixirASTDef.EUnary(Not, inner));
						case [_, _, f2, ENil] if (f2 != null):
							var inner2 = makeAST(ERemoteCall(makeAST(EVar("Kernel")), "is_nil", [
								makeAST(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(f2))]))
							]));
	...
	static function normalizeBody(body:ElixirAST):ElixirAST {
		...
		switch (body.def) {
			case EBlock(stmts) if (stmts.length == 1):
				...
				if (onlySimple && containsChangesetCallsDetect(only))
					return only;
			default:
		}
		...
		function patternAllWildcards(p:EPattern):Bool {
			return switch (p) {
				...
				case PTuple(es):
					var any = false;
					for (e in es)
						any = any || patternAllWildcards(e);
					any;
				case PList(es):
					var any2 = false;
					for (e in es)
						any2 = any2 || patternAllWildcards(e);
					any2;
		...
		if (binderAfterRequired == null)
			binderAfterRequired = csVar;
	...
	static function condContainsValidateLength(clauses:Array<ECondClause>):Bool {
		for (c in clauses) {
			if (c != null && c.body != null && isValidateLengthCall(c.body))
				return true;
		}
		return false;
	}
		...
		for (c in clauses) {
			...
			switch (newBody.def) {
				case ERemoteCall(mod, fn, args) if (isChangeset(mod) && fn == "validate_length"):
					var a = args.copy();
					if (a.length > 0)
						a[0] = makeAST(EVar(csVar));
					if (a.length > 1) {
						a[1] = (function(arg:ElixirAST) {
							return switch (arg.def) {
								case EString(s): makeAST(EAtom(s));
								case ERemoteCall({def: EVar(m)}, f, [fld]) if (m == "String"
									&& (f == "to_atom" || f == "to_existing_atom")):
									switch (fld.def) {
					...
					function rewriteOptsAccessInKeyword(n:ElixirAST):ElixirAST {
						return switch (n.def) {
							case EAccess({def: EVar(v)}, key) if (v == "opts"):
								...
								if (keyAtom != null) {
									makeASTWithMeta(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(keyAtom))]), n.metadata,
										n.pos);
								} else n;
							case EField({def: EVar(v2)}, fld) if (v2 == "opts"):
								makeASTWithMeta(ERemoteCall(makeAST(EVar("Map")), "get", [makeAST(EVar("opts")), makeAST(EAtom(fld))]), n.metadata, n.pos);
	...
	static function findChangesetVar(body:ElixirAST):Null<String> {
		...
		function visit(n:ElixirAST):Void {
			if (n == null || n.def == null || found != null)
				return;
			switch (n.def) {
