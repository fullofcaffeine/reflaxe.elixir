	...
	public static function pass(ast:ElixirAST):ElixirAST {
		...
		if (appPrefix == null) {
			try {
				...
				if (defineApp != null && defineApp.length > 0)
					appPrefix = defineApp;
			} catch (_e) {}
		}
		#end
		...
		function extractModuleName(moduleExpr:ElixirAST):Null<String> {
			if (moduleExpr == null || moduleExpr.def == null)
				return null;
		...
		return ElixirASTTransformer.transformNode(ast, function(node:ElixirAST):ElixirAST {
			...
			return switch (node.def) {
				case ERemoteCall(moduleExpr, funcName, args):
					...
					if ((moduleName != null && moduleName.indexOf("Component") != -1)
						|| (funcName != null && funcName.indexOf("Component") != -1)) {
					...
					if (compound != null && isPhoenixComponentFunction(compound)) {
						makeASTWithMeta(ERemoteCall(makeAST(EVar("Phoenix.Component")), compound, args), node.metadata, node.pos);
					} else if (isPhoenixComponentFunction(funcName) && moduleName != null && isAmbiguousComponentModule(moduleName)) {
						makeASTWithMeta(ERemoteCall(makeAST(EVar("Phoenix.Component")), funcName, args), node.metadata, node.pos);
					} else {
						node;
					}
				...
				case ECall(null, funcName, args):
					...
					if (compound != null && isPhoenixComponentFunction(compound)) {
						...
						&& isPhoenixComponentFunction(split.funcName)
						&& isAmbiguousComponentModule(split.moduleName)) {
						makeASTWithMeta(ERemoteCall(makeAST(EVar("Phoenix.Component")), split.funcName, args), node.metadata, node.pos);
					} else {
						node;
					}
