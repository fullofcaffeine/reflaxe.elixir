	...
	static function fixBody(body:ElixirAST):ElixirAST {
		return ElixirASTTransformer.transformNode(body, function(x:ElixirAST):ElixirAST {
			return switch (x.def) {
				...
				case EBinary(Match, {def: EVar(lv)}, {def: ERemoteCall({def: EVar(mod)}, "assign", [firstArg, secondArg])})
					if (lv == "_" && mod == "Phoenix.Component"):
					switch (firstArg.def) {
						case EVar(firstName):
							makeASTWithMeta(EBinary(Match, makeAST(EVar(firstName)),
								makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [firstArg, secondArg]))),
								x.metadata, x.pos);
						default:
							x;
					}
				...
				case EMatch(PWildcard, {def: ERemoteCall({def: EVar(mod2)}, "assign", [fa2, sa2])}) if (mod2 == "Phoenix.Component"):
					switch (fa2.def) {
						case EVar(firstName2):
							makeASTWithMeta(EMatch(PVar(firstName2), makeAST(ERemoteCall(makeAST(EVar("Phoenix.Component")), "assign", [fa2, sa2]))),
								x.metadata, x.pos);
						default:
							x;
					}
		...
		return ElixirASTTransformer.transformNode(body, function(node:ElixirAST):ElixirAST {
			return switch (node.def) {
				case EBlock(stmts):
					...
					for (i in 0...stmts.length) {
						switch (stmts[i].def) {
							case EMatch(PVar(lhs), {def: ERemoteCall({def: EVar(mod)}, "assign", [_fa, _sa])})
								if (lhs == "assigns" && mod == "Phoenix.Component"):
								assignsIdx = i;
							case EBinary(Match, {def: EVar(lhs2)}, {def: ERemoteCall({def: EVar(mod2)}, "assign", [_fa2, _sa2])})
					...
					for (i in 0...stmts.length) {
						...
						switch (cur.def) {
							case ESigil(type, content, _m) if (type == "H"):
								if (content.indexOf("Phoenix.HTML.raw(@content)") != -1
