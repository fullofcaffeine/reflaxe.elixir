...
class SubstitutionHelpers {
	...
	public static function replaceNullCoalVar(expr:TypedExpr, varId:Int, initExpr:TypedExpr):TypedExpr {
		return switch (expr.expr) {
			case TBinop(OpNullCoal, {expr: TLocal(v)}, defaultExpr) if (v.id == varId):
				...
				{
					expr: TIf({expr: TBinop(OpNotEq, initExpr, {expr: TConst(TNull), t: expr.t, pos: expr.pos}), t: expr.t, pos: expr.pos}, initExpr,
						defaultExpr),
					t: expr.t,
					pos: expr.pos
				};
			...
			case TIf(cond, thenExpr, elseExpr):
				...
				{
					expr: TIf(replaceNullCoalVar(cond, varId, initExpr), replaceNullCoalVar(thenExpr, varId, initExpr),
			...
			case TCall(e, el):
				...
				{
					expr: TCall(replaceNullCoalVar(e, varId, initExpr), el.map(arg -> replaceNullCoalVar(arg, varId, initExpr))),
					t: expr.t,
					pos: expr.pos
				};
	...
	public static function substituteVariable(expr:TypedExpr, varToReplace:TVar, replacement:TypedExpr):TypedExpr {
		return switch (expr.expr) {
			...
			case TCall(e, el):
				{
					expr: TCall(substituteVariable(e, varToReplace, replacement), el.map(arg -> substituteVariable(arg, varToReplace, replacement))),
					t: expr.t,
					pos: expr.pos
				};
			...
			case TIf(cond, thenExpr, elseExpr):
				{
					expr: TIf(substituteVariable(cond, varToReplace, replacement), substituteVariable(thenExpr, varToReplace, replacement),
			...
			case TBinop(op, e1, e2):
				{
					expr: TBinop(op, substituteVariable(e1, varToReplace, replacement), substituteVariable(e2, varToReplace, replacement)),
					t: expr.t,
					pos: expr.pos
				};
			...
			case TSwitch(e, cases, edef):
				{
					expr: TSwitch(substituteVariable(e, varToReplace, replacement), cases.map(c -> {
			...
			case TFor(v, e1, e2):
				...
				if (v.id == varToReplace.id) {
					...
					{
						expr: TFor(v, substituteVariable(e1, varToReplace, replacement), substituteVariable(e2, varToReplace, replacement)),
						t: expr.t,
						pos: expr.pos
					};
				}
			...
			case TWhile(econd, e, normalWhile):
				{
					expr: TWhile(substituteVariable(econd, varToReplace, replacement), substituteVariable(e, varToReplace, replacement), normalWhile),
					t: expr.t,
					pos: expr.pos
				};
			...
			case TTry(e, catches):
				{
					expr: TTry(substituteVariable(e, varToReplace, replacement), catches.map(c -> {
						v: c.v,
