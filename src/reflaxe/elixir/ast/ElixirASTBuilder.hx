...
class ElixirASTBuilder {
	...
	private static function buildFromTypedExprWithContext(expr:TypedExpr, context:reflaxe.elixir.CompilationContext):ElixirAST {
		...
		if (previousContext != null
			&& context.localVarInitValuesById != null
			&& previousContext.localVarInitValuesById != null
			&& Lambda.count(context.localVarInitValuesById) == 0
			&& Lambda.count(previousContext.localVarInitValuesById) > 0) {
		...
		if (previousContext != null) {
			if (context.currentClass == null)
				context.currentClass = previousContext.currentClass;
			if (context.currentModule == null)
				context.currentModule = previousContext.currentModule;
			if (!context.currentModuleHasPresence && previousContext.currentModuleHasPresence)
		...
		if (currentContext != null && currentContext.builderCache != null) {
			...
			if (cached != null) {
				#if debug_compilation_hang
				Sys.println('[HANG DEBUG] Cache hit for ' + reflaxe.elixir.util.EnumReflection.enumConstructor(expr.expr) + ' at ' + expr.pos);
				#end
	...
	static function convertExpression(expr:TypedExpr):ElixirASTDef {
		...
		return switch (expr.expr) {
			...
			case TVar(v, init):
						...
						case TField(obj, fa):
							...
							switch (obj.expr) {
								case TLocal(localVar):
									...
									var extractedVarName = VariableAnalyzer.toElixirVarName(localVar.name) + "_"
				...
				if (init != null) {
					switch (init.expr) {
						case TBlock(blockStmts) if (blockStmts.length >= 3):
							...
							var comprehensionAST = reflaxe.elixir.ast.builders.ComprehensionBuilder.tryBuildArrayComprehensionFromBlock(blockStmts,
								currentContext);
							...
							switch (blockStmts[0].expr) {
								case TVar(tempVar, tempInit) if (tempInit != null
									&& (tempVar.name.startsWith("g") || tempVar.name.startsWith("_g"))):
				...
				if (init != null) {
					switch (init.expr) {
						case TEnumParameter(e, _, index):
							...
							if ((tempVarName == "g"
								|| (tempVarName.length > 1 && tempVarName.charAt(0) == "g" && tempVarName.charAt(1) >= '0' && tempVarName.charAt(1) <= '9'))) {
				...
				var matchNode = if (init != null) {
					...
					var initValue = switch (init.expr) {
						case TBlock([
							{expr: TVar(tmpVar, tmpInit)},
							{expr: TBinop(OpNullCoal, {expr: TLocal(localVar)}, defaultExpr)}
							...
							var ifExpr = makeAST(EIf(makeAST(EBinary(NotEqual, makeAST(EMatch(PVar(tmpVarName), initAst)), makeAST(ENil))),
								makeAST(EVar(tmpVarName)), defaultAst));
									...
									if (stmts.length > 0) {
										switch (stmts[0].expr) {
											case TVar(v, initExpr) if (initExpr != null
												&& (v.name.startsWith("g") || v.name.startsWith("_g"))):
												#if debug_array_patterns
												#if debug_ast_builder
												#end
												#end
					...
					if (init != null
						&& currentContext.currentClauseContext != null
								...
								case TLocal(tempVar):
									...
									if (tempVarName == "g" || tempVarName == "_g") {
										...
										if (tempVarName.charAt(0) == "g" && tempVarName.charAt(1) >= '0' && tempVarName.charAt(1) <= '9') {
											isTempVar = true;
										} else if (tempVarName.length > 2 && tempVarName.charAt(0) == "_" && tempVarName.charAt(1) == "g"
											&& tempVarName.charAt(2) >= '0' && tempVarName.charAt(2) <= '9') {
											isTempVar = true;
										}
									}
									...
									if (isTempVar) {
										...
										if (elixirTempName == "g"
											|| (elixirTempName.charAt(0) == "g" && elixirTempName.length > 1 && elixirTempName.charAt(1) >= '0'
												&& elixirTempName.charAt(1) <= '9')) {
											shouldSkipAssignment = true;
											#if debug_ast_builder
											#end
											#if debug_enum_extraction
			...
			case TBinop(op, e1, e2):
				switch (op) {
					...
					default:
						...
						var ast = BinaryOpBuilder.buildBinopFromAST(op, leftAST, rightAST, e1, e2, // Keep original exprs for type checking
					...
					switch (fa) {
						...
						default:
							...
							if (currentContext != null
								&& currentContext.currentClass != null
								&& currentContext.currentClass.meta != null) {
			...
			case TBlock(el):
				...
				if (el.length == 2) {
					var hasVar = switch (el[0].expr) {
						case TVar(_, _): true;
						default: false;
					};
					var hasSwitch = switch (el[1].expr) {
				...
				if (el.length >= 2) {
						...
						if (declIdx != -1 && declVar != null && declInit != null) {
							...
							if (canonicalInit) {
								return BlockBuilder.withScopedInitTracking(el, currentContext, function() {
									...
									if (initAST != null)
										stmts.push({def: EMatch(PVar(varName), initAST), metadata: {}, pos: null});
					...
					switch (el[0].expr) {
						case TVar(declaredVar, declaredInit) if (declaredInit != null):
							...
							var isInfrastructureVar = (declaredName == "g"
								|| declaredName == "_g"
							...
							if (!isInfrastructureVar) {
								...
								if (canonicalInit) {
									return BlockBuilder.withScopedInitTracking(el, currentContext, function() {
										...
										if (declaredInitAST != null)
											seq.push({def: EMatch(PVar(declaredVarName), declaredInitAST), metadata: {}, pos: null});
										...
										for (i in 1...el.length) {
											...
											if (statementNode != null)
												seq.push(statementNode);
										}
										// If the last expression is not a return of the same variable, append it
										var lastIsSame = switch (el[el.length - 1].expr) {
											case TLocal(vx) if (vx.id == declaredVar.id || vx.name == declaredVar.name): true;
											default: false;
										};
										if (!lastIsSame)
											seq.push({def: EVar(declaredVarName), metadata: {}, pos: null});
										return EBlock(seq);
									});
								}
							}
					...
					if (declAt > 0 && declV != null && declI != null) {
						return BlockBuilder.withScopedInitTracking(el, currentContext, function() {
							...
							if (initASTx != null)
								seqx.push({def: EMatch(PVar(vname), initASTx), metadata: {}, pos: null});
							for (i in 0...el.length)
								if (i != declAt) {
							...
							var lastIsVar = switch (el[el.length - 1].expr) {
								case TLocal(vx) if (vx.id == declV.id || vx.name == declV.name): true;
								default: false;
							};
							if (!lastIsVar)
								seqx.push({def: EVar(vname), metadata: {}, pos: null});
							return EBlock(seqx);
						});
					}
				}
	 ...
	 */
	static function buildFieldPatternSwitch(rootObj:TypedExpr, fieldName:String, cases:Array<{values:Array<TypedExpr>, expr:TypedExpr}>, edef:Null<TypedExpr>,
			pos:Position, context:reflaxe.elixir.CompilationContext):ElixirASTDef {
		#if debug_ast_builder
		#end
	...
	static function createMetadata(expr:TypedExpr):ElixirMetadata {
		...
		switch (expr.expr) {
			case TFor(_, _, _) | TWhile(_, _, _):
				trace('[createMetadata] '
					+ reflaxe.elixir.util.EnumReflection.enumConstructor(expr.expr)
					+ ' loopContainsReturn='
					+ loopContainsReturn
					+ ' fromReturn='
					+ fromReturn);
	...
	static function containsIfStatement(expr:TypedExpr):Bool {
		return switch (expr.expr) {
			...
			case TSwitch(_, cases, _):
				for (c in cases) {
					if (containsIfStatement(c.expr))
						return true;
				}
				false;
			case TCall(_, args):
				for (arg in args) {
					if (containsIfStatement(arg))
						return true;
				}
				false;
	 ...
	 */
	static function tryExpandElixirInjection(methodExpr:TypedExpr, thisExpr:TypedExpr, args:Array<TypedExpr>,
							...
							switch (callArgs[0].expr) {
								case TConst(TString(code)):
									...
									inline function resolvePresenceModuleName():String {
										...
										if (currentContext != null && currentContext.currentModuleHasPresence == true) {
											...
											if (currentModule != null) {
												...
												if (webIndex > 0)
													appModulePrefix = currentModule.substring(0, webIndex);
											}
											if (appModulePrefix == null) {
												try
													appModulePrefix = reflaxe.elixir.PhoenixMapper.getAppModuleName()
												catch (e) {}
											}
											if (appModulePrefix != null)
												moduleName = appModulePrefix + "Web.Presence";
										}
										return moduleName;
									}
									...
									if (StringTools.startsWith(code, "Phoenix.Presence.track")) {
										...
										return makeAST(ERemoteCall(makeAST(EVar(presenceModuleName)), "update",
											[socketOrPidUpdateAst, topicUpdateAst, keyUpdateAst, metaUpdateAst]));
										...
										return makeAST(ERemoteCall(makeAST(EVar(presenceModuleName)), "untrack",
											[socketOrPidUntrackAst, topicUntrackAst, keyUntrackAst]));
	...
	static function applyParameterRenaming(ast:ElixirAST, renaming:Map<String, String>):ElixirAST {
		return switch (ast.def) {
			...
			case ECall(target, func, args):
				makeASTWithMeta(ECall(target != null ? applyParameterRenaming(target, renaming) : null, func,
			...
			case EIf(cond, then, else_):
				makeASTWithMeta(EIf(applyParameterRenaming(cond, renaming), applyParameterRenaming(then, renaming),
					else_ != null ? applyParameterRenaming(else_, renaming) : null),
					ast.metadata, ast.pos);

			case ECase(expr, clauses):
				makeASTWithMeta(ECase(applyParameterRenaming(expr, renaming), clauses.map(c -> {
	 ...
	 */
	static function createVariableMappingsForCase(caseExpr:TypedExpr, extractedParams:Array<String>, enumType:Null<EnumType>, values:Array<TypedExpr>,
			enumBindingPlan:Map<Int, {
			finalName:String,
			isUsed:Bool
		}> = null):Map<Int, String> {
	...
	static function isAssertClass(expr:TypedExpr):Bool {
		return switch (expr.expr) {
			case TTypeExpr(TClassDecl(classRef)): // Check if this is haxe.test.Assert
				var classType = classRef.get(); var pack = classType.pack.join("."); var name = classType.name; pack == "haxe.test" && name == "Assert";
			default: false;
		}
	}
	...
	static function typeToElixir(t:Type):String {
		if (t == null)
			return "any"; // Handle null types gracefully
		return switch (t) {
	...
	static function isVariableUsedInAST(varName:String, ast:ElixirAST):Bool {
		...
		return switch (ast.def) {
			...
			case EBlock(exprs):
				for (e in exprs) {
					if (isVariableUsedInAST(varName, e))
						return true;
				}
				false;
			case EIf(cond, thenBranch, elseBranch): isVariableUsedInAST(varName,
					cond) || isVariableUsedInAST(varName, thenBranch) || (elseBranch != null && isVariableUsedInAST(varName, elseBranch));
			case ETuple(values):
				for (v in values) {
					if (isVariableUsedInAST(varName, v))
						return true;
				}
				false;
			case EList(values):
				...
				false;
			case EBinary(_, left, right): isVariableUsedInAST(varName, left) || isVariableUsedInAST(varName, right);
			case ECall(target, funcName, args):
				if (target != null && isVariableUsedInAST(varName, target))
					return true;
				for (a in args) {
					if (isVariableUsedInAST(varName, a))
						return true;
				}
				false;
			case ERemoteCall(module, func, args):
				if (isVariableUsedInAST(varName, module))
					return true;
				for (a in args) {
					if (isVariableUsedInAST(varName, a))
						return true;
			...
			case EFn(clauses):
				for (c in clauses) {
					if (isVariableUsedInAST(varName, c.body))
						return true;
					if (c.guard != null && isVariableUsedInAST(varName, c.guard))
						return true;
				}
				false;
			case ECase(expr, clauses):
				if (isVariableUsedInAST(varName, expr))
					return true;
				for (c in clauses) {
					if (isVariableUsedInAST(varName, c.body))
						return true;
