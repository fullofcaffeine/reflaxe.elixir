...
class ElixirOutputIterator {
	...
	public function new(compiler:ElixirCompiler) {
		...
		maxIndex = compiler.classes.length + compiler.enums.length + compiler.typedefs.length + compiler.abstracts.length;
		...
		while (true) {
			...
			if (shouldSuppressEmission(transformedAST)) {
				...
				if (transformedAST.metadata != null) {
					if (transformedAST.metadata.suppressEmission == true)
						reason = "metadata.suppressEmission";
					else if (transformedAST.metadata.forceEmit == true)
						reason = "metadata.forceEmit";
				}
	...
	function setContextForCurrentOutput(astData:DataAndFileInfo<ElixirAST>):Void {
		if (astData == null || astData.baseType == null)
			return;
		...
		var typePath = astData.baseType.pack != null
			&& astData.baseType.pack.length > 0 ? astData.baseType.pack.join(".") + "." + astData.baseType.name : astData.baseType.name;
	 ...
	 */
	static function queueSourceMap(astData:DataAndFileInfo<reflaxe.elixir.ast.ElixirAST>, ast:ElixirAST, outputWithMarkers:String,
			markers:Null<SourceMapMarkers>):String {
		if (ast == null || outputWithMarkers == null)
			return outputWithMarkers;
		...
		var output = if (markers != null) emitMarkerMappings(writer, outputWithMarkers, markers, startPos) else {
	...
	static function shouldSuppressEmission(ast:ElixirAST):Bool {
		...
		if (ast.metadata != null) {
			...
			if (ast.metadata.forceEmit == true)
				return false;
			if (ast.metadata.suppressEmission == true)
				return true;
		}
		...
		return switch (ast.def) {
			...
			case EModule(_, _, body): body == null || body.length == 0;
			default:
				false;
		}
	}
	...
	function prepareExternalBootstraps():Void {
		...
		if (ModuleBuilder.getBootstrapStrategy() != BootstrapStrategy.External)
			return;
		...
		if (!haxe.macro.Context.defined("emit_bootstrap_scripts"))
			return;
		#end
		if (compiler.modulesWithBootstrap.length == 0)
			return;
