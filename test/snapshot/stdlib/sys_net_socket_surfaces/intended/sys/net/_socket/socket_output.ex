defmodule SocketOutput do
  def new(socket_ref_param) do
    struct = %{:__reflaxe_class__ => SocketOutput, :socket_ref => nil, :big_endian => nil}
    struct = %{struct | socket_ref: socket_ref_param}
    struct
  end
  def write_byte(struct, c) do
    SocketState.send_binary(struct.socket_ref, <<c::8>>)
  end
  def write_bytes(struct, buf, pos, len) do
    ...
    if (len == 0) do
      ...
      _ = SocketState.send_binary(struct.socket_ref, (fn ->
