defmodule Output do
  ...
  def write_byte(_struct, _c) do
    raise Reflaxe.Elixir.HaxeThrow, [value: NotImplementedException.new(nil, nil, %{file_name: "../../../../../std/elixir/_std/haxe/io/Output.hx", line_number: 38, class_name: "haxe.io.Output", method_name: "writeByte"})]
  end
  def write_bytes(struct, s, pos, len) do
    ...
    {_pos, _k} = Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {pos, k}, fn _, {acc_pos, acc_k} ->
      try do
        if (acc_k > 0) do
          apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, apply(Map.get(s, :__reflaxe_class__) || Map.get(s, :__struct__), :get, [s, acc_pos])])
          acc_pos = acc_pos + 1
          acc_k = (acc_k - 1)
          {:cont, {acc_pos, acc_k}}
        else
  ...
  def write_int8(struct, x) do
    ...
    apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
  end
  def write_int16(struct, x) do
    if (x < -32768 or x >= 32768) do
      raise Reflaxe.Elixir.HaxeThrow, [value: {:overflow}]
    end
    apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_u_int16, [struct, Bitwise.band(x, 65535)])
  end
  def write_u_int16(struct, x) do
    ...
    if (struct.big_endian) do
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.bsr(x, 8)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
    else
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.bsr(x, 8)])
  ...
  def write_int24(struct, x) do
    ...
    apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_u_int24, [struct, Bitwise.band(x, 16777215)])
  end
  def write_u_int24(struct, x) do
    ...
    if (struct.big_endian) do
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.bsr(x, 16)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(Bitwise.bsr(x, 8), 255)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
    else
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(Bitwise.bsr(x, 8), 255)])
  ...
  def write_int32(struct, x) do
    if (struct.big_endian) do
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.bsr(x, 24)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(Bitwise.bsr(x, 16), 255)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(Bitwise.bsr(x, 8), 255)])
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
    else
      apply(Map.get(struct, :__reflaxe_class__) || Map.get(struct, :__struct__), :write_byte, [struct, Bitwise.band(x, 255)])
