defmodule Main do
  def main() do
    ...
    code = apply(Map.get(p, :__reflaxe_class__) || Map.get(p, :__struct__), :exit_code, [p, nil])
    _ = apply(Map.get(p, :__reflaxe_class__) || Map.get(p, :__struct__), :close, [p])
    detached = Sys.IO.Process.new("sh", ["-c", "exit 3"], true)
    detached_code = apply(Map.get(detached, :__reflaxe_class__) || Map.get(detached, :__struct__), :exit_code, [detached, nil])
