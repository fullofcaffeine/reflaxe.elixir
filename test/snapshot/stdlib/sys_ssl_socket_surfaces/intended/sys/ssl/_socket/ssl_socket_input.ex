defmodule SslSocketInput do
  def new(socket_ref_param) do
    struct = %{:__reflaxe_class__ => SslSocketInput, :socket_ref => nil, :big_endian => nil}
    struct = %{struct | socket_ref: socket_ref_param}
    struct
  end
  def read_byte(struct) do
    result = SslSocketState.recv_binary(struct.socket_ref, 1)
  ...
  def read_bytes(_struct, buf, pos, len) do
    ...
    if (len == 0) do
      ...
      raise Reflaxe.Elixir.HaxeThrow, [value: {:custom, "sys.ssl.Socket.input.readBytes is not supported on the Elixir target because haxe.io.Bytes buffers are immutable in generated Elixir; use Socket.read() or Input.readByte() instead"}]
    end
  end
