defmodule Main do
  ...
  def main() do
    ...
    assert_that(length(items) == 3, "json array length failed")
    assert_that(Reflaxe.Elixir.HaxeFloat.eq(Enum.at(items, 0), 1), "json array int failed")
    assert_that(Reflaxe.Elixir.HaxeFloat.eq(Enum.at(items, 1), true), "json array bool failed")
    assert_that(Reflaxe.Elixir.HaxeFloat.eq(Enum.at(items, 2), nil), "json array null failed")
    assert_that(Reflaxe.Elixir.HaxeFloat.eq(Jason.decode!("\"ok\""), "ok"), "top-level json string failed")
    assert_that(Reflaxe.Elixir.HaxeFloat.eq(Jason.decode!("null"), nil), "top-level json null failed")
    try do
      Jason.decode!("{\"unterminated\":")
      assert_that(false, "invalid json should raise")
    rescue
      haxe_exception ->
