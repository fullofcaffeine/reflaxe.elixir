defmodule Main do
  ...
  def main() do
    ...
    _ = assert_that(Map.get(map_value, "one") == 1, "string map one failed")
    _ = assert_that(Map.get(map_value, "two") == 2, "string map two failed")
    serializer = Serializer.new()
    _ = apply(Map.get(serializer, :__reflaxe_class__) || Map.get(serializer, :__struct__), :serialize, [serializer, "prefix"])
    _ = apply(Map.get(serializer, :__reflaxe_class__) || Map.get(serializer, :__struct__), :serialize, [serializer, 7])
    instance_wire = apply(Map.get(serializer, :__reflaxe_class__) || Map.get(serializer, :__struct__), :to_string, [serializer])
    _ = assert_that(instance_wire == "y6:prefixi7", "instance serializer buffer failed")
    nil
  end
end
