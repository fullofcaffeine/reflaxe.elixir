defmodule Main do
  ...
  def main() do
    ...
    _ = assert_that(values.length == 3, "length should track add and push")
    _ = assert_that(apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :first, [values]) == "zero", "first should reflect pushed head")
    _ = assert_that(apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :last, [values]) == "two", "last should reflect appended tail")
    _ = assert_that(apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :to_string, [values]) == "{zero, one, two}", "toString should preserve order")
    entries = Array.new()
    ...
    _ = assert_that(Enum.join(entries, ",") == "0:zero,1:one,2:two", "keyValueIterator should preserve indices")
    {values, reflaxe_receiver_value_0} = apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :remove, [values, "one"])
    _ = assert_that(reflaxe_receiver_value_0, "remove should delete first matching value")
    {values, reflaxe_receiver_value_1} = apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :remove, [values, "missing"])
    _ = assert_that(not reflaxe_receiver_value_1, "remove should report missing values")
    {values, reflaxe_receiver_value_2} = apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :pop, [values])
    _ = assert_that(reflaxe_receiver_value_2 == "zero", "pop should remove the head")
    _ = assert_that(apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :to_string, [values]) == "{two}", "pop/remove should rebind receiver state")
    mapped = apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :map, [values, fn value -> value <> "!" end])
    _ = assert_that(apply(Map.get(mapped, :__reflaxe_class__) || Map.get(mapped, :__struct__), :to_string, [mapped]) == "{two!}", "map should return a new list")
    values = apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :clear, [values])
    _ = assert_that(apply(Map.get(values, :__reflaxe_class__) || Map.get(values, :__struct__), :is_empty, [values]), "clear should empty the list")
  end
end
