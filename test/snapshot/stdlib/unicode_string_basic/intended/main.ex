defmodule Main do
  def main() do
    test_iteration()
    test_key_value_iteration()
    test_validate_utf8()
  end
  defp test_iteration() do
    text = "Aé🌍中"
    expect("unicode length", String.length(text) == 4)
    codes = []
    g_offset = 0
    g_s = text
    ...
    expect("codepoint count", length(codes) == 4)
    expect("ascii codepoint", Enum.at(codes, 0) == 65)
    expect("latin codepoint", Enum.at(codes, 1) == 233)
    expect("astral codepoint", Enum.at(codes, 2) == 127757)
    expect("cjk codepoint", Enum.at(codes, 3) == 20013)
  end
