defmodule Main do
  ...
  def main() do
    ...
    try do
      Haxe.Crypto.BaseCode.new(Bytes.of_string("abc", {:utf8}))
      assert_that(false, "non-power-of-two dictionary should throw")
    rescue
      haxe_exception ->
    ...
    try do
      apply(Map.get(hex, :__reflaxe_class__) || Map.get(hex, :__struct__), :decode_bytes, [hex, Bytes.of_string("0g", {:utf8})])
      assert_that(false, "invalid encoded character should throw")
    rescue
      haxe_exception ->
        Process.put(:__reflaxe_last_stacktrace__, __STACKTRACE__)
        (case {(case haxe_exception do
