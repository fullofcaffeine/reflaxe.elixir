defmodule UserQueries do
  ...
  def get_active_posters(min_posts) do
    from("users", "u", %{:left_join => %{:table => "posts", :alias => "p", :on => "p.user_id == u.id"}, :group_by => "u.id", :having => "count(p.id) >= " <> StringBuf.to_string(min_posts), :select => %{:user => "u", :post_count => "count(p.id)"}})
  end
  ...
  def deactivate_old_users(days) do
    from("users", "u", %{:where => %{:last_login_lt => "ago(" <> StringBuf.to_string(days) <> ", \"day\")"}, :update => %{:active => false}})
  end
  ...
  def search_users(filters) do
    ...
    query = if Map.get(filters, :name) != nil do
      where(query, "u", %{:name_ilike => Map.get(filters, :name)})
    else
      query
    end
    query = if Map.get(filters, :email) != nil do
      where(query, "u", %{:email => Map.get(filters, :email)})
    else
      query
    end
    query = if Map.get(filters, :min_age) != nil do
