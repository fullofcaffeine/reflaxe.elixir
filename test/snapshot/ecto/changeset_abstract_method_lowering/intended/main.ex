defmodule Main do
  ...
  def fluent_chain(user, params) do
    ...
    this1 = Ecto.Changeset.validate_required(this1, Enum.map(["name"], &String.to_atom/1))
    this1 = Ecto.Changeset.validate_length(this1, String.to_atom("name"), Enum.filter([min: Map.get(%{:min => 2}, :min), max: Map.get(%{:min => 2}, :max), is: Map.get(%{:min => 2}, :is)], fn {_, v} -> v != nil end))
    cs = Ecto.Changeset.validate_inclusion(this1, String.to_atom("role"), ["admin", "user"])
    cs = Ecto.Changeset.validate_exclusion(cs, String.to_atom("role"), ["blocked"])
    cs
  end
  def direct_return(user, params) do
    ...
    cs = Ecto.Changeset.validate_required(cs, Enum.map(["name"], &String.to_atom/1))
    cs
  end
  def same_variable_reassignment(user, params) do
    ...
    changeset = changeset |> Ecto.Changeset.validate_required(Enum.map(["name"], &String.to_atom/1)) |> Ecto.Changeset.validate_length(String.to_atom("name"), Enum.filter([min: Map.get(%{:min => 2}, :min), max: Map.get(%{:min => 2}, :max), is: Map.get(%{:min => 2}, :is)], fn {_, v} -> v != nil end)) |> Ecto.Changeset.validate_inclusion(String.to_atom("role"), ["admin", "user"]) |> Ecto.Changeset.validate_exclusion(String.to_atom("role"), ["blocked"])
    changeset
  end
end
