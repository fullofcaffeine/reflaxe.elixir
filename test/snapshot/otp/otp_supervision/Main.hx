...
class Main {
	...
	static function testTask() {
		...
		var concurrentResults = Task.runConcurrently([function() return "a", function() return "b", function() return "c"]);
	...
	static function testTaskSupervisor() {
		...
		if (supervisorResult._0 == "ok") {
			...
			// Test async stream
			var stream = TaskSupervisor.asyncStream(supervisor, [10, 20, 30], function(x) return x + 1);
			...
			var concurrentResults = TaskSupervisor.runSupervisedConcurrently(supervisor, [function() return 100, function() return 200, function() return 300]);
