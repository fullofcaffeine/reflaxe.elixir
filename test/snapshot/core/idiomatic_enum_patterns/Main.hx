...
class Main {
	...
	static function messageToElixir(message:TodoMessage):Dynamic {
		var basePayload:Dynamic = switch (message) {
			case TodoCreated(todo): untyped {type: "todo_created", todo: todo};
			case TodoUpdated(todo): untyped {type: "todo_updated", todo: todo};
			case TodoDeleted(id): untyped {type: "todo_deleted", todoId: id};
			case BulkUpdate(action): untyped {type: "bulk_update", action: action};
			case UserOnline(userId): untyped {type: "user_online", userId: userId};
			case UserOffline(userId): untyped {type: "user_offline", userId: userId};
			case SystemAlert(message, level): untyped {type: "system_alert", message: message, level: level};
		};
