...
class Main {
	...
	static function buildUserProfile(userIdStr:String, emailStr:String, scoreStr:String):Result<UserProfile, String> {
		return UserId.parse(userIdStr).mapError(e -> 'Invalid UserId: ${e}').flatMap(userId -> {
			return Email.parse(emailStr.trim()).mapError(e -> 'Invalid Email: ${e}').flatMap(email -> {
				var scoreInt = Std.parseInt(scoreStr);
				if (scoreInt == null) {
					return Error('Invalid score: ${scoreStr}');
				}
				return PositiveInt.parse(scoreInt).mapError(e -> 'Invalid score: ${e}').map(score -> {
					return {
						userId: userId,
						email: email,
						score: score
	...
	static function createUser(userIdStr:String, emailStr:String, nameStr:String):Result<User, String> {
		return UserId.parse(userIdStr).mapError(e -> 'Invalid UserId: ${e}').flatMap(userId -> {
			return Email.parse(emailStr).mapError(e -> 'Invalid Email: ${e}').flatMap(email -> {
				return NonEmptyString.parseAndTrim(nameStr).mapError(e -> 'Invalid Name: ${e}').map(displayName -> {
					return {
						userId: userId,
						email: email,
	...
	static function validateConfiguration(timeoutStr:String, retriesStr:String, nameStr:String):Result<Configuration, String> {
		...
		return PositiveInt.parse(timeoutInt).mapError(e -> 'Invalid timeout: ${e}').flatMap(timeout -> {
			return PositiveInt.parse(retriesInt).mapError(e -> 'Invalid retries: ${e}').flatMap(retries -> {
				return NonEmptyString.parseAndTrim(nameStr).mapError(e -> 'Invalid name: ${e}').map(name -> {
					return {
						timeout: timeout,
						retries: retries,
						name: name
