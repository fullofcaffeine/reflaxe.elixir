defmodule StringTools do
  ...
  def lpad(s, c, l) do
    if (c.length <= 0), do: s
    buf = ""
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {l, buf, s, :ok}, fn _, {acc_l, acc_buf, acc_s, acc_state} ->
  if (acc_buf.length + acc_s.length < acc_l) do
    acc_buf = acc_buf <> c
    {:cont, {acc_l, acc_buf, acc_s, acc_state}}
  else
    {:halt, {acc_l, acc_buf, acc_s, acc_state}}
  end
end)
    buf <> s
  end
