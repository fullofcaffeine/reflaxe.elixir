...
class Main {
	...
	static function testMultipleInlineIfs() {
		...
		processThree(a ? "a_true" : "a_false", b ? "b_true" : "b_false", c > 5 ? "c_high" : "c_low");

		// Mixed with regular arguments
		processMixed("regular", a ? "conditional" : "alternative", 42, b ? 1 : 0);
	}
	...
	static function testNestedCalls() {
		...
		// Deeply nested
		var nested = processString(wrapString(level > 5 ? "high" : "low"));

		// Inline if containing function calls
		var complex = processString(enabled ? computeValue(10) : computeValue(5));
	}
