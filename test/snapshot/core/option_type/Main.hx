...
class Main {
	...
	static function processUser(user:Option<String>) {
		switch (user) {
			case Some(name):
				trace('Processing user: ${name}');
			case None:
				trace('No user to process');
		}
	}
	...
	static function testFunctionalOperations() {
		...
		// Chaining operations
		var finalResult = user.map(name -> name.toUpperCase()).filter(name -> name.length > 2).then(name -> Some(name + " [PROCESSED]"));
	...
	static function testNullSafety() {
		...
		var result = safeOption.map(s -> s.length).unwrap(0);
	...
	static function testCollectionOperations() {
		// Array of Options
		var options:Array<Option<Int>> = [Some(1), None, Some(3), Some(4), None];
		...
		var processedValues = options.map(opt -> opt.map(x -> x * 2)).filter(opt -> opt.isSome());
	}
}
 ...
 */
class UserService {
	static var users:Array<User> = [{name: "Alice", email: Some("alice@example.com")}, {name: "Bob", email: None}];
	...
	public static function notifyUser(name:String, message:String):Bool {
		return getUserEmail(name).map(email -> sendEmail(email, message)).unwrap(false);
	}
