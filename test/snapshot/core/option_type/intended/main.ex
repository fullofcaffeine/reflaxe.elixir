defmodule Main do
  ...
  defp test_option_construction() do
    ...
    name = "world"
    nullable_name = nil
    option_from_value = {:FromNullable, name}
    option_from_null = {:FromNullable, nullable_name}
    some_person = {:Some, "Alice"}
    no_person = :None
  end
  defp test_pattern_matching() do
    user = {:Some, "Bob"}
    result = case (user.elem(0)) do
  0 ->
    g = user.elem(1)
    name = g
    "Hello, " + name
  ...
  1 ->
    ...
    Main.process_user({:Some, "Charlie"})
    Main.process_user(:None)
  end
  ...
  defp test_functional_operations() do
    user = {:Some, "David"}
    upper_name = {:Map, user, fn name -> name.toUpperCase() end}
  ...
  defp test_beam_integration() do
    user = {:Some, "Eve"}
    user_result = {:ToResult, user, "User not found"}
    ok_result = {:Ok, "Frank"}
    error_result = {:Error, "Not found"}
    option_from_ok = {:FromResult, ok_result}
    option_from_error = {:FromResult, error_result}
  ...
  defp test_null_safety() do
    ...
    result = OptionTools.unwrap({:Map, safe_option, fn s -> s.length end}, 0)
  ...
  defp test_collection_operations() do
    ...
    combined = {:All, all_options}
    mixed_options = [{:Some, 1}, :None, {:Some, 3}]
    failed_combine = {:All, mixed_options}
    processed_values = Enum.filter(Enum.map(options, fn opt -> {:Map, opt, fn x -> x * 2 end} end), fn opt -> OptionTools.is_some(opt) end)
  end
end