defmodule Main do
  ...
  defp test_complex_indexed_operation() do
    ...
    g = 0
    g1 = numbers.length
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {sum, g, g1, :ok}, fn _, {acc_sum, acc_g, acc_g1, acc_state} ->
  if (acc_g < acc_g1) do
    i = acc_g = acc_g + 1
    acc_sum = acc_sum + numbers[i] * (i + 1)
    {:cont, {acc_sum, acc_g, acc_g1, acc_state}}
  else
    {:halt, {acc_sum, acc_g, acc_g1, acc_state}}
  end
end)
    sum
  end
