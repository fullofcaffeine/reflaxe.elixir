defmodule SysTools do
  ...
  def quote_win_arg(argument, escape_meta_characters) do
    if (not EReg.new("^(/)?[^ \t/\\\\\"]+$", "").match(argument)) do
      ...
      g = 0
      g1 = argument.length
      Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {g, result_b, bs_buf, :ok}, fn _, {acc_g, acc_result_b, acc_bs_buf, acc_state} ->
  g = acc_g
  ...
  if (g < g1) do
    ...
    {:cont, {g, result_b, bs_buf, acc_state}}
  else
    {:halt, {g, result_b, bs_buf, acc_state}}
  end
end)
      x = bs_buf.b
      result_b = result_b <> Std.string(x)
