defmodule EnumValueMap do
  ...
  defp compare(struct, k1, k2) do
    ...
    if (d != 0), do: d
    p_1 = Type.enum_parameters(k1)
    p_2 = Type.enum_parameters(k2)
    if (p.length == 0 && p.length == 0), do: 0
    struct.compareArgs(p, p)
  end
  ...
  defp compare_arg(struct, v1, v2) do
    if (Reflect.is_enum_value(v1) && Reflect.is_enum_value(v2)) do
      ...
      if (Std.is_of_type(v1, Array) && Std.is_of_type(v2, Array)) do
