defmodule StringTools do
  ...
  def html_escape(s, quotes) do
    ...
    _g_offset = 0
    _g_s = s
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), :ok, fn _, acc -> if (_g_offset < _g_s.length) do
  code = c = s = _g_s
index = _g_offset + 1
c = s.cca(index)
...
c
  ...
  def key_value_iterator(s) do
    StringKeyValueIterator.new(s)
  end
  def is_eof(c) do
    false
  end
  ...
  defp utf16_code_point_at(s, index) do
    c = s.cca(index)
    if (c >= 55296 && c <= 56319) do
      c = c - 55232 <<< 10 ||| s.cca(index + 1) &&& 1023
    end
