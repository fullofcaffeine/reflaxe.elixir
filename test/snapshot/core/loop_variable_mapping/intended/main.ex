defmodule Main do
  ...
  if (acc_g < acc_array.length) do
    ...
    g = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {g, sum, array, :ok}, fn _, {acc_g, acc_sum, acc_array, acc_state} ->
  if (acc_g < acc_array.length) do
    n = array[g]
    acc_g = acc_g + 1
    acc_sum = (acc_sum - n)
    {:cont, {acc_g, acc_sum, acc_array, acc_state}}
  else
    {:halt, {acc_g, acc_sum, acc_array, acc_state}}
  end
end)
