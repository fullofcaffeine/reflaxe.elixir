defmodule Main do
  ...
  def divide_numbers(a, b) do
    ResultTools.flat_map(parse_number(a), fn _num_a -> ResultTools.flat_map(parse_number(b), fn num_b -> if (num_b == 0), do: {:error, "Division by zero"}, else: {:ok, _num_a / num_b} end) end)
  end
  ...
  def test_extension_methods() do
    result = "hello"
    _upper_result = ResultTools.map(result, fn s -> s.toUpperCase() end)
    chained_result = ResultTools.flat_map(result, fn s -> if (s.length > 0), do: s <> "!", else: "empty" end)
