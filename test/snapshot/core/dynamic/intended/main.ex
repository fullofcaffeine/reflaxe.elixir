...
g = 0
Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), :ok, fn _, acc -> if (g < args.length) do
  arg = args[g]
  g + 1
  sum = sum + arg
  {:cont, acc}
...
sum end
  ...
  def dynamic_collections() do
    ...
    g = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), :ok, fn _, acc -> if (g < dyn_array.length) do
  item = dyn_array[g]
  g + 1
