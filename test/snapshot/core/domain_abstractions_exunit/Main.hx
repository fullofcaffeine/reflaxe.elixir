...
class Main extends TestCase {
	...
	function testResultChaining() {
		...
		var domainResult = Email.parse("test@example.com").map(email -> email.getDomain()).filter(domain -> domain == "example.com", "Wrong domain");
		...
		var failedFilter = Email.parse("test@wrong.com").map(email -> email.getDomain()).filter(domain -> domain == "example.com", "Wrong domain");
