defmodule PositiveInt_Impl_ do
  ...
  def parse(value) do
    if (value <= 0), do: {:error, "Value must be positive, got: " <> Kernel.to_string(value)}, else: {:ok, value}
  end
  ...
  def safe_sub(this1, other) do
    ...
    if (result <= 0), do: {:error, "Subtraction result would be non-positive: " <> Kernel.to_string(this1) <> " - " <> Kernel.to_string(to_int(other)) <> " = " <> Kernel.to_string(result)}, else: {:ok, result}
  end
  def safe_sub_int(this1, value) do
    result = (this1 - value)
    if (result <= 0), do: {:error, "Subtraction result would be non-positive: " <> Kernel.to_string(this1) <> " - " <> Kernel.to_string(value) <> " = " <> Kernel.to_string(result)}, else: {:ok, result}
  end
  def safe_div(this1, divisor) do
    divisor_int = to_int(divisor)
    if (rem(this1, divisor_int) != 0) do
      {:error, "Division not exact: " <> Kernel.to_string(this1) <> " / " <> Kernel.to_string(divisor_int) <> " has remainder " <> Kernel.to_string(rem(this1, divisor_int))}
    else
      result = trunc(this1 / divisor_int)
      if (result <= 0), do: {:error, "Division result would be non-positive: " <> Kernel.to_string(this1) <> " / " <> Kernel.to_string(divisor_int) <> " = " <> Kernel.to_string(result)}, else: {:ok, result}
    end
  end
  def div(this1, divisor) do
    trunc(this1 / to_int(divisor))
  end
