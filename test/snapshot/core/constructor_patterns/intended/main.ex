defmodule Main do
  ...
  defp test_multiple_instances() do
    users = []
    user = %ConstructorTest.User{}
    user = %{user | name: "User " <> Kernel.to_string(0)}
    user = %{user | email: "user" <> Kernel.to_string(0) <> "@example.com"}
    users = users ++ [user]
    user = %ConstructorTest.User{}
    user = %{user | name: "User " <> Kernel.to_string(1)}
    user = %{user | email: "user" <> Kernel.to_string(1) <> "@example.com"}
    users = users ++ [user]
    user = %ConstructorTest.User{}
    user = %{user | name: "User " <> Kernel.to_string(2)}
    user = %{user | email: "user" <> Kernel.to_string(2) <> "@example.com"}
    users = users ++ [user]
    user = %ConstructorTest.User{}
    user = %{user | name: "User " <> Kernel.to_string(3)}
    user = %{user | email: "user" <> Kernel.to_string(3) <> "@example.com"}
    users = users ++ [user]
    user = %ConstructorTest.User{}
    user = %{user | name: "User " <> Kernel.to_string(4)}
    user = %{user | email: "user" <> Kernel.to_string(4) <> "@example.com"}
    _ = users ++ [user]
