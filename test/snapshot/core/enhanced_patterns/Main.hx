...
class Main {
	...
	public static function testComplexGuardPerformance():String {
		...
		return switch ([metrics.cpu, metrics.memory, metrics.disk, metrics.network]) {
			case [cpu, mem, disk, net] if (cpu > 80 || mem > 90 || disk > 90 || net > 80): "Critical resource usage";

			case [cpu, mem, disk, net] if (cpu > 60 || mem > 75 || disk > 75 || net > 60): "High resource usage";

			case [cpu, mem, disk, net] if (cpu > 40 && mem > 50 && disk > 50 && net > 30): "Moderate resource usage";

			case [cpu, mem, disk, net] if (cpu <= 40 && mem <= 50 && disk <= 50 && net <= 30): "Low resource usage";
