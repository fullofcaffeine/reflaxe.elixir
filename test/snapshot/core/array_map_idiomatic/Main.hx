...
class Main {
	...
	static function testArrayFilter() {
		...
		// Chained operations
		var result = numbers.filter(n -> n > 5).map(n -> n * 2);
	}
	...
	static function testComplexTransformations() {
		var users = [{name: "Alice", age: 30}, {name: "Bob", age: 25}, {name: "Charlie", age: 35}];
		...
		var adults = users.filter(u -> u.age >= 30).map(u -> u.name);

		// Multiple transformations
		var processed = users.filter(u -> u.age > 20)
