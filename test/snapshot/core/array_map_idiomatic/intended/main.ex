defmodule Main do
  defp main() do
    Main.test_simple_map()
    Main.test_map_with_enum_construction()
    Main.test_nested_operations()
    Main.test_array_filter()
    Main.test_complex_transformations()
  end
  defp test_simple_map() do
    numbers = [1, 2, 3, 4, 5]
    doubled = Enum.map(numbers, fn n -> n * 2 end)
    strings = Enum.map(numbers, fn n -> "Number: " + n end)
    processed = Enum.map(numbers, fn n -> Main.process_number(n) end)
  end
  defp test_map_with_enum_construction() do
    tags = ["work", "personal", "urgent"]
    values = Enum.map(tags, fn t -> Main.string_value(t) end)
    tuples = Enum.map(tags, fn t -> %{:type => "string", :value => t} end)
    nested = Enum.map(tags, fn t -> Main.array_value([Main.string_value(t)]) end)
  end
  ...
  defp test_complex_transformations() do
    ...
    user_info = Enum.map(users, fn u -> %{:name => u[:name].toUpperCase(), :ageGroup => (if (u[:age] < 30), do: "young", else: "adult"), :id => Main.generate_id(u[:name])} end)
