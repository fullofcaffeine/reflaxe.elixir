defmodule EnumValueMap do
  @compile {:nowarn_unused_function, [compare: 3, compare_args: 3, compare_arg: 3]}

  defp compare(struct, k1, k2) do
    d = (MyApp.Type.enum_index(k1) - MyApp.Type.enum_index(k2))
    if d != 0, do: d
    p1 = MyApp.Type.enum_parameters(k1)
    p2 = MyApp.Type.enum_parameters(k2)
  ...
  defp compare_args(struct, a1, a2) do
    {a1} = Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {a1}, fn _, {a1} ->
  if 0 < length(a1) do
    i = 0 + 1
  ...
  defp compare_arg(struct, v1, v2) do
    if MyApp.Reflect.is_enum_value(v1) and MyApp.Reflect.is_enum_value(v2), do: struct.compare(v1, v2)
    MyApp.Reflect.compare(v1, v2)
  end
  def keys(struct) do
    struct.iterator()
  end
