...
class FormTemplate {
	...
	public static function renderForm(action:String, method:String):String {
		...
		return "<form action='" + action + "' method='" + method + "'>" + "<input type='hidden' name='_csrf_token' value='" + csrf + "'>"
...
class LiveViewComponents {
	...
	public static function renderCounter(count:Int):String {
		return "<div class='counter'>" + "<h3>Count: " + count + "</h3>" + "<button phx-click='increment'>+</button>"
			+ "<button phx-click='decrement'>-</button>" + "<button phx-click='reset'>Reset</button>" + "</div>";
	}
	...
	public static function renderLiveData(data:{temperature:Float, humidity:Float}):String {
		return "<div class='sensor-data' phx-update='replace'>" + "<div class='temperature'>" + "<span class='label'>Temperature:</span>"
			+ "<span class='value'>" + data.temperature + "°C</span>" + "</div>" + "<div class='humidity'>" + "<span class='label'>Humidity:</span>"
			+ "<span class='value'>" + data.humidity + "%</span>" + "</div>" + "</div>";
	}
}
