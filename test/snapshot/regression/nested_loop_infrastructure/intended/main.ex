defmodule Main do
  def paired_rows(source) do
    rows = []
    rows = Enum.reduce(source, rows, fn x, rows_acc ->
      row = []
      row = Enum.reduce(source, row, fn y, row_acc ->
        row_acc = Enum.concat(row_acc, [x])
        Enum.concat(row_acc, [y])
      end)
      Enum.concat(rows_acc, [row])
    end)
    rows
  end
  def main() do
    matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    results = []
    _ = Enum.reduce(matrix, results, fn row, results_acc -> Enum.reduce(row, results_acc, fn item, results_acc -> Enum.concat(results_acc, [item * 2]) end) end)
    g = []
    _doubled = Enum.reduce(matrix, g, fn row, g_acc ->
      _ = Enum.reduce(row, g, fn item, g_acc -> Enum.concat(g_acc, [item * 2]) end)
      g_acc
    end)
    _mapped = Enum.map(matrix, fn row -> Enum.map(row, fn item -> item * 2 end) end)
    sum_of_sums = 0
    _ = Enum.reduce(matrix, sum_of_sums, fn row, sum_of_sums_acc ->
      row_sum = 0
      row_sum = Enum.reduce(row, row_sum, fn item, row_sum_acc -> row_sum_acc + item end)
      sum_of_sums_acc + row_sum
    end)
    nil
  end
end
