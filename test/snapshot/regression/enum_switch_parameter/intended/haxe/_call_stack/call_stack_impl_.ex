defmodule CallStack_Impl_ do
  ...
  def subtract(this1, stack) do
    start_index = -1
    i = -1
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {g, i, this1, start_index, :ok}, fn _, {acc_g, acc_i, acc_this1, acc_start_index, acc_state} ->
  if (acc_i = acc_i + 1 < acc_this1.length) do
    acc_g = 0
    g1 = stack.length
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {acc_g, acc_i, acc_start_index, g1, :ok}, fn _, {acc_g, acc_i, acc_start_index, acc_g1, acc_state} ->
  if (acc_g < acc_g1) do
    j = acc_g = acc_g + 1
    ...
    {:cont, {acc_g, acc_i, acc_start_index, acc_g1, acc_state}}
  else
    {:halt, {acc_g, acc_i, acc_start_index, acc_g1, acc_state}}
  end
end)
    if (acc_start_index >= 0) do
      throw(:break)
    end
    {:cont, {acc_g, acc_i, acc_this1, acc_start_index, acc_state}}
  else
    {:halt, {acc_g, acc_i, acc_this1, acc_start_index, acc_state}}
  end
end)
    if (start_index >= 0) do
      if (end == nil) do
