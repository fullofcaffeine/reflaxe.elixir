defmodule Main do
  ...
  defp test_complex_loop() do
    ...
    sum = 0
    i = 0
    Enum.reduce_while(Stream.iterate(0, fn n -> n + 1 end), {i, sum, data, :ok}, fn _, {acc_i, acc_sum, acc_data, acc_state} ->
  if (acc_i < acc_data.length) do
    acc_sum = acc_sum + data[i]
    acc_i = acc_i + 1
    {:cont, {acc_i, acc_sum, acc_data, acc_state}}
  else
    {:halt, {acc_i, acc_sum, acc_data, acc_state}}
  end
end)
