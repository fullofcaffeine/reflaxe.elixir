...
class Main extends TestCase {
	...
	function testCustomAssertions() {
		...
		function assertBetween(value:Float, min:Float, max:Float, ?msg:String) {
			Assert.isTrue(value >= min && value <= max, msg != null ? msg : 'Value $value should be between $min and $max');
		}

		function assertContains<T>(array:Array<T>, element:T, ?msg:String) {
			Assert.isTrue(array.indexOf(element) >= 0, msg != null ? msg : 'Array should contain element');
		}
	...
	function testDataDriven() {
		...
		for (testCase in testCases) {
			...
			Assert.equals(testCase.expected, result, 'Input ${testCase.input} should produce ${testCase.expected}');
		}
	}
