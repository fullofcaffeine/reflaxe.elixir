 ...
 * Tests Supervisor, Task, and Task.Supervisor extern definitions
  ...
  (
  _g1 = 0
  while (_g1 < funs.length) do
  (
  fun = Enum.at(funs, _g1)
  _g1 + 1
  _g.push(Task.Task.async(fun))
)
end
)
  temp_array1 = _g
  ...
  (
  _g1 = 0
  while (_g1 < tasks2.length) do
  (
  task2 = Enum.at(tasks2, _g1)
  _g1 + 1
  _g.push(Task.Task.await(task2))
)
end
)
  temp_array = _g
  ...
  (
  task2 = Task.Task.async(fn  -> (
  Process.Process.sleep(50)
  "timed"
) end)
  result2 = Task.Task.yield(task2, 100)
  if (result2 != nil && result2._0 == "ok"), do: temp_maybe_maybe_string = result2._1, else: (
  ...
  Log.trace("Supervised tasks count: " + children.length, %{fileName: "Main.hx", lineNumber: 176, className: "Main", methodName: "testTaskSupervisor"})
  ...
  fun = Enum.at(funs, _g1)
  _g1 + 1
  _g.push(Supervisor.Task.Supervisor.async(supervisor2, fun))
)
end
)
  temp_array1 = _g
  ...
  (
  _g = 0
  while (_g < children_list.length) do
  (
