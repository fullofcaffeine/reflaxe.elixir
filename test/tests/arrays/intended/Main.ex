defmodule Main do
  ...
  def basic_array_ops() do
    ...
    Log.trace(length(numbers), %{"fileName" => "Main.hx", "lineNumber" => 15, "className" => "Main", "methodName" => "basicArrayOps"})
    numbers ++ [6]
    [0 | numbers]
  ...
  def array_iteration() do
    ...
    _g = _g + 1
    Log.trace("Fruit: " <> fruit, %{"fileName" => "Main.hx", "lineNumber" => 35, "className" => "Main", "methodName" => "arrayIteration"}) end)
    _g = 0
    _g = length(fruits)
    (
      try do
        loop_fn = fn ->
          if (_g < _g) do
            try do
              i = _g = _g + 1
    Log.trace("" <> Integer.to_string(i) <> ": " <> Enum.at(fruits, i), %{"fileName" => "Main.hx", "lineNumber" => 40, "className" => "Main", "methodName" => "arrayIteration"})
              loop_fn.()
            catch
              :break -> nil
              :continue -> loop_fn.()
  ...
  def multi_dimensional() do
    ...
    _g = _g + 1
    Log.trace("Element: " <> Integer.to_string(elem_), %{"fileName" => "Main.hx", "lineNumber" => 113, "className" => "Main", "methodName" => "multiDimensional"}) end) end)
    temp_array = nil
    _g = []
    temp_array1 = nil
    _g = []
  ...
  def functional_methods() do
    ...
    even_index = Enum.find_index(numbers, fn n -> n rem 2 == 0 end)
    ...
    Enum.each(numbers, fn n -> Log.trace("  - " <> Integer.to_string(n), %{"fileName" => "Main.hx", "lineNumber" => 174, "className" => "Main", "methodName" => "functionalMethods"}) end)
    ...
    flattened = Enum.flat_map(nested_arrays, fn arr -> temp_result = nil
    _g = []
    _g = 0
    _g = arr
