defmodule Mix.Tasks.Haxe.Gen.ExternTest do
  ...
  test "generates app-local boundary extern without loading the module" do
    tmp_root = tmp_root()
    out_dir = Path.join(tmp_root, "src_haxe")
    on_exit(fn -> File.rm_rf!(tmp_root) end)
    ...
    file_path = Path.join([out_dir, "my_app", "infrastructure", "PubSub.hx"])
    contents = File.read!(file_path)

    assert contents =~ ~s(package my_app.infrastructure;)
    assert contents =~ ~s|Generated by `mix haxe.gen.extern --boundary`|
    assert contents =~ ~s|@:native("MyApp.PubSub")|
    assert contents =~ ~s|@:unsafeExtern|
    assert contents =~ "extern class PubSub {}"
    refute contents =~ "import elixir.types.Term;"
    refute contents =~ "public static function"
    refute contents =~ "Dynamic"
  end
