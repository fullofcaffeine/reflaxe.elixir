...
- Pass registry changes: run `npm run docs:passes` and `npm run guard:pass-inventory`. Profile through `npm run profile:passes:baseline` or the module/output filters documented in the lean-pass guide; do not add unbounded per-pass logs or infer redundancy from a replay name alone.
