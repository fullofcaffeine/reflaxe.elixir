...
- External contribution boundary: do not open or update pull requests or issues in `SomeRanDev/reflaxe` or another external repository unless the user explicitly requests that external action. An "upstream candidate" is analysis, not permission. Keep required fixes vendored and documented in `vendor/reflaxe/PATCHES.md`; a backup branch may be pushed to the project-owned `fullofcaffeine/reflaxe` fork without opening an upstream PR.
...
- Deep compiler/runtime semantics: if a fix remains too complex, ambiguous, or architectural after deep local investigation, stop before broad changes and propose a rare manual GPT-5.6 Pro review. Use the globally installed `$oracle-review` skill for the detailed prompt, sanitized Repomix bundle, checked ZIP, pending-request check, and `/tmp/oracle` archive lifecycle. The prompt must still describe the failing Haxe source, generated Elixir shape, expected semantics, suspected pipeline files, AST-only/no-band-aid constraints, and adjacent risks. Do not land speculative broad repairs while waiting for that external review. Public source and install instructions: https://github.com/fullofcaffeine/caf-skills/tree/main/skills/oracle-review
...
- Pass registry changes: assign framework ownership by exact stable ID in `PassScopeManifest` and derive eligibility only from typed annotation metadata or structured ElixirAST in `PassApplicability`. Never classify a pass or module by name fragments, generated module names, file paths, or app names. Keep an uncertain pass core/global until evidence supports narrowing; document the reason and track upstream coupling separately.
- Run `npm run docs:passes`, `npm run guard:pass-inventory`, and `npm run guard:pass-scopes` after pass ownership, ordering, or registry changes. Profile through `npm run profile:passes:baseline` or the bounded filters documented in the lean-pass guide. `-D reflaxe_elixir_disable_pass_scopes` is verification-only, not a consumer profile or alternate output path.
- Do not infer redundancy from a replay/final name. Remove exact same-name registrations only when `RegistryCore` already proves later entries unreachable; merging distinct passes requires ordering, idempotence, focused regression, and scoped/all-pass byte-parity evidence.
...
- Abstraction-doc check (skeptical Elixir reader):
  ...
  - State one tradeoff explicitly (compile step, generated-shape indirection, or when direct Haxe->Elixir authoring is clearer).
...
Reserve Oracle for unresolved multi-subsystem compiler invariants, genuinely competing architectural fixes, or a new cross-target abstraction whose ownership cannot be established safely from local evidence. Use `$oracle-review` for the manual GPT-5.6 Pro package and lifecycle; keep the Elixir-specific AST, generated-source, Phoenix-faithfulness, runtime, and snapshot constraints in the request. The human performs the handoff and pastes the response back, and local tests and repository authorities remain decisive.
