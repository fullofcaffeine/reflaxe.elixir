# Quick Start Patterns for LLM Agents

**Purpose**: Copy-paste ready patterns for common Reflaxe.Elixir tasks.

**Target**: LLM agents who need immediate productivity with working examples.
...
class UserTemplate {
    ...
    public static function userList(users:Array<User>):String {
        var userItems = [for (user in users) userCard(user)].join("");
            ...
            <form phx-submit="${action}" class="user-form">
                ...
                <div class="field">
                    ...
                    <select name="user[role]" id="role">
                        <option value="user" ${changeset.data.role == "user" ? "selected" : ""}>User</option>
                        <option value="admin" ${changeset.data.role == "admin" ? "selected" : ""}>Admin</option>
                        <option value="moderator" ${changeset.data.role == "moderator" ? "selected" : ""}>Moderator</option>
                    </select>
...
1. **Read**: [API_REFERENCE_SKELETON.md](./API_REFERENCE_SKELETON.md) for complete API
2. **Study**: Working examples in the examples/ directory
3. **Test**: Create unit and integration tests
4. **Deploy**: Follow Phoenix deployment guides

## 🎯 LLM Agent Tips

1. **Always start with patterns** - Copy first, then customize
2. **Use annotations correctly** - They drive code generation
3. **Follow Phoenix conventions** - For familiar Elixir output
4. **Test incrementally** - Compile after each change
5. **Leverage type safety** - Let Haxe catch errors early
