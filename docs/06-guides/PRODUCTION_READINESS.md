...
| --- | --- | --- | --- |
| Haxe program behavior | **Conditional** | Generated-code checks and runtime tests cover normal and invalid inputs, loop control, nested list comprehensions, and callbacks inside `Result` branches | No known compiler bug affects those reviewed examples. Ordinary shared object/collection aliases are a separately recorded blocker and prevent a broader Haxe-semantics claim. |
| Haxe reference and mutable-collection semantics | **Blocked** | The pinned first tranche in the [reference-semantics audit](../05-architecture/HAXE_REFERENCE_SEMANTICS_AUDIT.md) compares Haxe 4.3.7 behavior with current generated persistent updates; the isolated runtime feasibility spike proves only foundational lease/graph behavior | Complete the type audit, typed representation ABI, managed core/collection runtime, closures, boundaries, package ABI, and alias-focused regressions. No managed behavior is shipped today. |
