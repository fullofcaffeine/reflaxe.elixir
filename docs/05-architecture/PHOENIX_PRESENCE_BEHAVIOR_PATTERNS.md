...
class InheritanceHookMacro {
    public static macro function build():Array<Field> {
        ...
        // If parent has onInherited method, call it
        if (cls.superClass != null) {
            var parentClass = cls.superClass.t.get();
            
            // Inject code that runs when child is defined
            Context.onAfterTyping(() -> {
                // Call parent's onInherited hook with child class info
                Reflect.callMethod(
                    parentClass,
                    Reflect.field(parentClass, "onInherited"),
                    [cls]
                );
            });
        }
...
class Child extends TrackableParent {
    // Parent is notified at compile-time!
}
```
