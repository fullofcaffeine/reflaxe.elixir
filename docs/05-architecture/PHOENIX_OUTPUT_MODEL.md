...
Those safeguards are implemented, not inferred from layout: the target root's
version 2 `_GeneratedFiles.json` owns exact paths and content digests; the compiler
formats staging, rejects unowned collisions, journals publication/stale deletion,
and commits the manifest last. Mix clean consumes the same record. See
...
In-place mode uses the same fail-closed protocol as isolated output:

- Write a missing generated file.
- Rewrite a manifest-owned file only when its current digest still matches.
- Refuse to overwrite an unowned app file.
- Remove stale manifest-owned files.
- Format and validate the complete staged set before changing live files.
- Recover or roll back interrupted publication before accepting another build or clean.

The version 2 manifest lives at the configured output root:

```text
<elixir_output>/_GeneratedFiles.json
```

Ownership is never inferred from a generated header, and there is no force-adoption flag. A team
transfers ownership by resolving the source/path collision explicitly, then regenerating into an
unoccupied path. See
[Generated Output Ownership](../02-user-guide/GENERATED_OUTPUT_OWNERSHIP.md) for the schema and
recovery contract.
...
- Ownership integration evidence for first/unchanged generation, legacy-manifest
  upgrade, manifest-owned rewrite, source rollback, unowned collision, modified-owned
  rejection, stale cleanup, namespace movement, interrupted recovery, clean, and
