...
| 3 | `BundleGuardsAndInterpolation` | `guards-interpolation` | `mixed` | `guards-interpolation.mixed` | source order | Case and guard normalization plus interpolation preparation (21 child passes) |
| 4 | `BundleCoreTransforms` | `core-lowering` | `mixed` | `core-lowering.mixed` | source order | Core idiom, control-flow, collection, and runtime lowering (222 child passes) |
| 5 | `BundleHeexPipeline` | `hxx-heex` | `mixed` | `hxx-heex.mixed` | source order | Typed HXX and HEEx lowering (88 child passes) |
...
| 7 | `BundleAbsoluteFinal` | `absolute-final` | `mixed` | `absolute-final.mixed` | source order | Absolute-final validation and narrowly scoped cleanup (175 child passes) |
