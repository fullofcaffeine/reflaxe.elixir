...
@:routes([
    {name: "root", method: HttpMethod.LIVE, path: "/", controller: "server.live.TodoLive", action: "index"},
    {name: "todosIndex", method: HttpMethod.LIVE, path: "/todos", controller: "server.live.TodoLive", action: "index"},
    {name: "todosShow", method: HttpMethod.LIVE, path: "/todos/:id", controller: "server.live.TodoLive", action: "show"},
    {name: "apiUsers", method: HttpMethod.GET, path: "/api/users", controller: "controllers.UserController", action: "index"},
    {name: "createUser", method: HttpMethod.POST, path: "/api/users", controller: "controllers.UserController", action: "create"},
...
class TodoAppRouter {
    ...
    // Type-safe enum provides compile-time validation!
}
```

**Module Naming**: Use @:native for Phoenix conventions. Controller references use full package paths for proper type resolution.
...
#### 1. RouterCompiler Interface Compatibility
The RouterCompiler (the main transpiler component) expects to find functions with `@:route` metadata. It scans all class methods looking for these annotations to generate Phoenix router code. Without functions, there's nothing for the RouterCompiler to process.

#### 2. Type-Safe Route Helpers
Generated functions return the path as a String, enabling compile-time route construction:
...
#### 3. IDE Code Completion and IntelliSense
...
#### 5. Haxe Reflection and Macro Processing
Functions are needed for:
- **Metadata extraction** - `@:route` annotations are attached to functions
- **Context.getBuildFields()** - Haxe macros operate on field structures
- **Type system integration** - Functions participate in normal type checking

### Generated Phoenix Router

The RouterCompiler processes these functions and generates Phoenix routes:
...
The generated functions are then processed normally by the RouterCompiler to produce Phoenix routes.
...
The RouterBuildMacro includes comprehensive snapshot testing:

- **test/tests/RouterBuildMacro/** - Build macro functionality tests
- **Route generation validation** - Ensures correct function creation
- **Phoenix output verification** - Validates generated router.ex content
- **Error handling tests** - Tests validation and error reporting

Run tests with:
```bash
npm test -- test=RouterBuildMacro
```
