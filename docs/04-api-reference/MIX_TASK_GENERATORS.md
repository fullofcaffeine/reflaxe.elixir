...
- If `--package` has extra segments, files are placed under matching package directories (for example `--out src_haxe/externs --package externs.ecto` writes `src_haxe/externs/ecto/<Module>.hx`).
