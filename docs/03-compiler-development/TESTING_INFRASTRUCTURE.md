...
| Shared language/issues | The installed-package smoke runs unchanged official numeric-separator and issue 10455 tests on BEAM | Broader applicable official Haxe coverage | Add cases through the source-record guard and existing ExUnit adapter | Baseline expansion |
...
| Package/install | The isolated Haxelib ZIP smoke compiles official shared-language, `unitstd`, and issue tests, checks generated Elixir strictly, and runs ExUnit on BEAM | Complete applicable official portable coverage | Expand the same package smoke from reviewed source records | Release evidence, then baseline expansion |
...
3. **Official representative smoke.** The public package path now runs shared-language, `unitstd`,
   and issue cases. Keep its source records, strict Elixir check, BEAM run, and bounded failures intact.
