...
| --- | --- | --- | --- | --- | --- |
| 1 | Ecto changesets | `std/ecto/Changeset.hx`, `docs/04-api-reference/ANNOTATIONS.md` | `castFields`, `validateRequired`, `validateLength`, `validateFormat`, and related APIs take field names as `String`/`Array<String>`. This loses autocomplete and lets renamed schema fields silently drift. | Generate or expose typed field tokens, for example `UserFields.email`, and add overloads that accept tokens and token arrays. Keep string compatibility behind clearly documented migration/unsafe APIs. | High / Medium |
